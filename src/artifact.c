 ...
 */
struct obj * mk_artifact(otmp, alignment)
...
void artifact_exists(otmp, name, mod)
    struct obj *otmp;const char *name;boolean mod;
{
    register const struct artifact *a;

    if (otmp && *name) for (a = artilist + 1; a->otyp; a++)
...
boolean shade_glare(struct obj *obj)
{
...
boolean restrict_name(struct obj *otmp, const char *name)
{
    ...
    sametype[otyp] = TRUE;
    if (!objects[otyp].oc_name_known && (odesc = OBJ_DESCR(objects[otyp])) != 0)
    {
...
/* used for monsters */
boolean defends_when_carried(int adtyp, struct obj *otmp)
{
    register const struct artifact *weap;
...
boolean protects(otmp, being_worn)
    struct obj *otmp;boolean being_worn;
{
    const struct artifact *arti;

    if (being_worn && objects[otmp->otyp].oc_oprop == PROTECTION) return TRUE;
 ...
 */
void set_artifact_intrinsic(otmp, on, wp_mask)
    register struct obj *otmp;boolean on;long wp_mask;
{
    long *mask = 0;
...
boolean arti_immune(struct obj *obj, int dtyp)
{
    register const struct artifact *weap = get_artifact(obj);

    if (!weap) return FALSE;
    if (dtyp == AD_PHYS) return FALSE; /* nothing is immune to phys dmg */
    return (boolean) (weap->attk.adtyp == dtyp || weap->defn.adtyp == dtyp
            || weap->cary.adtyp == dtyp);
}

STATIC_OVL boolean bane_applies(const struct artifact *oart, struct monst *mon)
{
...
STATIC_OVL int spec_applies(register const struct artifact *weap,
        struct monst *mtmp)
{
    struct permonst *ptr;
    boolean yours;
...
boolean undiscovered_artifact(m)
    xchar m;
{
    int i;
...
STATIC_OVL boolean Mb_hit(magr, mdef, mb, dmgptr, dieroll, vis, hittee)
    ...
    *dmgptr += rnd(4); /* (2..3)d4 */
    ...
    case MB_INDEX_CANCEL:
        ...
        else
        {
            ...
            if (youdefend)
            {
                if (youmonst.data != old_uasmon) *dmgptr = 0; /* rehumanized, so no more damage */
                if (u.uenmax > 0)
                {
            ...
            {
                if (mdef->data == &mons[PM_CLAY_GOLEM]) mdef->mhp = 1; /* cancelled clay golems will die */
 ...
 */
boolean artifact_hit(magr, mdef, otmp, dmgptr, dieroll)
    struct monst *magr, *mdef;struct obj *otmp;int *dmgptr;int dieroll; /* needed for Magicbane and vorpal blades */
{
    ...
    *dmgptr += spec_dbon(otmp, mdef, *dmgptr);
    ...
    if (spec_ability(otmp, SPFX_BEHEAD))
    {
                ...
                && (dieroll == 1 || mdef->data == &mons[PM_JABBERWOCK]))
        {
            ...
            { "%s beheads %s!", "%s decapitates %s!" };

            if (youattack && u.uswallow && mdef == u.ustuck) return FALSE;
...
static NEARDATA const char recharge_type[] =
...
STATIC_OVL int arti_invoke(obj)
    struct obj *obj;
{
    register const struct artifact *oart = get_artifact(obj);
    if (!obj)
...
boolean artifact_has_invprop(otmp, inv_prop)
    struct obj *otmp;uchar inv_prop;
{
    const struct artifact *arti = get_artifact(otmp);
...
STATIC_OVL uchar abil_to_adtyp(abil)
    long *abil;
{
...
STATIC_OVL unsigned long abil_to_spfx(abil)
    long *abil;
{
    ...
    long *abil;
{
    ...
    unsigned long spfx;
    long wornbits;
    long wornmask = (W_ARM | W_ARMC | W_ARMH | W_ARMS | W_ARMG | W_ARMF | W_ARMU
            | W_AMUL | W_RINGL | W_RINGR | W_TOOL
            /* [do W_ART and W_ARTI actually belong here?] */
            | W_ART | W_ARTI);

    if (u.twoweap) wornmask |= W_SWAPWEP;
...
const char * glow_color(arti_indx)
    int arti_indx;
{
...
int retouch_object(objp, loseit)
...
STATIC_OVL boolean untouchable(obj, drop_untouchable)
    struct obj *obj;boolean drop_untouchable;
{
    struct artifact *art;
...
void retouch_equipment(dropflag)
