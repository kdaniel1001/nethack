...
struct monst * make_familiar(otmp, x, y, quietly)
    register struct obj *otmp;xchar x, y;boolean quietly;
{
    struct permonst *pm;
    struct monst *mtmp = 0;
...
void mon_arrive(mtmp, with_you)
    struct monst *mtmp;boolean with_you;
{
    struct trap *t;
    ...
    if (mtmp == u.usteed) return; /* don't place steed on the map */
    if (with_you)
    {
...
void mon_catchup_elapsed_time(mtmp, nmv)
    struct monst *mtmp;long nmv; /* number of moves */
{
...
void migrate_to_level(mtmp, tolev, xyloc, cc)
    register struct monst *mtmp;xchar tolev; /* destination level */
...
int dogfood(mon, obj)
    struct monst *mon;register struct obj *obj;
{
    ...
    {
    case FOOD_CLASS:
        if (obj->otyp == CORPSE || obj->otyp == TIN || obj->otyp == EGG) fptr =
                &mons[obj->corpsenm];
        ...
        default:
            ...
            return (obj->otyp > SLIME_MOLD) ?
                    (carni ? ACCFOOD : MANFOOD) : (herbi ? ACCFOOD : MANFOOD);
        }
    default:
        if (obj->otyp == AMULET_OF_STRANGULATION
                || obj->otyp == RIN_SLOW_DIGESTION) return TABU;
        ...
        if (!obj->cursed && obj->oclass != BALL_CLASS
                && obj->oclass != CHAIN_CLASS) return APPORT;
        /*FALLTHRU*/
    case ROCK_CLASS:
        return UNDEF;
    }
 ...
 * succeeded.
 */
boolean tamedog(mtmp, obj)
    register struct monst *mtmp;register struct obj *obj;
{
    /* The Wiz, Medusa and the quest nemeses aren't even made peaceful. */
    if (mtmp->iswiz || mtmp->data == &mons[PM_MEDUSA]
            || (mtmp->data->mflags3 & M3_WANTSARTI)) return FALSE;

    /* worst case, at least it'll be peaceful. */
    mtmp->mpeaceful = 1;
    set_malign(mtmp);
    if (flags.moonphase == FULL_MOON && night() && rn2(6) && obj
    && mtmp->data->mlet == S_DOG) return FALSE;
    ...
    if (mtmp == u.ustuck)
    {
        ...
        else if (!(Upolyd && sticks(youmonst.data))) unstuck(mtmp);
    }
    ...
    if (mtmp->m_id == quest_status.leader_m_id) return FALSE;
    ...
    { /* thrown food */
        ...
        if (dog_eat(mtmp, obj, mtmp->mx, mtmp->my, TRUE) == 2) return TRUE; /* oops, it died... */
        /* `obj' is now obsolete */
    }
 ...
 */
void wary_dog(mtmp, was_dead)
    struct monst *mtmp;boolean was_dead;
{
    struct edog *edog;
