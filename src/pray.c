...
boolean critically_low_hp(only_if_injured)
    boolean only_if_injured; /* determines whether maxhp <= 5 matters */
{
    int divisor, hplim, curhp = Upolyd ? u.mh : u.uhp, maxhp =
...
STATIC_OVL int in_trouble()
{
    ...
    if (nohands(youmonst.data) || !freehand())
    {
        ...
        if (Upolyd && nohands(youmonst.data) && (!Unchanging || ((otmp =
                unchanger()) != 0 && otmp->cursed)))
    ...
    if (Cursed_obj(uarmg,
            GAUNTLETS_OF_FUMBLING) || Cursed_obj(uarmf, FUMBLE_BOOTS))
...
worst_cursed_item()
{
    ...
    if (welded(uwep) && (uright || bimanual(uwep)))
    { /* weapon */
        otmp = uwep;
    ...
    else if (uarms && uarms->cursed)
    { /* shield */
        otmp = uarms;
        /* then cloak due to body armor */
    }
    else if (uarmc && uarmc->cursed)
    { /* cloak */
        otmp = uarmc;
    }
    else if (uarm && uarm->cursed)
    { /* suit */
        otmp = uarm;
    }
    else if (uarmh && uarmh->cursed)
    { /* helmet */
        otmp = uarmh;
    }
    else if (uarmf && uarmf->cursed)
    { /* boots */
        otmp = uarmf;
    }
    else if (uarmu && uarmu->cursed)
    { /* shirt */
        otmp = uarmu;
    }
    else if (uamul && uamul->cursed)
    { /* amulet */
        otmp = uamul;
    }
    ...
    else if (ublindf && ublindf->cursed)
    { /* eyewear */
...
STATIC_OVL void fix_worst_trouble(trouble)
    int trouble;
{
    int i;
    struct obj *otmp = 0;
 ...
 */
STATIC_OVL void god_zaps_you(resp_god)
    aligntyp resp_god;
{
    if (u.uswallow)
    {
...
STATIC_OVL void fry_by_god(resp_god, via_disintegration)
    aligntyp resp_god;boolean via_disintegration;
{
    You("%s!",
            !via_disintegration ?
...
STATIC_OVL void angrygods(resp_god)
    aligntyp resp_god;
{
    int maxanger;
...
#define ok_wep(o) ((o) && ((o)->oclass == WEAPON_CLASS || is_weptool(o)))
            ...
            && !carrying(SPE_FINGER_OF_DEATH))
    {
        ...
        make_splbk: obj = mksobj(class_gift, TRUE, FALSE);
        bless(obj);
        obj->bknown = TRUE;
...
STATIC_OVL void pleased(g_align)
    aligntyp g_align;
{
 ...
 */
STATIC_OVL boolean water_prayer(bless_water)
    boolean bless_water;
{
    register struct obj *otmp;
    ...
    for (otmp = level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere)
    {
        ...
        else if (otmp->oclass == POTION_CLASS) other = TRUE;
    }
    if (!Blind && changed)
    {
        pline("%s potion%s on the altar glow%s %s for a moment.",
                ((other && changed > 1L) ?
                        "Some of the" : (other ? "One of the" : "The")),
...
STATIC_OVL void godvoice(g_align, words)
    aligntyp g_align;const char *words;
{
    const char *quot = "";
...
STATIC_OVL void gods_angry(g_align)
    aligntyp g_align;
{
...
STATIC_OVL void gods_upset(g_align)
    aligntyp g_align;
{
...
STATIC_OVL void consume_offering(otmp)
    register struct obj *otmp;
{
    if (Hallucination)
        switch (rn2(3))
...
#define MAXVALUE 24 /* Highest corpse value (besides Wiz) */
    ...
    if (otmp->otyp == AMULET_OF_YENDOR)
    {
        if (!highaltar)
        {
            too_soon: if (altaralign == A_NONE && Inhell)
    ...
    else
    {
        ...
        if (u.ualign.type != altaralign)
        {
            ...
            {
                if (u.ualignbase[A_CURRENT]
                        == u.ualignbase[A_ORIGINAL]&& altaralign != A_NONE)
                {
...
const char * align_gname(alignment)
    aligntyp alignment;
{
    const char *gnam;
...
const char * halu_gname(alignment)
    aligntyp alignment;
{
...
/* deity's title */
const char * align_gtitle(alignment)
    aligntyp alignment;
{
...
STATIC_OVL boolean blocked_boulder(dx, dy)
    int dx, dy;
{
    register struct obj *otmp;
    long count = 0L;

    for (otmp = level.objects[u.ux + dx][u.uy + dy]; otmp; otmp =
            otmp->nexthere)
    {
    ...
    if (!isok(u.ux + 2 * dx, u.uy + 2 * dy)) return TRUE;
    if (IS_ROCK(levl[u.ux + 2 * dx][u.uy + 2 * dy].typ)) return TRUE;
    if (sobj_at(BOULDER, u.ux + 2 * dx, u.uy + 2 * dy)) return TRUE;
