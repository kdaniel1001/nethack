...
void ballrelease(showmsg)
    boolean showmsg;
{
    if (carried(uball))
    {
        if (showmsg) pline("Startled, you drop the iron ball.");
        if (uwep == uball) setuwep((struct obj *) 0);
        if (uswapwep == uball) setuswapwep((struct obj *) 0);
        if (uquiver == uball) setuqwep((struct obj *) 0);
        ;
...
STATIC_OVL int bc_order()
{
    ...
    if (uchain->ox != uball->ox || uchain->oy != uball->oy || carried(uball)
            || u.uswallow) return BCPOS_DIFFER;
 ...
 */
void move_bc(before, control, ballx, bally, chainx, chainy)
 ...
 * and chain.
 */
boolean drag_ball(x, y, bc_control, ballx, bally, chainx, chainy, cause_delay,
        allow_drag)
    xchar x, y;int *bc_control;xchar *ballx, *bally, *chainx, *chainy;boolean *cause_delay;boolean allow_drag;
{
...
#undef CHAIN_IN_MIDDLE
    ...
    *bc_control = BC_BALL | BC_CHAIN;
    ...
    else
    {
         ...
         */
        if (dist2(x, y, uchain->ox, uchain->oy)
                == 4&& !IS_CHAIN_ROCK(newchainx, newchainy))
        {
