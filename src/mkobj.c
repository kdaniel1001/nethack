...
FDECL(save_mtraits, (struct obj *, struct monst *));
    ...
    struct obj *o;
{
    ...
    if (x)
    {
        ...
        if (x->omonst) free_omonst(o); /* 'o' rather than 'x' */
...
void new_omailcmd(otmp, response_cmd)
    struct obj *otmp;const char *response_cmd;
{
    if (!otmp->oextra) otmp->oextra = newoextra();
...
struct obj * mkobj_at(let, x, y, artif)
    char let;int x, y;boolean artif;
{
    struct obj *otmp;
...
struct obj * mksobj_at(otyp, x, y, init, artif)
    int otyp, x, y;boolean init, artif;
{
    struct obj *otmp;
...
struct obj * mkobj(oclass, artif)
    char oclass;boolean artif;
{
...
STATIC_OVL void mkbox_cnts(box)
    struct obj *box;
{
    register int n;
    ...
    struct obj *obj2, *obj1;
{
    ...
    if (has_omonst(obj1))
    {
        ...
        (void) memcpy((genericptr_t) OMONST(obj2), (genericptr_t) OMONST(obj1),
                sizeof(struct monst));
        ...
        OMONST(obj2)->m_id = context.ident++;
#endif
        if (OMONST(obj1)->mextra) copy_mextra(OMONST(obj2), OMONST(obj1));
    }
    if (has_omid(obj1))
    {
 ...
 */
struct obj * splitobj(obj, num)
    struct obj *obj;long num;
{
    struct obj *otmp;

    if (obj->cobj || num <= 0L || obj->quan <= num) panic("splitobj"); /* can't split containers */
    otmp = newobj();
    ...
    struct obj *obj;
{
    ...
    if (obj->o_id == context.objsplit.child_oid)
    {
        ...
        if (obj->nobj && obj->nobj->o_id == target_oid) oparent = obj->nobj;
    }
    else if (obj->o_id == context.objsplit.parent_oid)
    {
        ...
        if (obj->nobj && obj->nobj->o_id == target_oid) ochild = obj->nobj;
    }
...
{ "cancel", "drain", "uncharge", "unbless", "uncurse", "disenchant", "degrade",
        "dilute", "erase", "burn", "neutralize", "destroy", "splatter", "bite",
        "open", "break the lock on", "rust", "rot", "tarnish" };

/* possibly bill for an object which the player has just modified */
void costly_alteration(obj, alter_type)
    struct obj *obj;int alter_type;
{
    xchar ox, oy;
    char objroom;
    boolean set_bknown;
    ...
    case OBJ_INVENT:
        ...
        verbalize("You %s %s %s, you pay for %s!", alteration_verbs[alter_type],
                those, simpleonames(obj), them);
...
struct obj * mksobj(otyp, init, artif)
    int otyp;boolean init;boolean artif;
{
    int mndx, tryct;
    struct obj *otmp;
    ...
    *otmp = zeroobj;
    ...
    if ((otmp->otyp >= ELVEN_SHIELD && otmp->otyp <= ORCISH_SHIELD)
            || otmp->otyp == SHIELD_OF_REFLECTION) otmp->dknown = 0;
    ...
    if (init)
        ...
        case POTION_CLASS:
            otmp->fromsink = 0;
            if (otmp->otyp == POT_OIL) otmp->age = MAX_OIL_IN_FLASK; /* amount of oil */
            /* fall through */
        case SCROLL_CLASS:
#ifdef MAIL
...
case RING_CLASS:
    if (objects[otmp->otyp].oc_charged)
    {
        ...
        if (otmp->spe < 0 && rn2(5)) curse(otmp);
            ...
            && (otmp->otyp == RIN_TELEPORTATION || otmp->otyp == RIN_POLYMORPH
 ...
 */
void start_corpse_timeout(body)
    struct obj *body;
{
...
STATIC_OVL void maybe_adjust_light(obj, old_range)
    struct obj *obj;int old_range;
{
    char buf[BUFSZ];
    xchar ox, oy;
    ...
    register struct obj *otmp;
{
    ...
    if (otmp == uswapwep && u.twoweap) drop_uswapwep();
...
void blessorcurse(otmp, chance)
    register struct obj *otmp;register int chance;
{
    ...
    register struct obj *obj;
{
    ...
    if (SchroedingersBox(obj)) wt += mons[PM_HOUSECAT].cwt;
...
struct obj * mkcorpstat(objtype, mtmp, ptr, x, y, corpstatflags)
    ...
    struct monst *mtmp;struct permonst *ptr;int x, y;unsigned corpstatflags;
{
    register struct obj *otmp;
 ...
 */
struct obj * obj_attach_mid(obj, mid)
    struct obj *obj;unsigned mid;
{
...
static struct obj * save_mtraits(obj, mtmp)
    struct obj *obj;struct monst *mtmp;
{
 ...
 */
struct monst * get_mtraits(obj, copyof)
    struct obj *obj;boolean copyof;
{
...
struct obj * mk_tt_object(objtype, x, y)
    int objtype; /* CORPSE or STATUE */
    register int x, y;
{
...
struct obj * mk_named_object(objtype, ptr, x, y, nm)
    int objtype; /* CORPSE or STATUE */
    struct permonst *ptr;int x, y;const char *nm;
{
    struct obj *otmp;
...
void place_object(otmp, x, y)
    register struct obj *otmp;int x, y;
{
 ...
 */
void obj_ice_effects(x, y, do_buried)
    int x, y;boolean do_buried;
{
    struct obj *otmp;
 ...
 * restarted etc.
 */
long peek_at_iced_corpse_age(otmp)
    struct obj *otmp;
{
    ...
    if (otmp->otyp == CORPSE && otmp->on_ice)
    {
        ...
        debugpline3(
                ...
                s_suffix(doname(otmp)), otmp->age, retval);debugpline1("Effective age of corpse: %ld.", monstermoves - retval);
    }
    return retval;
}

STATIC_OVL void obj_timer_checks(otmp, x, y, force)
    struct obj *otmp;xchar x, y;int force; /* 0 = no force so do checks, <0 = force off, >0 force on */
{
    long tleft = 0L;
...
void extract_nobj(obj, head_ptr)
 ...
 */
void extract_nexthere(obj, head_ptr)
    struct obj *obj, **head_ptr;
{
    struct obj *curr, *prev;
 ...
 */
int add_to_minv(mon, obj)
    struct monst *mon;struct obj *obj;
{
    struct obj *otmp;
 ...
 */
struct obj * add_to_container(container, obj)
...
STATIC_OVL void container_weight(container)
 ...
 */
void dealloc_obj(obj)
    struct obj *obj;
{
    if (obj->where != OBJ_FREE) panic("dealloc_obj: obj not free");
    if (obj->nobj) panic("dealloc_obj with nobj");
    if (obj->cobj) panic("dealloc_obj with cobj");
     ...
     */
    if (obj_sheds_light(obj)) del_light_source(LS_OBJECT, obj_to_any(obj));
...
int hornoplenty(horn, tipping)
    struct obj *horn;boolean tipping; /* caller emptying entire contents; affects shop handling */
{
    int objcount = 0;
    ...
    else
    {
        ...
        else
        {
            ...
            if (obj->otyp == FOOD_RATION && !rn2(7)) obj->otyp =
            LUMP_OF_ROYAL_JELLY;
            what = "Some food";
        }
        ++objcount;
        ...
        if (horn->unpaid) addtobill(obj, FALSE, FALSE, tipping);
        ...
        else
        {
            ...
            else
            {
                if (IS_ALTAR(levl[u.ux][u.uy].typ))
                    ...
                    pline("%s %s to the %s.", Doname2(obj), otense(obj, "drop"),
                            surface(u.ux, u.uy));
                dropy(obj);
            }
        }
...
ofmt0[] = "%s obj %s %s: %s", ofmt3[] = "%s [not null] %s %s: %s",
...
STATIC_OVL void objlist_sanity(objlist, wheretype, mesg)
    ...
    for (obj = objlist; obj; obj = obj->nobj)
    {
        ...
        if (Has_contents(obj))
        {
            if (wheretype == OBJ_ONBILL)
                ...
                insane_object(obj, "%s obj contains something! %s %s: %s", mesg,
                        (struct monst *) 0);
...
STATIC_OVL void mon_obj_sanity(monlist, mesg)
...
{ "free", "floor", "contained", "invent", "minvent", "migrating", "buried",
        "onbill" };

STATIC_OVL const char * where_name(obj)
    struct obj *obj;
{
...
STATIC_OVL void insane_object(obj, fmt, mesg, mon)
    struct obj *obj;const char *fmt, *mesg;struct monst *mon;
{
    ...
    if (mon || (strstri(mesg, "minvent") && !strstri(mesg, "contained")))
    {
        ...
        if (mon) monnm = x_monnam(mon, ARTICLE_A, (char *) 0, EXACT_NAME, TRUE);
        pline(altfmt, mesg, fmt_ptr((genericptr_t) obj), where_name(obj), objnm,
...
STATIC_OVL void check_contained(container, mesg)
...
STATIC_OVL void sanity_check_worn(obj)
    struct obj *obj;
{
 ...
 */
struct obj * obj_nexto_xy(otyp, x, y, oid)
    int otyp, x, y;unsigned oid;
{
    struct obj *otmp;
    int fx, fy, ex, ey;
    short dx, dy;
 ...
 * is returned.
 */
struct obj * obj_absorb(obj1, obj2)
