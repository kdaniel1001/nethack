...
{
/* 0*/
{ ' ', "dark part of a room", C(NO_COLOR) }, /* stone */
...
void init_showsyms()
{
    ...
    {
        if (i == SYM_BOULDER)
            showsyms[i + SYM_OFF_X] =
                    iflags.bouldersym ?
                            iflags.bouldersym : def_oc_syms[ROCK_CLASS].sym;
        else if (i == SYM_INVISIBLE) showsyms[i + SYM_OFF_X] = DEF_INVISIBLE;
    }
}
    ...
    for (i = 0; i < MAXOTHER; i++)
    {
        if (i == SYM_BOULDER)
            l_syms[i + SYM_OFF_X] =
                    iflags.bouldersym ?
                            iflags.bouldersym : def_oc_syms[ROCK_CLASS].sym;
        else if (i == SYM_INVISIBLE) l_syms[i + SYM_OFF_X] = DEF_INVISIBLE;
    }
    ...
    for (i = 0; i < MAXOTHER; i++)
    {
        if (i == SYM_BOULDER)
            r_syms[i + SYM_OFF_X] =
                    iflags.bouldersym ?
                            iflags.bouldersym : def_oc_syms[ROCK_CLASS].sym;
        else if (i == SYM_INVISIBLE) r_syms[i + SYM_OFF_X] = DEF_INVISIBLE;
    }
...
void switch_symbols(nondefault)
    int nondefault;
{
    register int i;
...
void update_l_symset(symp, val)
    struct symparse *symp;int val;
{
    l_syms[symp->idx] = val;
}

void update_r_symset(symp, val)
    struct symparse *symp;int val;
{
    r_syms[symp->idx] = val;
}

void clear_symsetentry(which_set, name_too)
    int which_set;boolean name_too;
{
    if (symset[which_set].desc) free((genericptr_t) symset[which_set].desc);
