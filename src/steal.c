...
FDECL(equipname, (struct obj *));

STATIC_OVL const char * equipname(otmp)
...
STATIC_PTR int stealarm(VOID_ARGS)
{
    register struct monst *mtmp;
    register struct obj *otmp;
        ...
        if (otmp->o_id == stealoid)
        {
            for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
            {
                if (mtmp->m_id == stealmid)
                {
                    ...
                    goto botm;
                    if (otmp->unpaid) subfrombill(otmp, shop_keeper(*u.ushops));
                    freeinv(otmp);
...
void remove_worn_item(obj, unchain_ball)
    struct obj *obj;boolean unchain_ball; /* whether to unpunish or just unwield */
{
 ...
 */
int steal(mtmp, objnambuf)
    struct monst *mtmp;char *objnambuf;
{
    struct obj *otmp;
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj)
        if ((!uarm || otmp != uarmc)
                && otmp != uskin&& otmp->oclass != COIN_CLASS)
    ...
    gotobj: if (otmp->o_id == stealoid) return 0;
    ...
    if (monkey_business)
    {
        ...
        if (ostuck || can_carry(mtmp, otmp) == 0)
        {
            ...
            cant_take: pline("%s tries to %s %s%s but gives up.", Monnam(mtmp),
                    how[rn2(SIZE(how))],
                    (otmp->owornmask & W_ARMOR) ? "your " : "",
                    (otmp->owornmask & W_ARMOR) ?
                            equipname(otmp) : yname(otmp));
    ...
    if (otmp->owornmask & (W_ARMOR | W_ACCESSORY))
    {
        ...
        case ARMOR_CLASS:
            ...
            if (monkey_business)
            {
                ...
                if (armordelay >= 1 && !olddelay && rn2(10)) goto cant_take;
                remove_worn_item(otmp, TRUE);
                break;
            }
            else
            {
                ...
                if (flags.female)
                    pline("%s charms you.  You gladly %s your %s.",
                            ...
                            !slowly ? "hand over" :
                            was_doffing ?
                                    "continue removing" : "start removing",
                            equipname(otmp));
                else
                    pline("%s seduces you and %s off your %s.",
                            ...
                            !slowly ? "you take" :
                            was_doffing ?
                                    "you continue taking" : "you start taking",
                            equipname(otmp));
                named++;
    ...
    else if (otmp->owornmask) remove_worn_item(otmp, TRUE);
...
void maybe_absorb_item(mon, obj, ochance, achance)
    struct monst *mon;struct obj *obj;int ochance, achance; /* percent chance for ordinary item, artifact */
{
    if (obj == uball || obj == uchain || obj->oclass == ROCK_CLASS
            || obj_resists(obj, 100 - ochance, 100 - achance)
            || !touch_artifact(obj, mon)) return;
    ...
    else
    {
        ...
        if (canspotmon(mon)) pline("%s absorbs %s!", Monnam(mon), yname(obj));
    }
...
void mdrop_obj(mon, obj, verbosely)
    struct monst *mon;struct obj *obj;boolean verbosely;
{
...
void relobj(mtmp, show, is_pet)
    struct monst *mtmp;int show;boolean is_pet; /* If true, pet should keep wielded/worn items */
{
    struct obj *otmp;
    ...
    {
        if (canspotmon(mtmp)) pline("%s gold %s.", s_suffix(Monnam(mtmp)),
