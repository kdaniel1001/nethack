...
STATIC_DCL branch *
FDECL(add_branch, (int, int, struct proto_dungeon *));
...
STATIC_DCL void FDECL(traverse_mapseenchn, (BOOLEAN_P, winid,
                int, int, int *));
STATIC_DCL const char *
FDECL(seen_string, (XCHAR_P, const char *));
STATIC_DCL const char *
FDECL(br_string2, (branch *));
STATIC_DCL const char *
FDECL(endgamelevelname, (char *, int));
STATIC_DCL const char *
FDECL(shop_string, (int));
STATIC_DCL char *
FDECL(tunesuffix, (mapseen *, char *));
...
void save_dungeon(fd, perform_write, free_data)
    int fd;boolean perform_write, free_data;
{
    branch *curr, *next;
    ...
    if (free_data)
    {
        ...
        for (curr_ms = mapseenchn; curr_ms; curr_ms = next_ms)
        {
            ...
            if (curr_ms->custom) free((genericptr_t) curr_ms->custom);
...
static void Fread(ptr, size, nitems, stream)
    genericptr_t ptr;int size, nitems;
    dlb *stream;
{
    int cnt;
...
STATIC_OVL int find_branch(s, pd)
 ...
 */
STATIC_OVL xchar parent_dnum(s, pd)
    const char *s; /* dungeon name */
    struct proto_dungeon *pd;
{
    int i;
 ...
 *       end of the dungeon.
 */
STATIC_OVL int level_range(dgn, base, randc, chain, pd, adjusted_base)
    xchar dgn;int base, randc, chain;struct proto_dungeon *pd;int *adjusted_base;
{
...
STATIC_OVL xchar parent_dlevel(s, pd)
    const char *s;struct proto_dungeon *pd;
{
    ...
    num = level_range(dnum, pd->tmpbranch[i].lev.base,
            pd->tmpbranch[i].lev.rand, pd->tmpbranch[i].chain, pd, &base);
...
STATIC_OVL int correct_branch_type(tbr)
 ...
 */
void insert_branch(new_branch, extract_first)
    branch *new_branch;boolean extract_first;
{
    branch *curr, *prev;
...
STATIC_OVL branch * add_branch(dgn, child_entry_level, pd)
    int dgn;int child_entry_level;struct proto_dungeon *pd;
{
 ...
 * last entry.
 */
STATIC_OVL void add_level(new_lev)
    s_level *new_lev;
{
    s_level *prev, *curr;
...
STATIC_OVL void init_level(dgn, proto_index, pd)
    int dgn, proto_index;struct proto_dungeon *pd;
{
    s_level *new_level;
    ...
    if (!new_level->flags.align)
        new_level->flags.align = ((pd->tmpdungeon[dgn].flags & D_ALIGN_MASK)
                >> 4);
...
STATIC_OVL int possible_places(idx, map, pd)
...
STATIC_OVL xchar pick_level(map, nth)
 ...
 */
STATIC_OVL boolean place_level(proto_index, pd)
    int proto_index;struct proto_dungeon *pd;
{
...
void init_dungeons()
{
    ...
    for (i = 0; i < n_dgns; i++)
    {
        ...
        if (dungeons[i].num_dunlevs > MAXLEVEL) dungeons[i].num_dunlevs =
        MAXLEVEL;
...
xchar deepest_lev_reached(noquest)
    boolean noquest;
{
    ...
    xchar ledgerno;
{
    ...
    for (i = 0; i < n_dgns; i++)
        if (dungeons[i].ledger_start < ledgerno
                && ledgerno
                        <= dungeons[i].ledger_start + dungeons[i].num_dunlevs)
            return (xchar) i;
    ...
    int upflag;
{
    ...
    if (was_in_W_tower && On_W_tower_level(&u.uz))
        ...
        place_lregion(updest.lx, updest.ly, updest.hx, updest.hy, updest.nlx,
                updest.nly, updest.nhx, updest.nhy, LR_UPTELE, (d_level *) 0);
    else
        place_lregion(dndest.lx, dndest.ly, dndest.hx, dndest.hy, dndest.nlx,
                dndest.nly, dndest.nhx, dndest.nhy, LR_DOWNTELE, (d_level *) 0);
}

/* place you on the special staircase */
void u_on_sstairs(upflag)
    int upflag;
{
    if (sstairs.sx)
 ...
 */
void get_level(newlevel, levnum)
    d_level *newlevel;int levnum;
{
    branch *br;
    xchar dgn = u.uz.dnum;
...
/* go directly to hell... */
void goto_hell(at_stairs, falling)
...
/* dest = src + rn1(range) */
void assign_rnd_level(dest, src, range)
    d_level *dest, *src;int range;
{
    dest->dnum = src->dnum;
...
int induced_align(pct)
    int pct;
{
    s_level *lev = Is_special(&u.uz);
    aligntyp al;

    if (lev && lev->flags.align) if (rn2(100) < pct) return lev->flags.align;
    ...
    if ((slev = find_level(nam)) != 0)
    {
        ...
        || (u.uz.dnum == valley_level.dnum && dlev.dnum == medusa_level.dnum)
                || (u.uz.dnum == medusa_level.dnum
                        && dlev.dnum == valley_level.dnum)) && (/* either wizard mode or else seen and not forgotten */
...
STATIC_OVL boolean unplaced_floater(dptr)
    struct dungeon *dptr;
{
    branch *br;
...
STATIC_OVL boolean unreachable_level(lvl_p, unplaced)
    d_level *lvl_p;boolean unplaced;
{
    s_level *dummy;
...
static void tport_menu(win, entry, lchoices, lvl_p, unreachable)
    winid win;char *entry;struct lchoice *lchoices;d_level *lvl_p;boolean unreachable;
{
    char tmpbuf[BUFSZ];
    anything any;
...
STATIC_OVL const char * br_string(type)
    int type;
{
    switch (type)
    {
...
STATIC_OVL void print_branch(win, dnum, lower_bound, upper_bound, bymenu,
        lchoices_p)
    winid win;int dnum;int lower_bound;int upper_bound;boolean bymenu;struct lchoice *lchoices_p;
{
    branch *br;
    char buf[BUFSZ];
...
schar print_dungeon(bymenu, rlev, rdgn)
    boolean bymenu;schar *rlev;xchar *rdgn;
{
 ...
 */
void recbranch_mapseen(source, dest)
    d_level *source;d_level *dest;
{
    mapseen *mptr;
    branch *br;
    ...
    {
        impossible("Can't note branch for unseen level (%d, %d)", source->dnum,
                source->dlevel);
    }
}

char * get_annotation(lev)
    d_level *lev;
{
    mapseen *mptr;
...
STATIC_OVL mapseen * find_mapseen(lev)
    d_level *lev;
{
    mapseen *mptr;
...
void forget_mapseen(ledger_num)
    int ledger_num;
{
    mapseen *mptr;
    struct cemetery *bp;
...
STATIC_OVL void save_mapseen(fd, mptr)
    int fd;mapseen *mptr;
{
    branch *curr;
    int brindx;
...
STATIC_OVL mapseen * load_mapseen(fd)
    int fd;
{
    ...
    for (mptr = mapseenchn, prev = 0; mptr; prev = mptr, mptr = mptr->next)
        if (mptr->lev.dnum > init->lev.dnum
                || (mptr->lev.dnum == init->lev.dnum
                        && mptr->lev.dlevel > init->lev.dlevel)) break;
    if (!prev)
    {
...
STATIC_OVL boolean interest_mapseen(mptr)
    mapseen *mptr;
{
    ...
    if (mptr->flags.oracle || mptr->flags.bigroom || mptr->flags.roguelevel
            ...
            || mptr->flags.quest_summons || mptr->flags.questing) return TRUE;
    ...
    if (In_endgame(&u.uz)) return (boolean) In_endgame(&mptr->lev);
...
void recalc_mapseen()
{
    ...
    else if (mptr->flags.forgot) mptr->flags.bigroom = 0;
    ...
    for (x = 1; x < COLNO; x++)
    {
        for (y = 0; y < ROWNO; y++)
        {
            if (cansee(x, y) || (x == u.ux && y == u.uy && !Levitation))
            {
                ...
                if ((mtmp = m_at(x, y))
                        != 0&& mtmp->m_ap_type == M_AP_FURNITURE && canseemon(mtmp))
            ...
            case ALTAR:
                atmp = (Is_astralevel(&u.uz)
                        ...
                        MSA_NONE : Amask2msa(levl[x][y].altarmask);
                if (!mptr->feat.naltar)
                    mptr->feat.msalign = atmp;
                else if (mptr->feat.msalign != atmp) mptr->feat.msalign =
                MSA_NONE;
            ...
            case DRAWBRIDGE_DOWN:
                ...
                else if (Is_knox(&u.uz)) mptr->flags.ludios = 1;
                break;
            default:
                break;
            }
    ...
    struct monst *priest UNUSED; /* currently unused; might be useful someday */
{
    ...
    if (Is_valley(&u.uz))
        mptr->flags.valley = 1;
    else if (Is_sanctum(&u.uz)) mptr->flags.msanctum = 1;
}

/* room entry message has just been delivered so learn room even if blind */
void room_discovered(roomno)
    int roomno;
{
    mapseen *mptr = find_mapseen(&u.uz);
...
STATIC_OVL void traverse_mapseenchn(viewendgame, win, why, reason, lastdun_p)
    boolean viewendgame;winid win;int why, reason, *lastdun_p;
{
    mapseen *mptr;
    boolean showheader;
...
STATIC_OVL const char * seen_string(x, obj)
    xchar x;const char *obj;
{
...
/* better br_string */
STATIC_OVL const char * br_string2(br)
    branch *br;
{
...
STATIC_OVL const char * endgamelevelname(outbuf, indx)
    char *outbuf;int indx;
{
...
STATIC_OVL const char * shop_string(rtype)
    int rtype;
{
...
STATIC_OVL char * tunesuffix(mptr, outbuf)
    mapseen *mptr;char *outbuf;
{
    *outbuf = '\0';
...
STATIC_OVL void print_mapseen(win, mptr, final, how, printdun)
    winid win;mapseen *mptr;int final; /* 0: not final; 1: game over, alive; 2: game over, dead */
    ...
    boolean printdun;
{
    ...
    if (printdun)
    {
        ...
        || In_endgame(&mptr->lev))
            ...
            Sprintf (buf, "%s: levels %d up to %d", dungeons[dnum].dname,
    ...
    if (mptr->final_resting_place || final)
    {
        ...
        if (kncnt)
        {
            ...
            for (bp = mptr->final_resting_place; bp; bp = bp->next)
            {
                ...
                {
                    Sprintf (buf, "%s%s%s, %s%c", PREFIX, TAB, bp->who, bp->how,
                            --kncnt ? ',' : '.');
                    putstr(win, 0, buf);
                }
            }
