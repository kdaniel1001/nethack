...
STATIC_OVL
void cursetxt(mtmp, undirected)
    struct monst *mtmp;boolean undirected;
{
...
STATIC_OVL int choose_magic_spell(spellval)
    int spellval;
{
...
STATIC_OVL int choose_clerical_spell(spellnum)
    int spellnum;
{
 ...
 */
int castmu(mtmp, mattk, thinks_it_foundyou, foundyou)
    register struct monst *mtmp;register struct attack *mattk;boolean thinks_it_foundyou;boolean foundyou;
{
 ...
 */
STATIC_OVL
void cast_wizard_spell(mtmp, dmg, spellnum)
    struct monst *mtmp;int dmg;int spellnum;
{
...
STATIC_OVL
void cast_cleric_spell(mtmp, dmg, spellnum)
    struct monst *mtmp;int dmg;int spellnum;
{
...
STATIC_DCL
boolean is_undirected_spell(adtyp, spellnum)
...
STATIC_DCL
boolean spell_would_be_useless(mtmp, adtyp, spellnum)
    struct monst *mtmp;unsigned int adtyp;int spellnum;
{
    ...
    if (adtyp == AD_SPEL)
    {
        ...
        if (mtmp->mpeaceful
                && (spellnum == MGC_AGGRAVATION || spellnum == MGC_SUMMON_MONS
                        || spellnum == MGC_CLONE_WIZ)) return TRUE;
