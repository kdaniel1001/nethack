...
void wipeout_text(engr, cnt, seed)
    char *engr;int cnt;unsigned seed; /* for semi-controlled randomization */
{
    char *s;
...
boolean can_reach_floor(check_pit)
    boolean check_pit;
{
    struct trap *t;
    ...
    if (check_pit && !Flying && (t = t_at(u.ux, u.uy)) != 0
            && uteetering_at_seen_pit(t)) return FALSE;

    return (boolean) ((!Levitation || Is_airlevel(&u.uz) || Is_waterlevel(&u.uz))
            && (!u.uundetected || !is_hider(youmonst.data)
                    || u.umonnum == PM_TRAPPER));
}

/* give a message after caller has determined that hero can't reach */
void cant_reach_floor(x, y, up, check_pit)
    int x, y;boolean up, check_pit;
{
 ...
 */
int sengr_at(s, x, y, strict)
    const char *s;xchar x, y;boolean strict;
{
    ...
    {
        return strict ?
                (strncmpi(ep->engr_txt, s, strlen(s)) == 0) :
                (strstri(ep->engr_txt, s) != 0);
    }
    return FALSE;
}

void u_wipe_engr(cnt)
    int cnt;
{
    if (can_reach_floor(TRUE)) wipe_engr_at(u.ux, u.uy, cnt, FALSE);
}

void wipe_engr_at(x, y, cnt, magical)
...
void make_engr_at(x, y, s, e_time, e_type)
    int x, y;const char *s;long e_time;xchar e_type;
{
    struct engr *ep;
...
int doengrave()
{
    ...
    maxelen = BUFSZ - 1;
    if (is_demon(youmonst.data) || youmonst.data->mlet == S_VAMPIRE) type =
    ENGR_BLOOD;
    ...
    case WAND_CLASS:
        if (zappable(otmp))
        {
            ...
            case WAN_FIRE:
                ...
                Strcpy (post_engr_text,
                        Blind ? "You feel the wand heat up." : "Flames fly from the wand.");
                break;
            case WAN_LIGHTNING:
                ptext = TRUE;
                type = BURN;
        ...
        case TOWEL:
            ...
            if (oep)
                if (oep->engr_type == DUST || oep->engr_type == ENGR_BLOOD
                        || oep->engr_type == MARK)
                {
                    if (is_wet_towel(otmp)) dry_a_towel(otmp, -1, TRUE);
                    if (!Blind)
 ...
 */
void make_grave(x, y, str)
    int x, y;const char *str;
{
    char buf[BUFSZ];
    ...
    del_engr_at(x, y);
    if (!str) str = get_rnd_text(EPITAPHFILE, buf);
