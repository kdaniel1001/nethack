...
STATIC_DCL char *
FDECL(spellretention, (int, char *));
STATIC_DCL int NDECL(throwspell);
STATIC_DCL void NDECL(cast_protection);
STATIC_DCL void FDECL(spell_backfire, (int));
STATIC_DCL const char *
FDECL(spelltypemnemonic, (int));
...
STATIC_OVL int spell_let_to_idx(ilet)
    char ilet;
{
    int indx;
...
STATIC_OVL boolean cursed_book(bp)
    struct obj *bp;
{
...
STATIC_OVL boolean confused_book(spellbook)
    struct obj *spellbook;
{
    ...
    if (!rn2(3) && spellbook->otyp != SPE_BOOK_OF_THE_DEAD)
    {
        ...
        if (!objects[spellbook->otyp].oc_name_known
                && !objects[spellbook->otyp].oc_uname) docall(spellbook);
        useup(spellbook);
        gone = TRUE;
    }
    else
...
STATIC_OVL void deadbook(book2)
    struct obj *book2;
{
    ...
    if (invocation_pos(u.ux, u.uy) && !On_stairs(u.ux, u.uy))
    {
        ...
        { /* at least one artifact not prepared properly */
    ...
    if (book2->cursed)
    {
        ...
        NO_MINVENT)) != 0 || (mtmp = makemon(&mons[PM_NALFESHNEE], u.ux, u.uy,
        NO_MINVENT)) != 0))
        {
            mtmp->mpeaceful = 0;
            set_malign(mtmp);
...
STATIC_PTR int learn(VOID_ARGS)
{
    int i;
    short booktype;
    char splname[BUFSZ];
    ...
    { /* not if (context.spbook.delay++), so at end
     delay == 0 */
        context.spbook.delay++;
        return 1; /* still busy */
    }
    ...
    register struct obj *spellbook;
{
    ...
    if (context.spbook.book) context.spbook.o_id = context.spbook.book->o_id;
...
void book_substitution(old_obj, new_obj)
    struct obj *old_obj, *new_obj;
{
    if (old_obj == context.spbook.book)
    {
        context.spbook.book = new_obj;
        if (context.spbook.book) context.spbook.o_id =
                context.spbook.book->o_id;
    }
}
...
STATIC_OVL boolean rejectcasting()
{
 ...
 */
STATIC_OVL boolean getspell(spell_no)
    int *spell_no;
{
    int nspells, idx;
    ...
    if (rejectcasting()) return FALSE; /* no spell chosen */
...
STATIC_OVL const char * spelltypemnemonic(skill)
    int skill;
{
    switch (skill)
    {
...
STATIC_OVL void spell_backfire(spell)
    int spell;
{
...
STATIC_OVL boolean spellsortmenu()
{
    winid tmpwin;
    menu_item *selected;
    anything any;
...
STATIC_OVL boolean dospellmenu(prompt, splaction, spell_no)
    const char *prompt;int splaction; /* SPELLMENU_CAST, SPELLMENU_VIEW, or spl_book[] index */
    int *spell_no;
{
    winid tmpwin;
    int i, n, how, splnum;
...
STATIC_OVL int percent_success(spell)
    int spell;
{
    ...
    else if (uarmc && uarmc->otyp == ROBE) splcaster -= urole.spelarmr;
...
STATIC_OVL char * spellretention(idx, outbuf)
    int idx;char *outbuf;
{
