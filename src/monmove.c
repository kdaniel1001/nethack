    ...
    struct monst *mtmp;
{
    if (flags.verbose)
    {
        ...
        else if (!Deaf) You_hear("a distant explosion.");
    }
...
boolean monhaskey(mon, for_unlocking)
    struct monst *mon;boolean for_unlocking; /* true => credit card ok, false => not ok */
{
    if (for_unlocking && m_carrying(mon, CREDIT_CARD)) return TRUE;
    return m_carrying(mon, SKELETON_KEY) || m_carrying(mon, LOCK_PICK);
}

void mon_yells(mon, shout)
    struct monst *mon;const char *shout;
{
    if (canspotmon(mon))
...
STATIC_OVL void watch_on_duty(mtmp)
...
boolean onscary(x, y, mtmp)
    int x, y;struct monst *mtmp;
{
    ...
    return (epresent && ((u.ux == x && u.uy == y) || (Displaced&& mtmp->mux == x && mtmp->muy == y))
...
void mon_regen(mon, digest_meal)
    struct monst *mon;boolean digest_meal;
{
...
STATIC_OVL void release_hero(mon)
    struct monst *mon;
{
    if (mon == u.ustuck)
    {
...
void monflee(mtmp, fleetime, first, fleemsg)
    struct monst *mtmp;int fleetime;boolean first;boolean fleemsg;
{
    ...
    if (!first || !mtmp->mflee)
    {
        ...
        if (!mtmp->mflee && fleemsg
                && canseemon(mtmp) && mtmp->m_ap_type != M_AP_FURNITURE
                && mtmp->m_ap_type != M_AP_OBJECT)
        {
...
STATIC_OVL void distfleeck(mtmp, inrange, nearby, scared)
    register struct monst *mtmp;int *inrange, *nearby, *scared;
{
    ...
    *nearby = *inrange && monnear(mtmp, mtmp->mux, mtmp->muy);
    ...
    if (*nearby
            && (sawscary || (!mtmp->mpeaceful && in_your_sanctuary(mtmp, 0, 0))))
    {
        *scared = 1;
    ...
    register struct monst *mtmp;
{
    ...
    if ((mtmp->mstrategy & STRAT_WAITFORU)
            && (m_canseeu(mtmp) || mtmp->mhp < mtmp->mhpmax)) mtmp->mstrategy &=
            ~STRAT_WAITFORU;
    ...
    else if (is_mind_flayer(mdat) && !rn2(20))
    {
        ...
        else if (!u.uinvulnerable)
        {
            ...
            {
                int dmg;
                pline("It locks on to your %s!", m_sen ? "telepathy" :
            ...
            || (!mtmp->mcansee && !rn2(4)) || mtmp->mpeaceful)
    {
        ...
        tmp = m_move(mtmp, 0);
        if (tmp != 2) distfleeck(mtmp, &inrange, &nearby, &scared); /* recalc */
        ...
        case 3: /* absolutely no movement */
            ...
            if (mtmp->isgd && (mtmp->mhp < 1 || !mtmp->mx == 0)) return 1; /* behave as if it died */
        ...
        case 1: /* monster moved */
            ...
            if (!nearby && (ranged_attk(mdat) || find_offensive(mtmp))) break;
...
{ AMULET_CLASS, POTION_CLASS,
SCROLL_CLASS, WAND_CLASS,
RING_CLASS, SPBOOK_CLASS, 0 };
static NEARDATA const char indigestion[] =
{ BALL_CLASS, ROCK_CLASS, 0 };
static NEARDATA const char boulder_class[] =
{ ROCK_CLASS, 0 };
static NEARDATA const char gem_class[] =
{ GEM_CLASS, 0 };

boolean itsstuck(mtmp)
    register struct monst *mtmp;
{
 ...
 */
boolean should_displace(mtmp, poss, info, cnt, gx, gy)
    struct monst *mtmp;coord *poss; /* coord poss[9] */
    ...
    int cnt;xchar gx, gy;
{
    ...
    if (shortest_with_displacing > -1
            && (shortest_with_displacing < shortest_without_displacing
                    || !count_without_displacing)) return TRUE;
    return FALSE;
}
 ...
 */
int m_move(mtmp, after)
    register struct monst *mtmp;register int after;
{
    register int appr;
    ...
    if (hides_under(ptr) && OBJ_AT(mtmp->mx, mtmp->my) && rn2(10)) return 0; /* do not leave hiding place */
     ...
     */
    if (!Is_rogue_level(&u.uz)) can_tunnel = tunnels(ptr);
    ...
    if (is_covetous(ptr))
    {
        xchar tx = STRAT_GOALX(mtmp->mstrategy), ty = STRAT_GOALY(
                mtmp->mstrategy);
    ...
    {
        if (!Deaf && canseemon(mtmp)) verbalize("I'm late!");
        mongone(mtmp);
        return 2;
    }
#endif
    ...
    not_special: if (u.uswallow && !mtmp->mflee && u.ustuck != mtmp) return 1;
    omx = mtmp->mx;
    omy = mtmp->my;
    gx = mtmp->mux;
    gy = mtmp->muy;
...
#define SQSRCHRADIUS 5

    {
        ...
        {
            look_for_obj: oomx = min(COLNO - 1, omx + minr);
    ...
    if (mmoved)
    {
        ...
        if ((info[chi] & ALLOW_MDISP))
        {
            ...
            if ((mstatus & MM_AGR_DIED) || (mstatus & MM_DEF_DIED)) return 2;
    ...
    postmov: if (mmoved == 1 || mmoved == 3)
    {
        ...
        if (mmoved == 1)
        {
            ...
            { /* taken care of below */
                ...
                boolean btrapped = (here->doormask & D_TRAPPED), observeit =
                        canseeit && canspotmon(mtmp);
                ...
                else if (here->doormask & D_LOCKED && can_unlock)
                {
                    ...
                    else
                    {
                        if (flags.verbose)
                        {
                            ...
                            else if (!Deaf) You_hear("a door unlock and open.");
                        }
                    ...
                    else
                    {
                        if (flags.verbose)
                        {
                            if (observeit)
                                ...
                                You_see("a door open.");
                            else if (!Deaf) You_hear("a door open.");
                        }
                        here->doormask = D_ISOPEN;
                ...
                else if (here->doormask & (D_LOCKED | D_CLOSED))
                {
                    ...
                    else
                    {
                        if (flags.verbose)
                        {
                            ...
                            else if (!Deaf) You_hear("a door crash open.");
                        }
            ...
            /* possibly dig */
            if (can_tunnel && mdig_tunnel(mtmp)) return 2; /* mon died (position already updated) */
 ...
 * location however.
 */
boolean undesirable_disp(mtmp, x, y)
    struct monst *mtmp;xchar x, y;
{
 ...
 */
STATIC_OVL boolean stuff_prevents_passage(mtmp)
    struct monst *mtmp;
{
    ...
    for (obj = chain; obj; obj = obj->nobj)
    {
        ...
        if (obj->oclass != GEM_CLASS && !(typ >= ARROW && typ <= BOOMERANG)
                ...
                && !is_cloak(
                        ...
                        && typ != LOCK_PICK) return TRUE;
        if (Is_container(obj) && obj->cobj) return TRUE;
    }
    return FALSE;
}

boolean can_ooze(mtmp)
    struct monst *mtmp;
{
...
STATIC_OVL int vamp_shift(mon, ptr)
    struct monst *mon;struct permonst *ptr;
{
    int reslt = 0;

    if (mon->cham >= LOW_PM)
    {
        if (ptr == &mons[mon->cham]) mon->cham = NON_PM;
