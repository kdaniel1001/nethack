...
{ S_QUADRUPED, S_UNICORN, S_ANGEL,
S_CENTAUR, S_DRAGON, S_JABBERWOCK, '\0' };
    ...
    struct obj *otmp;
{
    ...
    if (!isok(u.ux + u.dx,
            u.uy
                    + u.dy) || !(mtmp = m_at(u.ux + u.dx, u.uy + u.dy)) || !canspotmon(mtmp))
    {
...
int doride()
{
    ...
    {
        if (wizard && yn("Force the mount to succeed?") == 'y') forcemount =
        TRUE;
...
boolean mount_steed(mtmp, force)
    ...
    boolean force; /* Quietly force this animal */
{
    ...
    if (!force
            && !is_floater(ptr) && !is_flyer(ptr) && Levitation && !Lev_at_will)
    {
    ...
    if (!force && (Confusion|| Fumbling || Glib || Wounded_legs || otmp->cursed
 ...
 */
STATIC_OVL boolean landing_spot(spot, reason, forceit)
    ...
    int reason;int forceit;
{
    ...
    if (forceit && min_distance < 0 && !enexto(spot, u.ux, u.uy, youmonst.data))
        return FALSE;
...
STATIC_OVL void maybewakesteed(steed)
    struct monst *steed;
{
...
boolean stucksteed(checkfeeding)
