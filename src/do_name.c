...
STATIC_OVL void getpos_help(force, goal)
    boolean force;const char *goal;
{
    char sbuf[BUFSZ];
...
int getpos(ccp, force, goal)
    coord *ccp;boolean force;const char *goal;
{
    int result = 0;
    int cx, cy, i, c;
    int sidx, tx, ty;
...
void new_mname(mon, lth)
    struct monst *mon;int lth; /* desired length (caller handles adding 1 for terminator) */
{
    if (lth)
    {
...
void new_oname(obj, lth)
    struct obj *obj;int lth; /* desired length (caller handles adding 1 for terminator) */
{
    if (lth)
    {
...
struct monst * christen_monst(mtmp, name)
    struct monst *mtmp;const char *name;
{
    int lth;
    char buf[PL_PSIZ];
...
struct obj * oname(obj, name)
    struct obj *obj;const char *name;
{
    int lth;
    char buf[PL_PSIZ];
...
STATIC_OVL void namefloorobj()
{
    ...
    if (getpos(&cc, FALSE, buf) < 0 || cc.x <= 0) return;
    ...
    Strcpy (buf,
            (obj->otyp != STRANGE_OBJECT) ?
                    simpleonames(obj) : obj_descr[STRANGE_OBJECT].oc_name);
    ...
    if (Hallucination)
    {
        ...
        pline("%s %s to call you \"%s.\"", The(buf),
                use_plural ? "decide" : "decides", unames[rn2(SIZE(unames))]);
    }
    else if (!objtyp_is_callable(obj->otyp))
    {
...
"Adri", "Andries", "Andreas", "Bert", "David", "Dirk", "Emile", "Frans", "Fred",
        "Greg", "Hether", "Jay", "John", "Jon", "Karnov", "Kay", "Kenny",
        "Kevin", "Maud", "Michiel", "Mike", "Peter", "Robert", "Ron", "Tom",
        "Wilmar", "Nick Danger", "Phoenix", "Jiro", "Mizue", "Stephan",
        "Lance Braccus", "Shadowhawk" };
 ...
 */
char * x_monnam(mtmp, article, adjective, suppress, called)
    register struct monst *mtmp;int article;
     ...
     */
    const char *adjective;int suppress;
    ...
    boolean called;
{
    ...
    else if (mdat == &mons[PM_HIGH_PRIEST] && mtmp->female) pm_name =
            "high priestess";
 ...
 */
char * noit_mon_nam(mtmp)
    struct monst *mtmp;
{
    return x_monnam(mtmp, ARTICLE_THE, (char *) 0,
            (has_mname(mtmp)) ? (SUPPRESS_SADDLE | SUPPRESS_IT) : SUPPRESS_IT,
            FALSE);
}
...
char * Adjmonnam(mtmp, adj)
    struct monst *mtmp;const char *adj;
{
...
char * distant_monnam(mon, article, outbuf)
    struct monst *mon;int article; /* only ARTICLE_NONE and ARTICLE_THE are handled here */
    char *outbuf;
{
...
STATIC_OVL char * bogusmon(buf, code)
    char *buf, *code;
{
    char *mname = buf;
...
roguename()
{
    ...
    return rn2(3) ?
            (rn2(2) ? "Michael Toy" : "Kenneth Arnold") : "Glenn Wichman";
}
...
{ "ultraviolet", "infrared", "bluish-orange", "reddish-green", "dark white",
        ...
        "paisley", "blotchy", "guernsey-spotted", "polka-dotted", "square",
        "round", "triangular", "cabernet", "sangria", "fuchsia", "wisteria",
        "lemon-lime", "strawberry-banana", "peppermint", "romantic",
        "incandescent", "octarine", /* Discworld: the Colour of Magic */
};

const char * hcolor(colorpref)
...
{ "Carnivorous Vulgaris", "Road-Runnerus Digestus", "Eatibus Anythingus",
        ...
        "Hardheadipus Oedipus", "Carnivorous Slobbius", "Hard-Headipus Ravenus",
        "Evereadii Eatibus", "Apetitius Giganticus", "Hungrii Flea-Bagius",
        "Overconfidentii Vulgaris", "Caninus Nervous Rex",
        "Grotesques Appetitus", "Nemesis Ridiculii", "Canis latrans" };

char * coyotename(mtmp, buf)
    struct monst *mtmp;char *buf;
{
    if (mtmp && buf)
    {
        Sprintf (buf, "%s - %s",
...
const char * lookup_novel(lookname, idx)
    const char *lookname;int *idx;
{
    int k;

    /* Take American or U.K. spelling of this one */
    if (!strcmpi(The(lookname), "The Color of Magic")) lookname =
            sir_Terry_novels[0];

    for (k = 0; k < SIZE(sir_Terry_novels); ++k)
    {
        if (!strcmpi(lookname,
                sir_Terry_novels[k]) || !strcmpi(The(lookname), sir_Terry_novels[k]))
        {
            if (idx) *idx = k;
