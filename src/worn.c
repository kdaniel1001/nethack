...
void setworn(obj, mask)
    register struct obj *obj;long mask;
{
            ...
            if (wp->w_mask & mask)
            {
                ...
                if (oobj)
                {
                    ...
                    if (wp->w_mask & ~(W_SWAPWEP | W_QUIVER))
                    {
                        ...
                        u.uprops[p].extrinsic = u.uprops[p].extrinsic
                                & ~wp->w_mask;
    ...
    struct obj *obj;
{
    ...
    case TOOL_CLASS:
        if (otyp == BLINDFOLD || otyp == TOWEL || otyp == LENSES)
            ...
            res = W_WEP | W_SWAPWEP;
        else if (otyp == SADDLE) res = W_SADDLE;
        break;
    case FOOD_CLASS:
...
void mon_adjust_speed(mon, adjust, obj)
    struct monst *mon;int adjust; /* positive => increase speed, negative => decrease */
...
void update_mon_intrinsics(mon, obj, on, silently)
    struct monst *mon;struct obj *obj;boolean on, silently;
{
    int unseen;
    uchar mask;
    struct obj *otmp;
 ...
 */
void m_dowear(mon, creation)
    register struct monst *mon;boolean creation;
{
    ...
    if (mindless(mon->data)
            && (!creation
                    || (mon->data->mlet != S_MUMMY
                            && mon->data != &mons[PM_SKELETON]))) return;
...
STATIC_OVL void m_dowear_type(mon, flag, creation, racialexception)
    struct monst *mon;long flag;boolean creation;boolean racialexception;
{
        ...
        case W_AMUL:
            if (obj->oclass != AMULET_CLASS
                    || (obj->otyp != AMULET_OF_LIFE_SAVING
                            && obj->otyp != AMULET_OF_REFLECTION)) continue;
            best = obj;
            goto outer_break;
    ...
    outer_break: if (!best || best == old) return;
...
struct obj * which_armor(mon, flag)
    struct monst *mon;long flag;
{
    if (mon == &youmonst)
    {
        switch (flag)
...
STATIC_OVL void m_lose_armor(mon, obj)
...
void bypass_objlist(objchain, on)
    struct obj *objchain;boolean on; /* TRUE => set, FALSE => clear */
{
...
struct obj * nxt_unbypassed_obj(objchain)
    struct obj *objchain;
{
    while (objchain)
    {
...
void mon_break_armor(mon, polyspot)
    struct monst *mon;boolean polyspot;
{
    ...
    if (handless_or_tiny)
    {
        ...
        {
            if (vis) pline("%s drops %s gloves%s!", Monnam(mon), ppronoun,
    ...
    if (!can_saddle(mon))
    {
        ...
        if (mon == u.usteed) goto noride;
    }
    else if (mon == u.usteed && !can_ride(mon))
    {
        noride: You("can no longer ride %s.", mon_nam(mon));
...
STATIC_OVL int extra_pref(mon, obj)
    struct monst *mon;struct obj *obj;
{
