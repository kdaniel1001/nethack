...
void erode_armor(mdef, hurt)
    struct monst *mdef;int hurt;
{
    struct obj *target;
...
boolean attack_checks(mtmp, wep)
    register struct monst *mtmp;struct obj *wep; /* uwep for attack(), null for kick_monster() */
{
    char qbuf[QBUFSZ];
    ...
    if (mtmp->m_ap_type && !Protection_from_shape_changers
            && !sensemon(
                    mtmp) && !glyph_is_warning(glyph_at(bhitpos.x, bhitpos.y)))
    {
...
int find_roll_to_hit(mtmp, aatyp, weapon, attk_count, role_roll_penalty)
    register struct monst *mtmp;uchar aatyp; /* usually AT_WEAP or AT_KICK */
    ...
    int *attk_count, *role_roll_penalty;
{
    ...
    *role_roll_penalty = 0; /* default is `none' */
    ...
    if (is_orc(
            mtmp->data) && maybe_polyd(is_elf(youmonst.data), Race_if(PM_ELF)))
        tmp++;
    ...
    register struct monst *mtmp;
{
    ...
    if (mdat->mlet == S_LEPRECHAUN && !mtmp->mfrozen && !mtmp->msleeping
            && !mtmp->mconf && mtmp->mcansee && !rn2(7) && (m_move(mtmp, 0) == 2 /* it died */
...
STATIC_OVL boolean known_hitum(mon, weapon, mhit, rollneeded, armorpenalty,
        uattk)
    register struct monst *mon;struct obj *weapon;int *mhit;int rollneeded,
            armorpenalty; /* for monks */
    struct attack *uattk;
{
    ...
    else
    {
        ...
        if (malive)
        {
            ...
            if (mon->wormno && *mhit) cutworm(mon, x, y, weapon);
        }
    }
    return malive;
}
...
STATIC_OVL boolean hitum(mon, uattk)
    struct monst *mon;struct attack *uattk;
{
    boolean malive, wep_was_destroyed = FALSE;
    struct obj *wepbefore = uwep;
    int armorpenalty, attknum = 0, x = u.ux + u.dx, y = u.uy + u.dy, tmp =
            find_roll_to_hit(mon, uattk->aatyp, uwep, &attknum, &armorpenalty);
...
boolean hmon(mon, obj, thrown)
    struct monst *mon;struct obj *obj;int thrown; /* HMON_xxx (0 => hand-to-hand, other => ranged) */
{
...
/* guts of hmon() */
STATIC_OVL boolean hmon_hitmon(mon, obj, thrown)
    struct monst *mon;struct obj *obj;int thrown; /* HMON_xxx (0 => hand-to-hand, other => ranged) */
{
    int tmp;
    ...
    else
    {
        ...
        if (obj->oclass == WEAPON_CLASS
                || is_weptool(obj) || obj->oclass == GEM_CLASS)
        {
...
#undef useup_eggs
    ...
    if (get_dmg_bonus && tmp > 0)
    {
        ...
        if (thrown != HMON_THROWN || !obj || !uwep
                || !ammo_and_launcher(obj, uwep)) tmp += dbon();
    }
    ...
    if (mon->mtame && tmp > 0)
    {
        ...
        if (mon->mtame && !destroyed) monflee(mon, 10 * rnd(tmp), FALSE, FALSE);
    }
...
STATIC_OVL boolean shade_aware(obj)
    struct obj *obj;
{
    if (!obj) return FALSE;
    /*
    ...
    if (obj->otyp == BOULDER || obj->otyp == HEAVY_IRON_BALL
            ...
            || objects[obj->otyp].oc_material == SILVER) return TRUE;
    return FALSE;
}

/* check whether slippery clothing protects from hug or wrap attack */
/* [currently assumes that you are the attacker] */
STATIC_OVL boolean m_slips_free(mdef, mattk)
    struct monst *mdef;struct attack *mattk;
{
    struct obj *obj;
    ...
    struct obj *obj; /* weapon */
{
    ...
    if (u.twoweap && P_SKILL(P_TWO_WEAPON_COMBAT) < skill_rating) skill_rating =
    ...
    if ((joust_dieroll = rn2(5)) < skill_rating)
    {
        if (joust_dieroll == 0 && rnl(50) == (50 - 1) && !unsolid(mon->data)
                && !obj_resists(obj, 0, 100)) return -1; /* hit that breaks lance */
...
STATIC_OVL boolean theft_petrifies(otmp)
    struct obj *otmp;
{
 ...
 */
STATIC_OVL void steal_it(mdef, mattk)
    struct monst *mdef;struct attack *mattk;
{
...
int damageum(mdef, mattk)
    register struct monst *mdef;register struct attack *mattk;
{
    ...
    case AD_FIRE:
        ...
        if (!Blind) pline("%s is %s!", Monnam(mdef), on_fire(pd, mattk));
    ...
    case AD_DCAY:
        ...
        break;
    case AD_DREN:
        if (!negated && !rn2(4)) xdrainenergym(mdef, TRUE);
        tmp = 0;
        break;
    case AD_DRST:
    case AD_DRDX:
    ...
    case AD_STCK:
        if (!negated && !sticks(pd)) u.ustuck = mdef; /* it's now stuck to you */
        break;
    case AD_WRAP:
        if (!sticks(pd))
        {
            ...
            else if (u.ustuck == mdef)
            {
                ...
                if (is_pool(u.ux, u.uy) && !is_swimmer(pd) && !amphibious(pd))
                {
    ...
    case AD_SLIM:
        ...
        if (!rn2(4) && !slimeproof(pd))
        {
            if (!munslime(mdef, TRUE) && mdef->mhp > 0)
            {
                ...
                if (newcham(mdef, &mons[PM_GREEN_SLIME], FALSE, FALSE)) pd =
                        mdef->data;
            }
...
STATIC_OVL int explum(mdef, mattk)
    register struct monst *mdef;register struct attack *mattk;
{
...
STATIC_OVL void start_engulf(mdef)
    struct monst *mdef;
{
    if (!Invisible)
    {
...
STATIC_OVL int gulpum(mdef, mattk)
    register struct monst *mdef;register struct attack *mattk;
{
    ...
    if (u.uhunger < 1500 && !u.uswallow)
    {
        ...
        else
        {
            ...
            case AD_FIRE:
                ...
                break;
            case AD_DREN:
                if (!rn2(4)) xdrainenergym(mdef, TRUE);
                dam = 0;
                break;
            }
            end_engulf();
...
void missum(mdef, mattk, wouldhavehit)
    register struct monst *mdef;register struct attack *mattk;boolean wouldhavehit;
{
 ...
 */
int passive(mon, mhit, malive, aatyp, wep_was_destroyed)
    register struct monst *mon;register boolean mhit;register int malive;uchar aatyp;boolean wep_was_destroyed;
{
    ...
    case AD_RUST:
        if (mhit && !mon->mcan)
        {
            if (aatyp == AT_KICK)
            {
                if (uarmf) (void) erode_obj(uarmf, xname(uarmf), ERODE_RUST,
                EF_GREASE | EF_VERBOSE);
            }
            else if (aatyp == AT_WEAP || aatyp == AT_CLAW || aatyp == AT_MAGC
                    || aatyp == AT_TUCH)
    ...
    case AD_CORR:
        if (mhit && !mon->mcan)
        {
            if (aatyp == AT_KICK)
            {
                if (uarmf) (void) erode_obj(uarmf, xname(uarmf), ERODE_CORRODE,
                EF_GREASE | EF_VERBOSE);
            }
            else if (aatyp == AT_WEAP || aatyp == AT_CLAW || aatyp == AT_MAGC
                    || aatyp == AT_TUCH)
 ...
 */
void passive_obj(mon, obj, mattk)
    register struct monst *mon;register struct obj *obj; /* null means pick uwep, uswapwep or uarmg */
    struct attack *mattk; /* null means we find one internally */
{
    ...
    if (!obj)
    {
        ...
        if (!obj && mattk->adtyp == AD_ENCH) obj = uarmg; /* no weapon? then must be gloves */
...
void stumble_onto_mimic(mtmp)
    struct monst *mtmp;
{
    const char *fmt = "Wait!  That's %s!", *generic = "a monster", *what = 0;

    if (!u.ustuck && !mtmp->mflee && dmgtype(mtmp->data, AD_STCK)) u.ustuck =
            mtmp;
    ...
    if (Blind)
    {
        ...
        else if (mtmp->m_ap_type == M_AP_MONSTER) what = a_monnam(mtmp); /* differs from what was sensed */
    }
    else
    {
    ...
    if (!canspotmon(mtmp) && !glyph_is_invisible(levl[mtmp->mx][mtmp->my].glyph))
        map_invisible(mtmp->mx, mtmp->my);
}

STATIC_OVL void nohandglow(mon)
    struct monst *mon;
{
...
int flash_hits_mon(mtmp, otmp)
    struct monst *mtmp;struct obj *otmp; /* source of flash */
{
        ...
        if (!resists_blnd(mtmp))
        {
            ...
            if (mtmp->data == &mons[PM_GREMLIN])
            {
                ...
                amt = otmp->otyp == WAN_LIGHT ?
                        d(1 + otmp->spe, 4) : rn2(min(mtmp->mhp, 4));
...
void light_hits_gremlin(mon, dmg)
    struct monst *mon;int dmg;
{
