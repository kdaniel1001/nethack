...
FDECL(restobjchn, (int, BOOLEAN_P, BOOLEAN_P));
STATIC_OVL void FDECL(restmon, (int, struct monst *));
STATIC_DCL struct monst *
FDECL(restmonchn, (int, BOOLEAN_P));
STATIC_DCL struct fruit *
FDECL(loadfruitchn, (int));
...
STATIC_OVL void restdamage(fd, ghostly)
    int fd;boolean ghostly;
{
    int counter;
        ...
        if (u.uz.dlevel)
        {
            ...
            for (shp = damaged_shops; *shp; shp++)
            {
                ...
                if (shkp && inhishop(shkp)
                        && repair_damage(shkp, tmp_dam, TRUE)) break;
            }
        }
        if (!shp || !*shp)
        {
...
/* restore one object */
STATIC_OVL void restobj(fd, otmp)
    int fd;struct obj *otmp;
{
    int buflen;
...
STATIC_OVL struct obj * restobjchn(fd, ghostly, frozen)
    register int fd;boolean ghostly, frozen;
{
    ...
    while (1)
    {
        ...
        {
            /* fix the pointers */
            if (otmp->o_id == context.victual.o_id) context.victual.piece =
                    otmp;
            if (otmp->o_id == context.tin.o_id) context.tin.tin = otmp;
            if (otmp->o_id == context.spbook.o_id) context.spbook.book = otmp;
        }
        otmp2 = otmp;
    }
    if (first && otmp2->nobj)
...
/* restore one monster */
STATIC_OVL void restmon(fd, mtmp)
    int fd;struct monst *mtmp;
{
    int buflen;
...
STATIC_OVL struct monst * restmonchn(fd, ghostly)
    register int fd;boolean ghostly;
{
    ...
    while (1)
    {
        ...
        if (!ghostly)
        {
            if (mtmp->m_id == context.polearm.m_id) context.polearm.hitmon =
                    mtmp;
        }
        mtmp2 = mtmp;
    }
...
STATIC_OVL struct fruit * loadfruitchn(fd)
    int fd;
{
    register struct fruit *flist, *fnext;

    flist = 0;
    while (fnext = newfruit(), mread(fd, (genericptr_t) fnext, sizeof *fnext), fnext->fid
            != 0)
    {
        fnext->nextf = flist;
        flist = fnext;
...
STATIC_OVL void freefruitchn(flist)
...
STATIC_OVL void ghostfruit(otmp)
    register struct obj *otmp;
{
...
STATIC_OVL
boolean restgamestate(fd, stuckid, steedid)
    register int fd;unsigned int *stuckid, *steedid;
{
    ...
    if (SYSOPT_CHECK_SAVE_UID && uid != (unsigned long) getuid())
    { /* strange ... */
...
#define ReadTimebuf(foo)                   \
    ...
    if (tmp_bc)
    {
        ...
        if (!uball || !uchain) impossible("restgamestate: lost ball & chain");
    }
    ...
    restore_oracles(fd);
    if (u.ustuck) mread(fd, (genericptr_t) stuckid, sizeof(*stuckid));
    if (u.usteed) mread(fd, (genericptr_t) steedid, sizeof(*steedid));
 ...
 */
STATIC_OVL void restlevelstate(stuckid, steedid)
...
/*ARGSUSED*/
STATIC_OVL int restlevelfile(fd, ltmp)
    ...
    register int fd;
{
    ...
    while (1)
    {
        ...
        if (restoreprocs.mread_flags == -1) break;
...
void restcemetery(fd, cemeteryaddr)
    int fd;struct cemetery **cemeteryaddr;
{
...
/*ARGSUSED*/
STATIC_OVL void rest_levl(fd, rlecomp)
    int fd;boolean rlecomp;
{
#ifdef RLECOMP
    short i, j;
    uchar len;
...
void getlev(fd, pid, lev, ghostly)
    int fd, pid;xchar lev;boolean ghostly;
{
    ...
    ftrap = 0;
    while (trap = newtrap(), mread(fd, (genericptr_t) trap,
            sizeof(struct trap)), trap->tx != 0)
    { /* need "!= 0" to work around DICE 3.0 bug */
        trap->ntrap = ftrap;
        ftrap = trap;
    }
    dealloc_trap(trap);
    ...
    if (ghostly)
    {
        ...
        if (br && u.uz.dlevel == 1)
        {
            ...
            case BR_PORTAL: /* max of 1 portal per level */
                ...
                if (!trap) panic("getlev: need portal but none found");
        ...
        else if (!br)
        {
            ...
            {
                ttmp = trap->ntrap;
                if (trap->ttyp == MAGIC_PORTAL) deltrap(trap);
            }
        }
    }
...
void get_plname_from_file(fd, plbuf)
    int fd;char *plbuf;
{
    int pltmpsiz = 0;
...
STATIC_OVL void restore_msghistory(fd)
    register int fd;
{
...
STATIC_OVL void add_id_mapping(gid, nid)
    unsigned gid, nid;
{
    int idx;
 ...
 * ID.
 */
boolean lookup_id_mapping(gid, nidp)
    unsigned gid, *nidp;
{
    int i;
    struct bucket *curr;

    if (n_ids_mapped) for (curr = id_map; curr; curr = curr->next)
    {
...
STATIC_OVL void reset_oattached_mids(ghostly)
    boolean ghostly;
{
    struct obj *otmp;
    unsigned oldid, nid;
...
void mread(fd, buf, len)
    register int fd;register genericptr_t buf;register unsigned int len;
{
...
STATIC_OVL void def_minit()
{
    return;
}

STATIC_OVL void def_mread(fd, buf, len)
    register int fd;register genericptr_t buf;register unsigned int len;
{
    register int rlen;
