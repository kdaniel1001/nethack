...
STATIC_OVL void simple_look(otmp, here)
...
int collect_obj_classes(ilets, otmp, here, filter, itemcount)
    char ilets[];register struct obj *otmp;boolean here;boolean FDECL((*filter), (OBJ_P));
int *itemcount;
{
 ...
 */
STATIC_OVL boolean query_classes(oclasses, one_at_a_time, everything, action,
        objs, here, menu_on_demand)
    char oclasses[];boolean *one_at_a_time, *everything;const char *action;struct obj *objs;boolean here;int *menu_on_demand;
{
    ...
    *one_at_a_time = *everything = m_seen = FALSE;
    ...
    { /* more than one choice available */
        ...
        ilets[iletct] = '\0';
        ask_again: oclasses[oclassct = 0] = '\0';
        *one_at_a_time = *everything = FALSE;
        ...
        for (p = inbuf; (sym = *p++);)
        {
            ...
            else if (sym == ':')
            {
                ...
                if (objs->where == OBJ_CONTAINED) objs->ocontainer->cknown = 1;
                goto ask_again;
            }
            else if (sym == 'i')
            {
...
STATIC_OVL boolean fatal_corpse_mistake(obj, remotely)
    struct obj *obj;boolean remotely;
{
...
boolean rider_corpse_revival(obj, remotely)
    struct obj *obj;boolean remotely;
{
...
STATIC_OVL void check_here(picked_some)
    boolean picked_some;
{
...
STATIC_OVL boolean n_or_more(obj)
    struct obj *obj;
{
...
STATIC_OVL boolean all_but_uchain(obj)
    struct obj *obj;
{
...
/*ARGUSED*/
boolean allow_all(obj)
    struct obj *obj UNUSED;
{
    return TRUE;
}

boolean allow_category(obj)
    struct obj *obj;
{
    ...
    if (class_filter && !index(valid_menu_classes, obj->oclass)) return FALSE;
    ...
    if (bucx_filter)
    {
        ...
        char bucx = !obj->bknown ? 'X' : obj->blessed ? 'B' :
                    obj->cursed ? 'C' : 'U';

        /* if its category is not in the list, reject */
        if (!index(valid_menu_classes, bucx)) return FALSE;
    }
    ...
    int what; /* should be a long */
{
    ...
    if (!u.uswallow)
    {
        ...
        if (autopickup
                && (context.nopick || !OBJ_AT(u.ux, u.uy)
    ...
    else
    {
        ...
        if (ct == 1 && count)
        {
            ...
            n_tried++;
            if (pickup_object(obj, lcount, FALSE) > 0) n_picked++; /* picked something */
            goto end_query;
        ...
        else if (ct >= 2)
        {
            ...
            if (!query_classes(oclasses, &selective, &all_of_a_type, "pick up",
                    objchain, traverse_how == BY_NEXTHERE, &via_menu))
            {
...
boolean is_autopickup_exception(obj, grab)
    struct obj *obj;boolean grab; /* forced pickup, rather than forced leave behind? */
{
    /*
    ...
    while (ape)
    {
        if (regex_match(objdesc, ape->regex)) return TRUE;
        ape = ape->next;
    }
    return FALSE;
}
 ...
 */
STATIC_OVL int autopick(olist, follow, pick_list)
 ...
 *			    pick nothing.
 */
int query_objlist(qstr, olist, qflags, pick_list, how, allow)
 ...
 *
 */
int query_category(qstr, olist, qflags, pick_list, how)
    const char *qstr; /* query string */
    struct obj *olist; /* the list to pick from */
    ...
    do
    {
        ...
        for (curr = olist; curr; curr = FOLLOW(curr, qflags))
        {
            if (curr->oclass == *pack)
            {
                if ((qflags & WORN_TYPES)
                        && !(curr->owornmask
                                & (W_ARMOR | W_ACCESSORY | W_WEAPON))) continue;
...
STATIC_OVL int count_categories(olist, qflags)
    struct obj *olist;int qflags;
{
    char *pack;
...
STATIC_OVL long carry_count(obj, container, count, telekinesis, wt_before,
        wt_after)
    struct obj *obj, *container; /* object to pick up, bag it's coming out of */
    long count;boolean telekinesis;int *wt_before, *wt_after;
{
    boolean adjust_wt = container && carried(container), is_gold = obj->oclass
            == COIN_CLASS;
    int wt, iw, ow, oow;
    ...
    long *cnt_p;boolean telekinesis;
{
            ...
            || (obj->otyp == BOULDER && throws_rocks(youmonst.data)))
    {
        if (inv_cnt(FALSE) < 52 || !carrying(obj->otyp)
                || merge_choice(invent, obj)) return 1; /* lift regardless of current situation */
    ...
    *cnt_p = carry_count(obj, container, *cnt_p, telekinesis, &old_wt, &new_wt);
    ...
    if (obj->otyp == SCR_SCARE_MONSTER && result <= 0 && !container) obj->spe =
            0;
    return result;
}
 ...
 * up, 1 if otherwise.
 */
int pickup_object(obj, count, telekinesis)
    struct obj *obj;long count;boolean telekinesis; /* not picking it up directly by hand */
{
    int res, nearload;
    ...
    else if (obj->otyp == CORPSE)
    {
        if (fatal_corpse_mistake(obj, telekinesis)
                || rider_corpse_revival(obj, telekinesis)) return -1;
    }
    else if (obj->otyp == SCR_SCARE_MONSTER)
    {
        ...
        else
        {
            ...
            if (!(objects[SCR_SCARE_MONSTER].oc_name_known)
                    && !(objects[SCR_SCARE_MONSTER].oc_uname)) docall(obj);
            useupf(obj, obj->quan);
    ...
    struct obj *otmp;
{
    ...
    if (!u.uswallow && otmp != uball && costly_spot(otmp->ox, otmp->oy))
    {
        ...
        if (!index(u.ushops, *fakeshop)) remote_burglary(otmp->ox, otmp->oy);
    }
...
STATIC_OVL int container_at(x, y, countem)
    int x, y;boolean countem;
{
...
STATIC_OVL boolean able_to_loot(x, y, looting)
    int x, y;boolean looting; /* loot vs tip */
{
...
STATIC_OVL boolean mon_beside(x, y)
    int x, y;
{
    int i, j, nx, ny;
    for (i = -1; i <= 1; i++)
...
STATIC_OVL boolean reverse_loot()
{
            ...
            if (coffers->otyp == CHEST)
            {
                ...
                if (!otmp
                        || distu(coffers->ox, coffers->oy)
                                < distu(otmp->ox, otmp->oy)) otmp = coffers; /* remember closest ordinary chest */
            }
 ...
 */
int loot_mon(mtmp, passed_info, prev_loot)
    struct monst *mtmp;int *passed_info;boolean *prev_loot;
{
    int c = -1;
    int timepassed = 0;
    struct obj *otmp;
 ...
 */
STATIC_OVL boolean mbag_explodes(obj, depthin)
    struct obj *obj;int depthin;
{
...
STATIC_PTR int in_container(obj)
    register struct obj *obj;
{
...
STATIC_PTR int out_container(obj)
    register struct obj *obj;
{
    register struct obj *otmp;
    ...
    {
        obj->age = monstermoves - obj->age; /* actual age */
        if (obj->otyp == CORPSE) start_corpse_timeout(obj);
    }
...
STATIC_OVL long mbag_item_gone(held, item)
    int held;struct obj *item;
{
    struct monst *shkp;
    long loss = 0L;
    ...
    {
        if (held ? (boolean) item->unpaid : costly_spot(u.ux, u.uy)) loss =
                stolen_value(item, u.ux, u.uy, (boolean) shkp->mpeaceful,
                TRUE);
    }
...
STATIC_OVL void observe_quantum_cat(box)
    struct obj *box;
{
    ...
    if (get_obj_location(box, &ox, &oy, 0)) box->ox = ox, box->oy = oy; /* in case it's being carried */
    ...
    {
        deadcat = mk_named_object(CORPSE, &mons[PM_HOUSECAT], box->ox, box->oy,
                sc);
        if (deadcat)
        {
...
STATIC_OVL void explain_container_prompt()
{
...
static const char stashable[] =
{ ALLOW_COUNT, COIN_CLASS, ALL_CLASSES, 0 };

int use_container(objp, held)
    struct obj **objp;int held;
{
    ...
    if (cursed_mbag)
    {
        ...
        if (loss) You("owe %ld %s for lost merchandise.", loss, currency(loss));
        current_container->owt = weight(current_container);
    }
    inokay = (invent != 0
    ...
    { /* repeats if '?' or ":' gets chosen */
        ...
        { /* TRADITIONAL, COMBINATION, or PARTIAL */
...
STATIC_OVL int traditional_loot(put_in)
    boolean put_in;
{
...
STATIC_OVL int menu_loot(retry, put_in)
    int retry;boolean put_in;
{
    int n, i, n_looted = 0;
...
STATIC_OVL char in_or_out_menu(prompt, obj, outokay, inokay, alreadyused)
    const char *prompt;struct obj *obj;boolean outokay, inokay, alreadyused;
{
...
static const char tippables[] =
{ ALL_CLASSES, TOOL_CLASS, 0 };
...
int dotip()
{
    ...
    if ((boxes = container_at(cc.x, cc.y, TRUE)) > 0)
    {
        ...
        if (!check_capacity(buf) && able_to_loot(cc.x, cc.y, FALSE))
        {
                            ...
                            || iflags.menu_requested))
            {
                ...
                if (n > 1 && otmp == &dummyobj) otmp = pick_list[1].item.a_obj;
                if (pick_list) free((genericptr_t) pick_list);
            ...
            else
            {
                for (cobj = level.objects[cc.x][cc.y]; cobj; cobj = nobj)
                {
                    ...
                    c = ynq(
                            safe_qbuf(qbuf, "There is ", " here, tip it?", cobj,
    ...
    if (spillage)
    {
        ...
        pline("Some %s %s onto the %s%s.", spillage, vtense(spillage, "spill"),
...
STATIC_OVL void tipcontainer(box)
    struct obj *box; /* or bag */
{
    ...
    else if (box->otyp == BAG_OF_TRICKS || box->otyp == HORN_OF_PLENTY)
    {
        ...
        if (box->spe < old_spe)
        {
            if (bag)
                pline(
                        (seen == 0) ? "Nothing seems to happen." :
                        (seen == 1) ?
    ...
    else if (SchroedingersBox(box))
    {
        ...
        if (!Has_contents(box)) /* evidently a live cat came out */
            ...
            /* holds cat corpse or other random stuff */
            empty_it = TRUE;
        box->cknown = 1;
    }
    ...
    if (empty_it)
    {
        ...
        boolean verbose = FALSE, highdrop = !can_reach_floor(TRUE), altarizing =
                IS_ALTAR(levl[ox][oy].typ), cursed_mbag = (Is_mbag(box)
                && box->cursed);
