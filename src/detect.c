 ...
 */
struct obj * o_in(obj, oclass)
    struct obj *obj;char oclass;
{
 ...
 * Return first found.
 */
struct obj * o_material(obj, material)
    struct obj *obj;unsigned material;
{
    register struct obj *otmp;
    struct obj *temp;
...
STATIC_OVL void do_dknown_of(obj)
    struct obj *obj;
{
    struct obj *otmp;
...
STATIC_OVL boolean check_map_spot(x, y, oclass, material)
    int x, y;char oclass;unsigned material;
{
    int glyph;
    register struct obj *otmp;
 ...
 * change occurs.
 */
STATIC_OVL boolean clear_stale_map(oclass, material)
    char oclass;unsigned material;
{
    register int zx, zy;
 ...
 */
int object_detect(detector, class)
    ...
    int class; /* an object class, 0 for all */
{
    ...
    if (sym && iflags.bouldersym && sym == iflags.bouldersym) boulder =
    ROCK_CLASS;
    ...
    for (x = 1; x < COLNO; x++)
        for (y = 0; y < ROWNO; y++)
            for (obj = level.objects[x][y]; obj; obj = obj->nexthere)
                if ((!class && !boulder) || (otmp = o_in(obj, class)) || (otmp =
                        o_in(obj, boulder)))
                {
                    if (class || boulder)
                    {
 ...
 * Returns 0 if something was detected.
 */
int monster_detect(otmp, mclass)
    ...
    int mclass; /* monster class, 0 for all */
{
    ...
    if (!mcnt)
    {
        if (otmp) strange_feeling(otmp,
        Hallucination ? "You get the heebie jeebies." : "You feel threatened.");
        return 1;
    }
    else
    {
...
STATIC_OVL void sense_trap(trap, x, y, src_cursed)
    struct trap *trap;xchar x, y;int src_cursed;
{
...
STATIC_OVL int detect_obj_traps(objlist, show_them, how)
    struct obj *objlist;boolean show_them;int how; /* 1 for misleading map feedback */
{
    struct obj *otmp;
    xchar x, y;
...
/* sobj is null if crystal ball, *scroll if gold detection scroll */
{
    ...
    if (!(glyph_is_trap(glyph) || glyph_is_object(glyph))) newsym(u.ux, u.uy);
    u.uinwater = iflags.save_uinwater, u.uburied = iflags.save_uburied;
...
const char * level_distance(where)
    d_level *where;
{
...
STATIC_OVL void show_map_spot(x, y)
    register int x, y;
{
    struct rm *lev;
    struct trap *t;
...
{
    register int zx, zy;
    int lo_y = (u.uy - 5 < 0 ? 0 : u.uy - 5), hi_y = (
            u.uy + 6 > ROWNO ? ROWNO : u.uy + 6), lo_x = (
            u.ux - 9 < 1 ? 1 : u.ux - 9), /* avoid column 0 */
...
STATIC_PTR void findone(zx, zy, num)
    int zx, zy;genericptr_t num;
{
    register struct trap *ttmp;
    register struct monst *mtmp;
...
STATIC_PTR void openone(zx, zy, num)
    int zx, zy;genericptr_t num;
{
    register struct trap *ttmp;
    register struct obj *otmp;
    ...
    {
        if (levl[zx][zy].typ == SDOOR) cvt_sdoor_to_door(&levl[zx][zy]); /* .typ = DOOR */
        if (levl[zx][zy].doormask & D_TRAPPED)
        {
            if (distu(zx, zy) < 3)
                ...
                Norep("You %s an explosion!",
                        cansee(zx, zy) ?
                                "see" : (!Deaf ? "hear" : "feel the shock of"));
    ...
    else if ((ttmp = t_at(zx, zy)) != 0)
    {
        ...
        if (openholdingtrap(mon, &dummy) || openfallingtrap(mon, TRUE, &dummy))
            (*num_p)++;
    }
    ...
    register int aflag; /* intrinsic autosearch vs explicit searching */
{
    ...
    else
    {
        ...
        if (ublindf && ublindf->otyp == LENSES && !Blind) fund += 2; /* JDS: lenses help searching */
        if (fund > 5) fund = 5;
...
void sokoban_detect()
{
    ...
    for (x = 1; x < COLNO; x++)
        for (y = 0; y < ROWNO; y++)
        {
            ...
            if ((obj = sobj_at(BOULDER, x, y)) != 0) map_object(obj, 1);
        }
...
void reveal_terrain(full, which_subset)
