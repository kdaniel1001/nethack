...
STATIC_DCL char *
FDECL(strprepend, (char *, const char *));
...
FDECL(Japanese_item_name, (int i));

STATIC_OVL char * strprepend(s, pref)
    register char *s;register const char *pref;
{
...
STATIC_OVL void releaseobuf(bufp)
    char *bufp;
{
    /* caller may not know whether bufp is the most recently allocated
     buffer; if it isn't, do nothing */
    if (bufp == obufs[obufidx]) obufidx = (obufidx - 1 + NUMOBUF) % NUMOBUF;
}

char * obj_typename(otyp)
    register int otyp;
{
    char *buf = nextobuf();
    ...
    if (Role_if(PM_SAMURAI) && Japanese_item_name(otyp)) actualn =
    ...
    int otyp;
{
    ...
    if ((pp = strstri(bufp, " (")) != 0) *pp = '\0'; /* strip the appended description */
    return bufp;
}

boolean obj_is_pname(obj)
    struct obj *obj;
{
...
char * xname_flags(obj, cxn_flags)
    register struct obj *obj;unsigned cxn_flags; /* bitmask of CXN_xxx values */
{
    register char *buf;
    ...
    if (Role_if(PM_SAMURAI) && Japanese_item_name(typ)) actualn =
    ...
    case TOOL_CLASS:
        ...
        if (typ == FIGURINE && omndx != NON_PM)
        {
            ...
            index(vowels, *mons[omndx].mname) ? "n" : "", mons[omndx].mname);
        }
        else if (is_wet_towel(obj))
        {
            if (wizard)
    ...
    case FOOD_CLASS:
        ...
        if (Is_pudding(obj))
        {
            Sprintf (buf, "%s%s",
                    ...
                    (obj->owt > 300) ? "large " : "", actualn);
            break;
        }

        Strcpy (buf, actualn);
        if (typ == TIN && known) tin_details(obj, omndx, buf);
        break;
    case COIN_CLASS:
    case CHAIN_CLASS:
        Strcpy (buf, actualn);
    ...
    struct obj *obj;
{
    ...
    bareobj.known =
            (obj->oclass == AMULET_CLASS) ? obj->known
    ...
    struct permonst *ptr;
{
    ...
    if (ptr == &mons[PM_HIGH_PRIEST] || ptr == &mons[PM_LONG_WORM_TAIL]) uniq =
    FALSE;
    /* Wizard no longer needs this; he's flagged as unique these days */
    if (ptr == &mons[PM_WIZARD_OF_YENDOR]) uniq = TRUE;
    return uniq;
}

STATIC_OVL void add_erosion_words(obj, prefix)
    struct obj *obj;char *prefix;
{
    ...
    if (rknown && obj->oerodeproof)
        Strcat (prefix,
                iscrys ? "fixed " : is_rustprone(obj) ? "rustproof " :
                is_corrodeable(obj) ? "corrodeproof " /* "stainless"? */
                :
                is_flammable(obj) ? "fireproof " : "");
}

static char * doname_base(obj, with_price)
    register struct obj *obj;boolean with_price;
{
    ...
    case ARMOR_CLASS:
        if (obj->owornmask & W_ARMOR)
            Strcat (bp,
                    (obj == uskin) ?
                            " (embedded in your skin)" : " (being worn)");
        goto plus;
    case TOOL_CLASS:
...
/* used from invent.c */
boolean not_fully_identified(otmp)
    register struct obj *otmp;
{
    ...
    if (!otmp->known || !otmp->dknown
            ...
            || !otmp->bknown
#endif
            || !objects[otmp->otyp].oc_name_known) return TRUE;
    if ((!otmp->cknown && (Is_container(otmp) || otmp->otyp == STATUE))
            || (!otmp->lknown && Is_box(otmp))) return TRUE;
...
char * corpse_xname(otmp, adjective, cxn_flags)
    struct obj *otmp;const char *adjective;unsigned cxn_flags; /* bitmask of CXN_xxx values */
{
    ...
    else
    {
        ...
        if (the_unique_pm(&mons[omndx]) || type_is_pname(&mons[omndx]))
        {
            ...
            else if (the_unique_pm(&mons[omndx]) && !no_prefix) the_prefix =
            TRUE;
        }
    }
    if (no_prefix)
...
char * short_oname(obj, func, altfunc, lenlimit)
    struct obj *obj;char *
FDECL    ((*func), (OBJ_P)), /* main formatting routine */
    *FDECL((*altfunc), (OBJ_P)); /* alternate for shortest result */
     ...
     */
const char * singular(otmp, func)
    register struct obj *otmp;char *
FDECL((*func), (OBJ_P));
{
    long savequan;
    char *nam;
...
char * aobjnam(otmp, verb)
    struct obj *otmp;const char *verb;
{
    char prefix[PREFIX];
    char *bp = cxname(otmp);
...
char * yobjnam(obj, verb)
    struct obj *obj;const char *verb;
{
...
{ "wand", "ring", "potion", "scroll", "gem", "amulet", "spellbook",
        "spell book",
...
"Cyclops", "Hippocrates", "Pelias", "aklys", "amnesia", "detect monsters",
        "paralysis", "shape changers", "nemesis", 0
...
char * vtense(subj, verb)
    register const char *subj;register const char *verb;
{
    ...
    if (subj)
    {
        ...
        if ((lowc(*spot) == 's' && spot != subj
                ...
                || !BSTRNCMPI(subj, spot - 3, "eeth",
                        4) || !BSTRNCMPI(subj, spot - 3, "feet", 4)
...
"boots", "shoes", "gloves", "lenses", "scales", "eyes", "gauntlets",
        ...
        "deer", "fish", "tuna", "yaki", "-hai", "krill", "manes", "ninja",
 ...
 */
STATIC_OVL boolean singplur_lookup(basestr, endstring, to_plural, alt_as_is)
    ...
    const char * const *alt_as_is; /* another set like as_is[] */
{
    ...
    {
        al = (int) strlen(*as);
        if (!BSTRCMPI(basestr, endstring - al, *as)) return TRUE;
    }
    if (alt_as_is)
    {
    ...
    for (sp = one_off; sp->sing; sp++)
    {
        ...
        al = (int) strlen(same);
        if (!BSTRCMPI(basestr, endstring - al, same)) return TRUE; /* use as-is */
...
STATIC_OVL char * singplur_compound(str)
    char *str;
{
    /* if new entries are added, be sure to keep compound_start[] in sync */
    static const char * const compounds[] =
    { " of ", " labeled ", " called ", " named ", " above", /* lurkers above */
    " versus ", " from ", " in ", " on ", " a la ", " with", /* " with "? */
    " de ", " d'", " du ", "-in-", "-at-", 0 }, /* list of first characters for all compounds[] entries */
    compound_start[] = " -";
    ...
    const char *oldstr;
{
     ...
     */
    if (!strncmpi(str, "pair of ", 8)) goto bottom;
    ...
    *(spot + 1) = 0;
    ...
    /* dispense with some words which don't need pluralization */
    {
        ...
        if (singplur_lookup(str, spot + 1, TRUE, already_plural)) goto bottom;

        /* more of same, but not suitable for blanket loop checking */
        if ((len == 2 && !strcmpi(str, "ya"))
                || (len >= 3 && !strcmpi(spot - 2, " ya"))) goto bottom;
    }
    ...
    const char *oldstr;
{
    ...
    if (singplur_lookup(bp, p, FALSE, special_subjs)) goto bottom;
    ...
    if (p >= bp + 1 && lowc(p[-1]) == 's')
    {
        if (p >= bp + 2 && lowc(p[-2]) == 'e')
        {
            ...
            {
                if (!BSTRCMPI(bp, p - 6,
                        "cloves") || !BSTRCMPI(bp, p - 6, "nerves")) goto mins;
                Strcasecpy(p - 3, "f"); /* ves -> f */
                goto bottom;
            }
            /* note: nurses, axes but boxes, wumpuses */
            if (!BSTRCMPI(bp, p - 4,
                    "eses") || !BSTRCMPI(bp, p - 4, "oxes") /* boxes, foxes */
                    || !BSTRCMPI(bp, p - 4, "nxes") /* lynxes */
                    || !BSTRCMPI(bp, p - 4, "ches") || !BSTRCMPI(bp, p - 4, "uses") /* lotuses */
                    || !BSTRCMPI(bp, p - 4, "sses") /* priestesses */
                    || !BSTRCMPI(bp, p - 5, "atoes") /* tomatoes */
                    || !BSTRCMPI(bp, p - 7, "dingoes") || !BSTRCMPI(bp, p - 7, "Aleaxes"))
            {
                *(p - 2) = '\0'; /* drop es */
                goto bottom;
        ...
        else if (!BSTRCMPI(bp, p - 2, "us"))
        { /* lotus, fungus... */
            if (BSTRCMPI(bp, p - 6, "tengus") /* but not these... */
            && BSTRCMPI(bp, p - 7, "hezrous")) goto bottom;
        }
    ...
    { /* input doesn't end in 's' */
        ...
        if (p - 4 >= bp && !strcmpi(p - 2, "ia") && index("lr", lowc(*(p - 3)))
                && lowc(*(p - 4)) == 'e')
        {
...
short rnd_otyp_by_wpnskill(skill)
    schar skill;
{
    int i, n = 0;
    ...
    if (n > 0)
    {
        ...
        for (i = bases[WEAPON_CLASS];
                i < NUM_OBJECTS && objects[i].oc_class == WEAPON_CLASS; i++)
            if (objects[i].oc_skill == skill) if (--n < 0) return i;
    }
    return otyp;
}
 ...
 * return null.
 */
struct obj * readobjnam(bp, no_wish)
    register char *bp;struct obj *no_wish;
{
    register char *p;
    register int i;
    register struct obj *otmp;
...
#define SPINACH 2
    ...
    for (;;)
    {
        ...
        else if (!strncmpi(bp, "blessed ", l = 8) || !strncmpi(bp, "holy ", l =
                5))
        {
            blessed = 1;
        }
        else if (!strncmpi(bp, "moist ", l = 6) || !strncmpi(bp, "wet ", l = 4))
        {
        ...
        else if (!strncmpi(bp, "cursed ", l = 7) || !strncmpi(bp, "unholy ", l =
                7))
        {
            iscursed = 1;
        }
        ...
        else if (!strncmpi(bp, "lit ", l = 4) || !strncmpi(bp, "burning ", l =
                8))
        {
            islit = 1;
        }
        ...
        else if (!strncmpi(bp, "rusty ", l = 6) || !strncmpi(bp, "rusted ", l =
                7) || !strncmpi(bp, "burnt ", l = 6)
    ...
    else
    {
                ...
                && !strstri(bp, "finger "))
        {
            if (((p = strstri(bp, "tin of ")) != 0)
                    && (tmp = tin_variety_txt(p + 7, &tinv)) && (mntmp =
                            name_to_mon(p + 7 + tmp)) >= LOW_PM)
            {
                *(p + 3) = 0;
                tvariety = tinv;
            }
            else if ((p = strstri(bp, " of "))
                    != 0&& (mntmp = name_to_mon(p + 4)) >= LOW_PM) *p = 0;
        }
    }
     ...
     */
    wiztrap: if (wizard && !program_state.wizkit_wishing)
    {
        struct rm *lev;
        ...
        if (!BSTRCMPI(bp, p - 5, "grave") || !BSTRCMPI(bp, p - 9, "headstone"))
        {
            make_grave(x, y, (char *) 0);
            pline("%s.",
            IS_GRAVE(lev->typ) ? "A grave" : "Can't place a grave here");
            newsym(x, y);
            return &zeroobj;
        }
    ...
    any: if (!oclass) oclass = wrpsym[rn2((int) sizeof(wrpsym))];
    typfnd: if (typ) oclass = objects[typ].oc_class;
    ...
    if (mntmp >= LOW_PM)
    {
        ...
        case CORPSE:
            if (!(mons[mntmp].geno & G_UNIQ)
                    && !(mvitals[mntmp].mvflags & G_NOCORPSE))
            {
                if (mons[mntmp].msound == MS_GUARDIAN) mntmp = genus(mntmp, 1);
    ...
    if (trapped)
    {
        if (Is_box(otmp) || typ == TIN) otmp->otrapped = (trapped == 1);
    }
...
STATIC_OVL const char * Japanese_item_name(i)
    int i;
{
    struct Jitem *j = Japanese_items;
...
const char * suit_simple_name(suit)
    struct obj *suit;
{
    ...
    else if (Is_dragon_scales(suit)) return "dragon scales";
...
const char * cloak_simple_name(cloak)
    struct obj *cloak;
{
    if (cloak)
    {
...
const char * helm_simple_name(helmet)
    struct obj *helmet;
{
    /*
...
const char * mimic_obj_name(mtmp)
    struct monst *mtmp;
{
 ...
 */
char * safe_qbuf(qbuf, qprefix, qsuffix, obj, func, altfunc, lastR)
    char *qbuf; /* output buffer */
    const char *qprefix, *qsuffix;struct obj *obj;char *
FDECL((*func), (OBJ_P)), *FDECL((*altfunc), (OBJ_P));
const char *lastR;
{
    char *bufp, *endp;
