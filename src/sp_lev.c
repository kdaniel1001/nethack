...
STATIC_DCL void FDECL(splev_stack_push, (struct splevstack *,
                struct opvar *));
STATIC_DCL struct opvar *
FDECL(splev_stack_pop, (struct splevstack *));
STATIC_DCL struct splevstack *
FDECL(splev_stack_reverse,
        (struct splevstack *));
STATIC_DCL struct opvar *
FDECL(opvar_new_str, (char *));
STATIC_DCL struct opvar *
FDECL(opvar_new_int, (long));
STATIC_DCL struct opvar *
FDECL(opvar_new_coord, (int, int));
#if 0
STATIC_DCL struct opvar * FDECL(opvar_new_region, (int,int, int,int));
#endif /*0*/
STATIC_DCL void FDECL(opvar_free_x, (struct opvar *));
STATIC_DCL struct opvar *
FDECL(opvar_clone, (struct opvar *));
STATIC_DCL struct opvar *
FDECL(opvar_var_conversion, (struct sp_coder *,
                struct opvar *));
STATIC_DCL struct splev_var *
FDECL(opvar_var_defined, (struct sp_coder *,
                char *));
STATIC_DCL struct opvar *
FDECL(splev_stack_getdat, (struct sp_coder *,
                XCHAR_P));
STATIC_DCL struct opvar *
FDECL(splev_stack_getdat_any, (struct sp_coder *));
...
STATIC_DCL struct mkroom *
FDECL(build_room, (room *, struct mkroom *));
...
STATIC_DCL struct sp_frame *
FDECL(frame_new, (long));
...
STATIC_DCL struct opvar *
FDECL(selection_not, (struct opvar *));
STATIC_DCL struct opvar *
FDECL(selection_logical_oper, (struct opvar *,
                struct opvar *, CHAR_P));
STATIC_DCL struct opvar *
FDECL(selection_filter_mapchar, (struct opvar *,
                struct opvar *));
    ...
    struct splevstack *st;
{
    if (st)
    {
        ...
        st->stackdata = (struct opvar **) alloc(
                st->depth_alloc * sizeof(struct opvar *));
    }
}

void splev_stack_done(st)
    struct splevstack *st;
{
    if (st)
    {
...
void splev_stack_push(st, v)
    struct splevstack *st;struct opvar *v;
{
    if (!st || !v) return;
...
struct opvar * splev_stack_pop(st)
    struct splevstack *st;
{
...
struct splevstack * splev_stack_reverse(st)
    struct splevstack *st;
{
    long i;
    struct opvar *tmp;
...
struct opvar * opvar_new_coord(x, y)
    int x, y;
{
    struct opvar *tmpov = (struct opvar *) alloc(sizeof(struct opvar));
...
struct opvar * opvar_var_conversion(coder, ov)
    struct sp_coder *coder;struct opvar *ov;
{
...
struct splev_var * opvar_var_defined(coder, name)
    struct sp_coder *coder;char *name;
{
    struct splev_var *tmp;
...
struct opvar * splev_stack_getdat(coder, typ)
    struct sp_coder *coder;xchar typ;
{
...
struct opvar * splev_stack_getdat_any(coder)
...
void variable_list_del(varlist)
...
void lvlfill_maze_grid(x1, y1, x2, y2, filling)
    int x1, y1, x2, y2;schar filling;
{
    int x, y;
...
void lvlfill_solid(filling, lit)
 ...
 */
STATIC_OVL void set_wall_property(x1, y1, x2, y2, prop)
    xchar x1, y1, x2, y2;int prop;
{
    register xchar x, y;
...
STATIC_OVL void count_features()
{
    ...
    for (y = 0; y < ROWNO; y++)
        for (x = 0; x < COLNO; x++)
        {
            ...
            else if (typ == SINK) level.flags.nsinks++;
        }
}
...
void maybe_add_door(x, y, droom)
    int x, y;struct mkroom *droom;
{
    if (droom->hx >= 0 && doorindex < DOORMAX && inside_room(droom, x, y))
        add_door(x, y, droom);
}

void link_doors_rooms()
{
    int x, y;
    int tmpi, m;
 ...
 */
STATIC_OVL void get_location(x, y, humidity, croom)
    schar *x, *y;int humidity;struct mkroom *croom;
{
    int cpt = 0;
    int mx, my, sx, sy;
...
STATIC_OVL boolean is_ok_location(x, y, humidity)
    register schar x, y;register int humidity;
{
    register int typ;
...
unpacked_coord get_unpacked_coord(loc, defhumidity)
    long loc;int defhumidity;
{
...
STATIC_OVL void get_location_coord(x, y, humidity, croom, crd)
    schar *x, *y;int humidity;struct mkroom *croom;long crd;
{
    unpacked_coord c;
...
STATIC_OVL void get_room_loc(x, y, croom)
    schar *x, *y;struct mkroom *croom;
{
    coord c;
 ...
 * negative values for x or y means RANDOM!
 */
STATIC_OVL void get_free_room_loc(x, y, croom, pos)
    schar *x, *y;struct mkroom *croom;
    packed_coord pos;
{
    schar try_x, try_y;
...
boolean check_room(lowx, ddx, lowy, ddy, vault)
    xchar *lowx, *ddx, *lowy, *ddy;boolean vault;
{
    ...
    chk: if (hix <= *lowx || hiy <= *lowy) return FALSE;
 ...
 */
boolean create_room(x, y, w, h, xal, yal, rtype, rlit)
    xchar x, y;xchar w, h;xchar xal, yal;xchar rtype, rlit;
{
    xchar xabs, yabs;
 ...
 */
STATIC_OVL boolean create_subroom(proom, x, y, w, h, rtype, rlit)
    struct mkroom *proom;xchar x, y;xchar w, h;xchar rtype, rlit;
{
    xchar width, height;
 ...
 */
STATIC_OVL void create_door(dd, broom)
    room_door *dd;struct mkroom *broom;
{
    int x = 0, y = 0;
    int trycnt = 0, wtry = 0;
    ...
    do
    {
        ...
        outdirloop: if (okdoor(x, y)) break;
        redoloop: ;
    }
    while (++trycnt <= 100);
    if (trycnt > 100)
 ...
 */
void create_secret_door(croom, walls)
    struct mkroom *croom;xchar walls; /* any of W_NORTH | W_SOUTH | W_EAST | W_WEST (or W_ANY) */
{
 ...
 */
STATIC_OVL void create_trap(t, croom)
    trap *t;struct mkroom *croom;
{
    schar x, y;
    coord tm;
 ...
 */
STATIC_OVL int noncoalignment(alignment)
    aligntyp alignment;
{
    int k;
...
STATIC_OVL boolean m_bad_boulder_spot(x, y)
    int x, y;
{
    struct rm *lev;
...
STATIC_OVL void create_monster(m, croom)
    monster *m;struct mkroom *croom;
{
    struct monst *mtmp;
    schar x, y;
    char class;
    ...
    amask = (m->align == AM_SPLEV_CO) ? Align2amask(u.ualignbase[A_ORIGINAL]) :
    ...
    if (In_mines(&u.uz) && pm && your_race(pm)
            && (Race_if(PM_DWARF) || Race_if(PM_GNOME)) && rn2(3)) pm =
            (struct permonst *) 0;
 ...
 */
STATIC_OVL void create_object(o, croom)
    object *o;struct mkroom *croom;
{
    struct obj *otmp;
    schar x, y;
    char c;
        ...
        if (!container_idx)
        {
            ...
            else
            {
                ...
                for (ci = 0; ci < container_idx; ci++)
                    if (container_obj[ci] == objcheck) inuse = ci;
                remove_object(otmp);
    ...
    else if ((otmp->otyp == AMULET_OF_REFLECTION || otmp->otyp == BAG_OF_HOLDING)
 ...
 */
STATIC_OVL void create_altar(a, croom)
    altar *a;struct mkroom *croom;
{
    schar sproom, x, y;
    aligntyp amask;
...
void replace_terrain(terr, croom)
    replaceterrain *terr;struct mkroom *croom;
{
 ...
 */
STATIC_OVL boolean search_door(croom, x, y, wall, cnt)
    struct mkroom *croom;xchar *x, *y;xchar wall;int cnt;
{
    int dx, dy;
    int xx, yy;
 ...
 */
boolean dig_corridor(org, dest, nxcor, ftyp, btyp)
    coord *org, *dest;boolean nxcor;schar ftyp, btyp;
{
 ...
 * (from a distance).
 */
STATIC_OVL void create_corridor(c)
    corridor *c;
{
    coord org, dest;
 ...
 */
void fill_room(croom, prefilled)
    struct mkroom *croom;boolean prefilled;
{
...
struct mkroom * build_room(r, mkr)
    room *r;struct mkroom *mkr;
{
    boolean okroom;
    struct mkroom *aroom;
 ...
 */
STATIC_OVL void light_region(tmpregion)
    region *tmpregion;
{
        ...
        for (x = x1; x <= x2; x++)
        {
            ...
            for (yy = lo_yy; yy <= hi_yy; yy++)
                for (xx = lo_xx; xx <= hi_xx; xx++)
                    if (IS_ROOM(
                            levl[xx][yy].typ) || levl[xx][yy].typ == CROSSWALL)
                    {
 ...
 */
STATIC_OVL void maze1xy(m, humidity)
    coord *m;int humidity;
{
 ...
 */
STATIC_OVL boolean sp_level_loader(fd, lvl)
    dlb *fd;sp_lev *lvl;
{
    long n_opcode = 0;
    struct opvar *opdat;
...
STATIC_OVL boolean sp_level_free(lvl)
    sp_lev *lvl;
{
...
struct sp_frame * frame_new(execptr)
    long execptr;
{
...
long sp_code_jmpaddr(curpos, jmpaddr)
    long curpos, jmpaddr;
{
    ...
    struct sp_coder *coder;
{
            ...
            && (OV_i(varparam) >= 0) && (OV_i(varparam) < SP_M_V_END))
    {
        ...
        case SP_M_V_APPEAR:
            ...
            break;
        case SP_M_V_ASLEEP:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.asleep = OV_i(parm);
            break;
        case SP_M_V_ALIGN:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.align = OV_i(parm);
            break;
        case SP_M_V_PEACEFUL:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.peaceful = OV_i(parm);
            break;
        case SP_M_V_FEMALE:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.female = OV_i(parm);
            break;
        case SP_M_V_INVIS:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.invis = OV_i(parm);
            break;
        case SP_M_V_CANCELLED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.cancelled = OV_i(parm);
            break;
        case SP_M_V_REVIVED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.revived = OV_i(parm);
            break;
        case SP_M_V_AVENGE:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.avenge = OV_i(parm);
            break;
        case SP_M_V_FLEEING:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.fleeing = OV_i(parm);
            break;
        case SP_M_V_BLINDED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.blinded = OV_i(parm);
            break;
        case SP_M_V_PARALYZED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.paralyzed = OV_i(parm);
            break;
        case SP_M_V_STUNNED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.stunned = OV_i(parm);
            break;
        case SP_M_V_CONFUSED:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.confused = OV_i(parm);
            break;
        case SP_M_V_SEENTRAPS:
            if (OV_typ(parm) == SPOVAR_INT) tmpmons.seentraps = OV_i(parm);
            break;
        case SP_M_V_END:
        ...
        case SP_O_V_CORPSENM:
            if (OV_typ(parm) == SPOVAR_MONST)
            {
                ...
                else
                {
                    ...
                    if (pm) tmpobj.corpsenm = monsndx(pm);
                }
            }
            break;
        case SP_O_V_CURSE:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.curse_state = OV_i(parm);
            break;
        case SP_O_V_SPE:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.spe = OV_i(parm);
            break;
        case SP_O_V_QUAN:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.quan = OV_i(parm);
            break;
        case SP_O_V_BURIED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.buried = OV_i(parm);
            break;
        case SP_O_V_LIT:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.lit = OV_i(parm);
            break;
        case SP_O_V_ERODED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.eroded = OV_i(parm);
            break;
        case SP_O_V_LOCKED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.locked = OV_i(parm);
            break;
        case SP_O_V_TRAPPED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.trapped = OV_i(parm);
            break;
        case SP_O_V_RECHARGED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.recharged = OV_i(parm);
            break;
        case SP_O_V_INVIS:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.invis = OV_i(parm);
            break;
        case SP_O_V_GREASED:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.greased = OV_i(parm);
            break;
        case SP_O_V_BROKEN:
            if (OV_typ(parm) == SPOVAR_INT) tmpobj.broken = OV_i(parm);
            break;
        case SP_O_V_COORD:
            if (OV_typ(parm) != SPOVAR_COORD) panic("no coord for obj?");
    ...
    struct sp_coder *coder;
{
    ...
    if (lflags & CHECK_INACCESSIBLES) coder->check_inaccessibles = TRUE;

    opvar_free(flagdata);
}

void spo_initlevel(coder)
    struct sp_coder *coder;
{
    ...
    || !OV_pop_i(filling) || !OV_pop_i(init_style)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(etyp) || !OV_pop_s(txt) || !OV_pop_c(ecoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    {
        struct opvar *rflags, *h, *w, *yalign, *xalign, *y, *x, *rlit, *chance,
                *rtype;
        room tmproom;
        struct mkroom *tmpcr;
        ...
        || !OV_pop_i(rlit) || !OV_pop_i(chance) || !OV_pop_i(rtype)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(up) || !OV_pop_c(scoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(up) || !OV_pop_c(lcoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(typ) || !OV_pop_s(txt) || !OV_pop_c(gcoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(al) || !OV_pop_i(shrine) || !OV_pop_c(acoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(type) || !OV_pop_c(tcoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    || !OV_pop_i(srcwall) || !OV_pop_i(srcdoor) || !OV_pop_i(srcroom)) return;
...
struct opvar * selection_opvar(nbuf)
    char *nbuf;
{
    struct opvar *ov;
...
xchar selection_getpoint(x, y, ov)
    int x, y;struct opvar *ov;
{
...
void selection_setpoint(x, y, ov, c)
    int x, y;struct opvar *ov;xchar c;
{
    ...
    struct opvar *s;
{
    ...
    for (x = 0; x < COLNO; x++)
        for (y = 0; y < ROWNO; y++)
            if (!selection_getpoint(x, y, s)) selection_setpoint(x, y, ov, 1);

    return ov;
}

struct opvar * selection_logical_oper(s1, s2, oper)
    struct opvar *s1, *s2;char oper;
{
    struct opvar *ov;
    int x, y;
...
struct opvar * selection_filter_mapchar(ov, mc)
    struct opvar *ov;struct opvar *mc;
{
    int x, y;
    schar mapc;
    xchar lit;
...
void selection_filter_percent(ov, percent)
    struct opvar *ov;int percent;
{
    int x, y;
...
STATIC_OVL int selection_rndcoord(ov, x, y, removeit)
    struct opvar *ov;schar *x, *y;boolean removeit;
{
    int idx = 0;
    int c;
    int dx, dy;
...
void selection_do_grow(ov, dir)
    struct opvar *ov;int dir;
{
    int x, y, c;
...
STATIC_OVL void set_selection_floodfillchk(f)
    int FDECL((*f), (int, int));
{
    selection_flood_check_func = f;
}

STATIC_OVL int floodfillchk_match_under(x, y)
    int x, y;
{
    return (floodfillchk_match_under_typ == levl[x][y].typ);
}

STATIC_OVL int floodfillchk_match_accessible(x, y)
    int x, y;
{
    return (ACCESSIBLE(levl[x][y].typ) || levl[x][y].typ == SDOOR
            || levl[x][y].typ == SCORR);
}

STATIC_OVL void selection_floodfill(ov, x, y, diagonals)
    struct opvar *ov;int x, y;boolean diagonals;
{
...
void selection_do_ellipse(ov, xc, yc, a, b, filled)
    struct opvar *ov;int xc, yc, a, b, filled;
...
long line_dist_coord(x1, y1, x2, y2, x3, y3)
...
void selection_do_gradient(ov, x, y, x2, y2, gtyp, mind, maxd, limit)
    struct opvar *ov;long x, y, x2, y2, gtyp, mind, maxd, limit;
{
    long dx, dy, dofs;
...
/* bresenham line algo */
void selection_do_line(x1, y1, x2, y2, ov)
    schar x1, y1, x2, y2;struct opvar *ov;
{
...
void selection_do_randline(x1, y1, x2, y2, rough, rec, ov)
    schar x1, y1, x2, y2, rough, rec;struct opvar *ov;
{
    int mx, my;
    int dx, dy;
...
void selection_iterate(ov, func, arg)
...
void sel_set_door(dx, dy, arg)
    int dx, dy;genericptr_t arg;
{
    xchar typ = *(xchar *) arg;
    xchar x = dx;
    xchar y = dy;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(msk) || !OV_pop_typ(sel, SPOVAR_SEL)) return;
...
void spo_replace_terrain(coder)
    struct sp_coder *coder;
{
    ...
    || !OV_pop_typ(from_ter, SPOVAR_MAPCHAR) || !OV_pop_r(reg)) return;
...
STATIC_OVL boolean generate_way_out_method(nx, ny, ov)
    int nx, ny;struct opvar *ov;
{
    ...
    if (Can_fall_thru(&u.uz))
    {
        ...
        while (selection_rndcoord(ov3, &x, &y, TRUE))
        {
            if (maketrap(x, y, rn2(2) ? HOLE : TRAPDOOR)) goto gotitdone;
        }
    }
...
STATIC_OVL void ensure_way_out()
{
    ...
    struct sp_coder *coder;
{
    ...
    if (OV_i(rtype) != OROOM) troom->needfill = (prefilled ? 2 : 1);
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(dir) || !OV_pop_i(db_open) || !OV_pop_c(dcoord)) return;
    ...
    struct sp_coder *coder;
{
    ...
    if (!OV_pop_i(wall) || !OV_pop_i(secret) || !OV_pop_i(mask)
            || !OV_pop_i(pos) || !coder->croom) return;
...
/*ARGSUSED*/
void sel_set_wallify(x, y, arg)
    int x, y;genericptr_t arg UNUSED;
{
    wallify_map(x, y, x, y);
}

void spo_wallify(coder)
    struct sp_coder *coder;
{
    ...
    struct sp_coder *coder;
{
    ...
    || !OV_pop_i(mpkeepr) || !OV_pop_i(mpzalign) || !OV_pop_c(mpa)) return;
...
void spo_jmp(coder, lvl)
    struct sp_coder *coder;sp_lev *lvl;
{
...
void spo_conditional_jump(coder, lvl)
    struct sp_coder *coder;sp_lev *lvl;
{
    ...
    if ((test) && (a >= 0) && (a < lvl->n_opcodes)
            && (a != coder->frame->n_opcode)) coder->frame->n_opcode = a;

    opvar_free(oa);
    opvar_free(oc);
}

void spo_var_init(coder)
    struct sp_coder *coder;
{
    ...
    if (!vname || !arraylen) panic("no values for SPO_VAR_INIT");
    ...
    else
    {
        ...
        if (OV_i(arraylen) < 0)
        {
            ...
            {
                idx = tmpvar->array_len;
                tmpvar->data.arrayvalues = (struct opvar **) alloc(
                        sizeof(struct opvar *) * idx);
                while (idx-- > 0)
                {
                    tmpvar->data.arrayvalues[idx] = opvar_clone(
                            tmp2->data.arrayvalues[idx]);
                }
            }
            else
            {
        ...
        {
            /* new array */
            create_new_array: idx = OV_i(arraylen);
 ...
 */
STATIC_OVL boolean sp_level_coder(lvl)
    sp_lev *lvl;
{
    ...
    while (coder->frame->n_opcode < lvl->n_opcodes && !coder->exit_script)
    {
        ...
        case SPO_MATH_ADD:
        {
            ...
            if (OV_typ(b) == OV_typ(a))
            {
                ...
                {
                    struct opvar *c;
                    char *tmpbuf = (char *) alloc(
                            strlen(OV_s(a)) + strlen(OV_s(b)) + 1);
        ...
        case SPO_SEL_LINE:
        {
            struct opvar *tmp = NULL, *tmp2 = NULL, *pt = selection_opvar(
                    (char *) 0);
            schar x1, y1, x2, y2;
        ...
        {
            struct opvar *tmp = NULL, *tmp2 = NULL, *tmp3, *pt =
        ...
        next_opcode: coder->frame->n_opcode++;
    } /*while*/
    ...
    if (coder->check_inaccessibles) ensure_way_out();
     ...
     * purpose.
     */
    if (!level.flags.corrmaze) wallification(1, 0, COLNO - 1, ROWNO - 1);
