 ...
 */
STATIC_OVL boolean wrong_elem_type(ptr)
    struct permonst *ptr;
{
    ...
    {
        if (!(is_flyer(ptr) && ptr->mlet != S_TRAPPER)
                && !is_floater(
                        ptr) && !amorphous(ptr) && !noncorporeal(ptr) && !is_whirly(ptr))
            return TRUE;
    }
    return FALSE;
}
...
STATIC_OVL void m_initgrp(mtmp, x, y, n)
    register struct monst *mtmp;register int x, y, n;
{
    coord mm;
...
STATIC_OVL
void m_initthrow(mtmp, otyp, oquan)
    struct monst *mtmp;int otyp, oquan;
{
...
STATIC_OVL void m_initweap(mtmp)
 ...
 */
void mkmonmoney(mtmp, amount)
    struct monst *mtmp;long amount;
{
...
STATIC_OVL void m_initinv(mtmp)
    register struct monst *mtmp;
{
    register int cnt;
    register struct obj *otmp;
    ...
    case S_HUMAN:
        if (is_mercenary(ptr))
        {
            ...
            else if (mac < 10 && rn2(2)) mac += 1 + mongets(mtmp, DENTED_POT);
            if (mac < 10 && rn2(3))
                mac += 1 + mongets(mtmp, SMALL_SHIELD);
            else if (mac < 10 && rn2(2)) mac += 2 + mongets(mtmp, LARGE_SHIELD);
            if (mac < 10 && rn2(3))
                mac += 1 + mongets(mtmp, LOW_BOOTS);
            else if (mac < 10 && rn2(2)) mac += 2 + mongets(mtmp, HIGH_BOOTS);
            if (mac < 10 && rn2(3))
...
struct monst * clone_mon(mon, x, y)
    struct monst *mon;xchar x, y; /* clone's preferred location or 0 (near mon) */
{
    coord mm;
    struct monst *m2;
 ...
 */
boolean propagate(mndx, tally, ghostly)
    int mndx;boolean tally;boolean ghostly;
{
    boolean result;
...
boolean makemon_rnd_goodpos(mon, gpflags, cc)
    struct monst *mon;unsigned gpflags;coord *cc;
{
    int tryct = 0;
    int nx, ny;
    boolean good;
        ...
        for (; bl < 2; bl++)
        {
            for (dx = 0; dx < COLNO; dx++)
                for (dy = 0; dy < ROWNO; dy++)
                {
                    ...
                    if (goodpos(nx, ny, mon, gpflags)) goto gotgood;
                }
 ...
 */
struct monst * makemon(ptr, x, y, mmflags)
    register struct permonst *ptr;register int x, y;int mmflags;
{
    register struct monst *mtmp;
    ...
    if (x == 0 && y == 0)
    {
        ...
        if (!makemon_rnd_goodpos(ptr ? &fakemon : (struct monst *) 0, gpflags,
                &cc)) return (struct monst *) 0;
        x = cc.x;
        y = cc.y;
    }
    ...
    *mtmp = zeromonst; /* clear all entries in structure */
    ...
    if (ptr->msound == MS_LEADER || ptr->msound == MS_NEMESIS) mtmp->mtrapseen =
            ~0;
    ...
    case S_SPIDER:
    case S_SNAKE:
        if (in_mklev) if (x && y) (void) mkobj_at(0, x, y, TRUE);
        (void) hideunder(mtmp);
        break;
    case S_LIGHT:
    case S_ELEMENTAL:
    ...
    case S_UNICORN:
        ...
        break;
    case S_BAT:
        if (Inhell && is_bat(ptr)) mon_adjust_speed(mtmp, 2, (struct obj *) 0);
        break;
    }
    ...
    if ((mcham = pm_to_cham(mndx)) != NON_PM)
    {
        ...
        if (Protection_from_shape_changers || mndx == PM_VLAD_THE_IMPALER)
        {
    ...
    else if (mndx == PM_WIZARD_OF_YENDOR)
    {
        ...
        if (context.no_of_wizards == 1 && Is_earthlevel(&u.uz)) mitem = SPE_DIG;
    }
    ...
    if (ptr->mflags3 && !(mmflags & MM_NOWAIT))
    {
        ...
        if (ptr->mflags3 & (M3_WAITMASK | M3_COVETOUS)) mtmp->mstrategy |=
        STRAT_APPEARMSG;
    }
...
boolean create_critters(cnt, mptr, neverask)
    int cnt;struct permonst *mptr; /* usually null; used for confused reading */
    boolean neverask;
{
    coord c;
    int x, y;
...
STATIC_OVL boolean uncommon(mndx)
    int mndx;
{
 ...
 */
STATIC_OVL int align_shift(ptr)
...
STATIC_OVL boolean mk_gen_ok(mndx, mvflagsmask, genomask)
 ...
 */
struct permonst * mkclass(class, spc)
    char class;int spc;
{
...
struct permonst * grow_up(mtmp, victim)
    struct monst *mtmp, *victim;
{
    ...
    if (mtmp->mhp <= 0) return (struct permonst *) 0;
    ...
    { /* killed a monster */
        ...
        if (oldtype != newtype && mons[newtype].mlevel > lev_limit) lev_limit =
                (int) mons[newtype].mlevel;
    ...
    if ((int) ++mtmp->m_lev >= mons[newtype].mlevel && newtype != oldtype)
    {
        ...
        else if (canspotmon(mtmp))
        {
            ...
            humanoid(ptr) ? "becomes" : "grows up into", an(ptr->mname));
        }
...
int mongets(mtmp, otyp)
    register struct monst *mtmp;int otyp;
{
    register struct obj *otmp;
    int spe;
    ...
    if (otmp)
    {
        ...
        if (is_prince(mtmp->data))
        {
            if (otmp->oclass == WEAPON_CLASS && otmp->spe < 1)
                otmp->spe = 1;
            else if (otmp->oclass == ARMOR_CLASS && otmp->spe < 0) otmp->spe =
                    0;
        }
    ...
    register struct permonst *ptr;
{
     ...
     */
    return (boolean) (!!rn2(
            16 + (u.ualign.record < -15 ? -15 : u.ualign.record))
            && !!rn2(2 + abs(mal)));
}
    ...
    register struct monst *mtmp;
{
    ...
    else
    {
        ...
        assign_sym: if (s_sym == MAXOCLASSES || s_sym == MAXOCLASSES + 1)
        {
            ap_type = M_AP_FURNITURE;
    ...
    if (ap_type == M_AP_OBJECT
            && (appear == STATUE || appear == CORPSE || appear == FIGURINE
                    || appear == EGG))
    {
        newmcorpsenm(mtmp);
...
int bagotricks(bag, tipping, seencount)
    struct obj *bag;boolean tipping; /* caller emptying entire contents; affects shop handling */
