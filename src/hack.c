...
FDECL(monstinroom, (struct permonst *, int));
...
boolean revive_nasty(x, y, msg)
    int x, y;const char *msg;
{
 ...
 *  when done.
 */
STATIC_OVL int still_chewing(x, y)
    xchar x, y;
{
...
void movobj(obj, ox, oy)
    register struct obj *obj;register xchar ox, oy;
{
...
STATIC_OVL void dosinkfall()
{
    ...
    int dmg;
    boolean lev_boots = (uarmf && uarmf->otyp == LEVITATION_BOOTS), innate_lev =
            ((HLevitation& (FROMOUTSIDE | FROMFORM)) != 0L),
...
boolean bad_rock(mdat, x, y)
    struct permonst *mdat;register xchar x, y;
{
    ...
    struct monst *mon;
{
    ...
    if (bigmonst(ptr)
            && !(amorphous(ptr) || is_whirly(ptr) || noncorporeal(ptr)
                    || slithy(ptr) || can_fog(mon))) return 1;
 ...
 */
boolean test_move(ux, uy, dx, dy, mode)
    int ux, uy, dx, dy;int mode;
{
    int x = ux + dx;
    int y = uy + dy;
    ...
    if (IS_ROCK(tmpr->typ) || tmpr->typ == IRONBARS)
    {
        ...
        else if (tmpr->typ == IRONBARS)
        {
            ...
            {
                if (iflags.mention_walls) You("cannot pass through the bars.");
                return FALSE;
            }
        }
        ...
        else
        {
            if (mode == DO_MOVE)
            {
                ...
                /* sokoban restriction stays even after puzzle is solved */
                        ...
                        && !may_passwall(x, y) && In_sokoban(&u.uz))
                    pline_The("Sokoban walls resist your ability.");
                else if (iflags.mention_walls) pline("It's a wall.");
            }
            return FALSE;
        }
    }
    ...
    if (sobj_at(BOULDER, x, y) && (Sokoban || !Passes_walls))
    {
        ...
        if (mode == DO_MOVE)
        {
            ...
            if (tunnels(youmonst.data) && !needspick(youmonst.data) && !Sokoban)
            {
 ...
 */
STATIC_OVL boolean findtravelpath(guess)
    boolean guess;
{
    ...
    if (u.tx != u.ux || u.ty != u.uy)
    {
        ...
        noguess: (void) memset((genericptr_t) travel, 0, sizeof(travel));
        travelstepx[0][0] = tx;
        travelstepy[0][0] = ty;
        ...
        if (guess)
        {
            ...
            if (px == u.ux && py == u.uy)
            {
                ...
                u.dx = sgn(u.tx - u.ux);
                u.dy = sgn(u.ty - u.uy);
                if (test_move(u.ux, u.uy, u.dx, u.dy, TEST_MOVE)) return TRUE;
                goto found;
            }
#ifdef DEBUG
            if (trav_debug)
...
STATIC_OVL boolean trapmove(x, y, desttrap)
    ...
    case TT_BURIEDBALL:
        ...
        if (anchored)
        {
            ...
            if (buried_ball(&cc) && dist2(x, y, cc.x, cc.y) <= 2)
            {
                ...
                if (flags.verbose) Norep("You move within the chain's reach.");
                return TRUE;
            }
        }
        if (--u.utrap)
        ...
        else
        {
            ...
            if (anchored) buried_ball_to_punishment();
        }
        break;
    default:
...
void domove()
{
    ...
    else
    {
        ...
        if (on_ice)
        {
            ...
            if ((uarmf && uarmf->otyp == skates)
                    || resists_cold(
                            &youmonst) || Flying || is_floater(youmonst.data)
        ...
        if (((trap = t_at(x, y)) && trap->tseen)
                || (Blind && !Levitation && !Flying
                        && !is_clinger(youmonst.data) && is_pool_or_lava(x, y)
                        && levl[x][y].seenv))
        {
            if (context.run >= 2)
            {
        ...
        if (u.ustuck && (x != u.ustuck->mx || y != u.ustuck->my))
        {
            ...
            else
            {
                ...
                case 1:
                case 2:
                    pull_free: You("pull free from %s.", mon_nam(u.ustuck));
                    u.ustuck = 0;
                    break;
                case 3:
    ...
    || (glyph_is_invisible(levl[x][y].glyph) && !context.nopick))
    {
        ...
        boolean explo = (Upolyd && attacktype(youmonst.data, AT_EXPL)), solid =
                !accessible(x, y);
        ...
        if (!Underwater)
        {
            ...
            if (glyph_is_statue(glyph)
                    || (Hallucination && glyph_is_monster(glyph))) boulder =
                    sobj_at(STATUE, x, y);
    ...
    if (context.run)
    {
        if (context.run < 8)
            if (IS_DOOR(
                    tmpr->typ) || IS_ROCK(tmpr->typ) || IS_FURNITURE(tmpr->typ))
                nomul(0);
    }

    if (hides_under(youmonst.data) || (youmonst.data->mlet == S_EEL) || u.dx
            || u.dy) (void) hideunder(&youmonst);
...
STATIC_OVL void switch_terrain()
{
    ...
    boolean pick;
{
    ...
    && (!spottrap || !trap || trap->ttyp == spottraptyp)) return;
...
STATIC_OVL struct monst * monstinroom(mdat, roomno)
    struct permonst *mdat;int roomno;
{
    register struct monst *mtmp;
...
char * in_rooms(x, y, typewanted)
    register xchar x, y;register int typewanted;
{
    static char buf[5];
    char rno, *ptr = &buf[4];
...
#define goodtype(rno)                                               \
    ...
    for (x = min_x; x <= max_x; x += step)
    {
        ...
        if (((rno = lev[y].roomno) >= ROOMOFFSET)
                && !index(ptr, rno) && goodtype(rno)) *(--ptr) = rno;
        y += step;
...
STATIC_OVL void move_update(newlev)
    register boolean newlev;
{
    ...
    {
        if (!index(u.urooms0, *ptr1)) *(ptr2++) = *ptr1;
        if (IS_SHOP(*ptr1 - ROOMOFFSET))
        {
            *(ptr3++) = *ptr1;
            if (!index(u.ushops0, *ptr1)) *(ptr4++) = *ptr1;
        }
    }
    *ptr2 = '\0';
    *ptr3 = '\0';
    *ptr4 = '\0';
    ...
    for (ptr1 = &u.ushops0[0], ptr2 = &u.ushops_left[0]; *ptr1; ptr1++)
        if (!index(u.ushops, *ptr1)) *(ptr2++) = *ptr1;
    *ptr2 = '\0';
}

void check_special_room(newlev)
    register boolean newlev;
{
    register struct monst *mtmp;
    char *ptr;
    ...
    if (*u.ushops0) u_left_shop(u.ushops_left, newlev);

    if (!*u.uentered && !*u.ushops_entered) /* implied by newlev */
    return; /* no entrance messages necessary */

    /* Did we just enter a shop? */
    if (*u.ushops_entered) u_entered_shop(u.ushops_entered);
        ...
        for (y = u.uy - 1; y <= u.uy + 1; y++)
        {
            ...
            { /* e.g. objects or trap or stairs */
                ...
                if (((x == u.ux - u.dx) && (y != u.uy + u.dy))
                        || ((y == u.uy - u.dy) && (x != u.ux + u.dx))) continue;
            }
            stop: nomul(0);
            return;
        } /* end for loops */
...
STATIC_OVL boolean doorless_door(x, y)
    int x, y;
{
    ...
    for (x = u.ux - 1; x <= u.ux + 1; x++)
        for (y = u.uy - 1; y <= u.uy + 1; y++)
        {
            ...
            if ((mtmp = m_at(x, y)) && mtmp->m_ap_type != M_AP_FURNITURE
                    ...
                    && !onscary(u.ux, u.uy, mtmp) && canspotmon(mtmp)) return 1;
        }
    return 0;
}
...
STATIC_OVL void maybe_wail()
{
    ...
    if (Role_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE))
    {
        ...
        else
        {
            ...
            pline(
                    powercnt >= 4 ?
                            "%s, all your powers will be lost..." :
...
void losehp(n, knam, k_format)
    register int n;register const char *knam;boolean k_format;
{
    if (Upolyd)
    {
        u.mh -= n;
    ...
    u.uhp -= n;
    if (u.uhp > u.uhpmax) u.uhpmax = u.uhp; /* perhaps n was negative */
    context.botl = 1;
    if (u.uhp < 1)
    {
...
int inv_weight()
{
    ...
    while (otmp)
    {
        ...
        else if (otmp->otyp != BOULDER || !throws_rocks(youmonst.data)) wt +=
                otmp->owt;
        otmp = otmp->nobj;
    }
    wc = weight_cap();
