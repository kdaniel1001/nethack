...
int dosave0()
{
    ...
    if (!program_state.something_worth_saving || !SAVEF[0]) return 0;
...
STATIC_OVL void savegamestate(fd, mode)
...
boolean tricked_fileremoved(fd, whynot)
    int fd;char *whynot;
{
    if (fd < 0)
    {
...
savelev0(fd, lev, mode)
#else
void savelev(fd, lev, mode)
#endif
    int fd;xchar lev;int mode;
{
#ifdef TOS
    short tlev;
#endif
...
STATIC_OVL void savelevl(fd, rlecomp)
...
void bwrite(fd, loc, num)
    int fd;genericptr_t loc;register unsigned num;
{
...
STATIC_OVL void def_bwrite(fd, loc, num)
    register int fd;register genericptr_t loc;register unsigned num;
{
    boolean failed;
...
STATIC_OVL void savelevchn(fd, mode)
    register int fd, mode;
{
...
void savecemetery(fd, mode, cemeteryaddr)
    int fd;int mode;struct cemetery **cemeteryaddr;
{
    ...
    if (perform_bwrite(mode)) bwrite(fd, (genericptr_t) &flag, sizeof flag);
    ...
    while ((thisbones = nextbones) != 0)
    {
        ...
        if (release_data(mode)) free((genericptr_t) thisbones);
    }
    if (release_data(mode)) *cemeteryaddr = 0;
}

STATIC_OVL void savedamage(fd, mode)
    register int fd, mode;
{
...
STATIC_OVL void saveobj(fd, otmp)
    int fd;struct obj *otmp;
{
    ...
    if (otmp->oextra)
    {
        ...
        if (buflen > 0) bwrite(fd, (genericptr_t) ONAME(otmp), buflen);
        ...
        if (OMID(otmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof buflen);
        if (buflen > 0) bwrite(fd, (genericptr_t) OMID(otmp), buflen);
        ...
        if (OLONG(otmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof buflen);
        if (buflen > 0) bwrite(fd, (genericptr_t) OLONG(otmp), buflen);
        ...
        if (OMAILCMD(otmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof buflen);
        if (buflen > 0) bwrite(fd, (genericptr_t) OMAILCMD(otmp), buflen);
    }
}

STATIC_OVL void saveobjchn(fd, otmp, mode)
    register int fd, mode;register struct obj *otmp;
{
    ...
    while (otmp)
    {
        ...
        if (release_data(mode))
        {
            ...
            if (otmp == context.victual.piece)
            {
                ...
                if (context.victual.o_id != otmp->o_id) context.victual.o_id =
                        otmp->o_id;
            ...
            if (otmp == context.spbook.book)
            {
                ...
                if (context.spbook.o_id != otmp->o_id) context.spbook.o_id =
                        otmp->o_id;
                /* invalidate the pointer; on reload it will get restored */
...
STATIC_OVL void savemon(fd, mtmp)
    int fd;struct monst *mtmp;
{
    ...
    if (mtmp->mextra)
    {
        if (MNAME(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof buflen);
        if (buflen > 0) bwrite(fd, (genericptr_t) MNAME(mtmp), buflen);
        ...
        if (EGD(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof(int));
        if (buflen > 0) bwrite(fd, (genericptr_t) EGD(mtmp), buflen);
        ...
        if (EPRI(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof(int));
        if (buflen > 0) bwrite(fd, (genericptr_t) EPRI(mtmp), buflen);
        ...
        if (ESHK(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof(int));
        if (buflen > 0) bwrite(fd, (genericptr_t) ESHK(mtmp), buflen);
        ...
        if (EMIN(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof(int));
        if (buflen > 0) bwrite(fd, (genericptr_t) EMIN(mtmp), buflen);
        ...
        if (EDOG(mtmp))
            ...
            buflen = 0;
        bwrite(fd, (genericptr_t) &buflen, sizeof(int));
        if (buflen > 0) bwrite(fd, (genericptr_t) EDOG(mtmp), buflen);
...
STATIC_OVL void savemonchn(fd, mtmp, mode)
    register int fd, mode;register struct monst *mtmp;
{
...
STATIC_OVL void savetrapchn(fd, trap, mode)
    register int fd, mode;register struct trap *trap;
{
...
STATIC_OVL void save_msghistory(fd, mode)
    int fd, mode;
{
    char *msg;
