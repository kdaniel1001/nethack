...
{ "Speleologist", 0 },
{ "Collector", 0 },
{ "Curator", 0 } }, "Quetzalcoatl", "Camaxtli", "Huhetotl", /* Central American */
...
{ "Conqueror", "Conqueress" } }, "Mitra", "Crom", "Set", /* Hyborian */
...
{ "Nomad", 0 },
{ "Rover", 0 },
{ "Pioneer", 0 } }, "Anu", "_Ishtar", "Anshar", /* Babylonian */
...
{ "Magister", "Magistra" },
{ "Physician", 0 },
{ "Chirurgeon", 0 } }, "_Athena", "Hermes", "Poseidon", /* Greek */
...
{ "Seignieur", "Dame" },
{ "Paladin", 0 } }, "Lugh", "_Brigit", "Manannan Mac Lir", /* Celtic */
...
{ "Student of Winds", 0 },
{ "Student of Fire", 0 },
{ "Master", 0 } }, "Shan Lai Ching", "Chih Sung-tzu", "Huan Ti", /* Chinese */
...
{ "Lama", 0 },
{ "Patriarch", "Matriarch" },
{ "High Priest", "High Priestess" } }, 0, 0, 0, /* deities from a randomly chosen other role will be used */
...
{ "Ranger", 0 },
{
        ...
        { "Archer", 0 },
        { "Sharpshooter", 0 },
        { "Marksman", "Markswoman" } }, "Mercury", "_Venus", "Mars", /* Roman/planets */
...
MH_HUMAN | MH_ELF | MH_GNOME | MH_ORC | ROLE_MALE | ROLE_FEMALE | ROLE_NEUTRAL
        | ROLE_CHAOTIC,
/* Str Int Wis Dex Con Cha */
{ 13, 13, 13, 9, 13, 7 },
{ 30, 10, 10, 20, 20, 10 },
...
{ "Voyager", 0 },
{ "Explorer", 0 },
{ "Adventurer", 0 } }, "Blind Io", "_The Lady", "Offler", /* Discworld */
...
{ "Necromancer", 0 },
{ "Wizard", 0 },
{ "Mage", 0 } }, "Ptah", "Thoth", "Anhur", /* Egyptian */
...
{ 0, 0 },
{ 0, 0 },
{ 0, 0 } }, "L", "N", "C", "Xxx", "home", "locate",
NON_PM, NON_PM, NON_PM, NON_PM, NON_PM, NON_PM, NON_PM, NON_PM, 0, 0, 0, 0,
/* Str Int Wis Dex Con Cha */
{ 7, 7, 7, 7, 7, 7 },
{ 20, 15, 15, 20, 20, 10 },
/* Init   Lower  Higher */
...
STATIC_OVL int randrole_filtered()
{
    ...
    for (i = 0; i < SIZE(roles); ++i)
        if (ok_role(i, ROLE_NONE, ROLE_NONE, ROLE_NONE)
                ...
                && ok_align(i, ROLE_NONE, ROLE_NONE, ROLE_RANDOM)) set[n++] = i;
    ...
    const char *str;
{
    ...
    if ((len == 1 && (*str == '*' || *str == '@'))
            || !strncmpi(str, randomstr, len)) return ROLE_RANDOM;

    /* Couldn't find anything appropriate */
    return ROLE_NONE;
}

boolean validrace(rolenum, racenum)
    int rolenum, racenum;
{
    /* Assumes validrole */
...
boolean validgend(rolenum, racenum, gendnum)
...
boolean validalign(rolenum, racenum, alignnum)
...
boolean ok_role(rolenum, racenum, gendnum, alignnum)
    int rolenum, racenum, gendnum, alignnum;
{
    ...
    {
        if (filter.roles[rolenum]) return FALSE;
...
int pick_role(racenum, gendnum, alignnum, pickhow)
...
boolean ok_race(rolenum, racenum, gendnum, alignnum)
    int rolenum, racenum, gendnum, alignnum;
{
    int i;
    short allow;
    ...
    if (racenum >= 0 && racenum < SIZE(races) - 1)
    {
        if (filter.mask & races[racenum].selfmask) return FALSE;
    ...
    else
    {
        ...
        {
            if (filter.mask & races[i].selfmask) continue;
            allow = races[i].allow;
            if (rolenum >= 0 && rolenum < SIZE(roles) - 1
                    && !(allow & roles[rolenum].allow & ROLE_RACEMASK))
                continue;
...
/* a single possibility */
int pick_race(rolenum, gendnum, alignnum, pickhow)
...
boolean ok_gend(rolenum, racenum, gendnum, alignnum)
    int rolenum, racenum, gendnum;int alignnum UNUSED;
{
    int i;
    short allow;
    ...
    if (gendnum >= 0 && gendnum < ROLE_GENDERS)
    {
        if (filter.mask & genders[gendnum].allow) return FALSE;
    ...
    else
    {
        ...
        for (i = 0; i < ROLE_GENDERS; i++)
        {
            if (filter.mask & genders[i].allow) continue;
...
/* a single possibility */
int pick_gend(rolenum, racenum, alignnum, pickhow)
...
boolean ok_align(rolenum, racenum, gendnum, alignnum)
    int rolenum, racenum;int gendnum UNUSED;int alignnum;
{
    int i;
    short allow;
    ...
    if (alignnum >= 0 && alignnum < ROLE_ALIGNS)
    {
        if (filter.mask & aligns[alignnum].allow) return FALSE;
    ...
    else
    {
        ...
        for (i = 0; i < ROLE_ALIGNS; i++)
        {
            if (filter.mask & aligns[i].allow) return FALSE;
...
/* a single possibility */
int pick_align(rolenum, racenum, gendnum, pickhow)
...
boolean gotrolefilter()
{
    int i;

    if (filter.mask) return TRUE;
    for (i = 0; i < SIZE(roles); ++i)
        if (filter.roles[i]) return TRUE;
    return FALSE;
}
...
STATIC_OVL char * promptsep(buf, num_post_attribs)
    char *buf;int num_post_attribs;
{
...
STATIC_OVL int role_gendercount(rolenum)
    int rolenum;
{
    int gendcount = 0;
...
STATIC_OVL int race_alignmentcount(racenum)
    int racenum;
{
    int aligncount = 0;
...
char * root_plselection_prompt(suppliedbuf, buflen, rolenum, racenum, gendnum,
        alignnum)
    char *suppliedbuf;int buflen, rolenum, racenum, gendnum, alignnum;
{
    ...
    *suppliedbuf = '\0';
    ...
    if (racenum != ROLE_NONE && racenum != ROLE_RANDOM) aligncount =
    ...
    if (racenum != ROLE_NONE && racenum != ROLE_RANDOM)
    {
        if (validrole(rolenum) && ok_race(rolenum, racenum, gendnum, alignnum))
        {
            ...
            Strcat (buf,
                    (rolenum == ROLE_NONE) ?
                            races[racenum].noun : races[racenum].adj);
            donefirst = TRUE;
        }
...
char * build_plselection_prompt(buf, buflen, rolenum, racenum, gendnum,
        alignnum)
    char *buf;int buflen, rolenum, racenum, gendnum, alignnum;
{
...
void plnamesuffix()
{
...
#ifdef GENERIC_USERNAMES
    {
        ...
        if ((sptr = strstri(uptr, plname)) != 0
                && (sptr == uptr || sptr[-1] == ' ')
                && (sptr[i] == ' ' || sptr[i] == '\0')) *plname = '\0'; /* call askname() */
    }
#endif
    ...
    do
    {
        ...
        while (eptr)
        {
            ...
            if ((i = str2role(sptr)) != ROLE_NONE)
                ...
                flags.initgend = i;
            else if ((i = str2align(sptr)) != ROLE_NONE) flags.initalign = i;
        }
    }
    while (!*plname);
...
void role_selection_prolog(which, where)
    int which;winid where;
{
    static const char NEARDATA choosing[] = " choosing now", not_yet[] =
            " not yet specified", rand_choice[] = " random";
    char buf[BUFSZ];
    ...
    if (r >= 0)
    {
        ...
        if ((allowmask & ROLE_RACEMASK) == MH_HUMAN)
            c = 0; /* races[human] */
        else if (c >= 0 && !(allowmask & ROLE_RACEMASK & races[c].allow)) c =
        ROLE_RANDOM;
        if ((allowmask & ROLE_GENDMASK) == ROLE_MALE)
            g = 0; /* role forces male (hypothetical) */
        else if ((allowmask & ROLE_GENDMASK) == ROLE_FEMALE) g = 1; /* role forces female (valkyrie) */
        if ((allowmask & ROLE_ALIGNMASK) == AM_LAWFUL)
            a = 0; /* aligns[lawful] */
        else if ((allowmask & ROLE_ALIGNMASK) == AM_NEUTRAL)
            a = 1; /* aligns[neutral] */
        else if ((allowmask & ROLE_ALIGNMASK) == AM_CHAOTIC) a = 2; /* alings[chaotic] */
    }
    if (c >= 0)
    {
    ...
    Strcat (buf,
            (which == RS_ROLE) ? choosing : (r == ROLE_NONE) ? not_yet :
    ...
    int which;winid where;
{
    ...
    case RS_RACE:
        ...
        if (r >= 0)
        {
            ...
            else if (f >= 0 && (allowmask & ~filter.mask) == races[f].selfmask)
            {
    ...
    case RS_GENDER:
        ...
        if (r >= 0)
        {
            ...
            if (allowmask == ROLE_MALE)
                g = 0; /* genders[male] */
            else if (allowmask == ROLE_FEMALE) g = 1; /* genders[female] */
            if (g >= 0)
            {
                constrainer = "role";
                forcedvalue = genders[g].adj;
            }
            else if (f >= 0 && (allowmask & ~filter.mask) == genders[f].allow)
            {
...
void role_init()
{
    ...
    if (mons[urace.malenum].mflags3 & M3_INFRAVISION)
    {
        ...
        if (urole.femalenum != NON_PM) mons[urole.femalenum].mflags3 |=
        M3_INFRAVISION;
    }
