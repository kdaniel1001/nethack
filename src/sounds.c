...
STATIC_OVL int mon_in_room(mon, rmtyp)
    struct monst *mon;int rmtyp;
{
    int rno = levl[mon->mx][mon->my].roomno;
    if (rno >= ROOMOFFSET) return rooms[rno - ROOMOFFSET].rtype == rmtyp;
    return FALSE;
}
...
void dosounds()
{
            ...
            && !(Is_astralevel(&u.uz) || Is_sanctum(&u.uz)))
    {
        for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        {
            ...
            && temple_occupied(u.urooms) != EPRI(mtmp)->shroom) break;
        }
        if (mtmp)
        {
            ...
            const char *msg;
            int trycount = 0, ax = EPRI(mtmp)->shrpos.x, ay =
            EPRI(mtmp)->shrpos.y;
            boolean speechless = (mtmp->data->msound <= MS_ANIMAL), in_sight =
            canseemon(mtmp) || cansee(ax, ay);
    ...
    if (Is_oracle_level(&u.uz) && !rn2(400))
    {
        ...
        {
            if (DEADMONSTER(mtmp)) continue;
            if (mtmp->data == &mons[PM_ORACLE]) break;
        }
        ...
        if (mtmp && (hallu || !canseemon(mtmp)))
        {
            ...
            "someone say \"No more woodchucks!\"", "a loud ZOT!" /* both rec.humor.oracle */
            };
...
{ "beep", "boing", "sing", "belche", "creak", "cough", "rattle", "ululate",
        "pop", "jingle", "sniffle", "tinkle", "eep", "clatter", "hum", "sizzle",
        "twitter", "wheeze", "rustle", "honk", "lisp", "yodel", "coo", "burp",
        "moo", "boom", "murmur", "oink", "quack", "rumble", "twang", "bellow",
        "toot", "gargle", "hoot", "warble" };

const char * growl_sound(mtmp)
...
STATIC_OVL boolean mon_is_gecko(mon)
    struct monst *mon;
{
    int glyph;
...
STATIC_OVL int domonnoise(mtmp)
    register struct monst *mtmp;
{
    char verbuf[BUFSZ];
    ...
    *verbl_msg_mcan = 0; /* verbalize() if cancelled */
    ...
    else if (Hallucination && mon_is_gecko(mtmp)) msound = MS_SELL;
    ...
    case MS_SELL: /* pitch, pay, total */
        ...
        else
        {
            ...
            Sprintf (verbuf, "15 minutes could save you 15 %s.", currency(15L)); /* "zorkmids" */
            verbl_msg = verbuf;
        }
        break;
    case MS_VAMPIRE:
    ...
    case MS_DJINNI:
        ...
        {
            if (ptr != &mons[PM_PRISONER]) verbl_msg =
    ...
    case MS_SOLDIER:
    {
        ...
        verbl_msg =
                mtmp->mpeaceful ?
                        soldier_pax_msg[rn2(3)] : soldier_foe_msg[rn2(3)];
        break;
    }
    case MS_RIDER:
        /* 3.6.0 tribute */
        ...
        else if (ptr == &mons[PM_DEATH] && !rn2(2)
