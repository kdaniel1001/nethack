    ...
    const char *s;
{
    ...
    {
        raw_printf("Window type %s not recognized.  The only choice is: %s.", s,
                winchoices[0].procs->name);
    }
    else
    {
...
void genl_putmsghistory(msg, is_restoring)
    const char *msg;boolean is_restoring;
{
    /* window ports can provide
...
hup_add_menu, hup_end_menu, hup_select_menu, genl_message_menu, hup_void_ndecl, /* update_inventory */
...
hup_void_ndecl, /* wait_synch */
        ...
        /* update_positionbar */
#endif
        hup_print_glyph, hup_void_fdecl_constchar_p, /* raw_print */
...
{
    char *
FDECL    ((*previnterface_getmsghistory), (BOOLEAN_P)) = 0;
...
static char hup_yn_function(prompt, resp, deflt)
    const char *prompt UNUSED, *resp UNUSED;char deflt;
{
    if (!deflt) deflt = '\033';
    return deflt;
}
...
static void hup_getlin(prompt, outbuf)
    const char *prompt UNUSED;char *outbuf;
{
    Strcpy (outbuf, "\033");
}
...
static void hup_init_nhwindows(argc_p, argv)
    int *argc_p UNUSED;char **argv UNUSED;
{
...
static int hup_select_menu(window, how, menu_list)
    winid window UNUSED;int how UNUSED;struct mi **menu_list UNUSED;
{
    return -1;
}

/*ARGSUSED*/
static void hup_add_menu(window, glyph, identifier, sel, grpsel, attr, txt,
        preselected)
    winid window UNUSED;int glyph UNUSED, attr UNUSED;const anything *identifier UNUSED;char
            sel UNUSED, grpsel UNUSED;const char *txt UNUSED;boolean preselected UNUSED;
{
    return;
}
...
static void hup_end_menu(window, prompt)
    winid window UNUSED;const char *prompt UNUSED;
{
    return;
}
...
static void hup_putstr(window, attr, text)
    winid window UNUSED;int attr UNUSED;const char *text UNUSED;
{
    return;
}
...
static void hup_print_glyph(window, x, y, glyph, bkglyph)
    winid window UNUSED;xchar x UNUSED, y UNUSED;int glyph UNUSED;int bkglyph UNUSED;
{
    return;
}
...
static void hup_outrip(tmpwin, how, when)
    winid tmpwin UNUSED;int how UNUSED;time_t when UNUSED;
{
    return;
}
...
static void hup_curs(window, x, y)
    winid window UNUSED;int x UNUSED, y UNUSED;
{
    return;
}
...
static void hup_display_nhwindow(window, blocking)
    winid window UNUSED;boolean blocking UNUSED;
{
    return;
}
...
static void hup_display_file(fname, complain)
    const char *fname UNUSED;boolean complain UNUSED;
{
    return;
}
