...
static NEARDATA const char c_armor[] = "armor", c_suit[] = "suit", c_shirt[] =
        "shirt", c_cloak[] = "cloak", c_gloves[] = "gloves",
        c_boots[] = "boots", c_helmet[] = "helmet", c_shield[] = "shield",
        c_weapon[] = "weapon", c_sword[] = "sword", c_axe[] = "axe", c_that_[] =
                "that";
...
STATIC_OVL
void toggle_stealth(obj, oldprop, on)
    struct obj *obj;long oldprop; /* prop[].extrinsic, with obj->owornmask stripped by caller */
    boolean on;
{
...
STATIC_OVL
void toggle_displacement(obj, oldprop, on)
    struct obj *obj;long oldprop; /* prop[].extrinsic, with obj->owornmask stripped by caller */
    boolean on;
{
    if (on ? initial_don : context.takeoff.cancelled_don) return;
...
STATIC_PTR int Cloak_on(VOID_ARGS)
{
    long oldprop = u.uprops[objects[uarmc->otyp].oc_oprop].extrinsic
            & ~WORN_CLOAK;
...
int Helmet_on(VOID_ARGS)
{
    ...
    case HELM_OF_OPPOSITE_ALIGNMENT:
        ...
        uchangealign(
                (u.ualign.type != A_NEUTRAL) ? -u.ualign.type :
...
STATIC_PTR
int Gloves_on(VOID_ARGS)
{
    long oldprop = u.uprops[objects[uarmg->otyp].oc_oprop].extrinsic
            & ~WORN_GLOVES;
...
STATIC_OVL void wielding_corpse(obj, voluntary)
    struct obj *obj;boolean voluntary; /* taking gloves off on purpose? */
{
    char kbuf[BUFSZ];
...
int Gloves_off(VOID_ARGS)
{
    ...
    if (uwep && uwep->otyp == CORPSE) wielding_corpse(uwep, on_purpose);
...
STATIC_PTR int Shield_on(VOID_ARGS)
{
...
STATIC_PTR int Shirt_on(VOID_ARGS)
{
 ...
 */
STATIC_PTR
int Armor_on(VOID_ARGS)
{
    return 0;
}

int Armor_off(VOID_ARGS)
{
...
STATIC_OVL void learnring(ring, observed)
    struct obj *ring;boolean observed;
{
...
STATIC_OVL void Ring_off_or_gone(obj, gone)
    register struct obj *obj;boolean gone;
{
    ...
    register struct obj *otmp;
{
    ...
    if (otmp->owornmask & W_WEAPON) remove_worn_item(otmp, FALSE);
    setworn(otmp, W_TOOL);
    on_msg(otmp);
...
void cancel_don()
{
    ...
    context.takeoff.cancelled_don = (afternmv == Boots_on
            || afternmv == Helmet_on || afternmv == Gloves_on
            || afternmv == Armor_on);
    afternmv = 0;
    nomovemsg = (char *) 0;
    multi = 0;
    ...
    struct obj *stolenobj; /* no message if stolenobj is already being doffing */
{
    ...
    if (putting_on || otmp != stolenobj)
    {
        Sprintf (buf, "You stop %s %s.",
                putting_on ? "putting on" : "taking off", thesimpleoname(otmp));
    }
    else
    {
    ...
    if (putting_on) remove_worn_item(otmp, FALSE);
...
STATIC_OVL void count_worn_stuff(which, accessorizing)
...
STATIC_OVL int armor_or_accessory_off(obj)
    struct obj *obj;
{
...
int dotakeoff()
{
    ...
    if (Narmorpieces != 1 || ParanoidRemove) otmp = getobj(clothes, "take off");
    if (!otmp) return 0;
    ...
    register struct obj *otmp;
{
    ...
    if (delay)
    {
        ...
        if (is_helmet(otmp))
        {
            /* ick... */
            nomovemsg =
                    !strcmp(helm_simple_name(otmp), "hat") ?
...
STATIC_OVL void already_wearing(cc)
    const char *cc;
{
    You("are already wearing %s%c", cc, (cc == c_that_) ? '!' : '.');
}

STATIC_OVL void already_wearing2(cc1, cc2)
    const char *cc1, *cc2;
{
 ...
 */
int canwearobj(otmp, mask, noisy)
    struct obj *otmp;long *mask;boolean noisy;
{
    int err = 0;
    const char *which;

    which = is_cloak(otmp) ? c_cloak : is_shirt(otmp) ? c_shirt :
    ...
    {
        if (noisy) You("cannot do that while holding your %s.",
        ...
        else if (Upolyd && has_horns(youmonst.data) && !is_flimsy(otmp))
        {
            ...
            if (noisy)
                pline_The("%s won't fit over your horn%s.",
                        helm_simple_name(otmp), plur(num_horns(youmonst.data)));
            err++;
        }
        else
            *mask = W_ARMH;
    ...
    else if (is_boots(otmp))
    {
        ...
        else if (Upolyd && youmonst.data->mlet == S_CENTAUR)
        {
            ...
            if (noisy) pline("You have too many hooves to wear %s.", c_boots); /* makeplural(body_part(FOOT)) yields
            ...
            else
            {
                if (noisy)
                    You_cant("wear that over your %s.",
                            (uarm && !uarmc) ?
                                    c_armor : cloak_simple_name(uarmc));
            }
            err++;
        }
        else
...
STATIC_OVL int accessory_or_armor_on(obj)
    struct obj *obj;
{
    long mask = 0L;
    ...
    if (armor)
    {
        if (!canwearobj(obj, &mask, TRUE)) return 0;
    ...
    if (!retouch_object(&obj, FALSE)) return 1; /* costs a turn even though it didn't get worn */
    ...
    if (armor)
    {
        ...
        if (obj->owornmask & W_WEAPON) remove_worn_item(obj, FALSE);
        setworn(obj, mask);
        ...
        if (delay)
        {
            ...
            if (is_boots(obj)) afternmv = Boots_on;
            if (is_helmet(obj)) afternmv = Helmet_on;
            if (is_gloves(obj)) afternmv = Gloves_on;
            if (obj == uarm) afternmv = Armor_on;
            nomovemsg = "You finish your dressing maneuver.";
        }
        else
        {
            if (is_cloak(obj)) (void) Cloak_on();
            if (is_shield(obj)) (void) Shield_on();
            if (is_shirt(obj)) (void) Shirt_on();
            on_msg(obj);
        }
        context.takeoff.mask = context.takeoff.what = 0L;
    }
    ...
    { /* not armor */
        ...
        if (give_feedback && is_worn(obj)) prinv((char *) 0, obj, 0L);
    }
    return 1;
}
...
void find_ac()
{
    ...
    else if (uac > 127) uac = 127; /* for completeness */
...
void glibr()
{
    ...
    if (otmp && !welded(otmp))
    {
        ...
        if (otherwep && strcmp(thiswep, makesingular(otherwep))) otherwep = 0;
        if (otmp->quan > 1L)
        {
        ...
        else if (wastwoweap) which = "right "; /* preceding msg was about left */
...
struct obj * stuck_ring(ring, otyp)
    struct obj *ring;int otyp;
{
...
int doddoremarm()
{
    ...
    if (flags.menu_style != MENU_TRADITIONAL
            || (result = ggetobj("take off", select_off, 0, FALSE,
                    (unsigned *) 0)) < -1) result = menu_remarm(result);
...
void adj_abon(otmp, delta)
    register struct obj *otmp;register schar delta;
{
...
boolean inaccessible_equipment(obj, verb, only_if_known_cursed)
    struct obj *obj;const char *verb; /* "dip" or "grease", or null to avoid messages */
