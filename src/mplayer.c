...
{
/* devteam */
"Dave", "Dean", "Eric", "Izchak", "Janet", "Jessie", "Ken", "Kevin", "Michael",
        "Mike", "Pat", "Paul", "Steve", "Timo", "Warwick",
        /* PC team */
...
STATIC_OVL void get_mplname(mtmp, nam)
    register struct monst *mtmp;char *nam;
{
    ...
    Strcat (nam, " the ");
    Strcat (nam,
            rank_of((int) mtmp->m_lev, monsndx(mtmp->data),
                    (boolean) mtmp->female));
}

STATIC_OVL void mk_mplayer_armor(mon, typ)
    struct monst *mon;short typ;
{
    struct obj *obj;
...
struct monst * mk_mplayer(ptr, x, y, special)
    register struct permonst *ptr;xchar x, y;register boolean special;
{
    register struct monst *mtmp;
    char nam[PL_NSIZ];

    if (!is_mplayer(ptr)) return ((struct monst *) 0);
    ...
    if ((mtmp = makemon(ptr, x, y, NO_MM_FLAGS)) != 0)
    {
        ...
        short armor = rnd_class(GRAY_DRAGON_SCALE_MAIL,
        ...
        if (weapon != STRANGE_OBJECT)
        {
            ...
            if (objects[otmp->otyp].oc_merge && !otmp->oartifact) otmp->quan +=
                    (long) rn2(is_spear(otmp) ? 4 : 8);
 ...
 */
void create_mplayers(num, special)
    register int num;boolean special;
{
    int pm, x, y;
