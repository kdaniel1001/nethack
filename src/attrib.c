...
STATIC_DCL const struct innate *
FDECL(check_innate_abil, (long *, long));
STATIC_DCL int FDECL(innately, (long *));

/* adjust an attribute; return TRUE if change is made, FALSE otherwise */
boolean adjattrib(ndx, incr, msgflg)
    int ndx, incr;int msgflg; /* positive => no message, zero => message, and */
...
void gainstr(otmp, incr, givemsg)
    struct obj *otmp;int incr;boolean givemsg;
{
    int num = incr;
 ...
 */
void poisoned(reason, typ, pkiller, fatal, thrown_weapon)
    ...
    boolean thrown_weapon; /* thrown weapons are less deadly */
{
    ...
    else
    {
        ...
        if (adjattrib(typ, -loss, 1)) poisontell(typ, TRUE);
    }
...
void exercise(i, inc_or_dec)
    int i;boolean inc_or_dec;
{
...
STATIC_OVL void exerper()
{
    if (!(moves % 10))
    {
        ...
        int hs = (u.uhunger > 1000) ? SATIATED :
                 (u.uhunger > 150) ? NOT_HUNGRY : (u.uhunger > 50) ? HUNGRY :
    ...
    if (moves >= context.next_attrib_check && !multi)
    {
        ...
        for (i = 0; i < A_MAX; ++i)
        {
            ...
            if (adjattrib(i, mod_val, -1))
            {
                ...
                You("%s %s.", (mod_val > 0) ? "must have been" : "haven't been",
...
STATIC_OVL const struct innate * check_innate_abil(ability, frommask)
    long *ability;long frommask;
{
    ...
    else if (frommask == FROMRACE) switch (Race_switch)
    {
    case PM_ELF:
        abil = elf_abil;
        break;
 ...
 * otherwise returns 0
 */
STATIC_OVL int innately(ability)
    long *ability;
{
        ...
        if (propidx >= 0)
        {
            ...
            if (innate == 2)
                ...
                Sprintf (buf, because_of,
                        obj->oartifact ?
                                bare_artifactname(obj) : ysimple_name(obj));
            ...
            if ((p = strstri(buf, " pair of ")) != 0)
                copynchars(p + 1, p + 9, BUFSZ); /* overlapping buffers ok */
                    ...
                    && (p = strstri(buf, " of strangulation")) != 0) *p = '\0';

        }
        else
        { /* negative property index */
...
void adjabil(oldlevel, newlevel)
        ...
        else if (oldlevel >= abil->ulevel && newlevel < abil->ulevel)
        {
            *(abil->ability) &= ~mask;
            if (!(*(abil->ability) & INTRINSIC))
            {
                ...
                else if (*(abil->gainstr)) You_feel("less %s!", abil->gainstr);
            }
        }
    ...
    int x;
{
    ...
    else if (x == A_CHA)
    {
        if (tmp < 18
                && (youmonst.data->mlet == S_NYMPH || u.umonnum == PM_SUCCUBUS
                        || u.umonnum == PM_INCUBUS)) return (schar) 18;
    }
    else if (x == A_INT || x == A_WIS)
    {
         ...
         */
        if (uarmh && uarmh->otyp == DUNCE_CAP) return (schar) 6;
    }
#ifdef WIN32_BUG
    ...
    int attrindx;
{
    ...
    if (attrindx == A_STR)
    {
        ...
        if (uarmg && uarmg->otyp == GAUNTLETS_OF_POWER) lolimit = hilimit;
    }
    ...
    {
        if (uarmh && uarmh->otyp == DUNCE_CAP) hilimit = lolimit = 6;
    }
    ...
    int n;
{
    ...
    if (n < 0)
    {
        if (newalign < u.ualign.record) u.ualign.record = newalign;
    }
    else if (newalign > u.ualign.record)
    {
        u.ualign.record = newalign;
        if (u.ualign.record > ALIGNLIM) u.ualign.record = ALIGNLIM;
    }
}

/* change hero's alignment type, possibly losing use of artifacts */
void uchangealign(newalign, reason)
    int newalign;int reason; /* 0==conversion, 1==helm-of-OA on, 2==helm-of-OA off */
{
    ...
    if (reason == 0)
    {
        ...
        if (!uarmh || uarmh->otyp != HELM_OF_OPPOSITE_ALIGNMENT) u.ualign.type =
    ...
    else
    {
        ...
        else if (reason == 2) Your("mind is %s.",
