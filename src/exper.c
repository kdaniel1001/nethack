...
int experience(mtmp, nk)
    register struct monst *mtmp;register int nk;
{
...
void more_experienced(exper, rexp)
    register int exper, rexp;
{
    ...
    if (u.urexp >= (Role_if(PM_WIZARD) ? 1000 : 2000)) flags.beginner = 0;
}
    ...
    const char *drainer; /* cause of death, if drain should be fatal */
{
    ...
    if (drainer && !strcmp(drainer, "#levelchange"))
        drainer = 0;
    else if (resists_drli(&youmonst)) return;
