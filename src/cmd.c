...
STATIC_DCL void FDECL(enlght_line, (const char *, const char *, const char *,
                const char *));
STATIC_DCL char *
FDECL(enlght_combatinc, (const char *, int, int, char *));
...
STATIC_PTR int doprev_message(VOID_ARGS)
{
    return nh_doprev_message();
}

/* Count down by decrementing multi */
STATIC_PTR int timed_occupation(VOID_ARGS)
{
    (*timed_occ_fn)();
    if (multi > 0) multi--;
    return multi > 0;
 ...
 */
void set_occupation(fn, txt, xtime)
    int NDECL((*fn));
const char *txt;
int xtime;
{
...
char pgetchar() /* courtesy of aeb@cwi.nl */
{
    register int ch;
    ...
    char ch;
{
    if (!in_doagain)
    {
        ...
        else if (shead < BSIZE) saveq[shead++] = ch;
    }
    return;
}

/* here after # - now read a full-word command */
STATIC_PTR int doextcmd(VOID_ARGS)
{
    int idx, retval;
    int NDECL((*func));
...
STATIC_PTR int domonability(VOID_ARGS)
{
...
int enter_explore_mode(VOID_ARGS)
{
    if (wizard)
    {
...
STATIC_PTR int dooverview_or_wiz_where(VOID_ARGS)
{
    if (wizard)
        return wiz_where();
    else
...
STATIC_PTR int wiz_wish(VOID_ARGS) /* Unlimited wishes for debug mode by Paul Polderman */
{
    if (wizard)
    {
...
STATIC_PTR int wiz_identify(VOID_ARGS)
{
    if (wizard)
    {
        iflags.override_ID = (int) cmd_from_func(wiz_identify);
        if (display_inventory((char *) 0, TRUE) == -1) identify_pack(0, FALSE);
...
STATIC_PTR int wiz_map(VOID_ARGS)
{
    if (wizard)
    {
        struct trap *t;
...
STATIC_PTR int wiz_genesis(VOID_ARGS)
{
    if (wizard)
...
STATIC_PTR int wiz_where(VOID_ARGS)
{
    if (wizard)
...
STATIC_PTR int wiz_detect(VOID_ARGS)
{
    if (wizard)
        (void) findit();
    else
...
STATIC_PTR int wiz_level_tele(VOID_ARGS)
{
    if (wizard)
        level_tele();
    else
...
STATIC_PTR int wiz_mon_polycontrol(VOID_ARGS)
{
...
STATIC_PTR int wiz_level_change(VOID_ARGS)
{
    char buf[BUFSZ];
    int newlevel;
    int ret;
...
STATIC_PTR int wiz_panic(VOID_ARGS)
{
...
STATIC_PTR int wiz_polyself(VOID_ARGS)
{
    polyself(1);
    return 0;
}
...
/* #seenv command */
STATIC_PTR int wiz_show_seenv(VOID_ARGS)
{
    winid win;
...
/* #vision command */
STATIC_PTR int wiz_show_vision(VOID_ARGS)
{
    winid win;
    int x, y, v;
    char row[COLNO + 1];
...
/* #wmode command */
STATIC_PTR int wiz_show_wmodes(VOID_ARGS)
{
    winid win;
    int x, y;
    char row[COLNO + 1];
...
STATIC_OVL void wiz_map_levltyp(VOID_ARGS)
{
    winid win;
    int x, y, terrain;
    char row[COLNO + 1];
    ...
    for (y = 0; y < ROWNO; y++)
    {
        ...
        for (x = 1; x < COLNO; x++)
        {
            ...
            row[x - 1] = (char) (
                    (terrain == 0 && !may_dig(x, y)) ? '*' :
                    (terrain < 10) ? '0' + terrain :
                    (terrain < 36) ? 'a' + terrain - 10 : 'A' + terrain - 36);
        }
...
{ "stone", "vertical wall", "horizontal wall", "top-left corner wall",
        ...
        "bottom-right corner wall", "cross wall", "tee-up wall",
        "tee-down wall", "tee-left wall", "tee-right wall", "drawbridge wall",
        "tree", "secret door", "secret corridor", "pool", "moat", "water",
        "drawbridge up", "lava pool", "iron bars", "door", "corridor", "room",
        "stairs", "ladder", "fountain", "throne", "sink", "grave", "altar",
        "ice", "drawbridge down", "air", "cloud",
...
STATIC_OVL void wiz_levltyp_legend(VOID_ARGS)
{
    winid win;
    int i, j, last, c;
    const char *dsc, *fmt;
...
STATIC_PTR int wiz_smell(VOID_ARGS)
{
    int ans = 0;
...
STATIC_PTR int wiz_rumor_check(VOID_ARGS)
{
    rumor_check();
    return 0;
}
...
STATIC_PTR int doterrain(VOID_ARGS)
{
    winid men;
    menu_item *sel;
    anything any;
    ...
    add_menu(men, NO_GLYPH, &any, 0, 0, ATR_NONE,
            ...
            MENU_UNSELECTED);
    any.a_int = 3;
    add_menu(men, NO_GLYPH, &any, 0, 0, ATR_NONE, "known map without monsters",
    MENU_UNSELECTED);
    if (discover || wizard)
    {
        any.a_int = 4;
    ...
    case 2:
        reveal_terrain(0, 1);
        break; /* known map with traps */
    case 3:
        reveal_terrain(0, 1 | 2);
        break; /* known map w/ traps & objs */
    case 4:
        reveal_terrain(1, 0);
        break; /* full map */
    case 5:
...
static const char You_[] = "You ", are[] = "are ", were[] = "were ", have[] =
...
static void enlght_line(start, middle, end, ps)
...
static char * enlght_combatinc(inctyp, incamt, final, outbuf)
    const char *inctyp;int incamt, final;char *outbuf;
{
...
STATIC_OVL void enlght_halfdmg(category, final)
    int category;int final;
{
...
STATIC_OVL boolean walking_on_water()
{
    if (u.uinwater || Levitation || Flying) return FALSE;
    return (boolean) (Wwalking && (is_pool(u.ux, u.uy) || is_lava(u.ux, u.uy)));
}
...
STATIC_OVL boolean cause_known(propindx)
    int propindx; /* index of a property which can be conveyed by worn item */
{
    ...
    for (o = invent; o; o = o->nobj)
    {
        ...
        if ((int) objects[o->otyp].oc_oprop == propindx
                && objects[o->otyp].oc_name_known && o->dknown) return TRUE;
    }
    return FALSE;
}

/* format a characteristic value, accommodating Strength's strangeness */
STATIC_OVL char * attrval(attrindx, attrvalue, resultbuf)
    int attrindx, attrvalue;char resultbuf[]; /* should be at least [7] to hold "18/100\0" */
{
...
STATIC_OVL void background_enlightenment(unused_mode, final)
    int unused_mode UNUSED;int final;
{
    ...
    if (!strcmpi(rank_titl, role_titl))
    {
        ...
        Sprintf (eos(buf), "%s, level %d %s%s", an(rank_titl), u.ulevel, tmpbuf,
                urace.noun);
    }
    else
    {
    ...
    Sprintf (buf, " %s%s%s, %son a mission for %s", You_, !final ? are : were,
...
STATIC_OVL void characteristics_enlightenment(mode, final)
    int mode;int final;
{
...
STATIC_OVL void one_characteristic(mode, final, attrindx)
...
STATIC_OVL void status_enlightenment(mode, final)
    int mode;int final;
{
    ...
    && !(final == ENL_GAMEOVERDEAD && !strcmp(killer.name, "riding accident")));
    ...
    else if (walking_on_water())
    {
        ...
        Sprintf (buf, "walking on %s",
                ...
                is_lava(u.ux, u.uy) ? "lava" : surface(u.ux, u.uy)); /* catchall; shouldn't happen */
    ...
    if (Blind)
    {
        ...
        Sprintf (buf, "%s blind",
                ...
                :
                Blindfolded_only ? "deliberately" : "temporarily");
    ...
    if (Hunger)
    {
        ...
        enl_msg(You_, "hunger", "hungered", " rapidly", from_what(HUNGER));
    }
...
void attributes_enlightenment(unused_mode, final)
    int unused_mode UNUSED;int final;
{
    ...
    if (u.uevent.uhand_of_elbereth)
    {
        static const char * const hofe_titles[3] =
                { "the Hand of Elbereth", "the Envoy of Balance",
                        "the Glory of Arioch" };
...
/* ^X command */
STATIC_PTR int doattributes(VOID_ARGS)
{
...
void youhiding(via_enlghtmt, msgflag)
 ...
 */
STATIC_PTR int doconduct(VOID_ARGS)
{
    show_conduct(0);
    return 0;
}

void show_conduct(final)
    int final;
{
    char buf[BUFSZ];
    int ngenocided;
...
void add_debug_extended_commands()
{
    ...
    for (i = 0; debug_extcmdlist[i].ef_txt; i++)
    {
        ...
        if (n + 2 >= SIZE(extcmdlist)) panic("Too many debugging commands!");
        for (j = 0; j < n; j++)
...
STATIC_OVL char cmd_from_func(fn)
    int NDECL((*fn));
{
    int i;
...
STATIC_OVL void count_obj(chain, total_count, total_size, top, recurse)
    struct obj *chain;long *total_count;long *total_size;boolean top;boolean recurse;
{
    long count, size;
    struct obj *obj;
...
STATIC_OVL void obj_chain(win, src, chain, total_count, total_size)
    winid win;const char *src;struct obj *chain;long *total_count;long *total_size;
{
    char buf[BUFSZ];
    long count = 0, size = 0;
...
STATIC_OVL void mon_invent_chain(win, src, chain, total_count, total_size)
    winid win;const char *src;struct monst *chain;long *total_count;long *total_size;
{
    char buf[BUFSZ];
    long count = 0, size = 0;
    struct monst *mon;
...
STATIC_OVL void contained(win, src, total_count, total_size)
    winid win;const char *src;long *total_count;long *total_size;
{
    char buf[BUFSZ];
    long count = 0, size = 0;
    struct monst *mon;
...
STATIC_OVL int size_monst(mtmp)
    struct monst *mtmp;
{
...
STATIC_OVL void mon_chain(win, src, chain, total_count, total_size)
    winid win;const char *src;struct monst *chain;long *total_count;long *total_size;
{
    char buf[BUFSZ];
    long count, size;
...
STATIC_OVL boolean accept_menu_prefix(cmd_func)
    int NDECL((*cmd_func));
{
...
void rhack(cmd)
    register char *cmd;
{
    boolean do_walk, do_rush, prefix_seen, bad_command, firsttime = (cmd == 0);
    ...
    case 'g':
        ...
        break;
    case '5':
        if (!Cmd.num_pad) break; /* else FALLTHRU */
    case 'G':
...
int get_adjacent_loc(prompt, emsg, x, y, cc)
    const char *prompt, *emsg;xchar x, y;coord *cc;
{
    xchar new_x, new_y;
    if (!getdir(prompt))
    {
    ...
    const char *s;
{
    ...
    retry: if (in_doagain || *readchar_queue)
        dirsym = readchar();
    else
        ...
        if (!index(quitchars, dirsym))
        {
            ...
            if (help_requested || iflags.cmdassist)
            {
                did_help = help_dir((s && *s == '^') ? dirsym : 0,
                        help_requested ?
                                (const char *) 0 : "Invalid direction key!");
...
STATIC_OVL boolean help_dir(sym, msg)
    char sym;const char *msg;
{
    char ctrl;
    winid win;
 ...
 */
const char * click_to_cmd(x, y, mod)
    int x, y, mod;
{
    int dir;
    static char cmd[4];
    ...
    if (flags.travelcmd)
    {
        ...
        if (x == 0 && y == 0)
        {
            /* here */
            if (IS_FOUNTAIN(
                    levl[u.ux][u.uy].typ) || IS_SINK(levl[u.ux][u.uy].typ))
            {
    ...
    else
    {
        cmd[0] = (
                Cmd.num_pad ?
                        M(Cmd.dirchars[dir]) : (Cmd.dirchars[dir] - 'a' + 'A')); /* run command */
    }
...
#endif
    if (!Cmd.num_pad || (foo = readchar()) == 'n') for (;;)
    {
        foo = readchar();
        if (foo >= '0' && foo <= '9')
        {
...
char readchar()
{
    ...
    else if (sym == '\033' && alt_esc)
    {
        ...
        if (sym == EOF || sym == 0)
            sym = '\033';
        else if (sym != '\033') sym |= 0200; /* force 8th bit on */
#endif                   /*ALTMETA*/
    }
    else if (sym == 0)
    {
...
STATIC_PTR int dotravel(VOID_ARGS)
{
 ...
 */
char yn_function(query, resp, def)
...
boolean paranoid_query(be_paranoid, prompt)
    boolean be_paranoid;const char *prompt;
{
    ...
    if (be_paranoid)
    {
        ...
        const char *promptprefix = "", *responsetype =
        ParanoidConfirm ? "(yes|no)" : "(yes) [no]";
