 ...
 */
boolean boulder_hits_pool(otmp, rx, ry, pushing)
    struct obj *otmp;register int rx, ry;boolean pushing;
{
 ...
 * away.
 */
boolean flooreffects(obj, x, y, verb)
    struct obj *obj;int x, y;const char *verb;
{
    struct trap *t;
    struct monst *mtmp;
    struct obj *otmp;
...
STATIC_DCL void polymorph_sink()
{
...
STATIC_DCL boolean teleport_sink()
{
    int cx, cy;
    int cnt = 0;
    struct trap *trp;
...
STATIC_OVL void dosinkring(obj)
    register struct obj *obj;
{
    ...
    if (ideed)
        trycall(obj);
    else if (!nosink) You_hear("the ring bouncing down the drainpipe.");
...
boolean canletgo(obj, word)
    struct obj *obj;const char *word;
{
    ...
    register struct obj *obj;
{
    ...
    if (!u.uswallow)
    {
        ...
        if (IS_ALTAR(levl[u.ux][u.uy].typ)) doaltarobj(obj); /* set bknown */
    }
    dropy(obj);
}
...
void dropz(obj, with_impact)
    struct obj *obj;boolean with_impact;
{
    ...
    else
    {
        ...
        if (obj == uball)
            drop_ball(u.ux, u.uy);
        else if (level.flags.has_shop) sellobj(obj, u.ux, u.uy);
        stackobj(obj);
...
STATIC_OVL int menu_drop(retry)
    int retry;
{
    int n, i, n_dropped = 0;
    long cnt;
    ...
    else if (flags.menu_style == MENU_FULL)
    {
        ...
        n = query_category("Drop what type of items?", invent,
                UNPAID_TYPES | ALL_TYPES | CHOOSE_ALL | BUC_BLESSED | BUC_CURSED
                        | BUC_UNCURSED | BUC_UNKNOWN, &pick_list, PICK_ANY);
        if (!n) goto drop_done;
        for (i = 0; i < n; i++)
        {
...
void goto_level(newlevel, at_stairs, falling, portal)
    d_level *newlevel;boolean at_stairs, falling, portal;
{
    int fd, l_idx;
    xchar new_ledger;
    boolean cant_go_back, up = (depth(newlevel) < depth(&u.uz)), newdungeon =
            (u.uz.dnum != newlevel->dnum), was_in_W_tower = In_W_tower(u.ux,
            u.uy, &u.uz), familiar = FALSE;
    boolean new = FALSE; /* made a new level? */
    struct monst *mtmp;
    char whynot[BUFSZ];
    char *annotation;

    if (dunlev(newlevel) > dunlevs_in_dungeon(newlevel)) newlevel->dlevel =
            ...
            && (dunlev(&u.uz) < dunlevs_in_dungeon(&u.uz) - 3))
    {
        if (!rn2(4))
        {
            ...
            if (diff == 0) assign_level(newlevel, &u.uz);
    ...
    if (u.utrap && u.utraptype == TT_BURIEDBALL) buried_ball_to_punishment(); /* (before we save/leave old level) */
    ...
    if ((mtmp = m_at(u.ux, u.uy)) != 0 && mtmp != u.usteed)
    {
        ...
        if ((mtmp = m_at(u.ux, u.uy)) != 0)
        {
            ...
            migrate_to_level(mtmp, ledger_no(&u.uz), MIGR_RANDOM, (coord *) 0);
        }
    }
    ...
    if (Is_waterlevel(&u.uz) || Is_airlevel(&u.uz))
        movebubbles();
    else if (Is_firelevel(&u.uz)) fumaroles();
    ...
    if (In_endgame(&u.uz))
    {
        ...
        else if (newdungeon && u.uhave.amulet) resurrect(); /* force confrontation with Wizard */
    }
...
void schedule_goto(tolev, at_stairs, falling, portal_flag, pre_msg, post_msg)
    d_level *tolev;boolean at_stairs, falling;int portal_flag;const char
            *pre_msg, *post_msg;
{
    ...
    if (pre_msg) dfr_pre_msg = dupstr(pre_msg);
    if (post_msg) dfr_post_msg = dupstr(post_msg);
}
...
STATIC_PTR int wipeoff(VOID_ARGS)
{
    if (u.ucreamed < 4)
        u.ucreamed = 0;
    else
...
void set_wounded_legs(side, timex)
    register long side;register int timex;
{
    /* KMH -- STEED
