...
STATIC_OVL boolean its_dead(int rx, int ry, int *resp)
{
    ...
    struct obj *corpse = sobj_at(CORPSE, rx, ry), *statue = sobj_at(STATUE, rx,
            ry);
        ...
        else if (corpse->quan == 1L && !more_corpses)
        {
            ...
            if (mtmp)
            {
                ...
                if ((humanoid(mptr) || (mptr->geno & G_UNIQ)
                        || type_is_pname(mptr)) && !is_neuter(mptr)) gndr =
                        (int) mtmp->female;
            }
            else
            {
    ...
    {
        boolean here = (rx == u.ux && ry == u.uy), one = (corpse->quan == 1L
                && !more_corpses), reviver = FALSE;
    ...
    { /* statue */
        ...
        { /* ignore statue->dknown; it'll always be set */
            Sprintf (buf, "%s %s", (rx == u.ux && ry == u.uy) ? "This" : "That",
...
STATIC_OVL int use_stethoscope(register struct obj *obj)
{
    ...
    if ((mtmp = m_at(rx, ry)) != 0)
    {
        ...
        if (mtmp->mundetected)
        {
            if (!canspotmon(mtmp)) There("is %s hidden there.", mnm);
            mtmp->mundetected = 0;
    ...
    if (!its_dead(rx, ry, &res)) You("hear nothing special."); /* not You_hear()  */
    return res;
}
...
void m_unleash(mtmp, feedback)
    register struct monst *mtmp;boolean feedback;
{
...
static boolean leashable(struct monst *mtmp)
{
...
STATIC_OVL int use_mirror(struct obj *obj)
{
    ...
    if (obj->cursed && !rn2(2))
    {
        if (!Blind) pline_The("%s fogs up and doesn't reflect!", mirror);
        return 1;
    }
    if (!u.dx && !u.dy && !u.dz)
    {
    ...
    if (!mtmp || !haseyes(mtmp->data) || notonhead) return 1;
...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    ...
    {
        if (vis) pline("%s fails to notice your %s.", Monnam(mtmp), mirror);
    ...
    else if ((how_seen & SEENMON) == MONSEEN_INFRAVIS)
    {
        if (vis)
            /* (redundant) */
            pline("%s is too far away to see %sself in the dark.", Monnam(mtmp),
                    mhim(mtmp));
...
STATIC_OVL void use_bell(struct obj **optr)
{
    ...
    struct monst *mtmp;
    boolean wakem = FALSE, learno = FALSE, ordinary = (obj->otyp
            != BELL_OF_OPENING || !obj->spe), invoking = (obj->otyp
            == BELL_OF_OPENING && invocation_pos(u.ux, u.uy)
            && !On_stairs(u.ux, u.uy));
...
boolean catch_lit(struct obj *obj)
{
    ...
    if (!obj->lamplit && (obj->otyp == MAGIC_LAMP || ignitable(obj)))
    {
        if ((obj->otyp == MAGIC_LAMP || obj->otyp == CANDELABRUM_OF_INVOCATION)
                && obj->spe == 0)
            return FALSE;
        else if (obj->otyp != MAGIC_LAMP && obj->age == 0) return FALSE;
...
STATIC_OVL void light_cocktail(struct obj *obj) /* obj is a potion of oil */
{
    char buf[BUFSZ];
    boolean split1off;
...
static NEARDATA const char cuddly[] =
{ TOOL_CLASS, GEM_CLASS, 0 };
...
boolean is_valid_jump_pos(x, y, magic, showmsg)
    int x, y, magic;boolean showmsg;
{
...
void display_jump_positions(int state)
{
    if (state == 0)
    {
...
int jump(int magic) /* 0=Physical, otherwise skill level */
{
    ...
    if (getpos(&cc, TRUE, "the desired position") < 0) return 0; /* user pressed ESC */
...
STATIC_OVL void use_tinning_kit(obj)
    struct obj *obj;
{
    ...
    if (did_attr)
        pline("This makes you feel %s!",
                (did_prop + did_attr) == (trouble_count + unfixable_trbl) ?
                        "great" : "better");
    else if (!did_prop) pline("Nothing seems to happen.");
 ...
 */
void fig_transform(arg, timeout)
    anything *arg;long timeout;
{
    ...
    if (mtmp)
    {
        ...
        if ((mtmp->minvis && !See_invisible)
                || (mtmp->data->mlet == S_MIMIC
                        && mtmp->m_ap_type != M_AP_NOTHING)) suppress_see =
        TRUE;
        ...
        case OBJ_MINVENT:
            if (cansee_spot && !silent && !suppress_see)
            {
                ...
                You_see("%s %s out of %s%s!", monnambuf,
                        locomotion(mtmp->data, "drop"), carriedby, and_vanish);
            }
            break;
#if 0
            case OBJ_MIGRATING:
...
STATIC_OVL boolean figurine_location_checks(obj, cc, quietly)
    struct obj *obj;coord *cc;boolean quietly;
{
    xchar x, y;
    ...
    {
        if (!quietly) You("cannot place a figurine in %s!",
...
STATIC_OVL void use_figurine(optr)
    struct obj **optr;
{
    register struct obj *obj = *optr;
    xchar x, y;
...
static NEARDATA const char lubricables[] =
{ ALL_CLASSES, ALLOW_NONE, 0 };

STATIC_OVL void use_grease(obj)
    struct obj *obj;
{
    struct obj *otmp;
...
STATIC_OVL void use_stone(tstone)
    struct obj *tstone;
{
    struct obj *obj;
    boolean do_scratch;
    ...
    static const char allowall[3] =
    { COIN_CLASS, ALL_CLASSES, 0 };
    static const char coins_gems[3] =
    { COIN_CLASS, GEM_CLASS, 0 };
    ...
    choices =
            (tstone->otyp == TOUCHSTONE && tstone->dknown
...
static const char not_enough_room[] =
        ...
        "won't hit anything if you can't see that spot.", cant_reach[] =
        "can't reach that spot from here.";

/* find pos of monster in range, if only one monster */
boolean find_poleable_mon(pos, min_range, max_range)
    coord *pos;int min_range, max_range;
{
    struct monst *mtmp;
...
void display_polearm_positions(state)
    int state;
{
    if (state == 0)
    {
    ...
    struct obj *obj;
{
    ...
    if (getpos(&cc, TRUE, "the spot to hit") < 0) return res; /* ESC; uses turn iff polearm became wielded */
...
STATIC_OVL int use_cream_pie(obj)
    struct obj *obj;
{
...
STATIC_OVL int use_grapple(obj)
    struct obj *obj;
{
    ...
    cc.x = u.ux;
    cc.y = u.uy;
    if (getpos(&cc, TRUE, "the spot to hit") < 0) return res; /* ESC; uses turn iff grapnel became wielded */
...
STATIC_OVL int do_break_wand(obj)
    struct obj *obj;
{
    ...
    ParanoidBreakwand,
            safe_qbuf(confirm, "Are you really sure you want to break ", "?",
                    obj, yname, ysimple_name, "the wand"))) return 0;
    ...
    case WAN_MAGIC_MISSILE:
        wanexpl: explode(u.ux, u.uy, -(obj->otyp), dmg, WAND_CLASS, expltype);
    ...
    if (obj->otyp == WAN_LIGHT) litroom(TRUE, obj); /* only needs to be done once */

    discard_broken_wand: obj = current_wand; /* [see dozap() and destroy_item()] */
    current_wand = 0;
    if (obj) delobj(obj);
    nomul(0);
    return 1;
...
STATIC_OVL void add_class(cl, class)
    char *cl;char class;
{
    char tmp[2];
...
STATIC_OVL void setapplyclasses(class_list)
    char class_list[];
{
    register struct obj *otmp;
    int otyp;
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj)
    {
        ...
        if (otyp == TOUCHSTONE
            ...
            addstones = TRUE;
        if (otyp == CREAM_PIE || otyp == EUCALYPTUS_LEAF) addfood = TRUE;
    }
...
int doapply()
{
    ...
    if (!retouch_object(&obj, FALSE)) return 1; /* evading your grasp costs a turn; just be
