...
static struct Comp_Opt
{
    ...
    int optflags;
        ...
        { "altkeyhandler", "alternate key handler", 20, DISP_IN_GAME },
                ...
                SET_IN_FILE },
                { "menu_deselect_page",
                        "deselect all items on this page of a menu", 4,
                        SET_IN_FILE },
                { "menu_first_page", "jump to the first page in a menu", 4,
                SET_IN_FILE },
                { "menu_headings", "text attribute for menu headings", 9,
                SET_IN_GAME },
                ...
                DISP_IN_GAME },
#ifdef TTY_GRAPHICS
                {   "msg_window", "the type of message window required", 1,
                    SET_IN_GAME},
#else
                { "msg_window", "the type of message window required", 1,
                SET_IN_FILE },
                ...
                PL_NSIZ,
                DISP_IN_GAME },
                { "number_pad", "use the number pad for movement", 1,
                SET_IN_GAME },
                ...
                { "paranoid_confirmation",
                        "extra prompting in certain situations", 28,
                        SET_IN_GAME },
                ...
                { "roguesymset",
                        ...
                        SET_IN_GAME },
                { "suppress_alert",
                        "suppress alerts about version-specific features", 8,
                        SET_IN_GAME },
...
#endif
#ifdef BACKWARD_COMPAT
                { "DECgraphics", "load DECGraphics display symbols", 70,
                SET_IN_FILE },
                { "IBMgraphics", "load IBMGraphics display symbols", 70,
                SET_IN_FILE },
...
STATIC_DCL char *
FDECL(string_for_opt, (char *, BOOLEAN_P));
STATIC_DCL char *
FDECL(string_for_env_opt, (const char *, char *, BOOLEAN_P));
...
FDECL(get_compopt_value, (const char *, char *));
...
boolean match_optname(user_string, opt_name, min_length, val_allowed)
    const char *user_string, *opt_name;int min_length;boolean val_allowed;
{
    ...
    return (boolean) (len >= min_length && !strncmpi(opt_name, user_string, len));
}
...
void initoptions_init()
{
    ...
    if ((opts = nh_getenv ("TERM"))
            ...
            && !strncmpi (opts, "vt", 2) && AS && AE && index (AS, '\016')
            && index (AE, '\017'))
    {
        if (!symset[PRIMARY].name)
...
void initoptions_finish()
{
    ...
    if (opts)
    {
        if (*opts == '/' || *opts == '\\' || *opts == '@')
        {
            ...
            if (strlen(opts) < BUFSZ / 2) read_config_file(opts, SET_IN_FILE);
        }
        else
        {
...
STATIC_OVL void nmcpy(dest, src, maxlen)
    char *dest;const char *src;int maxlen;
{
    int count;
...
STATIC_OVL void escapes(cp, tp)
    const char *cp;char *tp;
{
    static NEARDATA const char oct[] = "01234567", dec[] = "0123456789", hex[] =
...
STATIC_OVL char * string_for_opt(opts, val_optional)
    char *opts;boolean val_optional;
{
    char *colon, *equals;
...
STATIC_OVL char * string_for_env_opt(optname, opts, val_optional)
    const char *optname;char *opts;boolean val_optional;
{
    if (!initial)
    {
...
STATIC_OVL void bad_negation(optname, with_parameter)
    const char *optname;boolean with_parameter;
{
...
STATIC_OVL void warning_opts(opts, optype)
    register char *opts;const char *optype;
{
    ...
    for (i = 0; i < WARNCOUNT; i++)
        translate[i] = (i >= length) ? 0 :
                       opts[i] ? (uchar) opts[i] : def_warnsyms[i].sym;
...
STATIC_OVL boolean duplicate_opt_detection(opts, iscompound)
    const char *opts;int iscompound; /* 0 == boolean option, 1 == compound */
{
    int i, *optptr;
...
const char * clr2colorname(clr)
    int clr;
{
    int i;

    for (i = 0; i < SIZE(colornames); i++)
        if (colornames[i].color == clr) return colornames[i].name;
    return (char *) 0;
}

const char * attr2attrname(attr)
    int attr;
{
    int i;
...
boolean msgtype_add(typ, pattern)
    int typ;char *pattern;
{
    struct plinemsg_type *tmp = (struct plinemsg_type *) alloc(
            sizeof(struct plinemsg_type));
...
boolean add_menu_coloring_parsed(str, c, a)
    char *str;int c, a;
{
...
boolean add_menu_coloring(str)
    char *str;
{
    int i, c = NO_COLOR, a = ATR_NONE;
    char *tmps, *cs, *amp;

    if (!str || (cs = index(str, '=')) == 0) return FALSE;

    tmps = cs + 1; /* advance past '=' */
    mungspaces(tmps);
    if ((amp = index(tmps, '&')) != 0) *amp = '\0';
...
boolean get_menu_coloring(str, color, attr)
    char *str;int *color, *attr;
{
...
void free_one_menu_coloring(idx)
    int idx; /* 0 .. */
{
    struct menucoloring *tmp = menu_colorings;
...
void parseoptions(opts, tinitial, tfrom_file)
    register char *opts;boolean tinitial, tfrom_file;
{
    register char *op;
    unsigned num;
    ...
    {
        if (duplicate_opt_detection(opts, 0)) complain_about_duplicate(opts, 0);
    ...
    if (match_optname(opts, fullname, sizeof("align") - 1, TRUE))
    {
        ...
        else if ((op = string_for_env_opt(fullname, opts, FALSE)) != 0)
        {
            ...
            if (val_negated)
            {
                if (!setrolefilter(op)) badoption(opts);
            }
            else
            {
    ...
    if (match_optname(opts, fullname, 7, TRUE))
    {
        ...
        else if ((op = string_for_opt(opts, FALSE)) != 0)
        {
            ...
            if (!read_sym_file(ROGUESET))
            {
                ...
                raw_printf("Unable to load symbol set \"%s\" from \"%s\".", op,
                SYMBOLS);
                wait_synch();
            }
            else
    ...
    if (match_optname(opts, "fruit", 2, TRUE))
    {
        ...
        goodfruit: nmcpy(pl_fruit, op, PL_FSIZ);
    ...
    if (match_optname(opts, fullname, 7, TRUE))
    {
        ...
        if (def_char_to_monclass(opts[0]) != MAXMCLASSES)
            clash = 1;
        else if (opts[0] >= '1' && opts[0] <= '5') clash = 2;
        if (clash)
        {
        ...
        if (!op)
        {
                    ...
                    || flags.menu_style == MENU_COMBINATION)
            {
                ...
                if (abuf[0] == '\0' || abuf[0] == '\033')
                    op = tbuf; /* restore */
                else if (abuf[0] == 'm') use_menu = TRUE;
            }
            if (use_menu)
            {
            ...
            *(boolopt[i].addr) = !negated;
                    ...
                    || ((boolopt[i].addr) == &flags.dark_room))
            {
                ...
                if (iflags.use_color) need_redraw = TRUE; /* darkroom refresh */
                    ...
                    || (boolopt[i].addr)
                            == &flags.showrace|| (boolopt[i].addr) == &iflags.hilite_pet)
            {
                need_redraw = TRUE;
#ifdef TEXTCOLOR
            }
...
int handle_add_list_remove(optname, numtotal)
    const char *optname;int numtotal;
{
    winid tmpwin;
    anything any;
...
STATIC_OVL boolean special_handling(optname, setinitial, setfromfile)
    const char *optname;boolean setinitial, setfromfile;
{
    winid tmpwin;
    anything any;
    int i;
    ...
    else if (!strcmp("paranoid_confirmation", optname))
    {
        ...
        for (i = 0; paranoia[i].flagmask != 0; ++i)
        {
            ...
            add_menu(tmpwin,
            NO_GLYPH, &any, *paranoia[i].argname, 0,
    ...
    else if (!strcmp("number_pad", optname))
    {
        ...
        {
            any.a_int = i + 1;
            add_menu(tmpwin, NO_GLYPH, &any, 'a' + i, 0, ATR_NONE, npchoices[i],
            MENU_UNSELECTED);
        }
    ...
    else if (!strcmp("msgtype", optname))
    {
        ...
        msgtypes_again: nmt = msgtype_count();
        ...
        { /* add new */
            ...
            if (*mtbuf == '\033' || !*mtbuf) goto msgtypes_again;
            mttyp = query_msgtype();
            if (mttyp == -1) goto msgtypes_again;
        ...
        { /* list or remove */
            ...
            pick_cnt = select_menu(tmpwin,
                    (opt_idx == 1) ? PICK_NONE : PICK_ANY, &pick_list);
            if (pick_cnt > 0)
            {
                for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx)
                    free_one_msgtype(
                            pick_list[pick_idx].item.a_int - 1 - pick_idx);
                free((genericptr_t) pick_list), pick_list = (menu_item *) 0;
            }
            destroy_nhwindow(tmpwin);
            if (pick_cnt >= 0) goto msgtypes_again;
        }
    }
    else if (!strcmp("menucolors", optname))
    {
        ...
        menucolors_again: nmc = count_menucolors();
    ...
    {
        int opt_idx, pass, totalapes = 0, numapes[2] =
        { 0, 0 };
        char apebuf[1 + BUFSZ]; /* so &apebuf[1] is BUFSZ long for getlin() */
        struct autopickup_exception *ape;

        ape_again: totalapes = count_ape_maps(&numapes[AP_LEAVE],
                &numapes[AP_GRAB]);
    ...
    else if (!strcmp("symset", optname) || !strcmp("roguesymset", optname))
    {
        menu_item *symset_pick = (menu_item *) 0;
        boolean primaryflag = (*optname == 's'), rogueflag = (*optname == 'r'),
                ready_to_switch = FALSE, nothing_to_do = FALSE;
        ...
        if (res && symset_list)
        {
            ...
            {
                /* check restrictions */
                if ((!rogueflag && sl->rogue) || (!primaryflag && sl->primary))
                {
                    sl = sl->next;
                    continue;
                }
        ...
        if (!symset[which_set].name && symset_name) symset[which_set].name =
                symset_name; /* not dupstr() here */
        ...
        {
            if (rogueflag) assign_graphics(ROGUESET);
        }
...
STATIC_OVL const char * get_compopt_value(optname, buf)
    const char *optname;char *buf;
{
    char ocl[MAXOCLASSES + 1];
    static const char none[] = "(none)", randomrole[] = "random", to_be_done[] =
            "(to be done)", defopt[] = "default", defbrief[] = "def";
    int i;

    buf[0] = '\0';
    if (!strcmp(optname, "align_message"))
        Sprintf (buf, "%s",
                iflags.wc_align_message == ALIGN_TOP ? "top" :
                iflags.wc_align_message == ALIGN_LEFT ? "left" :
                iflags.wc_align_message == ALIGN_BOTTOM ? "bottom" :
                iflags.wc_align_message == ALIGN_RIGHT ? "right" : defopt);
    else if (!strcmp(optname, "align_status"))
        Sprintf (buf, "%s",
                iflags.wc_align_status == ALIGN_TOP ? "top" :
                iflags.wc_align_status == ALIGN_LEFT ? "left" :
                iflags.wc_align_status == ALIGN_BOTTOM ? "bottom" :
                iflags.wc_align_status == ALIGN_RIGHT ? "right" : defopt);
    ...
    else if (!strcmp(optname, "number_pad"))
    {
        ...
        { "0=off", "1=on", "2=on, MSDOS compatible", "3=on, phone-style layout",
                "4=on, phone layout, MSDOS compatible", "-1=off, y & z swapped", /*[5]*/
        };
        int indx =
                Cmd.num_pad ?
                        (Cmd.phone_layout ?
                                (Cmd.pcHack_compat ? 4 : 3) :
    ...
    else if (!strcmp(optname, "windowcolors"))
    {
        Sprintf (buf, "%s/%s %s/%s %s/%s %s/%s",
                ...
                iflags.wc_foregrnd_message ?
                        iflags.wc_foregrnd_message : defbrief,
                iflags.wc_backgrnd_message ?
                        iflags.wc_backgrnd_message : defbrief,
                iflags.wc_foregrnd_status ?
                        iflags.wc_foregrnd_status : defbrief,
                iflags.wc_backgrnd_status ?
                        iflags.wc_backgrnd_status : defbrief,
...
int load_symset(s, which_set)
    const char *s;int which_set;
{
    clear_symsetentry(which_set, TRUE);

    if (symset[which_set].name) free((genericptr_t) symset[which_set].name);
 ...
 */
int fruitadd(str, replace_fruit)
    char *str;struct fruit *replace_fruit;
{
    register int i;
    ...
    if (user_specified)
    {
        ...
        if (found || numeric || !strncmp(str, "cursed ", 7)
                ...
                || ((str_end_is(str, " corpse") || str_end_is(str, " egg"))
    ...
    for (f = ffruit; f; f = f->nextf)
    {
        ...
        if (!strncmp(str, f->fname, PL_FSIZ - 1)
                || (*altname && !strcmp(altname, f->fname))) goto nonew;
    }
    ...
    f->nextf = ffruit;
    ffruit = f;
    nonew: if (user_specified) context.current_fruit = f->fid;
    return f->fid;
}
...
int choose_classes_menu(prompt, category, way, class_list, class_select)
    const char *prompt;int category;boolean way;char *class_list;char *class_select;
{
    menu_item *pick_list = (menu_item *) 0;
    winid win;
    anything any;
...
void set_wc_option_mod_status(optmask, status)
    unsigned long optmask;int status;
{
    int k = 0;
    ...
    const char *optnam;
{
    ...
    while (wc_options[k].wc_name)
    {
        if (!strcmp(wc_options[k].wc_name, optnam)
                && (windowprocs.wincap & wc_options[k].wc_bit)) return TRUE;
        k++;
    }
    return FALSE;
}
    ...
    const char *optnam;
{
    ...
    while (wc2_options[k].wc_name)
    {
        if (!strcmp(wc2_options[k].wc_name, optnam)
                && (windowprocs.wincap2 & wc2_options[k].wc_bit)) return TRUE;
        k++;
    }
    return FALSE;
}
    ...
    char *op;
{
    ...
    { "mnu", "msg", "sts", "txt" };
    static char **fgp[] =
    { &iflags.wc_foregrnd_menu, &iflags.wc_foregrnd_message,
            &iflags.wc_foregrnd_status, &iflags.wc_foregrnd_text };
    static char **bgp[] =
    { &iflags.wc_backgrnd_menu, &iflags.wc_backgrnd_message,
    ...
    while (newop && *newop)
    {
        ...
        for (j = 0; j < 4; ++j)
        {
            if (!strcmpi(wn, wnames[j]) || !strcmpi(wn, shortnames[j]))
            {
                if (tfg && !strstri(tfg, " "))
                {
                    if (*fgp[j]) free((genericptr_t) *fgp[j]);
                    *fgp[j] = dupstr(tfg);
                }
                if (tbg && !strstri(tbg, " "))
                {
                    if (*bgp[j]) free((genericptr_t) *bgp[j]);
                    *bgp[j] = dupstr(tbg);
                }
                break;
            }
