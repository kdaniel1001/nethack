...
static struct valuable_data gems[LAST_GEM + 1 - FIRST_GEM + 1], /* 1 extra for glass */
...
#ifndef NO_SIGNAL
/*ARGSUSED*/
STATIC_PTR void done_intr(sig_unused)
...
void done_in_by(mtmp, how)
    struct monst *mtmp;int how;
{
    char buf[BUFSZ];
    struct permonst *mptr = mtmp->data, *champtr = (
            (mtmp->cham >= LOW_PM) ? &mons[mtmp->cham] : mptr);
    boolean distorted = (boolean) (Hallucination && canspotmon(mtmp)),
            mimicker = (mtmp->m_ap_type == M_AP_MONSTER), imitator = (mptr
                    != champtr || mimicker);
    ...
    if (imitator)
    {
        ...
        Sprintf (eos(buf),
                alt ? "%s in %s form" :
                mimicker ? "%s disguised as %s" : "%s imitating %s", realnm,
                shape);
    ...
    {
        const char *shknm = shkname(mtmp), *honorific =
                shkname_is_pname(mtmp) ? "" : mtmp->female ? "Ms. " : "Mr. ";
    ...
    else if (mptr == &mons[PM_GHOUL]) u.ugrave_arise = PM_GHOUL;
...
VA_DECL(const char *, str)
{
    ...
    raw_print(
            program_state.gameover ? "Postgame wrapup disrupted." :
    ...
    if (!wizard)
    {
        ...
        if (sysopt.support)
            ...
            raw_printf("Report error to \"%s\"%s", WIZARD_NAME, maybe_rebuild);
    }
#endif
...
STATIC_OVL boolean should_query_disclose_option(category, defquery)
    int category;char *defquery;
{
    int idx;
    char *dop;
...
STATIC_OVL void disclose(how, taken)
    int how;boolean taken;
{
    char c = '\0', defquery;
    char qbuf[QBUFSZ];
    boolean ask = FALSE;
    ...
    if (!done_stopprint)
    {
        ...
        c = ask ?
                yn_function("Do you want to see the dungeon overview?",
                        ynqchars, defquery) :
                defquery;
        if (c == 'y') show_overview((how >= PANICKED) ? 1 : 2, how);
        if (c == 'q')
        done_stopprint++;
    }
}
 ...
 * intact.
 */
STATIC_OVL void get_valuables(list)
 ...
 */
STATIC_OVL void sort_valuables(list, size)
    struct valuable_data list[];int size; /* max value is less than 20 */
{
    register int i, j;
...
STATIC_OVL boolean odds_and_ends(list, what)
    struct obj *list;int what;
{
    struct obj *otmp;
    for (otmp = list; otmp; otmp = otmp->nobj)
    {
        ...
        case CAT_CHECK: /* Schroedinger's Cat */
            ...
            if (SchroedingersBox(otmp)) return rn2(2);
            break;
        }
        if (Has_contents(otmp)) return odds_and_ends(otmp->cobj, what);
    }
    return FALSE;
}

/* called twice; first to calculate total, then to list relevant items */
STATIC_OVL void artifact_score(list, counting, endwin)
    struct obj *list;boolean counting; /* true => add up points; false => display them */
    winid endwin;
{
    char pbuf[BUFSZ];
    struct obj *otmp;
            ...
            else
            {
                ...
                Sprintf (pbuf, "%s%s (worth %ld %s and %ld points)",
                        ...
                        otmp->oartifact ?
                                ...
                                OBJ_NAME(objects[otmp->otyp]), value,
                        currency(value), points);
    ...
    int how;
{
    ...
    if (!killer.name[0] && (how == STARVING || how == BURNING)) killer.format =
    KILLED_BY;
...
STATIC_OVL void really_done(how)
    int how;
{
    boolean taken;
    char pbuf[BUFSZ];
    ...
    else if (how == TURNED_SLIME) u.ugrave_arise = PM_GREEN_SLIME;
    ...
    if (how == ESCAPED || how == PANICKED) killer.format = NO_KILLER_PREFIX;
    ...
    if (how == ESCAPED || how == ASCENDED)
    {
        ...
        if (!done_stopprint)
        {
            Sprintf (eos(pbuf), "%s with %ld point%s,",
                    how == ASCENDED ?
                            "went to your reward" : "escaped from the dungeon",
                    u.urexp, plur(u.urexp));
            putstr(endwin, 0, pbuf);
        }

        if (!done_stopprint) artifact_score(invent, FALSE, endwin); /* list artifacts */
...
void container_contents(list, identified, all_containers, reportempty)
    ...
    for (box = list; box; box = box->nobj)
    {
        if (Is_container(box) || box->otyp == STATUE)
        {
            ...
            else if (box->cobj)
            {
                ...
                nextclass: for (obj = box->cobj; obj; obj = obj->nobj)
                {
                    if (!flags.sortpack || obj->oclass == *invlet)
                    {
                        objarr_set(obj, i++, oarray,
...
STATIC_OVL void list_vanquished(defquery, ask)
    char defquery;boolean ask;
{
    register int i, lev;
...
STATIC_OVL void list_genocided(defquery, ask)
    char defquery;boolean ask;
{
    register int i;
...
void delayed_killer(id, format, killername)
    int id;int format;const char *killername;
{
...
struct kinfo * find_delayed_killer(id)
    int id;
{
    struct kinfo *k;
