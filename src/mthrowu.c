...
int thitu(tlev, dam, obj, name)
    int tlev, dam;struct obj *obj;const char *name; /* if null, then format `obj' */
{
    const char *onm, *knm;
    boolean is_acid;
 ...
 */
STATIC_OVL int drop_throw(obj, ohit, x, y)
    register struct obj *obj;boolean ohit;int x, y;
{
    int retvalu = 1;
    int create;
    struct monst *mtmp;
                    ...
                    && ((t->ttyp == PIT) || (t->ttyp == SPIKED_PIT))))
    {
        ...
        if (!objgone)
        {
            ...
            { /* don't double-dip on damage */
                ...
                if (!mtmp && x == u.ux && y == u.uy) mtmp = &youmonst;
...
int ohitmon(mtmp, otmp, range, verbose)
...
void m_throw(mon, x, y, dx, dy, range, obj)
    ...
    struct obj *obj; /* missile (or stack providing it) */
{
    ...
    if (obj->quan == 1L)
    {
        ...
        if (MON_WEP(mon) == obj) setmnotwielded(mon, obj);
    ...
    { /* Actually the loop is always exited by break */
        ...
        default:
            ...
            {
                hitv++;
                if (MON_WEP(mon) && MON_WEP(mon)->otyp == ELVEN_BOW) hitv++;
    ...
    struct monst *mtmp;
{
    ...
    if (is_pole(otmp))
    {
        ...
        if (dist2(mtmp->mx, mtmp->my, mtmp->mux, mtmp->muy) > POLE_LIM
                || !couldsee(mtmp->mx, mtmp->my)) return; /* Out of range, or intervening wall */
                        ...
                        otmp->oclass == WEAPON_CLASS) && !mtmp->mconf)
    {
        ...
        if ((is_elf(mtmp->data) && otmp->otyp == ELVEN_ARROW && mwep
                ...
                || (is_orc(mtmp->data) && otmp->otyp == ORCISH_ARROW && mwep
                        && mwep->otyp == ORCISH_BOW)) multishot++;
    ...
    if (lined_up(mtmp))
    {
        ...
        if (!mtmp->mspec_used && rn2(3))
        {
            if ((typ >= AD_MAGM) && (typ <= AD_ACID))
            {
                ...
                if (typ == AD_SLEE && !Sleep_resistance) mtmp->mspec_used +=
                        rnd(20);
            }
            else
...
boolean linedup(ax, ay, bx, by, boulderhandling)
    register xchar ax, ay, bx, by;int boulderhandling; /* 0=block, 1=ignore, 2=conditionally block */
{
    ...
    {
        if ((ax == u.ux && ay == u.uy) ?
                (boolean) couldsee(bx, by) : clear_path(ax, ay, bx, by))
            return TRUE;
        ...
        do
        {
            ...
            bx += dx, by += dy;
            if (IS_ROCK(levl[bx][by].typ) || closed_door(bx, by)) return FALSE;
...
struct obj * m_carrying(mtmp, type)
    struct monst *mtmp;int type;
{
...
boolean hits_bars(obj_p, x, y, always_hit, whodidit)
    struct obj **obj_p; /* *obj_p will be set to NULL if object breaks */
    int x, y;int always_hit; /* caller can force a hit for items which would fit through */
    int whodidit; /* 1==hero, 0=other, -1==just check whether it'll pass thru */
{
    ...
    case WEAPON_CLASS:
    {
        ...
        hits = (oskill != -P_BOW && oskill != -P_CROSSBOW && oskill != -P_DART && oskill != -P_SHURIKEN && oskill != P_SPEAR
    ...
    case ROCK_CLASS: /* includes boulder */
        if (obj_type != STATUE || mons[otmp->corpsenm].msize > MZ_TINY) hits =
        TRUE;
        break;
    case FOOD_CLASS:
