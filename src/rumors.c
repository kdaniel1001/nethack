...
STATIC_OVL void init_rumors(fp)
    dlb *fp;
{
    ...
    if (sscanf(line, rumors_header, &true_count, &true_rumor_size,
            ...
            &false_rumor_start, &eof_offset) == 7 && true_rumor_size > 0L
            && false_rumor_size > 0L)
    {
 ...
 */
char * getrumor(truth, rumor_buf, exclude_cookie)
    int truth; /* 1=true, -1=false, 0=either */
    char *rumor_buf;boolean exclude_cookie;
{
    dlb *rumors;
...
void rumor_check()
{
    ...
    if (true_rumor_size < 0L)
    { /* we couldn't open RUMORFILE */
        no_rumors: pline("rumors not accessible.");
        return;
    }
    ...
    if (rumors)
    {
        ...
        { /* if this is 1st outrumor() */
            init_rumors(rumors);
            if (true_rumor_size < 0L) goto no_rumors;
            /* init failed */
        }
...
STATIC_OVL void init_oracles(fp)
    dlb *fp;
{
    register int i;
    char line[BUFSZ];
...
void outoracle(special, delphi)
    boolean special;boolean delphi;
{
    char line[COLNO];
    char *endp;
    dlb *oracles;
    ...
    if (oracle_flg < 0 || (oracle_flg > 0 && oracle_cnt == 0)) return;
