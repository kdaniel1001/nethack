 ...
 */
STATIC_OVL boolean md_start(startp)
    coord *startp;
{
    ...
    {
        if (!enexto(startp, u.ux, u.uy, (struct permonst *) 0)) return FALSE; /* no good positions */
        return TRUE;
    }
            ...
            if (dd > max_distance)
            {
                if (lax)
                {
                    ...
                    startp->x = viz_rmin[row];
                        ...
                        (struct permonst *) 0) && !cansee(testcc.x, testcc.y)
 ...
 */
STATIC_OVL boolean md_stop(stopp, startp)
 ...
 * TRUE otherwise.
 */
STATIC_OVL boolean md_rush(md, tx, ty)
    struct monst *md;register int tx, ty; /* destination of mail daemon */
{
    ...
    while (1)
    {
        ...
        for (dx = -1; dx <= 1; dx++)
            for (dy = -1; dy <= 1; dy++)
                if ((dx || dy)
                        && isok(fx + dx,
                                fy
                                        + dy) && !IS_STWALL(levl[fx + dx][fy + dy].typ))
                {
        ...
        else if (fx == u.ux && fy == u.uy) verbalize("Excuse me.");
    ...
    struct mail_info *info;
{
    ...
    if (info->message_typ)
    {
        ...
        if (info->object_nam) obj = oname(obj, info->object_nam);
    ...
    go_back: (void) md_rush(md, start.x, start.y);
    mongone(md);
    /* deliver some classes of messages even if no daemon ever shows up */
    give_up: if (!message_seen && info->message_typ == MSG_OTHER)
...
ckmailstatus()
{
    ...
    while (broadcasts > 0)
    { /* process all trapped broadcasts [until] */
        broadcasts--;
