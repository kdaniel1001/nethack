...
STATIC_OVL boolean clear_fcorr(grd, forceshow)
    struct monst *grd;boolean forceshow;
{
    ...
    if (sawcorridor) pline_The("corridor disappears.");
    ...
    int x, y;
{
    ...
    for (i = x - 1; i <= x + 1; ++i)
        for (j = y - 1; j <= y + 1; ++j)
        {
            ...
            if (lev->typ == STONE) lev->lit = lev->waslit = 0;
            /* mark <i,j> as not having been seen from <x,y> */
            unset_seenv(lev, x, y, i, j);
        }
}

STATIC_OVL void restfakecorr(grd)
    struct monst *grd;
{
    ...
    struct monst *grd;
{
    ...
    if (!dispose)
    {
        ...
        grd->mhp = 0;
        if (grd == context.polearm.hitmon) context.polearm.hitmon = 0;
...
STATIC_OVL boolean in_fcorridor(grd, x, y)
    struct monst *grd;int x, y;
{
    register int fci;
    struct egd *egrd = EGD(grd);
...
findgd()
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
    {
        ...
        if (mtmp->isgd && on_level(&(EGD(mtmp)->gdlevel), &u.uz)) return mtmp;
    }
    return (struct monst *) 0;
}

char vault_occupied(array)
    char *array;
{
    register char *ptr;
...
void invault()
{
    ...
    { /* if time ok and no guard now. */
                ...
                for (x = u.ux - dd; x <= u.ux + dd; lx = x, x++)
                {
                    ...
                    if (levl[x][y].typ == CORR)
                    {
                        ...
                        if (levl[lx][ly].typ != STONE
                                && levl[lx][ly].typ != CORR) goto incr_radius;
                        goto fnd;
                    }
                }
            }
        ...
        {
            if (!umoney) verbalize("You have hidden gold.");
...
STATIC_OVL void move_gold(gold, vroom)
    struct obj *gold;int vroom;
{
    xchar nx, ny;
...
STATIC_OVL void wallify_vault(grd)
    struct monst *grd;
{
    int x, y, typ;
    int vlt = EGD(grd)->vroom;
    char tmp_viz;
    xchar lox = rooms[vlt].lx - 1, hix = rooms[vlt].hx + 1, loy = rooms[vlt].ly
            - 1, hiy = rooms[vlt].hy + 1;
    struct monst *mon;
    struct obj *gold;
    struct trap *trap;
    boolean fixed = FALSE;
    ...
    register struct monst *grd;
{
    ...
    if (abs(egrd->ogx - grd->mx) > 1 || abs(egrd->ogy - grd->my) > 1) return -1; /* teleported guard - treat as monster */
    ...
    if (egrd->fcend == 1)
    {
        if (u_in_vault && (u_carry_gold || um_dist(grd->mx, grd->my, 1)))
        {
            ...
            if (egrd->warncnt == 7)
            {
                m = grd->mx;
                n = grd->my;
                if (!Deaf) verbalize("You've been warned, knave!");
                mnexto(grd);
    ...
    if (egrd->fcend > 1)
    {
        ...
        || (egrd->fcend > 1 && u_in_vault)))
        {
            ...
            {
                egrd->warncnt = 6;
                if (!Deaf) verbalize("Drop all your gold, scoundrel!");
                return 0;
            }
            else
            {
...
void vault_gd_watching(activity)
