...
FDECL(find_branch_room, (coord *));
STATIC_DCL struct mkroom *
FDECL(pos_to_room, (XCHAR_P, XCHAR_P));
...
STATIC_OVL void finddpos(cc, xl, yl, xh, yh)
    coord *cc;xchar xl, yl, xh, yh;
{
    register xchar x, y;
...
STATIC_OVL void do_room_or_subroom(croom, lowx, lowy, hix, hiy, lit, rtype,
        special, is_room)
    register struct mkroom *croom;int lowx, lowy;register int hix, hiy;boolean lit;schar rtype;boolean special;boolean is_room;
{
    register int x, y;
    struct rm *lev;
...
void add_room(lowx, lowy, hix, hiy, lit, rtype, special)
    int lowx, lowy, hix, hiy;boolean lit;schar rtype;boolean special;
{
...
void add_subroom(proom, lowx, lowy, hix, hiy, lit, rtype, special)
    struct mkroom *proom;int lowx, lowy, hix, hiy;boolean lit;schar rtype;boolean special;
{
    register struct mkroom *croom;
...
STATIC_OVL void makerooms()
{
    ...
    while (nroom < MAXNROFROOMS && rnd_rect())
    {
        ...
        else if (!create_room(-1, -1, -1, -1, -1, -1, OROOM, -1)) return;
    }
    return;
}

STATIC_OVL void join(a, b, nxcor)
    register int a, b;boolean nxcor;
{
...
void makecorridors()
{
    ...
    if (nroom > 2) for (i = rn2(nroom) + 4; i; i--)
    {
        a = rn2(nroom);
        b = rn2(nroom - 2);
        if (b >= a) b += 2;
...
void add_door(x, y, aroom)
    register int x, y;register struct mkroom *aroom;
{
    register struct mkroom *broom;
    register int tmp;
    int i;

    if (aroom->doorct == 0) aroom->fdoor = doorindex;
...
STATIC_OVL void dosdoor(x, y, aroom, type)
    register xchar x, y;struct mkroom *aroom;int type;
{
...
STATIC_OVL boolean place_niche(aroom, dy, xx, yy)
    register struct mkroom *aroom;int *dy, *xx, *yy;
{
    coord dd;
...
STATIC_OVL void makeniche(trap_type)
    int trap_type;
{
            ...
            if (trap_type || !rn2(4))
            {
                ...
                if (trap_type)
                {
                    if ((trap_type == HOLE || trap_type == TRAPDOOR)
                            && !Can_fall_thru(&u.uz)) trap_type = ROCKTRAP;
...
STATIC_OVL void make_niches()
{
    ...
    boolean ltptr = (!level.flags.noteleport && dep > 15), vamp = (dep > 5
            && dep < 25);
...
STATIC_OVL void makelevel()
{
    ...
    if (!Is_botlevel(&u.uz)) mkstairs(somex(croom), somey(croom), 0, croom); /* down */
    if (nroom > 1)
    {
        troom = croom;
    ...
    if (do_vault())
    {
        ...
        {
            fill_vault: add_room(vault_x, vault_y, vault_x + w, vault_y + h,
            TRUE, VAULT,
            FALSE);
    ...
    for (croom = rooms; croom->hx > 0; croom++)
    {
        ...
        if (u.uhave.amulet || !rn2(3))
        {
            ...
            if (tmonst && tmonst->data == &mons[PM_GIANT_SPIDER]
                    && !occupied(x, y)) (void) maketrap(x, y, WEB);
        }
        ...
        if (!rn2(3)) (void) mkgold(0L, somex(croom), somey(croom));
 ...
 */
void mineralize(kelp_pool, kelp_moat, goldprob, gemprob, skip_lvl_checks)
    int kelp_pool, kelp_moat, goldprob, gemprob;boolean skip_lvl_checks;
{
    s_level *sp;
    struct obj *otmp;
    int x, y, cnt;
    ...
    if (!skip_lvl_checks
            && (In_hell(&u.uz) || In_V_tower(&u.uz) || Is_rogue_level(&u.uz)
                    ...
                    || ((sp = Is_special(&u.uz)) != 0 && !Is_oracle_level(&u.uz)
                            && (!In_mines(&u.uz) || sp->flags.town)))) return;
...
STATIC_OVL struct mkroom * find_branch_room(mp)
    coord *mp;
{
...
STATIC_OVL struct mkroom * pos_to_room(x, y)
    xchar x, y;
{
    int i;
    struct mkroom *curr;
...
void dodoor(x, y, aroom)
    int x, y;struct mkroom *aroom;
{
    if (doorindex >= DOORMAX)
    {
...
void mktrap(num, mazeflag, croom, tm)
    int num, mazeflag;struct mkroom *croom;coord *tm;
{
    register int kind;
    coord m;
    ...
    if ((kind == TRAPDOOR || kind == HOLE) && !Can_fall_thru(&u.uz)) kind =
    ROCKTRAP;
...
void mkstairs(x, y, up, croom)
    xchar x, y;char up;struct mkroom *croom;
{
    if (!x)
    {
...
STATIC_OVL void mkfount(mazeflag, croom)
    int mazeflag;struct mkroom *croom;
{
    coord m;
    register int tryct = 0;
 ...
 */
STATIC_OVL void mkinvpos(x, y, dist)
    xchar x, y;int dist;
{
    struct trap *ttmp;
    struct obj *otmp;
 ...
 */
STATIC_OVL void mk_knox_portal(x, y)
    xchar x, y;
{
