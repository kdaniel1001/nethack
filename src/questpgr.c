...
STATIC_DCL struct qtmsg *
FDECL(construct_qtlist, (long));
...
STATIC_OVL struct qtmsg * construct_qtlist(hdr_offset)
    long hdr_offset;
{
    ...
    msg_list = (struct qtmsg *) alloc(
            (unsigned) (n_msgs + 1) * sizeof(struct qtmsg));
...
boolean is_quest_artifact(otmp)
    struct obj *otmp;
{
...
/* replace deity, leader, nemesis, or artifact name with pronoun;
 overwrites cvt_buf[] */
STATIC_OVL void qtext_pronoun(who, which)
...
STATIC_OVL struct qtmsg * msg_in(qtm_list, msgnum)
    struct qtmsg *qtm_list;int msgnum;
{
    struct qtmsg *qt_msg;
...
STATIC_OVL void convert_line(in_line, out_line)
    char *in_line, *out_line;
{
    ...
    if (cc > &out_line[BUFSZ - 1]) panic("convert_line: overflow");
    *cc = 0;
    return;
}

STATIC_OVL void deliver_by_pline(qt_msg)
    struct qtmsg *qt_msg;
{
    long size;
...
STATIC_OVL void deliver_by_window(qt_msg, how)
    struct qtmsg *qt_msg;int how;
{
    long size;
    char in_line[BUFSZ], out_line[BUFSZ];
    ...
    *out_line = '\0';
    ...
    else if (qt_msg->delivery == 'c')
    { /* skip for 'qtdump' of 'p' */
