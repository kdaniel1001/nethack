...
STATIC_OVL boolean rm_waslit()
{
    ...
    if (levl[u.ux][u.uy].typ == ROOM && levl[u.ux][u.uy].waslit) return TRUE;
 ...
 */
STATIC_OVL void mkcavepos(x, y, dist, waslit, rockit)
    xchar x, y;int dist;boolean waslit, rockit;
{
    ...
    if (rockit)
    {
        ...
        if (!passes_walls(mtmp->data)) (void) rloc(mtmp, TRUE);
    }
...
STATIC_OVL void mkcavearea(rockit)
...
int dig_typ(otmp, x, y)
    struct obj *otmp;xchar x, y;
{
    boolean ispick;
...
boolean dig_check(madeby, verbose, x, y)
    struct monst *madeby;boolean verbose;int x, y;
{
...
STATIC_OVL int dig(VOID_ARGS)
{
    ...
    context.digging.effort += 10 + rn2(5) + abon() + uwep->spe
            - greatest_erosion(uwep) + u.udaminc;
    if (Race_if(PM_DWARF)) context.digging.effort *= 2;
    if (context.digging.down)
    {
        ...
        if (context.digging.effort <= 50
                || (ttmp
                        && (ttmp->ttyp == TRAPDOOR || ttmp->ttyp == PIT
    ...
    if (context.digging.effort > 100)
    {
        ...
        if (!does_block(dpx, dpy, &levl[dpx][dpy])) unblock_point(dpx, dpy); /* vision:  can see through */
        feel_newsym(dpx, dpy);
        ...
        cleanup: context.digging.lastdigtime = moves;
    ...
    { /* not enough effort has been spent yet */
        ...
        if (IS_WALL(lev->typ) || dig_target == DIGTYP_DOOR)
        {
            if (*in_rooms(dpx, dpy, SHOPBASE))
            {
                ...
                || (dig_target == DIGTYP_ROCK && !IS_ROCK(lev->typ))) return 0; /* statue or boulder got taken */
...
schar fillholetyp(x, y, fill_if_any)
    int x, y;boolean fill_if_any; /* force filling if it exists at all */
{
    register int x1, y1;
    int lo_x = max(1, x - 1), hi_x = min(x + 1, COLNO - 1), lo_y = max(0,
            y - 1), hi_y = min(y + 1, ROWNO - 1);
...
void digactualhole(x, y, madeby, ttyp)
    register int x, y;struct monst *madeby;int ttyp;
{
    ...
    if (ttyp == PIT)
    {
        ...
        else if (mtmp)
        {
            ...
            else if (mtmp != madeby) (void) mintrap(mtmp);
        }
    }
    else
    { /* was TRAPDOOR now a HOLE*/
 ...
 * in apply.c.
 */
void liquid_flow(x, y, typ, ttmp, fillmsg)
    xchar x, y;schar typ;struct trap *ttmp;const char *fillmsg;
{
    ...
    if (u_spot && !(Levitation || Flying))
    {
        if (typ == LAVAPOOL)
            (void) lava_effects();
        else if (!Wwalking) (void) drown();
    }
}

/* return TRUE if digging succeeded, FALSE otherwise */
boolean dighole(pit_only, by_magic, cc)
    boolean pit_only, by_magic;coord *cc;
{
    register struct trap *ttmp;
    struct rm *lev;
...
STATIC_OVL void dig_up_grave(cc)
    coord *cc;
{
    struct obj *otmp;
    xchar dig_x, dig_y;
    ...
    case 1:
        ...
        if (!!(otmp = mk_tt_object(CORPSE, dig_x, dig_y))) otmp->age -= 100; /* this is an *OLD* corpse */
        ;
        break;
    case 2:
        if (!Blind)
    ...
    case 3:
        if (!Blind) pline(
        Hallucination ? "I want my mummy!" : "You've disturbed a tomb!");
 ...
 */
void watch_dig(mtmp, x, y, zap)
    struct monst *mtmp;xchar x, y;boolean zap;
{
    struct rm *lev = &levl[x][y];
    ...
    register struct monst *mtmp;
{
    ...
    if ((here->wall_info & W_NONDIGGABLE) != 0)
    {
        impossible("mdig_tunnel:  %s at (%d,%d) is undiggable",
                (IS_WALL(here->typ) ? "wall" :
                 IS_TREE(here->typ) ? "tree" : "stone"), (int) mtmp->mx,
                (int) mtmp->my);
    ...
    if (IS_WALL(here->typ))
    {
        ...
        if (flags.verbose && !rn2(5)) You_hear("crashing rock.");
        if (*in_rooms(mtmp->mx, mtmp->my, SHOPBASE))
            add_damage(mtmp->mx, mtmp->my, 0L);
        if (level.flags.is_maze_lev)
        {
    ...
    {
        here->typ = ROOM;
        if (pile && pile < 5) (void) rnd_treefruit_at(mtmp->mx, mtmp->my);
    }
    else
    {
        here->typ = CORR;
        ...
        if (!Hallucination)
            ...
            You_feel("like you are %s.",
                    (ACURR(A_STR) < 6 || ACURR(A_DEX) < 6 || ACURR(A_CON) < 6
                            || ACURR(A_CHA) < 6 || ACURR(A_INT) < 6
                            || ACURR(A_WIS) < 6) ? "4-F" : "1-A");
    }
    else
    {
        if (!Hallucination)
...
void zap_dig()
{
    ...
    if (u.utrap && u.utraptype == TT_PIT && (trap_with_u = t_at(u.ux, u.uy)))
    {
        ...
        {
            if (xdir[diridx] == u.dx && ydir[diridx] == u.dy) break;
    ...
    while (--digdepth >= 0)
    {
        ...
        else if (closed_door(zx, zy) || room->typ == SDOOR)
        {
            ...
            if (room->typ == SDOOR)
                room->typ = DOOR;
            else if (cansee(zx, zy)) pline_The("door is razed!");
        ...
        else if (maze_dig)
        {
            if (IS_WALL(room->typ))
            {
                ...
                else if (!Blind) pline_The("wall glows then fades.");
                break;
            }
            ...
            { /* check trees before stone */
                ...
                else if (!Blind) pline_The("tree shudders but is unharmed.");
                break;
            }
            else if (room->typ == STONE || room->typ == SCORR)
            {
                ...
                else if (!Blind) pline_The("rock glows then fades.");
                break;
            }
        }
    ...
    if (pitflow && isok(flow_x, flow_y))
    {
        ...
        if (ttmp && (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT))
        {
            ...
            if (filltyp != ROOM) pit_flow(ttmp, filltyp);
        }
    }
 ...
 * down in the pit.
 */
STATIC_OVL int adj_pit_checks(cc, msg)
    coord *cc;char *msg;
{
    int ltyp;
    struct rm *room;
 ...
 */
STATIC_OVL void pit_flow(trap, filltyp)
    struct trap *trap;schar filltyp;
{
...
void buried_ball_to_punishment()
{
    coord cc;
    struct obj *ball;
    cc.x = u.ux;
...
/* move objects from fobj/nexthere lists to buriedobjlist, keeping position
 information */
struct obj * bury_an_obj(otmp, dealloced)
    struct obj *otmp;boolean *dealloced;
{
    struct obj *otmp2;
    boolean under_ice;
 ...
 */
void rot_corpse(arg, timeout)
    anything *arg;long timeout;
{
    xchar x = 0, y = 0;
    struct obj *obj = arg->a_obj;
    boolean on_floor = obj->where == OBJ_FLOOR, in_invent = obj->where
            == OBJ_INVENT;
    ...
    {
        if (obj == MON_WEP(obj->ocarry)) setmnotwielded(obj->ocarry, obj);
    }
    ...
    if (on_floor)
    {
        ...
        if (mtmp
                && !OBJ_AT(x,
                        y) && mtmp->mundetected && hides_under(mtmp->data))
        {
            mtmp->mundetected = 0;
        }
        else if (x
                == u.ux&& y == u.uy && u.uundetected && hides_under(youmonst.data))
