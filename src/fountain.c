...
STATIC_OVL void dowatersnakes()
{
    ...
    if (!(mvitals[PM_WATER_MOCCASIN].mvflags & G_GONE))
    {
        ...
        while (num-- > 0)
            ...
            NO_MM_FLAGS)) != 0 && t_at(mtmp->mx, mtmp->my))
                (void) mintrap(mtmp);
    }
    else
...
STATIC_OVL void dowaterdemon()
{
    ...
    if (!(mvitals[PM_WATER_DEMON].mvflags & G_GONE))
    {
        ...
        NO_MM_FLAGS)) != 0)
        {
            ...
            else if (t_at(mtmp->mx, mtmp->my)) (void) mintrap(mtmp);
        }
    }
    else
        pline_The("fountain bubbles furiously for a moment, then calms.");
...
STATIC_PTR void gush(x, y, poolcnt)
    int x, y;genericptr_t poolcnt;
{
    register struct monst *mtmp;
    ...
    if (((x + y) % 2) || (x == u.ux && y == u.uy)
            ...
            || (sobj_at(BOULDER, x, y)) || nexttodoor(x, y)) return;
...
void dryup(x, y, isyou)
    xchar x, y;boolean isyou;
{
    ...
    register struct obj *obj;
{
            ...
            && !exist_artifact(LONG_SWORD, artiname(ART_EXCALIBUR)))
    {
        ...
        newsym(u.ux, u.uy);
        level.flags.nfountains--;
        if (in_town(u.ux, u.uy)) (void) angry_guards(FALSE);
        return;
    }
    else
    {
            ...
            if (money > 10)
            {
                ...
                for (otmp = invent; otmp && money > 0; otmp = otmp->nobj)
                    if (otmp->oclass == COIN_CLASS)
                    {
                        ...
                        long coin_loss = (money + denomination - 1)
                                / denomination;
    ...
    case 29: /* You see coins */
        ...
        ;
        (void) mkgold(
                (long) (rnd((dunlevs_in_dungeon(&u.uz) - dunlev(&u.uz) + 1) * 2)
                        + 5), u.ux, u.uy);
        if (!Blind)
