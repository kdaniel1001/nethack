    ...
    int x, y;
{
    ...
    type = levl[x][y].typ;
    return (boolean) (IS_WALL(type) || IS_DOOR(type) || type == SDOOR
            || type == IRONBARS);
}

STATIC_OVL boolean iswall_or_stone(x, y)
    int x, y;
{
    register int type;
    ...
    type = levl[x][y].typ;
    return (boolean) (type == STONE || IS_WALL(type) || IS_DOOR(type)
            || type == SDOOR || type == IRONBARS);
}

/* return TRUE if out of bounds, wall or rock */
STATIC_OVL boolean is_solid(x, y)
    int x, y;
{
 ...
 *              . W W
 */
STATIC_OVL int extend_spine(locale, wall_there, dx, dy)
    int locale[3][3];int wall_there, dx, dy;
{
    int spine, nx, ny;
 ...
 */
void wallification(x1, y1, x2, y2)
    int x1, y1, x2, y2;
{
    uchar type;
    register int x, y;
...
STATIC_OVL boolean okay(x, y, dir)
    int x, y;register int dir;
{
    mz_move(x, y, dir);
    mz_move(x, y, dir);
    if (x < 3 || y < 3 || x > x_maze_max || y > y_maze_max
            || levl[x][y].typ != 0) return FALSE;
    return TRUE;
}

/* find random starting point for maze generation */
STATIC_OVL void maze0xy(cc)
    coord *cc;
{
 ...
 */
boolean bad_location(x, y, lx, ly, hx, hy)
    xchar x, y;xchar lx, ly, hx, hy;
{
...
void place_lregion(lx, ly, hx, hy, nlx, nly, nhx, nhy, rtype, lev)
    xchar lx, ly, hx, hy;xchar nlx, nly, nhx, nhy;xchar rtype;d_level *lev;
{
    int trycnt;
    boolean oneshot;
    xchar x, y;
...
STATIC_OVL boolean put_lregion_here(x, y, nlx, nly, nhx, nhy, rtype, oneshot,
        lev)
    xchar x, y;xchar nlx, nly, nhx, nhy;xchar rtype;boolean oneshot;d_level *lev;
{
...
STATIC_OVL void fixup_special()
{
    ...
    for (x = 0; x < num_lregions; x++, r++)
    {
        ...
        case LR_UPSTAIR:
        case LR_DOWNSTAIR:
            place_it: place_lregion(r->inarea.x1, r->inarea.y1, r->inarea.x2,
                    r->inarea.y2, r->delarea.x1, r->delarea.y1, r->delarea.x2,
    ...
    register const char *s;
{
    ...
    if (!level.flags.corrmaze) wallification(2, 2, x_maze_max, y_maze_max);
        ...
        for (x = 0; x < (x_maze_max >> 1) - 1; x++)
            for (y = 0; y < (y_maze_max >> 1) - 1; y++)
            {
                ...
                if (levl[cc->x][cc->y].typ
                        == (level.flags.corrmaze ? CORR : ROOM)) return;
            }
...
void mkportal(x, y, todnum, todlevel)
...
void fumaroles()
{
    ...
    if (snd && !Deaf) Norep("You hear a %swhoosh!", loud ? "loud " : "");
}
                ...
                for (j = 0, y = b->y; j < (int) b->bm[1]; j++, y++)
                    if (b->bm[j + 2] & (1 << i))
                    {
                        ...
                        if (OBJ_AT(x, y))
                        {
                            ...
                            struct container *cons = (struct container *) alloc(
    ...
    xchar x, y;
{
    ...
    lev = &levl[x][y];
    ltyp = lev->typ;
    if (ltyp == DRAWBRIDGE_UP) ltyp = db_under_typ(lev->drawbridgemask);
    ...
    if (ltyp == LAVAPOOL)
        ...
        return "swamp";
    else if (ltyp == MOAT && !Is_medusa_level(&u.uz)) return "moat";
...
STATIC_OVL void mk_bubble(x, y, n)
    register int x, y, n;
{
    /*
 ...
 */
STATIC_OVL void mv_bubble(b, dx, dy, ini)
    register struct bubble *b;register int dx, dy;register boolean ini;
{
