...
STATIC_DCL struct wseg *
FDECL(create_worm_tail, (int));
...
int get_wormno()
{
    ...
    {
        if (!wheads[new_wormno]) return new_wormno; /* found an empty wtails[] slot at new_wormno
         */
        new_wormno++;
    }
 ...
 */
STATIC_OVL
void toss_wsegs(curr, display_update)
 ...
 */
void detect_wsegs(worm, use_detection_glyph)
    struct monst *worm;boolean use_detection_glyph;
{
    int num;
    struct wseg *curr = wtails[worm->wormno];
 ...
 */
void place_worm_tail_randomly(worm, x, y)
    struct monst *worm;xchar x, y;
{
    int wnum = worm->wormno;
 ...
 */
STATIC_OVL
void random_dir(x, y, nx, ny)
    register xchar x, y;register xchar *nx, *ny;
{
    *nx = x;
    *ny = y;
 ...
 */
STATIC_OVL
struct wseg * create_worm_tail(num_segs)
    int num_segs;
{
    register int i = 0;
...
boolean worm_cross(x1, y1, x2, y2)
    int x1, y1, x2, y2;
{
    struct monst *worm;
