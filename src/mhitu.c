...
STATIC_OVL void hitmsg(mtmp, mattk)
    register struct monst *mtmp;register struct attack *mattk;
{
    int compat;
...
/* monster missed you */
STATIC_OVL void missmu(mtmp, nearmiss, mattk)
    struct monst *mtmp;boolean nearmiss;struct attack *mattk;
{
...
/* monster swings obj */
STATIC_OVL void mswings(mtmp, otemp)
    struct monst *mtmp;struct obj *otemp;
{
...
const char * mpoisons_subj(mtmp, mattk)
...
STATIC_OVL void wildmiss(mtmp, mattk)
    register struct monst *mtmp;register struct attack *mattk;
{
    int compat;
...
void expels(mtmp, mdat, message)
    struct monst *mtmp;struct permonst *mdat; /* if mtmp is polymorphed, mdat != mtmp->data */
    boolean message;
{
    if (message)
    {
...
struct attack * getmattk(mptr, indx, prev_result, alt_attk_buf)
    struct permonst *mptr;int indx, prev_result[];struct attack *alt_attk_buf;
{
    ...
    register struct monst *mtmp;
{
    ...
    if (u.uundetected && !range2 && foundyou && !u.uswallow)
    {
        ...
        if (is_hider(youmonst.data) && u.umonnum != PM_TRAPPER)
        {
            ...
            if (youmonst.data->mlet != S_PIERCER) return 0; /* lurkers don't attack */
    ...
    for (i = 0; i < NATTK; i++)
    {
        ...
        if ((u.uswallow && mattk->aatyp != AT_ENGL)
                || (skipnonmagc && mattk->aatyp != AT_MAGC)) continue;
                ...
                if (foundyou)
                {
                    if (tmp > (j = rnd(20 + i)))
                    {
                        if (mattk->aatyp != AT_KICK
                                || !thick_skinned(youmonst.data)) sum[i] =
                                hitmu(mtmp, mattk);
                    }
                    else
        ...
        case AT_HUGS: /* automatic if prev two attacks succeed */
            ...
            if ((!range2 && i >= 2 && sum[i - 1] && sum[i - 2])
                    || mtmp == u.ustuck) sum[i] = hitmu(mtmp, mattk);
            break;
...
STATIC_OVL boolean diseasemu(mdat)
...
STATIC_OVL boolean u_slip_free(mtmp, mattk)
    struct monst *mtmp;struct attack *mattk;
{
...
int magic_negation(mon)
    struct monst *mon;
{
    struct obj *o;
    long wearmask;
    int armpro, mc = 0;
    boolean is_you = (mon == &youmonst), gotprot = is_you ? (EProtection!= 0L)
 ...
 *             attacking you
 */
STATIC_OVL int hitmu(mtmp, mattk)
    register struct monst *mtmp;register struct attack *mattk;
{
        ...
        { /* hand to hand weapon */
            if (mattk->aatyp == AT_WEAP && otmp)
            {
                ...
                if (!(otmp->oartifact
                    ...
                    hitmsg(mtmp, mattk);
                if (!dmg) break;
                if (objects[otmp->otyp].oc_material == SILVER && Hate_silver)
                {
    ...
    case AD_DRIN:
        ...
        if (!uarmh || uarmh->otyp != DUNCE_CAP)
        {
            ...
            if (eat_brains(mtmp, &youmonst, TRUE, (int *) 0) == MM_MISS) break;
        }
    ...
    if (dmg)
    {
        if (Half_physical_damage
                ...
                || (Role_if(PM_PRIEST) && uarmh && is_quest_artifact(uarmh)
                        && (is_undead(mtmp->data) || is_demon(mtmp->data)
                                || is_vampshifter(mtmp)))) dmg = (dmg + 1) / 2;
        ...
        { /* Death's life force drain */
            ...
            if (*hpmax_p - permdmg > lowerlimit)
                *hpmax_p -= permdmg;
            else if (*hpmax_p > lowerlimit) *hpmax_p = lowerlimit;
            /* else unlikely...
...
STATIC_OVL int gulpmu(mtmp, mattk)
    register struct monst *mtmp;register struct attack *mattk;
{
    ...
    { /* swallows you */
        ...
        if ((t && ((t->ttyp == PIT) || (t->ttyp == SPIKED_PIT)))
                && sobj_at(BOULDER, u.ux, u.uy)) return 0;
        ...
        if (mtmp->data == &mons[PM_FOG_CLOUD])
        {
            You("are laden with moisture and %s",
                    ...
                    amphibious(youmonst.data) ?
                            "feel comforted." : "can barely breathe!");
...
STATIC_OVL int explmu(mtmp, mattk, ufound)
    register struct monst *mtmp;register struct attack *mattk;boolean ufound;
{
    ...
    register struct monst *mtmp;register struct attack *mattk;
{
    ...
    case AD_STON:
                ...
                && mtmp->data == &mons[PM_MEDUSA])
        {
            ...
            if (mon_reflects(mtmp,
                    !useeit ?
                            (char *) 0 :
                            "The gaze is reflected away by %s %s!")) break;
            if (!m_canseeu(mtmp))
    ...
    if (react >= 0)
    {
        ...
        pline("%s looks %s%s.", Monnam(mtmp),
                !rn2(3) ? "" :
                already ? "quite " : (!rn2(2) ? "a bit " : "somewhat "),
                reactions[react]);
    }
    return 0;
}
...
void mdamageu(mtmp, n)
    register struct monst *mtmp;register int n;
{
    context.botl = 1;
    if (Upolyd)
    {
 ...
 */
int could_seduce(magr, mdef, mattk)
    struct monst *magr, *mdef;struct attack *mattk;
{
    ...
    if (agrinvis && !defperc
            && (!SYSOPT_SEDUCE || (mattk && mattk->adtyp != AD_SSEX))) return 0;
    ...
    for (ring = invent; ring; ring = nring)
    {
        ...
        if (fem)
        {
            ...
            if (rn2(20) < ACURR(A_CHA))
            {
                (void) safe_qbuf(qbuf, "\"That ",
                        " looks pretty.  May I have it?\"", ring, xname,
                        simpleonames, "ring");
        ...
        else
        {
            if (uleft && uright && uleft->otyp == RIN_ADORNMENT
                    && uright->otyp == RIN_ADORNMENT) break;
...
STATIC_OVL void mayberem(obj, str)
    register struct obj *obj;const char *str;
{
    char qbuf[QBUFSZ];
...
STATIC_OVL int passiveum(olduasmon, mtmp, mattk)
    struct permonst *olduasmon;register struct monst *mtmp;register struct attack *mattk;
{
    int i, tmp;
        ...
        case AD_PLYS: /* Floating eye */
            ...
            if (u.umonnum == PM_FLOATING_EYE)
            {
                        ...
                        && (perceives(mtmp->data) || !Invis))
                {
                    ...
                    {
                        if (mon_reflects(mtmp,
                                "Your gaze is reflected by %s %s.")) return 1;
