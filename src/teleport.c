 ...
 */
boolean goodpos(x, y, mtmp, gpflags)
    int x, y;struct monst *mtmp;unsigned gpflags;
{
    ...
    if (mtmp != &youmonst && x == u.ux && y == u.uy
            && (!u.usteed || mtmp != u.usteed)) return FALSE;
 ...
 */
boolean enexto(cc, xx, yy, mdat)
    coord *cc;register xchar xx, yy;struct permonst *mdat;
{
    return enexto_core(cc, xx, yy, mdat, 0);
}

boolean enexto_core(cc, xx, yy, mdat, entflags)
    coord *cc;register xchar xx, yy;struct permonst *mdat;unsigned entflags;
{
#define MAX_GOOD 15
 ...
 */
STATIC_OVL boolean tele_jump_ok(x1, y1, x2, y2)
    int x1, y1, x2, y2;
{
    if (dndest.nlx > 0)
    {
        ...
        if (within_bounded_area(x1, y1, dndest.nlx, dndest.nly, dndest.nhx,
                dndest.nhy) && !within_bounded_area(x2, y2, dndest.nlx, dndest.nly,
                        dndest.nhx, dndest.nhy)) return FALSE;
        /* and if outside, can't teleport inside */
        if (!within_bounded_area(x1, y1, dndest.nlx, dndest.nly, dndest.nhx,
                dndest.nhy) && within_bounded_area(x2, y2, dndest.nlx, dndest.nly, dndest.nhx,
                        dndest.nhy)) return FALSE;
    }
    if (updest.nlx > 0)
    { /* ditto */
        if (within_bounded_area(x1, y1, updest.nlx, updest.nly, updest.nhx,
                updest.nhy) && !within_bounded_area(x2, y2, updest.nlx, updest.nly,
                        updest.nhx, updest.nhy)) return FALSE;
        if (!within_bounded_area(x1, y1, updest.nlx, updest.nly, updest.nhx,
                updest.nhy) && within_bounded_area(x2, y2, updest.nlx, updest.nly, updest.nhx,
                        updest.nhy)) return FALSE;
    }
    return TRUE;
}

STATIC_OVL boolean teleok(x, y, trapok)
    register int x, y;boolean trapok;
{
...
void teleds(nux, nuy, allow_drag)
    register int nux, nuy;boolean allow_drag;
{
    ...
    ball_active = (Punished && uball->where != OBJ_FREE), ball_still_in_range =
    FALSE;
...
boolean safe_teleds(allow_drag)
    boolean allow_drag;
{
...
boolean teleport_pet(mtmp, force_it)
    register struct monst *mtmp;boolean force_it;
{
    ...
    if (mtmp->mleashed)
    {
        ...
        else
        {
            ...
            release_it: m_unleash(mtmp, FALSE);
            return TRUE;
        }
    }
    return TRUE;
    ...
    struct obj *scroll;
{
    ...
    if ((u.uhave.amulet || On_W_tower_level(&u.uz)) && !rn2(3))
    {
        ...
        if (!wizard || yn("Override?") != 'y') return FALSE;
    }
...
void level_tele()
{
    ...
    if ((u.uhave.amulet || In_endgame(&u.uz) || In_sokoban(&u.uz)) && !wizard)
    {
    ...
    if (u.utrap && u.utraptype == TT_BURIEDBALL) buried_ball_to_punishment();
    ...
    {
        if (!(wizard && force_dest)) find_hell(&newlevel);
    }
    else
    {
        ...
        {
            newlev = dungeons[u.uz.dnum].depth_start + dunlevs_in_dungeon(&u.uz)
                    - 2;
            pline("Sorry...");
        }
         ...
         * current dungeon.
         */
        if (!(wizard && force_dest)) get_level(&newlevel, newlev);
    }
    schedule_goto(&newlevel, FALSE, FALSE, 0, (char *) 0, (char *) 0);
...
STATIC_OVL boolean rloc_pos_ok(x, y, mtmp)
    ...
    if (!xx)
    {
        ...
        if (updest.lx && (yy & 1) != 0) /* moving up */
            return (within_bounded_area(x, y, updest.lx, updest.ly, updest.hx,
                    updest.hy)
                    && (!updest.nlx
                            || !within_bounded_area(x, y, updest.nlx,
                                    updest.nly, updest.nhx, updest.nhy)));
        if (dndest.lx && (yy & 1) == 0) /* moving down */
            return (within_bounded_area(x, y, dndest.lx, dndest.ly, dndest.hx,
                    dndest.hy)
                    && (!dndest.nlx
                            || !within_bounded_area(x, y, dndest.nlx,
                                    dndest.nly, dndest.nhx, dndest.nhy)));
    }
    else
    {
 ...
 * the worm.
 */
void rloc_to(mtmp, x, y)
    struct monst *mtmp;register int x, y;
{
...
boolean rloc(mtmp, suppress_impossible)
    ...
    boolean suppress_impossible;
{
    ...
    found_xy: rloc_to(mtmp, x, y);
    return TRUE;
}

STATIC_OVL void mvault_tele(mtmp)
    struct monst *mtmp;
{
    register struct mkroom *croom = search_special(VAULT);
    coord c;
...
void mtele_trap(mtmp, trap, in_sight)
    struct monst *mtmp;struct trap *trap;int in_sight;
{
    char *monname;
...
int mlevel_tele_trap(mtmp, trap, force_it, in_sight)
    struct monst *mtmp;struct trap *trap;boolean force_it;int in_sight;
{
    int tt = trap->ttyp;
    ...
    while (!goodpos(tx, ty, (struct monst *) 0, 0)
            || (restricted_fall
                    && (!within_bounded_area(tx, ty, dndest.lx, dndest.ly,
                            dndest.hx, dndest.hy)
                            || (dndest.nlx
                                    && within_bounded_area(tx, ty, dndest.nlx,
                                            dndest.nly, dndest.nhx, dndest.nhy))))
            ...
            || (dndest.nlx && On_W_tower_level(&u.uz)
                    && within_bounded_area(tx, ty, dndest.nlx, dndest.nly,
                            dndest.nhx, dndest.nhy)
                            != within_bounded_area(otx, oty, dndest.nlx,
                                    dndest.nly, dndest.nhx, dndest.nhy)));
...
int random_teleport_level()
{
    ...
    if (!rn2(5) || Is_knox(&u.uz) || In_endgame(&u.uz)) return cur_depth;
    ...
    if (In_quest(&u.uz))
    {
        int bottom = dunlevs_in_dungeon(&u.uz), qlocate_depth =
        qlocate_level.dlevel;

        /* if hero hasn't reached the middle locate level yet,
         no one can randomly teleport past it */
        if (dunlev_reached(&u.uz) < qlocate_depth) bottom = qlocate_depth;
...
boolean u_teleport_mon(mtmp, give_feedback)
    struct monst *mtmp;boolean give_feedback;
{
    coord cc;
