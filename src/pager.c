...
FDECL(lookat, (int, int, char *, char *));
...
STATIC_OVL boolean is_swallow_sym(c)
    int c;
{
    int i;
 ...
 */
STATIC_OVL int append_str(buf, new_str)
...
boolean object_from_map(glyph, x, y, obj_p)
    int glyph, x, y;struct obj **obj_p;
{
    ...
    int x, y, glyph;
{
    ...
    if (otmp)
    {
        ...
        if (fakeobj) dealloc_obj(otmp), otmp = 0;
    }
    else
...
STATIC_OVL void look_at_monster(buf, monbuf, mtmp, x, y)
    char *buf, *monbuf; /* buf: output, monbuf: optional output */
    struct monst *mtmp;int x, y;
{
 ...
 */
STATIC_OVL struct permonst * lookat(x, y, buf, monbuf)
    int x, y;char *buf, *monbuf;
{
    ...
    if (u.ux == x && u.uy == y && canspotself())
    {
        ...
        if (Role_if(PM_WIZARD) && Race_if(PM_GNOME) && !Upolyd) pm =
                &mons[PM_WIZARD];
...
STATIC_OVL void checkfile(inp, pm, user_typed_name, without_asking)
    char *inp;struct permonst *pm;boolean user_typed_name, without_asking;
{
    dlb *fp;
    ...
    if (!strncmp(dbase_str, "a ", 2))
        ...
        dbase_str += 3;
    else if (!strncmp(dbase_str, "the ", 4)) dbase_str += 4;
    if (!strncmp(dbase_str, "tame ", 5))
        dbase_str += 5;
    else if (!strncmp(dbase_str, "peaceful ", 9)) dbase_str += 9;
    if (!strncmp(dbase_str, "invisible ", 10)) dbase_str += 10;
    if (!strncmp(dbase_str, "saddled ", 8)) dbase_str += 8;
    if (!strncmp(dbase_str, "statue of ", 10))
        dbase_str[6] = '\0';
    else if (!strncmp(dbase_str, "figurine of ", 12)) dbase_str[8] = '\0';
    ...
    if (*dbase_str)
    {
         ...
         */
        if (!alt) alt = makesingular(dbase_str);
    ...
    if (found_in_file)
    {
        ...
        {
            bad_data_file: impossible(
                    "'data' file in wrong format or corrupted");
...
int do_screen_description(cc, looked, sym, out_str, firstmatch)
    coord cc;boolean looked;int sym;char *out_str;const char **firstmatch;
{
    ...
    if ((looked ?
            (sym == showsyms[S_HUMAN + SYM_OFF_M] && cc.x == u.ux
                    && cc.y == u.uy) :
            (sym == def_monsyms[S_HUMAN].sym && !flags.showrace))
            && !(Race_if(PM_HUMAN) || Race_if(PM_ELF)) && !Upolyd) found +=
            append_str(out_str, "you"); /* tack on "or you" */
...
#define is_cmap_drawbridge(i) ((i) >= S_vodbridge && (i) <= S_hcdbridge)
    ...
    for (hit_trap = FALSE, i = 0; i < MAXPCHARS; i++)
    {
        ...
        {
            if (alt_i++ == 2) i = 0; /* undo loop increment */
        ...
        if (sym == (looked ? showsyms[i] : defsyms[i].sym) && *x_str)
        {
            ...
            int article =
                    strstri(x_str, " of a room") ?
                            2 :
                            !(alt_i <= 2 || strcmp(x_str, "air") == 0
...
int do_look(mode, click_cc)
    int mode;coord *click_cc;
{
    ...
    if (!clicklook)
    {
        ...
        if (i != 'y')
        {
            ...
            ATR_NONE, "something on the map", MENU_UNSELECTED);
            any.a_char = 'i';
            add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 0,
            ATR_NONE, "something you're carrying", MENU_UNSELECTED);
            any.a_char = '?';
            add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 'n',
            ATR_NONE, "something else (by symbol or name)",
            MENU_UNSELECTED);
            if (!u.uswallow && !Hallucination)
            {
                ...
                any.a_char = 'm';
                add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 0,
                ATR_NONE, "nearby monsters", MENU_UNSELECTED);
                any.a_char = 'M';
                add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 0,
                ATR_NONE, "all monsters shown on map", MENU_UNSELECTED);
                any.a_char = 'o';
                add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 0,
                ATR_NONE, "nearby objects", MENU_UNSELECTED);
                any.a_char = 'O';
                add_menu(win, NO_GLYPH, &any, flags.lootabc ? 0 : any.a_char, 0,
                ATR_NONE, "all objects shown on map", MENU_UNSELECTED);
            }
        ...
        case 'i':
        {
            ...
            *out_str = '\0';
            ...
            if (*out_str) checkfile(out_str, pm, TRUE, TRUE);
            return 0;
        }
        case '?':
            from_screen = FALSE;
...
int doidtrap()
{
    ...
    for (trap = ftrap; trap; trap = trap->ntrap)
        if (trap->tx == x && trap->ty == y)
        {
            ...
            if (u.dz)
            {
                if (u.dz < 0 ? (tt == TRAPDOOR || tt == HOLE) : tt == ROCKTRAP)
                    break;
            }
            tt = what_trap(tt);
            pline("That is %s%s%s.",
...
STATIC_OVL boolean help_menu(sel)
    int *sel;
{
    ...
    start_menu(tmpwin);
    if (!wizard)
        help_menu_items[WIZHLP_SLOT] = "", help_menu_items[WIZHLP_SLOT + 1] =
                (char *) 0;
