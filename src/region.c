...
NhRegion *
FDECL(create_region, (NhRect *, int));
 ...
 */
boolean inside_region(reg, x, y)
    NhRegion *reg;int x, y;
{
    int i;
 ...
 */
NhRegion * create_region(rects, nrect)
    NhRect *rects;int nrect;
{
    int i;
    NhRegion *reg;
 ...
 */
void add_rect_to_reg(reg, rect)
    NhRegion *reg;NhRect *rect;
{
    NhRect *tmp_rect;
 ...
 */
void add_mon_to_reg(reg, mon)
    NhRegion *reg;struct monst *mon;
{
    int i;
    unsigned *tmp_m;
    ...
    {
        tmp_m = (unsigned *) alloc(
                sizeof(unsigned) * (reg->max_monst + MONST_INC));
        if (reg->max_monst > 0)
        {
 ...
 */
void remove_mon_from_reg(reg, mon)
    NhRegion *reg;struct monst *mon;
{
    register int i;
    ...
    NhRegion *reg;
{
    if (reg)
    {
        ...
        if (reg->enter_msg) free((genericptr_t) reg->enter_msg);
        if (reg->leave_msg) free((genericptr_t) reg->leave_msg);
...
void run_regions()
{
    ...
    {
        /* Make the region age */
        if (regions[i]->ttl > 0L) regions[i]->ttl--;
 ...
 */
boolean m_in_out_region(mon, x, y)
 ...
 */
void update_monster_region(mon)
    struct monst *mon;
{
    register int i;
 ...
 */
NhRegion * visible_region_at(x, y)
    xchar x, y;
{
    register int i;
    ...
    for (i = 0; i < n_regions; i++)
        if (inside_region(regions[i], x, y) && regions[i]->visible
                && regions[i]->ttl != -2L) return regions[i];
    return (NhRegion *) 0;
}

void show_region(reg, x, y)
    NhRegion *reg;xchar x, y;
{
    ...
    int fd;int mode;
{
    ...
    skip_lots: if (release_data(mode)) clear_regions();
}

void rest_regions(fd, ghostly)
    int fd;boolean ghostly; /* If a bones file restore */
{
    int i, j;
    unsigned n;
    long tmstamp;
    ...
    for (i = 0; i < n_regions; i++)
    {
        ...
        if (regions[i]->nrects > 0)
            regions[i]->rects = (NhRect *) alloc(
                    sizeof(NhRect) * regions[i]->nrects);
        for (j = 0; j < regions[i]->nrects; j++)
        ...
        if (regions[i]->ttl >= 0L)
            regions[i]->ttl =
                    (regions[i]->ttl > tmstamp) ?
                            regions[i]->ttl - tmstamp : 0L;
        ...
        if (regions[i]->n_monst > 0)
            regions[i]->monsters = (unsigned *) alloc(
                    sizeof(unsigned) * regions[i]->n_monst);
        else
...
STATIC_OVL void reset_region_mids(reg)
    NhRegion *reg;
{
 ...
 */
/*ARGSUSED*/
boolean expire_gas_cloud(p1, p2)
    genericptr_t p1;genericptr_t p2 UNUSED;
{
    NhRegion *reg;
    int damage;
...
boolean inside_gas_cloud(p1, p2)
    genericptr_t p1;genericptr_t p2;
{
    NhRegion *reg;
    struct monst *mtmp;
    int dam;
...
NhRegion * create_gas_cloud(x, y, radius, damage)
    xchar x, y;int radius;int damage;
{
    NhRegion *cloud;
    int i, nrect;
    NhRect tmprect;
