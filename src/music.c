...
STATIC_OVL void awaken_monsters(distance)
    int distance;
{
...
STATIC_OVL void put_monsters_to_sleep(distance)
    int distance;
{
    register struct monst *mtmp;
...
STATIC_OVL void charm_snakes(distance)
    int distance;
{
    register struct monst *mtmp;
...
STATIC_OVL void calm_nymphs(distance)
    int distance;
{
    register struct monst *mtmp;
 ...
 */
STATIC_OVL void charm_monsters(distance)
    int distance;
{
 ...
 */
STATIC_OVL void do_earthquake(force)
    int force;
{
    register int x, y;
    struct monst *mtmp;
                ...
                case CORR: /* Try to make a pit */
                    ...
                    if (mtmp)
                    {
                        if (!is_flyer(mtmp->data) && !is_clinger(mtmp->data))
                        {
                            ...
                            if (mtmp->mhp > 0)
                                if ((mtmp->mhp -= rnd(m_already_trapped ? 4 : 6))
                                        <= 0)
                                {
                                    if (!cansee(x, y))
                    ...
                    else if (x == u.ux && y == u.uy)
                    {
                                ...
                                || (u.utrap && u.utraptype != TT_PIT))
                        {
                            ...
                            u.utrap = rn1(6, 2);
                            u.utraptype = TT_PIT;
                            losehp(Maybe_Half_Phys(rnd(6)), "fell into a chasm",
                            NO_KILLER_PREFIX);
                            selftouch("Falling, you");
                        }
                        else if (u.utrap && u.utraptype == TT_PIT)
                        {
                            boolean keepfooting = ((Fumbling && !rn2(5))
 ...
 */
STATIC_OVL int do_improvisation(instr)
    struct obj *instr;
{
            ...
            if (!strcmp(buf, tune))
            {
                ...
                for (y = u.uy - 1; y <= u.uy + 1; y++)
                    for (x = u.ux - 1; x <= u.ux + 1; x++)
                        if (isok(x, y)) if (find_drawbridge(&x, &y))
                        {
                    ...
                    for (x = u.ux - 1; x <= u.ux + 1 && !ok; x++)
                        if (isok(x, y))
                            if (IS_DRAWBRIDGE(levl[x][y].typ)
                                    || is_drawbridge_wall(x, y) >= 0) ok = TRUE;
                if (ok)
...
STATIC_OVL void speaker(instr, buf)
    struct obj *instr;char *buf;
{
    /*
