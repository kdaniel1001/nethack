...
STATIC_PTR char *
FDECL(safeq_xprname, (struct obj *));
STATIC_PTR char *
FDECL(safeq_shortxprname, (struct obj *));
...
FDECL(find_unpaid, (struct obj *, struct obj **));
...
sortloot_cmp(obj1, obj2)
    struct obj *obj1;struct obj *obj2;
{
    int val1 = 0;
    int val2 = 0;
     ...
     * to 0. */
    val1 = obj1->bknown ?
            (obj1->blessed << 2) + ((!obj1->blessed && !obj1->cursed) << 1)
                    + obj1->cursed :
            0;
    val2 = obj2->bknown ?
            (obj2->blessed << 2) + ((!obj2->blessed && !obj2->cursed) << 1)
                    + obj2->cursed :
            0;
    if (val1 != val2)
    {
...
struct obj ** objarr_init(n)
    int n;
{
    return (struct obj **) alloc(n * sizeof(struct obj *));
}

void objarr_set(otmp, idx, oarray, dosort)
    struct obj *otmp;int idx;struct obj **oarray;boolean dosort;
{
    if (dosort)
    {
        int j;
...
struct obj * merge_choice(objlist, obj)
    ...
    struct obj *obj;
{
    ...
    else if ((obj->otyp == AMULET_OF_REFLECTION || obj->otyp == BAG_OF_HOLDING)
    ...
    struct obj *obj;
{
    ...
    for (prev = 0, otmp = invent; otmp; prev = otmp, otmp = otmp->nobj)
        ...
        {
            obj = otmp;
            if (!obj) panic("addinv: null obj after merge otyp=%d", saved_otyp);
            goto added;
        }
 ...
 */
struct obj * hold_another_object(obj, drop_fmt, drop_arg, hold_msg)
    struct obj *obj;const char *drop_fmt, *drop_arg, *hold_msg;
{
    char buf[BUFSZ];
        ...
        else
        {
            if (flags.autoquiver && !uquiver && !obj->owornmask
                    && (is_missile(obj) || ammo_and_launcher(obj, uwep)
                            || ammo_and_launcher(obj, uswapwep))) setuqwep(obj);
...
void consume_obj_charge(obj, maybe_unpaid)
    struct obj *obj;boolean maybe_unpaid; /* false if caller handles shop billing */
{
    ...
    struct obj *obj;
{
    ...
    else if (obj->oartifact)
    {
        if (is_quest_artifact(obj))
        {
            if (!u.uhave.questart) impossible("don't have quest artifact?");
            u.uhave.questart = 0;
        }
...
struct obj * nxtobj(obj, type, by_nexthere)
    struct obj *obj;int type;boolean by_nexthere;
{
    register struct obj *otmp;
...
u_have_novel()
{
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj)
        if (otmp->otyp == SPE_NOVEL) return otmp;
    return (struct obj *) 0;
}

struct obj * o_on(id, objchn)
    unsigned int id;register struct obj *objchn;
{
    struct obj *temp;
...
boolean obj_here(obj, x, y)
    register struct obj *obj;int x, y;
{
    register struct obj *otmp;
...
STATIC_OVL void compactify(buf)
    register char *buf;
{
...
STATIC_OVL boolean splittable(obj)
    struct obj *obj;
{
...
STATIC_OVL boolean taking_off(action)
    const char *action;
{
...
STATIC_OVL boolean putting_on(action)
    const char *action;
{
    ...
    register const char *let, *word;
{
    ...
    if (usegold && !strcmp(word, "eat")
            && (!metallivorous(youmonst.data)
                    || youmonst.data == &mons[PM_RUST_MONSTER])) usegold =
    FALSE;
    ...
    for (otmp = firstobj; otmp; otmp = otmp->nobj)
    {
                ...
                || (useboulder && otmp->otyp == BOULDER))
        {
            ...
            if ((taking_off(word) /* exclude if not worn */
             ...
             */
                    ...
                    || (!strcmp(word, "use or apply")
                            ...
                            && ((otmp->oclass == WEAPON_CLASS && !is_pick(otmp)
                                    ...
                                    || (otmp->oclass == GEM_CLASS
                                            && !is_graystone(otmp))))
                    || (!strcmp(word, "invoke") && !otmp->oartifact
        ...
        else
        {
            ...
            if ((!strcmp(word, "read") && is_readable(otmp))) allowall =
                    usegold = TRUE;
        }
    }
    ...
    *ap = '\0';
    ...
    for (;;)
    {
        ...
        if (allowcnt == 2 && !strcmp(word, "throw"))
        {
            ...
            if (ilet != def_oc_syms[COIN_CLASS].sym
                    && !(otmp && otmp->oclass == COIN_CLASS)) allowcnt = 1;
...
void silly_thing(word, otmp)
    const char *word;struct obj *otmp;
{
...
STATIC_PTR int ckvalidcat(otmp)
    struct obj *otmp;
{
...
STATIC_PTR char * safeq_xprname(obj)
    struct obj *obj;
{
    return xprname(obj, (char *) 0, safeq_xprn_ctx.let, safeq_xprn_ctx.dot, 0L,
            0L);
}

/* alternate safe_qbuf() -> short_oname() callback */
STATIC_PTR char * safeq_shortxprname(obj)
    struct obj *obj;
{
...
int ggetobj(word, fn, mx, combo, resultflags)
    const char *word;int FDECL((*fn), (OBJ_P)), mx;
     ...
     */
int askchain(objchn, olets, allflag, fn, ckfn, mx, word)
    struct obj **objchn;register int allflag, mx;register const char *olets,
...
void fully_identify_obj(otmp)
    struct obj *otmp;
{
    makeknown(otmp->otyp);
    if (otmp->oartifact) discover_artifact((xchar) otmp->oartifact);
    otmp->known = otmp->dknown = otmp->bknown = otmp->rknown = 1;
    if (Is_container(otmp) || otmp->otyp == STATUE) otmp->cknown =
            otmp->lknown = 1;
...
STATIC_OVL void menu_identify(id_limit)
    int id_limit;
{
    menu_item *pick_list;
    ...
    while (id_limit)
    {
        ...
        SIGNAL_NOMENU | SIGNAL_ESCAPE | USE_INVLET | INVORDER_SORT, &pick_list,
...
void identify_pack(id_limit, learning_id)
    int id_limit;boolean learning_id; /* true if we just read unknown identify scroll */
{
...
STATIC_OVL char obj_to_let(obj)
    struct obj *obj;
{
 ...
 */
void prinv(prefix, obj, quan)
    const char *prefix;struct obj *obj;long quan;
{
    if (!prefix) prefix = "";
...
char * xprname(obj, txt, let, dot, cost, quan)
    struct obj *obj;const char *txt; /* text to print instead of obj */
 ...
 * containers.
 */
STATIC_OVL struct obj * find_unpaid(list, last_found)
    ...
    while (list)
    {
        if (list->unpaid)
        {
            if (*last_found)
            {
                ...
                if (list == *last_found) *last_found = (struct obj *) 0;
            }
            else
 ...
 */
STATIC_OVL char display_pickinv(lets, want_reply, out_cnt)
    register const char *lets;boolean want_reply;long *out_cnt;
{
    struct obj *otmp;
    char ilet, ret;
    ...
    if (wizard && iflags.override_ID)
    {
        ...
        add_menu(win, NO_GLYPH, &any, '_', iflags.override_ID, ATR_NONE, prompt,
        MENU_UNSELECTED);
    }
    nextclass: classcount = 0;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (!flags.sortpack || otmp->oclass == *invlet)
        {
            ...
            {
                add_menu(win,
                NO_GLYPH, &any, 0, 0, iflags.menu_headings,
...
char display_inventory(lets, want_reply)
    const char *lets;boolean want_reply;
{
 ...
 *
 */
STATIC_OVL char display_used_invlets(avoidlet)
    char avoidlet;
{
    struct obj *otmp;
    char ilet, ret = 0;
 ...
 */
int count_buc(list, type)
    struct obj *list;int type;
{
    int count = 0;
...
STATIC_OVL void tally_BUCX(list, bcp, ucp, ccp, xcp, ocp)
    struct obj *list;int *bcp, *ucp, *ccp, *xcp, *ocp;
{
...
long count_contents(container, nested, quantity, everything)
    ...
    everything; /* all objects vs only unpaid objects */
{
    ...
    for (otmp = container->cobj; otmp; otmp = otmp->nobj)
    {
        ...
        if (everything || otmp->unpaid) count += quantity ? otmp->quan : 1L;
    }
    return count;
}
...
STATIC_OVL void dounpaid()
{
    ...
    if (count == 1)
    {
        ...
        pline1(
                xprname(otmp, distant_name(otmp, doname), carried(otmp) ? otmp->invlet : CONTAINED_SYM, TRUE, cost, 0L));
...
STATIC_OVL boolean this_type_only(obj)
    struct obj *obj;
{
...
int dotypeinv()
{
    ...
    if (traditional)
    {
        ...
        if (oclass == COIN_CLASS) return doprgold();
        if (index(types, c) > index(types, '\033'))
        {
            ...
            if (which) You("have no %s objects.", which);
            return 0;
        }
        this_type = oclass;
    }
...
const char * dfeature_at(x, y, buf)
    int x, y;char *buf;
{
    ...
    if (IS_DOOR(ltyp))
    {
        ...
        if (is_drawbridge_wall(x, y) >= 0) dfeature =
                "open drawbridge portcullis", cmap = -1;
    }
    ...
    else if (IS_ALTAR(ltyp))
    {
        ...
        cmap = S_tree; /* "tree" */
    else if (ltyp == IRONBARS) dfeature = "set of iron bars";
...
int look_here(obj_cnt, picked_some)
    ...
    boolean picked_some;
{
    ...
    if (dfeature && !strcmp(dfeature, "pool of water") && Underwater) dfeature =
            0;
...
boolean will_feel_cockatrice(otmp, force_touch)
    struct obj *otmp;boolean force_touch;
{
...
void feel_cockatrice(otmp, force_touch)
    struct obj *otmp;boolean force_touch;
{
    char kbuf[BUFSZ];
...
STATIC_OVL boolean mergable(otmp, obj)
    register struct obj *otmp, *obj;
{
    ...
    if (obj->unpaid != otmp->unpaid || obj->spe != otmp->spe
            ...
            || obj->bypass != otmp->bypass) return FALSE;
    ...
    if ((obj->oclass == WEAPON_CLASS || obj->oclass == ARMOR_CLASS)
            && (obj->oerodeproof != otmp->oerodeproof
                    || obj->rknown != otmp->rknown)) return FALSE;
    ...
    if ((obj->otyp == EGG && (obj->timed || otmp->timed))
            || (obj->otyp == CORPSE && otmp->corpsenm >= LOW_PM
                    && is_reviver(&mons[otmp->corpsenm]))) return FALSE;
    ...
    || has_omid(otmp) || has_olong(otmp)) return FALSE;
...
STATIC_OVL void noarmor(report_uskin)
...
STATIC_OVL boolean tool_in_use(obj)
    struct obj *obj;
{
 ...
 */
void useupf(obj, numused)
    register struct obj *obj;long numused;
{
    register struct obj *otmp;
...
char * let_to_name(let, unpaid, showsym)
    char let;boolean unpaid, showsym;
{
...
int doorganize() /* inventory organizer by Del Lamb */
{
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj)
        if (otmp != obj && !mergable(otmp, obj))
        {
            ...
            if (let >= 'a' && let <= 'z')
                alphabet[let - 'a'] = ' ';
            else if (let >= 'A' && let <= 'Z') alphabet[let - 'A' + 26] = ' ';
        }
    ...
    for (trycnt = 1;; ++trycnt)
    {
        ...
        {
            noadjust: if (splitting) (void) merged(&splitting, &obj);
            pline1(Never_mind);
            return 0;
        }
        if ((letter(let) && let != '@') || index(buf, let)) break; /* got one */
...
STATIC_OVL void invdisp_nothing(hdr, txt)
    const char *hdr, *txt;
{
    winid win;
    anything any;
...
STATIC_OVL boolean worn_wield_only(obj)
    struct obj *obj;
{
#if 1
 ...
 */
struct obj * display_minventory(mon, dflags, title)
    register struct monst *mon;int dflags;char *title;
{
    struct obj *ret;
    char tmp[QBUFSZ];
    int n;
    menu_item *selected = 0;
    int do_all = (dflags & MINV_ALL) != 0, incl_hero = (do_all && u.uswallow
            && mon == u.ustuck), have_inv = (mon->minvent != 0), have_any =
            (have_inv || incl_hero);
    ...
    if (do_all ? have_any : (mon->misc_worn_check || MON_WEP(mon)))
    {
        ...
        INVORDER_SORT | (incl_hero ? INCLUDE_HERO : 0), &selected,
 ...
 */
struct obj * display_cinventory(obj)
    register struct obj *obj;
{
    struct obj *ret;
    char qbuf[QBUFSZ];
...
STATIC_OVL boolean only_here(obj)
    struct obj *obj;
{
 ...
 */
int display_binventory(x, y, as_if_seen)
    int x, y;boolean as_if_seen;
{
    struct obj *obj;
    menu_item *selected = 0;
    int n;
