...
static NEARDATA const char beverages[] =
{ POTION_CLASS, 0 };
...
STATIC_OVL long itimeout_incr(old, incr)
    long old;int incr;
{
...
void incr_itimeout(which, incr)
    long *which;int incr;
{
    set_itimeout(which, itimeout_incr(*which, incr));
}

void make_confused(xtime, talk)
    long xtime;boolean talk;
{
    long old = HConfusion;
...
void make_sick(xtime, cause, talk, type)
    long xtime;const char *cause; /* sickness cause */
...
void make_slimed(xtime, msg)
    long xtime;const char *msg;
{
    long old = Slimed;
...
void make_stoned(xtime, msg, killedby, killername)
    long xtime;const char *msg;int killedby;const char *killername;
{
    long old = Stoned;
...
boolean make_hallucinated(xtime, talk, mask)
    long xtime; /* nonzero if this is an attempt to turn on hallucination */
    boolean talk;long mask; /* nonzero if resistance status should change by mask */
{
    ...
    register struct obj *otmp;
{
    ...
    if (otmp->dknown && !objects[otmp->otyp].oc_name_known)
    {
        ...
        else if (!objects[otmp->otyp].oc_uname) docall(otmp);
    }
    useup(otmp);
    return 1;
}
    ...
    register struct obj *otmp;
{
    ...
    case POT_HALLUCINATION:
        ...
        (void) make_hallucinated(itimeout_incr(HHallucination, rn1(200, 600 - 300 * bcsign(otmp))),
        TRUE, 0L);
        break;
        case POT_WATER:
...
void healup(nhp, nxtra, curesick, cureblind)
    int nhp, nxtra;register boolean curesick, cureblind;
{
    if (nhp)
    {
        if (Upolyd)
...
void strange_feeling(obj, txt)
    struct obj *obj;const char *txt;
{
    ...
    if (obj->dknown && !objects[obj->otyp].oc_name_known
            && !objects[obj->otyp].oc_uname) docall(obj);
...
STATIC_OVL boolean H2Opotion_dip(potion, targobj, useeit, objphrase)
    struct obj *potion, *targobj;boolean useeit;const char *objphrase; /* "Your widget glows" or "Steed's saddle glows" */
{
...
void potionhit(mon, obj, your_fault)
    register struct monst *mon;register struct obj *obj;boolean your_fault;
{
    ...
    else
    {
        ...
        if (((mon->misc_worn_check & W_SADDLE)
                ...
                && (!rn2(10)
                        || (obj->otyp == POT_WATER
                                && ((rnl(10) > 7 && obj->cursed)
                                        || (rnl(10) < 4 && obj->blessed)
                                        || !rn2(3))))) hit_saddle = TRUE;
        ...
        if (rn2(5) && mon->mhp > 1 && !hit_saddle) mon->mhp--;
    }
    ...
    if (isyou)
    {
        ...
        {
        case POT_OIL:
            if (obj->lamplit) explode_oil(obj, u.ux, u.uy);
            break;
        case POT_POLYMORPH:
            You_feel("a little %s.", Hallucination ? "normal" : "strange");
    ...
    else
    {
        ...
        case POT_SICKNESS:
            ...
            do_illness: if ((mon->mhpmax > 3)
                    && !resist(mon, POTION_CLASS, 0, NOTELL)) mon->mhpmax /= 2;
        ...
        case POT_WATER:
            ...
            || is_were(mon->data) || is_vampshifter(mon))
            {
                ...
                else if (obj->cursed)
                {
                    ...
                    && !Protection_from_shape_changers) new_were(mon); /* transform into beast */
                }
            }
            ...
            break;
        case POT_OIL:
            if (obj->lamplit) explode_oil(obj, mon->mx, mon->my);
            break;
        case POT_ACID:
...
STATIC_OVL short mixtype(o1, o2)
...
int dodip()
{
        ...
        if (yn(upstart(qtoo)) == 'y')
        {
            if (Levitation)
            {
                floating_above(pooltype);
                    ...
                    && !is_swimmer(
                            u.usteed->data) && P_SKILL(P_RIDING) < P_BASIC)
            {
    ...
    if (potion->otyp == POT_WATER)
    {
        ...
        if (H2Opotion_dip(potion, obj, useeit, obj_glows)) goto poof;
    }
    ...
    potion->in_use = FALSE; /* didn't go poof */
    if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST) && (mixture =
            mixtype(obj, potion)) != 0)
    {
        ...
        singlepotion = hold_another_object(singlepotion,
                "You juggle and drop %s!", doname(singlepotion),
                (const char *) 0);
        update_inventory();
        return 1;
    }
    ...
    poof: if (!objects[potion->otyp].oc_name_known
            && !objects[potion->otyp].oc_uname) docall(potion);
    useup(potion);
    return 1;
}

/* *monp grants a wish and then leaves the game */
void mongrantswish(monp)
    struct monst **monp;
{
...
struct monst * split_mon(mon, mtmp)
