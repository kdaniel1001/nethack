...
{ /* also used in buzzmu(mcastu.c) */"magic missile", /* Wands must be 0-9 */
"bolt of fire", "bolt of cold", "sleep ray", "death ray", "bolt of lightning",
        "", "", "", "",
...
int bhitm(mtmp, otmp)
    struct monst *mtmp;struct obj *otmp;
{
    ...
    if (u.uswallow && mtmp == u.ustuck) reveal_invis = FALSE;
    ...
    case POT_POLYMORPH:
        ...
        else if (!resist(mtmp, otmp->oclass, 0, NOTELL))
        {
            ...
            {
                if (!Hallucination && canspotmon(mtmp)) learn_it = TRUE;
            }
        }
        break;
    ...
    if (wake)
    {
        ...
        else if (mtmp->m_ap_type) seemimic(mtmp); /* might unblock if mimicing a boulder/door */
    }
 ...
 */
boolean get_obj_location(obj, xp, yp, locflags)
    struct obj *obj;xchar *xp, *yp;int locflags;
{
    switch (obj->where)
    {
    case OBJ_INVENT:
...
boolean get_mon_location(mon, xp, yp, locflags)
    struct monst *mon;xchar *xp, *yp;int locflags; /* non-zero means get location even if monster is buried */
{
    if (mon == &youmonst)
    {
        *xp = u.ux;
...
struct monst * montraits(obj, cc)
    struct obj *obj;coord *cc;
{
    struct monst *mtmp = (struct monst *) 0;
    struct monst *mtmp2 = (struct monst *) 0;

    if (has_omonst(obj)) mtmp2 = get_mtraits(obj, TRUE);
    if (mtmp2)
    {
 ...
 * if applicable.
 */
struct monst * get_container_location(obj, loc, container_nesting)
    struct obj *obj;int *loc;int *container_nesting;
{
    if (!obj || !loc) return 0;
 ...
 */
struct monst * revive(corpse, by_hero)
    struct obj *corpse;boolean by_hero;
{
    struct monst *mtmp = 0;
    ...
    if (MON_AT(x, y))
    {
        if (enexto(&xy, x, y, mptr)) x = xy.x, y = xy.y;
    }
    ...
    if (mons[montype].mlet == S_EEL && !IS_POOL(levl[x][y].typ))
    {
        if (by_hero && cansee(x, y))
            pline("%s twitches feebly.",
                    upstart(corpse_xname(corpse, (const char *) 0,
                    CXN_PFX_THE)));
        return (struct monst *) 0;
    }
    ...
    if (by_hero)
    {
        ...
        if (costly_spot(x, y)) shkp = shop_keeper(*in_rooms(x, y, SHOPBASE));
    ...
    if (has_oname(corpse)) mtmp = christen_monst(mtmp, ONAME(corpse));
    /* partially eaten corpse yields wounded monster */
    if (corpse->oeaten) mtmp->mhp = eaten_stat(mtmp->mhp, corpse);
    ...
    case OBJ_FLOOR:
        ...
        if (corpse->quan > 1L) corpse = splitobj(corpse, 1L);
        delobj(corpse);
        newsym(x, y);
        break;
    case OBJ_MINVENT:
...
STATIC_OVL void revive_egg(obj)
    struct obj *obj;
{
    /*
    ...
    struct monst *mon;
{
    ...
    while ((otmp = otmp2) != 0)
    {
        ...
        if (youseeit)
        Strcpy (corpse, corpse_xname(otmp, (const char *) 0, CXN_SINGULAR));
    ...
    register struct obj *obj;
{
    ...
    case RIN_INCREASE_ACCURACY:
        if ((obj->owornmask & W_RING) && u_ring) u.uhitinc -= obj->spe;
        break;
    case RIN_INCREASE_DAMAGE:
        if ((obj->owornmask & W_RING) && u_ring) u.udaminc -= obj->spe;
        break;
    ...
    {
        if (obj->spe
                != ((obj->oclass == WAND_CLASS) ? -1 : 0)&& otyp != WAN_CANCELLATION /* can't cancel cancellation */
    ...
    register struct obj *obj;
{
    ...
    if (!obj
            ...
            || obj->spe <= 0) return FALSE;
    if (defends(AD_DRLI, obj) || defends_when_carried(AD_DRLI, obj)
            || obj_resists(obj, 10, 90)) return FALSE;
    ...
    case RIN_ADORNMENT:
        ...
        break;
    case RIN_INCREASE_ACCURACY:
        if ((obj->owornmask & W_RING) && u_ring) u.uhitinc--;
        break;
    case RIN_INCREASE_DAMAGE:
        if ((obj->owornmask & W_RING) && u_ring) u.udaminc--;
        break;
    case HELM_OF_BRILLIANCE:
        if ((obj->owornmask & W_ARMH) && (obj == uarmh))
        {
...
boolean obj_resists(obj, ochance, achance)
    struct obj *obj;int ochance, achance; /* percent chance for ordinary objects, artifacts */
{
 ...
 */
STATIC_OVL void polyuse(objhdr, mat, minwt)
    struct obj *objhdr;int mat, minwt;
{
 ...
 */
STATIC_OVL void create_polymon(obj, okind)
    struct obj *obj;int okind;
{
 ...
 */
struct obj * poly_obj(obj, id)
    struct obj *obj;int id;
{
    struct obj *otmp;
    xchar ox, oy;
    ...
    { /* preserve symbol */
        ...
        while (--try_limit > 0 && objects[otmp->otyp].oc_magic != magic_obj);
    }
    else
    {
...
#undef USES_CORPSENM
    ...
    case GEM_CLASS:
        if (otmp->quan
                > (long) rnd(4) && objects[obj->otyp].oc_material == MINERAL
    ...
    {
        long old_wornmask = obj->owornmask & ~(W_ART | W_ARTI), new_wornmask =
                wearslot(otmp);
        ...
        {
            if (was_twohanded || !bimanual(otmp)) setuswapwep(otmp);
            if (was_twoweap && uswapwep) u.twoweap = TRUE;
        }
        else if ((old_wornmask & W_WEP) != 0L)
        {
            if (was_twohanded || !bimanual(otmp) || !uarms) setuwep(otmp);
    ...
    if ((!carried(otmp) || obj->unpaid)
            ...
            && costly_spot(ox, oy))
    {
        register struct monst *shkp = shop_keeper(*in_rooms(ox, oy, SHOPBASE));
...
STATIC_OVL int stone_to_flesh_obj(obj)
    struct obj *obj;
{
...
int bhitpile(obj, fhito, tx, ty, zz)
    struct obj *obj;int FDECL((*fhito), (OBJ_P, OBJ_P));
int tx, ty;
schar zz;
{
    int hitanything = 0;
    ...
    register struct obj *obj;
{
    ...
    if (known)
    {
        if (!objects[obj->otyp].oc_name_known) more_experienced(0, 10);
...
static NEARDATA const char zap_syms[] =
{ WAND_CLASS, 0 };
...
int zapyourself(obj, ordinary)
    struct obj *obj;boolean ordinary;
{
...
int lightdamage(obj, ordinary, amt)
 ...
 */
STATIC_OVL boolean zap_steed(obj)
 ...
 */
boolean cancel_monst(mdef, obj, youattack, allow_cancel_kill, self_cancel)
    register struct monst *mdef;register struct obj *obj;boolean youattack,
            allow_cancel_kill, self_cancel;
{
    ...
    if (youdefend ?
            (!youattack && Antimagic) : resist(mdef, obj->oclass, 0, NOTELL))
...
STATIC_OVL boolean zap_updown(obj)
    struct obj *obj; /* wand or spell */
{
 ...
 */
STATIC_OVL int spell_hit_bonus(skill)
    int skill;
{
    int hit_bon = 0;
    int dex = ACURR(A_DEX);
...
void hit(str, mtmp, force)
    const char *str;struct monst *mtmp;const char *force; /* usually either "." or "!" */
{
...
void miss(str, mtmp)
    register const char *str;register struct monst *mtmp;
{
...
STATIC_OVL void skiprange(range, skipstart, skipend)
 ...
 */
struct monst * bhit(ddx, ddy, range, weapon, fhitm, fhito, pobj)
     ...
     */
struct monst * boomhit(obj, dx, dy)
    struct obj *obj;int dx, dy;
{
    register int i, ct;
...
int zhitm(mon, type, nd, ootmp)
    register struct monst *mon;register int type, nd;struct obj **ootmp; /* to return worn armor for caller to disintegrate */
{
    register int tmp = 0;
    ...
    *ootmp = (struct obj *) 0;
    ...
    if (is_hero_spell(type) && (Role_if(PM_KNIGHT) && u.uhave.questart)) tmp *=
            2;
    if (tmp > 0 && type >= 0
...
STATIC_OVL void zhitu(type, nd, fltxt, sx, sy)
    int type, nd;const char *fltxt;xchar sx, sy;
{
 ...
 */
int burn_floor_objects(x, y, give_feedback, u_caused)
    int x, y;boolean give_feedback; /* caller needs to decide about visibility checks */
    boolean u_caused;
{
    struct obj *obj, *obj2;
    ...
    for (obj = level.objects[x][y]; obj; obj = obj2)
    {
                        ...
                        && obj->otyp == GLOB_OF_GREEN_SLIME))
        {
            if (obj->otyp == SCR_FIRE || obj->otyp == SPE_FIREBALL
                    || obj_resists(obj, 2, 100)) continue;
...
STATIC_OVL int zap_hit(ac, type)
    int ac;int type; /* either hero cast spell type or 0 */
{
    int chance = rn2(20);
...
STATIC_OVL void disintegrate_mon(mon, type, fltxt)
    struct monst *mon;int type; /* hero vs other */
    const char *fltxt;
{
 ...
 */
void buzz(type, nd, sx, sy, dx, dy)
    register int type, nd;register xchar sx, sy;register int dx, dy;
{
    ...
    if (u.uswallow)
    {
        ...
        if (tmp == MAGIC_COOKIE) u.ustuck->mhp = 0;
    ...
    while (range-- > 0)
    {
        ...
        if (!ZAP_POS(lev->typ) || (closed_door(sx, sy) && (range >= 0)))
        {
            ...
            {
                pline("%s %s!", The(fltxt),
                Is_airlevel(&u.uz) ? "vanishes into the aether" : "bounces");
...
void melt_ice(x, y, msg)
    xchar x, y;const char *msg;
{
    ...
    if (x == u.ux && y == u.uy) spoteffects(TRUE); /* possibly drown, notice objects */
}
 ...
 * permanent instead.
 */
void start_melt_ice_timeout(x, y, min_time)
    xchar x, y;long min_time; /* <x,y>'s old melt timeout (deleted by time we get here) */
{
    int when;
    long where;
 ...
 */
void melt_ice_away(arg, timeout)
    anything *arg;long timeout UNUSED;
{
    xchar x, y;
 ...
 */
int zap_over_floor(x, y, type, shopdamage, exploding_wand_typ)
    xchar x, y;int type;boolean *shopdamage;short exploding_wand_typ;
{
    const char *zapverb;
    struct monst *mon;
    struct trap *t;
    ...
    case ZT_FIRE:
        ...
        {
            if (see_it) pline("Steam billows from the fountain.");
            rangemod -= 1;
            dryup(x, y, type > 0);
        }
        break; /* ZT_FIRE */
    ...
    case ZT_COLD:
        if (is_pool(x, y) || is_lava(x, y))
        {
            ...
            else
            {
                ...
                else if (!lava) You_hear("a crackling sound.");
    ...
    case ZT_ACID:
        if (lev->typ == IRONBARS)
        {
            ...
            {
                rangemod -= 3;
                if (see_it) Norep("The %s melt.", defsyms[S_bars].explanation);
    ...
    if (levl[x][y].typ == SDOOR)
    {
        ...
        if (see_it)
            pline("%s %s reveals a secret door.", yourzap ? "Your" : "The",
                    zapverb);
        else if (Is_rogue_level(&u.uz)) draft_message(FALSE); /* "You feel a draft." (open doorway) */
    }
    ...
    if (closed_door(x, y))
    {
        ...
        case ZT_LIGHTNING:
            ...
            break;
        default:
            def_case: if (exploding_wand_typ > 0)
            {
    ...
    if ((mon = m_at(x, y)) != 0)
    {
        ...
        if (type >= 0)
        {
            ...
            if (mon->ispriest && *in_rooms(mon->mx, mon->my, TEMPLE))
                ghod_hitsu(mon);
            if (mon->isshk && !*u.ushops) hot_pursuit(mon);
        }
    }
    return rangemod;
}
    ...
    register struct obj *obj; /* no texts here! */
{
    ...
    if (by_you && obj->otyp == BOULDER) sokoban_guilt();
    ...
    register struct obj *obj;
{
    ...
    if (trap && trap->ttyp == STATUE_TRAP
            && activate_statue_trap(trap, obj->ox, obj->oy, TRUE)) return FALSE;
...
int destroy_mitem(mtmp, osym, dmgtyp)
    struct monst *mtmp;int osym, dmgtyp;
{
    struct obj *obj, *obj2;
    int skip, tmp = 0;
    long i, cnt, quan;
...
int resist(mtmp, oclass, damage, tell)
    struct monst *mtmp;char oclass;int damage, tell;
{
    int resisted;
    int alev, dlev;
...
STATIC_OVL void wishcmdassist(triesleft)
    int triesleft;
{
    static NEARDATA const char
            * wishinfo[] =
                    { "Wish details:", "",
                            ...
                            0, }, preserve_wishless[] =
                    "Doing so will preserve 'wishless' conduct.", retry_info[] =
            ...
            *cardinals[] =
            { "zero", "one", "two", "three", "four", "five" }, too_many[] =
                    "too many";
    int i;
    winid win;
    char buf[BUFSZ];
