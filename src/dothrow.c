...
STATIC_OVL int throw_obj(obj, shotlimit)
    struct obj *obj;int shotlimit;
{
    struct obj *otmp;
    int multishot;
    schar skill;
    ...
    if (is_wet_towel(obj)) dry_a_towel(obj, -1, FALSE);
    ...
    if (multishot > 1 || shotlimit > 0)
    {
        ...
        You("%s %d %s.", m_shot.s ? "shoot" : "throw", multishot, /* (might be 1 if player gave shotlimit) */
...
STATIC_OVL boolean ok_to_throw(shotlimit_p)
 ...
 */
boolean walk_path(src_cc, dest_cc, check_proc, arg)
    coord *src_cc;coord *dest_cc;boolean FDECL((*check_proc), (genericptr_t, int, int));
genericptr_t arg;
{
 ...
 */
boolean hurtle_step(arg, x, y)
    genericptr_t arg;int x, y;
{
    ...
    if ((ttmp = t_at(x, y)) != 0)
    {
        ...
        else if (ttmp->ttyp == FIRE_TRAP)
        {
            dotrap(ttmp, 0);
                ...
                || ttmp->ttyp == HOLE || ttmp->ttyp == TRAPDOOR) && Sokoban)
        {
...
STATIC_OVL boolean mhurtle_step(arg, x, y)
    genericptr_t arg;int x, y;
{
 ...
 */
void hurtle(dx, dy, range, verbose)
 ...
 */
void mhurtle(mon, dx, dy, range)
    struct monst *mon;int dx, dy, range;
{
    coord mc, cc;
     ...
     */
    if (mon->data->msize >= MZ_HUGE || mon == u.ustuck || mon->mtrapped) return;
...
STATIC_OVL void check_shop_obj(obj, x, y, broken)
    struct obj *obj;xchar x, y;boolean broken;
{
    struct monst *shkp = shop_keeper(*u.ushops);

    if (!shkp) return;

    if (broken || !costly_spot(x, y) || *in_rooms(x, y, SHOPBASE) != *u.ushops)
    {
 ...
 */
STATIC_OVL boolean toss_up(obj, hitsroof)
    struct obj *obj;boolean hitsroof;
{
    const char *action;
    ...
    { /* neither potion nor other breaking object */
        ...
        { /* probably wasn't a weapon; base damage on weight */
            ...
            if (youmonst.data
                    == &mons[PM_SHADE]&& objects[obj->otyp].oc_material != SILVER)
                dmg = 0;
        }
        ...
        {
            petrify: killer.format = KILLED_BY;
...
void throwit(obj, wep_mask, twoweap)
    struct obj *obj;long wep_mask; /* used to re-equip returning boomerang */
    boolean twoweap; /* used to restore twoweapon mode if wielded weapon returns */
{
    ...
    if ((u.dx || u.dy || (u.dz < 1))
            ...
            && obj->owt
                    > (unsigned) ((Upolyd ? u.mh : u.uhp) * 2)&& !Is_airlevel(&u.uz))
    {
    ...
    {
        if (Is_airlevel(&u.uz) || Levitation) hurtle(-u.dx, -u.dy, 1, TRUE);
    ...
    else
    {
        ...
        if (is_ammo(obj))
        {
            ...
            else if (obj->oclass != GEM_CLASS) range /= 2;
        }
        ...
        else if (obj == uball && u.utrap && u.utraptype == TT_INFLOOR) range =
                1;
    ...
    if (mon)
    {
        ...
        if (mon && mon->isshk
                && (!inside_shop(u.ux, u.uy)
                        || !index(in_rooms(mon->mx, mon->my, SHOPBASE),
                                *u.ushops))) hot_pursuit(mon);
    ...
    else
    {
        ...
        if (obj->oartifact == ART_MJOLLNIR && Role_if(PM_VALKYRIE) && rn2(100))
        {
            ...
            else
            {
                ...
                {
                    pline(
                            Blind ? "%s lands %s your %s." : "%s back to you, landing %s your %s.",
                ...
                else
                {
                    dmg += rnd(3);
                    pline(
                            Blind ? "%s your %s!" : "%s back toward you, hitting your %s!",
 ...
 */
int omon_adj(mon, obj, mon_notices)
    struct monst *mon;struct obj *obj;boolean mon_notices;
{
    int tmp = 0;
...
STATIC_OVL void tmiss(obj, mon, maybe_wakeup)
    struct obj *obj;struct monst *mon;boolean maybe_wakeup;
{
 ...
 */
int thitmonst(mon, obj)
    register struct monst *mon;register struct obj *obj; /* thrownobj or kickedobj or uwep */
{
    ...
    else if (otyp == HEAVY_IRON_BALL)
    {
        ...
        if (tmp >= rnd(20))
        {
            ...
            if (!hmon(mon, obj, hmode))
            { /* mon killed */
                if (was_swallowed && !u.uswallow && obj == uball) return 1; /* already did placebc() */
            }
        }
        else
        {
...
STATIC_OVL int gem_accept(mon, obj)
    register struct monst *mon;register struct obj *obj;
{
    char buf[BUFSZ];
    ...
    if (*u.ushops || obj->unpaid) check_shop_obj(obj, mon->mx, mon->my, TRUE);
    (void) mpickobj(mon, obj); /* may merge and free obj */
    ret = 1;
 ...
 */
int hero_breaks(obj, x, y, from_invent)
    struct obj *obj;xchar x, y; /* object location (ox, oy may not be right) */
...
void release_camera_demon(obj, x, y)
    struct obj *obj;xchar x, y;
{
    struct monst *mtmp;
    if (!rn2(3) && (mtmp = makemon(&mons[rn2(3) ? PM_HOMUNCULUS : PM_IMP], x, y,
    NO_MM_FLAGS)) != 0)
    {
        if (canspotmon(mtmp)) pline("%s is released!",
        Hallucination ? An(rndmonnam(NULL)) : "The picture-painting demon");
 ...
 */
void breakobj(obj, x, y, hero_caused, from_invent)
    struct obj *obj;xchar x, y; /* object location (ox, oy may not be right) */
    ...
    boolean from_invent;
{
    ...
    if (hero_caused)
    {
        ...
        {
            if (*u.ushops || obj->unpaid) check_shop_obj(obj, x, y, TRUE);
        }
...
STATIC_OVL void breakmsg(obj, in_view)
    struct obj *obj;boolean in_view;
{
    ...
    case POT_WATER: /* really, all potions */
        if (!in_view)
            ...
            pline("%s shatter%s%s!", Doname2(obj), (obj->quan == 1L) ? "s" : "",
                    to_pieces);
        break;
    case EGG:
    case MELON:
    ...
    struct obj *obj;
{
    ...
    {
        pline(
        is_animal(u.ustuck->data) ? "%s in the %s's entrails." : "%s into %s.",
    ...
    if (u.dz > 0) pline_The("gold hits the %s.", surface(bhitpos.x, bhitpos.y));
    place_object(obj, bhitpos.x, bhitpos.y);
