...
void new_light_source(x, y, range, type, id)
    xchar x, y;int range, type;anything *id;
{
    light_source *ls;
 ...
 */
void del_light_source(type, id)
    int type;anything *id;
{
    ...
    char **cs_rows;
{
    ...
    for (ls = light_base; ls; ls = ls->next)
    {
        ...
        {
            if (get_obj_location(ls->id.a_obj, &ls->x, &ls->y, 0)) ls->flags |=
            LSF_SHOW;
        }
...
struct monst * find_mid(nid, fmflags)
    unsigned nid;unsigned fmflags;
{
    struct monst *mtmp;

    if (!nid) return &youmonst;
    if (fmflags & FM_FMON) for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
        if (!DEADMONSTER(mtmp) && mtmp->m_id == nid) return mtmp;
    if (fmflags & FM_MIGRATE)
        for (mtmp = migrating_mons; mtmp; mtmp = mtmp->nmon)
            if (mtmp->m_id == nid) return mtmp;
    if (fmflags & FM_MYDOGS) for (mtmp = mydogs; mtmp; mtmp = mtmp->nmon)
        if (mtmp->m_id == nid) return mtmp;
    return (struct monst *) 0;
}

/* Save all light sources of the given range. */
void save_light_sources(fd, mode, range)
    int fd, mode, range;
{
...
void relink_light_sources(ghostly)
    boolean ghostly;
{
    char which;
    unsigned nid;
 ...
 */
STATIC_OVL int maybe_write_ls(fd, range, write_it)
...
void light_sources_sanity_check()
{
    light_source *ls;
    struct monst *mtmp;
    struct obj *otmp;
    ...
    for (ls = light_base; ls; ls = ls->next)
    {
        if (!ls->id.a_monst) panic("insane light source: no id!");
        if (ls->type == LS_OBJECT)
        {
...
STATIC_OVL void write_ls(fd, ls)
    int fd;light_source *ls;
{
    anything arg_save;
    struct obj *otmp;
    struct monst *mtmp;
...
void obj_move_light_source(src, dest)
    struct obj *src, *dest;
{
    light_source *ls;

    for (ls = light_base; ls; ls = ls->next)
        if (ls->type == LS_OBJECT && ls->id.a_obj == src) ls->id.a_obj = dest;
    src->lamplit = 0;
    dest->lamplit = 1;
}
...
void obj_split_light_source(src, dest)
    struct obj *src, *dest;
{
...
void obj_merge_light_sources(src, dest)
    struct obj *src, *dest;
{
    light_source *ls;
...
void obj_adjust_light_radius(obj, new_radius)
    struct obj *obj;int new_radius;
{
    light_source *ls;
...
const char * arti_light_description(obj)
    struct obj *obj;
{
