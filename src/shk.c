...
STATIC_DCL struct bill_x *
FDECL(onbill, (struct obj *, struct monst *,
                BOOLEAN_P));
STATIC_DCL struct monst *
FDECL(next_shkp, (struct monst *, BOOLEAN_P));
STATIC_DCL long FDECL(shop_debt, (struct eshk *));
STATIC_DCL char *
FDECL(shk_owns, (char *, struct obj *));
STATIC_DCL char *
FDECL(mon_owns, (char *, struct obj *));
...
FDECL(shk_embellish, (struct obj *, long));
...
STATIC_DCL struct obj *
FDECL(bp_to_obj, (struct bill_x *));
    ...
    struct monst *mon;long amount;
{
    ...
    else if (ygold->owornmask) remove_worn_item(ygold, FALSE); /* quiver */
    freeinv(ygold);
...
STATIC_OVL struct monst * next_shkp(shkp, withbill)
    register struct monst *shkp;register boolean withbill;
{
...
void set_residency(shkp, zero_out)
    register struct monst *shkp;register boolean zero_out;
{
...
void restshk(shkp, ghostly)
    struct monst *shkp;boolean ghostly;
{
    if (u.uz.dlevel)
    {
...
STATIC_OVL void clear_unpaid(list)
...
STATIC_OVL long addupbill(shkp)
    register struct monst *shkp;
{
...
STATIC_OVL void call_kops(shkp, nearshop)
    register struct monst *shkp;register boolean nearshop;
{
    ...
    {
        if (flags.verbose && !Deaf) pline("But no one seems to respond to it.");
        return;
    }
...
void u_left_shop(leavestring, newlev)
    char *leavestring;boolean newlev;
{
    struct monst *shkp;
    struct eshk *eshkp;
    ...
    if (!shkp || !inhishop(shkp)) return; /* shk died, teleported, changed levels... */
...
STATIC_OVL boolean rob_shop(shkp)
    struct monst *shkp;
{
    struct eshk *eshkp;
    long total;
    ...
    /*const*/char *enterstring;
{
    ...
    if (Blind && !(Blind_telepat || Detect_monsters)) ++n; /* force feedback to be less specific */

    pline("This shop %s %s.", (m < n) ? "seems to be" : "is",
            !n ? "deserted" : "untended");
}

void u_entered_shop(enterstring)
    char *enterstring;
{
    register int rt;
    register struct monst *shkp;
    ...
    if (!inhishop(shkp))
    {
        ...
        if (!index(empty_shops, *enterstring)) deserted_shop(enterstring);
        Strcpy (empty_shops, u.ushops);
        u.ushops[0] = '\0';
        return;
    }
    ...
    if (!inside_shop(u.ux, u.uy))
    {
        ...
        int cnt;
        const char *tool;
        struct obj *pick = carrying(PICK_AXE), *mattock = carrying(
        DWARVISH_MATTOCK);
 ...
 */
STATIC_OVL long shop_debt(eshkp)
    struct eshk *eshkp;
{
    struct bill_x *bp;
    int ct;
...
void shopper_financial_report()
{
    ...
    for (pass = this_shkp ? 0 : 1; pass <= 1; pass++)
                ...
                shkp = next_shkp(shkp->nmon, FALSE))
        {
            ...
            else if (shkp == this_shkp) You("have no credit in here.");
            if ((amt = shop_debt(eshkp)) != 0)
                You("owe %s %ld %s.", shkname(shkp), amt, currency(amt));
            else if (shkp == this_shkp) You("don't owe any money here.");
        }
}
...
STATIC_OVL struct bill_x * onbill(obj, shkp, silent)
    register struct obj *obj;register struct monst *shkp;register boolean silent;
{
    if (shkp)
    {
        register struct bill_x *bp = ESHK(shkp)->bill_p;
...
STATIC_OVL long check_credit(tmp, shkp)
    long tmp;register struct monst *shkp;
{
...
void home_shk(shkp, killkops)
    register struct monst *shkp;register boolean killkops;
{
...
STATIC_OVL void pacify_shk(shkp)
    register struct monst *shkp;
{
...
STATIC_OVL void rouse_shk(shkp, verbosely)
    struct monst *shkp;boolean verbosely;
{
...
void make_happy_shk(shkp, silentkops)
    register struct monst *shkp;register boolean silentkops;
{
    ...
    eshkp->following = 0;
    eshkp->robbed = 0L;
    if (!Role_if(PM_ROGUE)) adjalign(sgn(u.ualign.type));
    if (!inhishop(shkp))
    {
        char shk_nam[BUFSZ];
    ...
    else if (wasmad) pline("%s calms down.", Monnam(shkp));
 ...
 */
void make_angry_shk(shkp, ox, oy)
    register struct monst *shkp;register xchar ox, oy;
{
    xchar sx, sy;
    struct eshk *eshkp = ESHK(shkp);
...
STATIC_OVL long cheapest_item(shkp)
...
int dopay()
{
            ...
            shkp = next_shkp(shkp->nmon, FALSE))
    {
        ...
        if (inhishop(shkp) && (*u.ushops == ESHK(shkp)->shoproom)) resident =
                shkp;
    }
    ...
    else
    {
        ...
        cc.x = u.ux;
        cc.y = u.uy;
        if (getpos(&cc, TRUE, "the creature you want to pay") < 0) return 0; /* player pressed ESC */
        cx = cc.x;
        cy = cc.y;
        if (cx < 0)
        {
    ...
    if (shkp != resident && NOTANGRY(shkp))
    {
        ...
        else
        {
            if (umoney > ltmp)
            {
                You("give %s the %ld gold piece%s %s asked for.", shkname(shkp),
                        ltmp, plur(ltmp), mhe(shkp));
                pay(ltmp, shkp);
            }
            else
            {
    ...
    if (!eshkp->billct && !eshkp->debit)
    {
        ...
        if (!ltmp && NOTANGRY(shkp))
        {
            ...
            if (!umoney) pline(no_money, stashed_gold ? " seem to" : "");
        }
        else if (ltmp)
        {
 ...
 */
STATIC_OVL int dopayobj(shkp, bp, obj_p, which, itemize)
    register struct monst *shkp;register struct bill_x *bp;struct obj **obj_p;int which; /* 0 => used-up item, 1 => other (unpaid or lost) */
    boolean itemize;
{
    ...
    if (itemize)
    {
        ...
        { /* partly used goods */
            ...
            ANGRY(shkp) ? "Pay" : "Please pay", simpleonames(obj), /* short name suffices */
    ...
    if (consumed)
    {
        ...
        { /* completely used-up, so get rid of it */
            obj_extract_self(obj);
            /* assert( obj == *obj_p ); */
            dealloc_obj(obj);
...
STATIC_OVL boolean inherits(shkp, numsk, croaked)
    struct monst *shkp;int numsk;int croaked;
{
    long loss = 0L;
    long umoney;
    struct eshk *eshkp = ESHK(shkp);
    ...
    clear: shkp->minvis = save_minvis;
    setpaid(shkp);
    return taken;
}

STATIC_OVL void set_repo_loc(shkp)
    struct monst *shkp;
{
    ...
    { /* already inside this shk's shop */
        ox = u.ux;
        oy = u.uy;
    }
...
STATIC_OVL struct obj * bp_to_obj(bp)
    register struct bill_x *bp;
{
    register struct obj *obj;
...
long get_cost_of_shop_item(obj)
    register struct obj *obj;
{
    struct monst *shkp;
    xchar x, y;
    ...
    if (*u.ushops && obj->oclass != COIN_CLASS && obj != uball && obj != uchain
...
STATIC_OVL long get_cost(obj, shkp)
    register struct obj *obj;register struct monst *shkp; /* if angry, impose a surcharge */
{
 ...
 */
long contained_cost(obj, shkp, price, usell, unpaid_only)
    struct obj *obj;struct monst *shkp;long price;boolean usell;boolean unpaid_only;
{
    ...
    register struct obj *obj;
{
    ...
    for (otmp = obj->cobj; otmp; otmp = otmp->nobj)
        if (otmp->oclass == COIN_CLASS)
            value += otmp->quan;
        else if (Has_contents(otmp)) value += contained_gold(otmp);

    return value;
}

STATIC_OVL void dropped_container(obj, shkp, sale)
    register struct obj *obj;register struct monst *shkp;register boolean sale;
{
    register struct obj *otmp;
    ...
    for (otmp = obj->cobj; otmp; otmp = otmp->nobj)
    {
        ...
        if (!otmp->unpaid && !(sale && saleable(shkp, otmp))) otmp->no_charge =
                1;
...
STATIC_OVL boolean special_stock(obj, shkp, quietly)
    struct obj *obj;struct monst *shkp;boolean quietly;
{
...
STATIC_OVL long set_cost(obj, shkp)
    register struct obj *obj;register struct monst *shkp;
{
    ...
    if (!obj->dknown || !objects[obj->otyp].oc_name_known)
    {
        ...
        else if (tmp > 1L && !rn2(4)) multiplier *= 3L, divisor *= 4L;
    }
...
void alter_cost(obj, amt)
    struct obj *obj;long amt; /* if 0, use regular shop pricing, otherwise force amount;
...
long unpaid_cost(unp_obj, include_contents)
...
STATIC_OVL void add_one_tobill(obj, dummy, shkp)
    struct obj *obj;boolean dummy;struct monst *shkp;
{
    struct eshk *eshkp;
    struct bill_x *bp;
    int bct;
...
STATIC_OVL void add_to_billobjs(obj)
    struct obj *obj;
{
...
STATIC_OVL void bill_box_content(obj, ininv, dummy, shkp)
    register struct obj *obj;register boolean ininv, dummy;register struct monst *shkp;
{
    register struct obj *otmp;
...
STATIC_OVL void shk_names_obj(shkp, obj, fmt, amt, arg)
    struct monst *shkp;struct obj *obj;const char *fmt; /* "%s %ld %s %s", doname(obj), amt, plur(amt), arg */
    long amt;const char *arg;
{
    ...
    if (!objects[obj->otyp].oc_magic && saleable(shkp, obj)
            && (obj->oclass == WEAPON_CLASS || obj->oclass == ARMOR_CLASS
                    ...
                    || obj->oclass == SPBOOK_CLASS || obj->otyp == MIRROR))
    {
...
boolean billable(shkpp, obj, roomno, reset_nocharge)
    struct monst **shkpp; /* in: non-null if shk has been validated; out: shk */
    struct obj *obj;char roomno;boolean reset_nocharge;
{
    ...
    if (obj->no_charge)
    {
        if (!Has_contents(obj)
                || (contained_gold(obj) == 0L
                        && contained_cost(obj, shkp, 0L, FALSE, !reset_nocharge)
                                == 0L)) shkp = 0; /* not billable */
...
void addtobill(obj, ininv, dummy, silent)
    struct obj *obj;boolean ininv, dummy, silent;
{
    struct monst *shkp = 0;
        ...
        else
        {
            ...
            pline("%s %ld %s %s %s%s.\"", buf, ltmp, currency(ltmp),
                    ...
                    (contentscount && !obj->unpaid) ?
                            "for the contents of this" : "for this", xname(obj),
    ...
    char *buf;
{
    ...
    if (is_vampire(youmonst.data))
        ...
        Strcat (buf,
                !is_human(youmonst.data) ? " creature" :
                (flags.female) ? " lady" : " sir");
}

void splitbill(obj, otmp)
...
STATIC_OVL void sub_one_frombill(obj, shkp)
    register struct obj *obj;register struct monst *shkp;
{
    register struct bill_x *bp;
    ...
    register struct obj *obj;register struct monst *shkp;
{
    ...
    if (Has_contents(obj)) for (otmp = obj->cobj; otmp; otmp = otmp->nobj)
    {
        if (otmp->oclass == COIN_CLASS) continue;
...
STATIC_OVL long stolen_container(obj, shkp, price, ininv)
    struct obj *obj;struct monst *shkp;long price;boolean ininv;
{
    struct obj *otmp;
    struct bill_x *bp;
    long billamt;
...
long stolen_value(obj, x, y, peaceful, silent)
    struct obj *obj;xchar x, y;boolean peaceful, silent;
{
    ...
    else
    {
        if (billamt)
            value += billamt;
        else if (!obj->no_charge) value += obj->quan * get_cost(obj, shkp);
...
void sellobj(obj, x, y)
    register struct obj *obj;xchar x, y;
{
    register struct monst *shkp;
    register struct eshk *eshkp;
    ...
    { /* they become shop-objects, no pay */
        if (!muteshk(shkp)) verbalize("Thank you, scum!");
    ...
    if (isgold || cgold)
    {
        ...
        else
        {
            ...
            if (eshkp->credit == delta)
                You("have established %ld %s credit.", delta, currency(delta));
            else
    ...
    if (!shkmoney)
    {
        ...
        else if (sell_response != 'n')
        {
            ...
            Sprintf (qbuf, "Will you accept %ld %s in credit for ", tmpcr,
                    currency(tmpcr));
            c = ynaq(
                    safe_qbuf(qbuf, qbuf, "?", obj, doname, thesimpleoname,
...
STATIC_OVL long getprice(obj, shk_buying)
    register struct obj *obj;boolean shk_buying;
{
    ...
    case TOOL_CLASS:
        if (Is_candle(obj)
                && obj->age < 20L * (long) objects[obj->otyp].oc_cost) tmp /=
                2L;
        break;
    }
    return tmp;
...
struct monst * shkcatch(obj, x, y)
    register struct obj *obj;register xchar x, y;
{
    register struct monst *shkp;
...
void add_damage(x, y, cost)
    register xchar x, y;long cost;
{
 ...
 */
STATIC_OVL
void remove_damage(shkp, croaked)
    struct monst *shkp;boolean croaked;
{
    ...
    if (saw_walls)
    {
        ...
        if (saw_untrap) pline("%s!", upstart(trapmsg));
    }
    else
    {
 ...
 * 3: untrap
 */
int repair_damage(shkp, tmp_dam, catchup)
    register struct monst *shkp;register struct damage *tmp_dam;boolean catchup; /* restoring a level */
{
    ...
    {
        if (x == u.ux && y == u.uy) if (!Passes_walls) return 0;
...
#define vert(i) ((i / 3) - 1)
                        ...
                        || (uball->ox == x && uball->oy == y)))
        {
             ...
             */
            if (!muteshk(shkp)) verbalize("Get your junk out of my wall!");
    ...
    register struct monst *shkp;
{
    ...
    if (eshkp->following || ((z = holetime()) >= 0 && z * z <= udist))
    {
        ...
        if (udist > 4 && eshkp->following && !eshkp->billct) return -1; /* leave it to m_move */
        gx = u.ux;
        gy = u.uy;
    }
    else if (ANGRY(shkp))
    ...
    register int fall;
{
    ...
    if (shkp->msleeping || !shkp->mcanmove || is_silent(shkp->data))
        ...
        lang = 1;
    else if (shkp->data->msound >= MS_HUMANOID) lang = 2;
...
void pay_for_damage(dmgstr, cant_mollify)
    const char *dmgstr;boolean cant_mollify;
{
            ...
            tmp_dam = tmp_dam->next)
    {
        ...
        for (shp = shops_affected; *shp; shp++)
        {
            ...
            if (tmp_shk == shkp)
            {
                unsigned int damage_distance = distu(tmp_dam->place.x,
                        tmp_dam->place.y);
    ...
    if ((um_dist(x, y, 1) && !uinshp) || cant_mollify
            ...
            || !rn2(50))
    {
        getcad: if (muteshk(shkp))
        {
            if (animal && shkp->mcanmove && !shkp->msleeping) yelp(shkp);
        }
    ...
    Sprintf (qbuf, "%sYou did %ld %s worth of damage!%s  Pay?",
            !animal ? cad(TRUE) : "", cost_of_damage, currency(cost_of_damage),
            !animal ? "\"" : "");
    if (yn(qbuf) != 'n')
    {
    ...
    register struct obj *first_obj;
{
    ...
    for (otmp = first_obj; otmp; otmp = otmp->nexthere)
    {
        ...
        cost = (otmp->no_charge || otmp == uball || otmp == uchain) ?
                0L : get_cost(otmp, (struct monst *) 0);
        contentsonly = !cost;
        if (Has_contents(otmp))
...
STATIC_OVL const char * shk_embellish(itm, cost)
    register struct obj *itm;long cost;
{
    if (!rn2(3))
    {
...
STATIC_OVL void kops_gone(silent)
    boolean silent;
{
    register int cnt = 0;
    register struct monst *mtmp, *mtmp2;
...
STATIC_OVL long cost_per_charge(shkp, otmp, altusage)
    struct monst *shkp;struct obj *otmp;boolean altusage; /* some items have an "alternate" use with different cost */
{
    long tmp = 0L;
    ...
    || otmp->otyp == BRASS_LANTERN
            || (otmp->otyp >= MAGIC_FLUTE && otmp->otyp <= DRUM_OF_EARTHQUAKE) /* 5 - 9 */
 ...
 */
void check_unpaid_usage(otmp, altusage)
    struct obj *otmp;boolean altusage;
{
    struct monst *shkp;
    ...
    if (!otmp->unpaid || !*u.ushops
            || (otmp->spe <= 0 && objects[otmp->otyp].oc_charged)) return;
    if (!(shkp = shop_keeper(*u.ushops)) || !inhishop(shkp)) return;
...
void check_unpaid(otmp)
    struct obj *otmp;
{
    check_unpaid_usage(otmp, FALSE); /* normal item use */
}

void costly_gold(x, y, amount)
    register xchar x, y;register long amount;
{
    register long delta;
    register struct monst *shkp;
    register struct eshk *eshkp;
    ...
    register xchar x, y;
{
    ...
    if (!(IS_DOOR(levl[u.ux][u.uy].typ) && levl[u.ux][u.uy].doormask == D_BROKEN))
        return FALSE;
    ...
    if (ESHK(shkp)->shd.x != u.ux || ESHK(shkp)->shd.y != u.uy) return FALSE;
...
char * shk_your(buf, obj)
    char *buf;struct obj *obj;
{
...
STATIC_OVL char * shk_owns(buf, obj)
    char *buf;struct obj *obj;
{
    struct monst *shkp;
    xchar x, y;
...
STATIC_OVL char * mon_owns(buf, obj)
    char *buf;struct obj *obj;
{
...
STATIC_OVL const char * cad(altusage)
