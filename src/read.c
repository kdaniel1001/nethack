...
STATIC_DCL char *
FDECL(erode_obj_text, (struct obj *, char *));
...
STATIC_OVL boolean learnscrolltyp(scrolltyp)
    short scrolltyp;
{
...
void learnscroll(sobj)
    struct obj *sobj;
{
    /* it's implied that sobj->dknown is set;
     we couldn't be reading this scroll otherwise */
    if (sobj->oclass != SPBOOK_CLASS) (void) learnscrolltyp(sobj->otyp);
}

char * erode_obj_text(otmp, buf)
    struct obj *otmp;char *buf;
{
...
char * tshirt_text(tshirt, buf)
    struct obj *tshirt;char *buf;
{
    static const char *shirt_msgs[] =
            {
                    ...
                    "Aberzombie and Fitch", "cK -- Cockatrice touches the Kop",
                    "Don't ask me, I only adventure here", "Down with pants!",
                    "d, your dog or a killer?", "FREE PUG AND NEWT!",
                    "Go team ant!", "Got newt?", "Hello, my darlings!", /* Charlie Drake */
                    ...
                    "Lichen Park", "LOST IN THOUGHT - please send search party",
                    "Meat is Mordor", "Minetown Better Business Bureau",
                    "Minetown Watch",
                    "Ms. Palm's House of Negotiable Affection -- A Very Reputable House Of Disrepute",
                    "Protection Racketeer", "Real men love Crom",
                    "Somebody stole my Mojo!", "The Hellhound Gang",
                    "The Werewolves", "They Might Be Storm Giants",
                    ...
                    "Mind flayers suck", "I'm not wearing any pants",
                    "Down with the living!", "Pudding farmer", "Vegetarian",
                    "Hello, I'm War!", };
...
char * apron_text(apron, buf)
    struct obj *apron;char *buf;
{
    static const char *apron_msgs[] =
            { "Kiss the cook", "I'm making SCIENCE!",
                    "Don't mess with the chef", "Don't make me poison you",
                    "Gehennom's Kitchen", "Rat: The other white meat",
...
int doread()
{
    ...
    else if (scroll->otyp == T_SHIRT || scroll->otyp == ALCHEMY_SMOCK)
    {
        ...
        pline("\"%s\"",
                (scroll->otyp == T_SHIRT) ?
                        tshirt_text(scroll, buf) : apron_text(scroll, buf));
        return 1;
    }
    ...
    else if (scroll->oclass == COIN_CLASS)
    {
        ...
        else if (flags.verbose) You("read:");
    ...
    else if (scroll->otyp == CANDY_BAR)
    {
        ...
        "Snacky Cake", "Chocolate Nuggie", "The Small Bar", "Crispy Yum Yum",
                "Nilla Crunchie", "Berry Bar", "Choco Nummer", "Om-nom", /* Cat Macro */
    ...
    if (scroll->otyp != SPE_BOOK_OF_THE_DEAD && scroll->otyp != SPE_BLANK_PAPER
            && scroll->otyp != SCR_BLANK_PAPER && scroll->otyp != SPE_NOVEL)
        u.uconduct.literate++;
    ...
    if (scroll->otyp != SCR_BLANK_PAPER)
    {
        ...
        if (confused)
        {
            if (Hallucination)
                ...
                is_silent(youmonst.data) ? "misunderstand" : "mispronounce");
        }
    }
    if (!seffects(scroll))
    {
        if (!objects[scroll->otyp].oc_name_known)
        {
            if (known)
                learnscroll(scroll);
            else if (!objects[scroll->otyp].oc_uname) docall(scroll);
        }
        scroll->in_use = FALSE;
        if (scroll->otyp != SCR_BLANK_PAPER) useup(scroll);
    }
    return 1;
}

STATIC_OVL void stripspe(obj)
...
STATIC_OVL void p_glow1(otmp)
    register struct obj *otmp;
{
    pline("%s briefly.", Yobjnam2(otmp, Blind ? "vibrate" : "glow"));
}

STATIC_OVL void p_glow2(otmp, color)
    register struct obj *otmp;register const char *color;
{
...
void recharge(obj, curse_bless)
    struct obj *obj;int curse_bless;
{
    register int n;
    ...
    {
        not_chargable: You("have a feeling of loss.");
    }
}

/* Forget known information about this object type. */
STATIC_OVL void forget_single_object(obj_id)
    int obj_id;
{
...
STATIC_OVL void randomize(indices, count)
 ...
 */
STATIC_OVL void forget(howmuch)
    int howmuch;
{
...
STATIC_OVL int maybe_tame(mtmp, sobj)
    struct monst *mtmp;struct obj *sobj;
{
...
boolean is_valid_stinking_cloud_pos(x, y, showmsg)
    int x, y;boolean showmsg;
{
...
void display_stinking_cloud_positions(state)
    int state;
{
    if (state == 0)
    {
    ...
    struct obj *sobj; /* scroll, or fake spellbook object for scroll-like spell */
{
    ...
    if (objects[otyp].oc_magic) exercise(A_WIS, TRUE); /* just for trying */
    ...
    case SCR_ENCHANT_ARMOR:
    {
        ...
        s = scursed ? -1 : otmp->spe >= 9 ? (rn2(otmp->spe) == 0) :
        ...
        pline("%s %s%s%s%s for a %s.", Yname2(otmp), s == 0 ? "violently " : "",
                otense(otmp, Blind ? "vibrate" : "glow"),
                (!Blind && !same_color) ? " " : "",
                (Blind || same_color) ?
                        "" : hcolor(scursed ? NH_BLACK : NH_SILVER),
...
void drop_boulder_on_player(confused, helmet_protects, byu, skip_uswallow)
...
boolean drop_boulder_on_monster(x, y, confused, byu)
            ...
            if (is_metallic(helmet))
            {
                if (canspotmon(mtmp))
                    pline("Fortunately, %s is wearing a hard helmet.",
                            mon_nam(mtmp));
                else if (!Deaf) You_hear("a clanging sound.");
                if (mdmg > 2) mdmg = 2;
            }
            else
            {
...
void wand_explode(obj, chg)
    struct obj *obj;int chg; /* recharging */
{
 ...
 */
STATIC_PTR void set_lit(x, y, val)
    int x, y;genericptr_t val;
{
    struct monst *mtmp;
...
void litroom(on, obj)
    register boolean on;struct obj *obj;
{
    ...
    { /* on */
        ...
        else if (!Blind) pline("A lit field surrounds you!");
    }

    /* No-op when swallowed or in water */
    if (u.uswallow || Underwater || Is_waterlevel(&u.uz)) return;
    /*
...
STATIC_OVL void do_class_genocide()
{
    ...
    for (j = 0;; j++)
    {
        ...
        if (class == 0 && (i = name_to_mon(buf)) != NON_PM) class =
                mons[i].mlet;
...
/* 5 (4 | 1) = normal genocide from throne */
{
    ...
    else
    {
        ...
        if (!(mons[mndx].geno & G_UNIQ)
                && !(mvitals[mndx].mvflags & (G_GENOD | G_EXTINCT))) for (i =
                rn1(3, 4); i > 0; i--)
        {
            if (!makemon(ptr, u.ux, u.uy, NO_MINVENT)) break; /* couldn't make one */
            ++cnt;
 ...
 */
boolean cant_revive(mtype, revival, from_obj)
    int *mtype;boolean revival;struct obj *from_obj;
{
...
boolean create_particular()
{
    ...
    else
    {
        ...
        for (i = 0; i <= multi; i++)
        {
            ...
            else if (randmonst) whichpm = rndmonst();
