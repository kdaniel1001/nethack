...
STATIC_OVL void give_may_advance_msg(skill)
    int skill;
{
    You_feel("more confident in your %sskills.",
            skill == P_NONE ? "" : skill <= P_LAST_WEAPON ? "weapon " :
            skill <= P_LAST_SPELL ? "spell casting " : "fighting ");
}
...
STATIC_DCL char *
FDECL(skill_level_name, (int, char *));
 ...
 */
int hitval(otmp, mon)
    struct obj *otmp;struct monst *mon;
{
    int tmp = 0;
    ...
    if (Is_weapon && otmp->blessed
            && (is_demon(ptr) || is_undead(ptr) || is_vampshifter(mon))) tmp +=
            2;
...
FDECL(oselect, (struct monst *, int));
#define Oselect(x)                      \
    if ((otmp = oselect(mtmp, x)) != 0) \
        return otmp;

STATIC_OVL struct obj * oselect(mtmp, x)
    struct monst *mtmp;int x;
{
    struct obj *otmp;
    ...
    register struct monst *mtmp;
{
    ...
    for (i = 0; i < SIZE(rwep); i++)
    {
        ...
        if (prop < 0)
        {
            ...
            && mtmp->weapon_check == NO_WEAPON_WANTED) propellor = 0;
        }
    ...
    for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj)
    {
        if (otmp->oclass == WEAPON_CLASS && otmp->oartifact
                ...
                && ((strong && !wearing_shield)
                        || !objects[otmp->otyp].oc_bimanual)) return otmp;
    }
 ...
 */
void possibly_unwield(mon, polyspot)
    struct monst *mon;boolean polyspot;
{
...
void wet_a_towel(obj, amt, verbose)
    struct obj *obj;int amt; /* positive: new value; negative: increment by -amt; zero: no-op */
    boolean verbose;
{
    ...
    if (obj == uwep) unweapon = !is_wet_towel(obj);
}

/* decrease a towel's wetness */
void dry_a_towel(obj, amt, verbose)
    struct obj *obj;int amt; /* positive: new value; negative: decrement by -amt; zero: no-op */
    boolean verbose;
{
    int newspe = (amt <= 0) ? obj->spe + amt : amt;
...
STATIC_OVL char * skill_level_name(skill, buf)
    int skill;char *buf;
{
    const char *ptr;
...
STATIC_OVL int slots_required(skill)
    int skill;
{
...
STATIC_OVL boolean could_advance(skill)
    int skill;
{
    if (P_RESTRICTED(skill) || P_SKILL(skill) >= P_MAX_SKILL(skill)
    || u.skills_advanced >= P_SKILL_LIMIT) return FALSE;
...
STATIC_OVL boolean peaked_skill(skill)
    int skill;
{
...
STATIC_OVL void skill_advance(skill)
    int skill;
{
    u.weapon_slots -= slots_required(skill);
    P_SKILL(skill)++;
                    ...
                    i++)
            {
                ...
                if (wizard)
                {
                    if (!iflags.menu_tab_sep)
                        Sprintf (buf, " %s%-*s %-12s %5d(%4d)", prefix, longest,
 ...
 */
void unrestrict_weapon_skill(skill)
    int skill;
{
...
void use_skill(skill, degree)
    int skill;int degree;
{
    ...
    struct obj *obj;
{
    ...
    if (!obj) return P_BARE_HANDED_COMBAT; /* Not using a weapon */
    if (obj->oclass != WEAPON_CLASS && obj->oclass != TOOL_CLASS
            && obj->oclass != GEM_CLASS) return P_NONE; /* Not a weapon, weapon-tool, or ammo */
...
int uwep_skill_type()
{
    if (u.twoweap) return P_TWO_WEAPON_COMBAT;
