...
void copynchars(dst, src, n)
    char *dst;const char *src;int n;
{
...
char * strsubst(bp, orig, replacement)
    char *bp;const char *orig, *replacement;
{
...
static boolean pmatch_internal(patrn, strng, ci, sk)
    const char *patrn, *strng;boolean ci; /* True => case-insensitive, False => case-sensitive */
...
int strncmpi(s1, s2, n)
    /*{ aka strncasecmp }*/
    register const char *s1, *s2;register int n; /*(should probably be size_t, which is unsigned)*/
{
    register char t1, t2;
...
boolean fuzzymatch(s1, s2, ignore_chars, caseblind)
    const char *s1, *s2;const char *ignore_chars;boolean caseblind;
{
    register char c1, c2;
...
time_t time_from_yyyymmddhhmmss(buf)
    char *buf;
{
    int k;
