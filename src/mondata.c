...
void set_mon_data(mon, ptr, flag)
    struct monst *mon;struct permonst *ptr;int flag;
{
    mon->data = ptr;
...
struct attack * attacktype_fordmg(ptr, atyp, dtyp)
    struct permonst *ptr;int atyp, dtyp;
{
    struct attack *a;
...
boolean attacktype(ptr, atyp)
    struct permonst *ptr;int atyp;
{
    return attacktype_fordmg(ptr, atyp, AD_ANY) ? TRUE : FALSE;
}

/* returns True if monster doesn't attack, False if it does */
boolean noattacks(ptr)
    struct permonst *ptr;
{
    int i;
    ...
    struct monst *mon;
{
    ...
    || ptr == &mons[PM_DEATH] || is_vampshifter(mon)) return TRUE;
    ...
    struct monst *mon;
{
    ...
    || (uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))) slotmask |=
    W_WEP;
...
boolean can_blnd(magr, mdef, aatyp, obj)
    struct monst *magr; /* NULL == no specific aggressor */
    struct monst *mdef;uchar aatyp;struct obj *obj; /* aatyp == AT_WEAP, AT_SPIT */
{
    ...
    case AT_NONE:
        ...
        else if (obj && (obj->otyp == BLINDING_VENOM))
        {
            ...
            if (is_you && (ublindf || u.ucreamed)) return FALSE;
            check_visor = TRUE;
        }
    ...
    case AT_ENGL:
        if (is_you && (Blindfolded || Unaware || u.ucreamed)) return FALSE;
    ...
    if (check_visor)
    {
        ...
        for (; o; o = o->nobj)
            if ((o->owornmask & W_ARMH)
                    ...
                    && !strcmp(s, "visored helmet")) return FALSE;
    }
...
boolean passes_bars(mptr)
    struct permonst *mptr;
{
    return (boolean) (passes_walls(mptr) || amorphous(mptr) || unsolid(mptr)
            || is_whirly(mptr) || verysmall(mptr) || dmgtype(mptr, AD_CORR)
            || dmgtype(mptr, AD_RUST) || (slithy(mptr) && !bigmonst(mptr)));
}

/* returns True if monster can blow (whistle, etc) */
boolean can_blow(mtmp)
    ...
    struct permonst *ptr;
{
    ...
    if (ptr->mlet == S_RODENT && ptr != &mons[PM_ROCK_MOLE]
            && ptr != &mons[PM_WOODCHUCK]) return TRUE;
    return FALSE;
}
...
/* does monster-type deal out a particular type of damage from a particular
 type of attack? */
struct attack * dmgtype_fromattack(ptr, dtyp, atyp)
    struct permonst *ptr;int dtyp, atyp;
{
    struct attack *a;
...
boolean dmgtype(ptr, dtyp)
    struct permonst *ptr;int dtyp;
{
    ...
    struct permonst *pm1, *pm2;
{
    ...
    if (let1 == let2)
    {
        ...
        for (prv = m1, nxt = big_to_little(m1); nxt != prv; prv = nxt, nxt =
                big_to_little(nxt))
            if (nxt == m2) return TRUE;
        for (prv = m1, nxt = little_to_big(m1); nxt != prv; prv = nxt, nxt =
                little_to_big(nxt))
            if (nxt == m2) return TRUE;
    }
...
int name_to_monclass(in_str, mndx_p)
    const char *in_str;int *mndx_p;
{
 ...
 */
const struct permonst * raceptr(mtmp)
    struct monst *mtmp;
{
...
static const char *levitate[4] =
{ "float", "Float", "wobble", "Wobble" };
static const char *flys[4] =
{ "fly", "Fly", "flutter", "Flutter" };
static const char *flyl[4] =
{ "fly", "Fly", "stagger", "Stagger" };
static const char *slither[4] =
{ "slither", "Slither", "falter", "Falter" };
static const char *ooze[4] =
{ "ooze", "Ooze", "tremble", "Tremble" };
static const char *immobile[4] =
{ "wiggle", "Wiggle", "pulsate", "Pulsate" };
static const char *crawl[4] =
{ "crawl", "Crawl", "falter", "Falter" };

const char * locomotion(ptr, def)
    const struct permonst *ptr;const char *def;
{
    ...
    return (is_floater(ptr) ? levitate[capitalize] :
            ...
            nolimbs(ptr) ? crawl[capitalize] : def);
}

const char * stagger(ptr, def)
    const struct permonst *ptr;const char *def;
{
...
const char * on_fire(mptr, mattk)
    struct permonst *mptr;struct attack *mattk;
{
    const char *what;
    ...
    struct permonst *mdat;
{
    ...
    || mdat->mlet == S_LIGHT) return FALSE;
    return TRUE;
}
