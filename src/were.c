    ...
    register struct monst *mon;
{
    ...
    if (is_human(mon->data))
    {
        if (!Protection_from_shape_changers
                && !rn2(
                        night() ?
                                (flags.moonphase == FULL_MOON ? 3 : 30) :
...
int were_summon(ptr, yours, visible, genbuf)
    ...
    register struct permonst *ptr;register boolean yours;int *visible; /* number of visible helpers created */
    char *genbuf;
{
    ...
    *visible = 0;
    ...
    for (i = rnd(5); i > 0; i--)
    {
        ...
        if (yours && mtmp) (void) tamedog(mtmp, (struct obj *) 0);
    }
    return total;
}
