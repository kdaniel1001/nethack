    ...
    for (obj = mon->minvent; obj; obj = obj->nobj)
    {
        ...
        case DWARVISH_MATTOCK:
            ...
            if (pickaxe && pickaxe->otyp == PICK_AXE && pickaxe != wep
                    && (!pickaxe->oartifact || obj->oartifact)) return pickaxe; /* drop the one we earlier decided to keep */
            /*FALLTHRU*/
        case PICK_AXE:
        ...
        case SKELETON_KEY:
            ...
            if (key && key->otyp == LOCK_PICK
                    && (!key->oartifact || obj->oartifact)) return key; /* drop the one we earlier decided to keep */
            /*FALLTHRU*/
        case LOCK_PICK:
            /* keep lock-pick in preference to credit card */
            if (key && key->otyp == CREDIT_CARD
                    && (!key->oartifact || obj->oartifact)) return key;
            /*FALLTHRU*/
        case CREDIT_CARD:
...
int dog_eat(mtmp, obj, x, y, devour)
    register struct monst *mtmp;register struct obj *obj; /* if unpaid, then thrown or kicked by hero */
    ...
    boolean devour;
{
    ...
    if (obj->quan > 1L && obj->oclass == FOOD_CLASS) obj = splitobj(obj, 1L);
    ...
    else
    {
        ...
        boolean seeobj = cansee(mtmp->mx, mtmp->my), sawpet = cansee(
                x, y) && mon_visible(mtmp);
...
STATIC_OVL boolean dog_hunger(register struct monst *mtmp,
        register struct edog *edog)
{
    if (monstermoves > edog->hungrytime + 500)
    {
        ...
        {
            dog_died: if (mtmp->mleashed && mtmp != u.usteed)
 ...
 */
STATIC_OVL int dog_invent(register struct monst *mtmp,
        register struct edog *edog, int udist)
{
    ...
    if (droppables(mtmp))
    {
        if (!rn2(udist + 1) || !rn2(edog->apport)) if (rn2(10) < edog->apport)
        {
                    ...
                    && could_reach_item(mtmp, obj->ox, obj->oy))
            {
                if (rn2(20) < edog->apport + 3)
                {
                    if (rn2(udist) || !rn2(edog->apport))
                    {
                        ...
                        if (attacktype(mtmp->data,
                        AT_WEAP) && mtmp->weapon_check == NEED_WEAPON)
                        {
...
STATIC_OVL int dog_goal(register struct monst *mtmp, struct edog *edog,
...
#define SQSRCHRADIUS 5
        ...
        for (obj = fobj; obj; obj = obj->nobj)
        {
            ...
            if (nx >= min_x && nx <= max_x && ny >= min_y && ny <= max_y)
            {
                ...
                if (!could_reach_item(mtmp, nx, ny)
                        || !can_reach_location(mtmp, mtmp->mx, mtmp->my, nx,
                                ny)) continue;
                if (otyp < MANFOOD)
                {
    ...
    if (gtyp == UNDEF
            || (gtyp != DOGFOOD && gtyp != APPORT
                    && monstermoves < edog->hungrytime))
    {
        gx = u.ux;
        gy = u.uy;
        if (after && udist <= 4 && gx == u.ux && gy == u.uy) return -2;
        appr = (udist >= 9) ? 1 : (mtmp->mflee) ? -1 : 0;
        if (udist > 1)
        {
            if (!IS_ROOM(levl[u.ux][u.uy].typ) || !rn2(4) || whappr
                    || (dog_has_minvent && rn2(edog->apport))) appr = 1;
        }
...
#define GDIST(x, y) (dist2(x, y, gx, gy))
    ...
    if (tunnels(mtmp->data) && !Is_rogue_level(&u.uz)) /* same restriction as m_move() */
    ...
    for (i = 0; i < cnt; i++)
    {
        ...
        if ((info[i] & ALLOW_MDISP) && MON_AT(nx, ny) && better_with_displacing
    ...
    else if (mtmp->mleashed && distu(omx, omy) > 4)
    {
        ...
        cc.x = mtmp->mx;
        cc.y = mtmp->my;
        dognext: if (!m_in_out_region(mtmp, nix, niy)) return 1;
...
STATIC_OVL boolean could_reach_item(mon, nx, ny)
    struct monst *mon;xchar nx, ny;
{
 ...
 * calls deep.
 */
STATIC_OVL boolean can_reach_location(mon, mx, my, fx, fy)
    struct monst *mon;xchar mx, my, fx, fy;
{
    int i, j;
    int dist;
    ...
    for (i = mx - 1; i <= mx + 1; i++)
    {
        for (j = my - 1; j <= my + 1; j++)
        {
            ...
            if (IS_ROCK(levl[i][j].typ) && !passes_walls(mon->data)
                    && (!may_dig(i, j) || !tunnels(mon->data))) continue;
            if (IS_DOOR(levl[i][j].typ)
                    && (levl[i][j].doormask & (D_CLOSED | D_LOCKED))) continue;
...
STATIC_PTR void wantdoor(x, y, distance)
    int x, y;genericptr_t distance;
{
    int ndist;
...
STATIC_OVL void quickmimic(mtmp)
    struct monst *mtmp;
{
    ...
    {
        idx = rn2(SIZE(qm));
        if (qm[idx].mndx != 0 && monsndx(mtmp->data) == qm[idx].mndx) break;
        if (qm[idx].mlet != 0 && mtmp->data->mlet == qm[idx].mlet) break;
        if (qm[idx].mndx == 0 && qm[idx].mlet == 0) break;
    }
    while (--trycnt > 0);
