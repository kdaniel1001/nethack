    ...
    while (cnt > 0)
    {
        ...
        if (mtmp)
        {
            ...
            if (dtype == PM_ANGEL)
            {
                ...
                EMIN(mtmp)->renegade = (atyp != u.ualign.type)
                        ^ !mtmp->mpeaceful;
            }
        }
        cnt--;
...
void summon_minion(alignment, talk)
    aligntyp alignment;boolean talk;
{
...
void gain_guardian_angel()
{
    ...
    { /* fervent */
                ...
                TRUE)) != 0)
        {
            ...
            if ((otmp = which_armor(mtmp, W_ARMS))
                    == 0|| otmp->otyp != SHIELD_OF_REFLECTION)
            {
