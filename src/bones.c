...
STATIC_OVL boolean no_bones_level(lev)
    d_level *lev;
{
...
STATIC_OVL void resetobjs(ochain, restore)
    struct obj *ochain;boolean restore;
{
...
void drop_upon_death(mtmp, cont, x, y)
    struct monst *mtmp;struct obj *cont;int x, y;
{
    struct obj *otmp;
...
STATIC_OVL boolean fixuporacle(oracle)
    struct monst *oracle;
{
    coord cc;
    int ridx, o_ridx;
    ...
    if (!Is_oracle_level(&u.uz)) return FALSE;

    oracle->mpeaceful = 1;
    o_ridx = levl[oracle->mx][oracle->my].roomno - ROOMOFFSET;
    if (o_ridx >= 0 && rooms[o_ridx].rtype == DELPHI) return TRUE; /* no fixup needed */
...
boolean can_make_bones()
{
    ...
    if (ledger_no(&u.uz) <= 0 || ledger_no(&u.uz) > maxledgerno()) return FALSE;
    if (no_bones_level(&u.uz)) return FALSE; /* no bones for specific levels */
    if (u.uswallow)
    {
...
void savebones(how, when, corpse)
    int how;time_t when;struct obj *corpse;
{
    int fd, x, y;
    struct trap *ttmp;
    struct monst *mtmp;
    ...
    else if (u.ugrave_arise < LOW_PM)
    {
        ...
        if (corpse) (void) obj_attach_mid(corpse, mtmp->m_id);
    }
    else
    {
...
int getbones()
{
    ...
    {
        if (!wizard) pline("Discarding unuseable bones; no need to panic...");
        ok = FALSE;
    }
    else
    {
