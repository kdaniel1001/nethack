...
FDECL(other_mon_has_arti, (struct monst *, SHORT_P));
STATIC_DCL struct obj *
FDECL(on_ground, (SHORT_P));
...
void amulet()
{
            ...
            && !rn2(15))
    {
        for (ttmp = ftrap; ttmp; ttmp = ttmp->ntrap)
        {
            if (ttmp->ttyp == MAGIC_PORTAL)
            {
                ...
                else if (du <= 144) pline("%s warm.", Tobjnam(amu, "feel"));
    ...
    if (!context.no_of_wizards) return;
    ...
    register struct monst *mtmp;
{
    ...
    for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj)
        if (otmp->otyp == AMULET_OF_YENDOR
                || is_quest_artifact(otmp) || otmp->otyp == BELL_OF_OPENING
...
STATIC_OVL short which_arti(mask)
    register int mask;
{
    switch (mask)
    {
 ...
 */
STATIC_OVL boolean mon_has_arti(mtmp, otyp)
    register struct monst *mtmp;register short otyp;
{
    register struct obj *otmp;
...
STATIC_OVL struct monst * other_mon_has_arti(mtmp, otyp)
    register struct monst *mtmp;register short otyp;
{
    ...
    for (mtmp2 = fmon; mtmp2; mtmp2 = mtmp2->nmon)
        ...
        if (mtmp2 != mtmp) if (mon_has_arti(mtmp2, otyp)) return mtmp2;

    return (struct monst *) 0;
}

STATIC_OVL struct obj * on_ground(otyp)
    register short otyp;
{
    register struct obj *otmp;
...
STATIC_OVL boolean you_have(mask)
    register int mask;
{
    switch (mask)
    {
...
STATIC_OVL unsigned long target_on(mask, mtmp)
    register int mask;register struct monst *mtmp;
{
    register short otyp;
    register struct obj *otmp;
    register struct monst *mtmp2;
    ...
    if (!mon_has_arti(mtmp, otyp))
    {
        ...
        && (otyp != AMULET_OF_YENDOR || (!mtmp2->iswiz && !inhistemple(mtmp2))))
            return STRAT(STRAT_MONSTR, mtmp2->mx, mtmp2->my, mask);
    }
    return (unsigned long) STRAT_NONE;
}

STATIC_OVL unsigned long strategy(mtmp)
    register struct monst *mtmp;
{
    ...
    || (mtmp->ispriest && inhistemple(mtmp))) return (unsigned long) STRAT_NONE;
    ...
    register struct monst *mtmp;
{
    ...
    mtmp->mstrategy = (mtmp->mstrategy & (STRAT_WAITMASK | STRAT_APPEARMSG))
            | strat;
...
int pick_nasty()
{
    ...
    if (Is_rogue_level(&u.uz)
            && !('A' <= mons[res].mlet && mons[res].mlet <= 'Z')) res =
            nasties[rn2(SIZE(nasties))];
            ...
            for (j = 0; j < 20; j++)
            {
                ...
                if (mtmp)
                {
                    ...
                    if (mtmp->data->maligntyp == 0
                            || sgn(mtmp->data->maligntyp) == castalign) break;
                }
            }
    }
    ...
    else
    {
        ...
        while ((mtmp = *mmtmp) != 0)
        {
                    ...
                    && (elapsed = monstermoves - mtmp->mlstmv) > 0L)
            {
                ...
                elapsed /= 50L;
                if (mtmp->msleeping && rn2((int) elapsed + 1)) mtmp->msleeping =
                        0;
