 ...
 */
void explode(x, y, type, dam, olet, expltype)
    int x, y;int type; /* the same as in zap.c; passes -(wand typ) for some WAND_CLASS */
    int dam;char olet;int expltype;
{
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
        {
            ...
            if (!mtmp && i + x - 1 == u.ux && j + y - 1 == u.uy) mtmp =
                    u.usteed;
            if (mtmp)
            {
                if (mtmp->mhp < 1)
            ...
            if (mtmp && cansee(i + x - 1, j + y - 1) && !canspotmon(mtmp))
                map_invisible(i + x - 1, j + y - 1);
                    ...
                    && glyph_is_invisible(levl[i + x - 1][j + y - 1].glyph))
            {
    ...
    if (dam) for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
        {
            ...
            if (type >= 0 && !u.uswallow) (void) zap_over_floor((xchar) (i + x -1 ), (xchar) (j + y - 1),
    ...
    {
        pay_for_damage(
                adtyp == AD_FIRE ? "burn away" : adtyp == AD_COLD ? "shatter" :
...
long scatter(sx, sy, blastforce, scflags, obj)
    int sx, sy; /* location of objects to scatter */
    int blastforce; /* force behind the scattering */
    unsigned int scflags;struct obj *obj; /* only scatter this obj        */
{
...
void explode_oil(obj, x, y)
    struct obj *obj;int x, y;
{
