...
void formatkiller(buf, siz, how)
    char *buf;unsigned siz;int how;
{
...
STATIC_OVL xchar observable_depth(lev)
    d_level *lev;
{
#if 0
...
STATIC_OVL void readentry(rfile, tt)
    FILE *rfile;struct toptenentry *tt;
{
    ...
    FILE *rfile;struct toptenentry *tt;
{
    ...
    (void) fprintf(rfile, fmt0, tt->ver_major, tt->ver_minor, tt->patchlevel,
            tt->points, tt->deathdnum, tt->deathlev, tt->maxlvl, tt->hp,
            tt->maxhp, tt->deaths, tt->deathdate, tt->birthdate, tt->uid);
    if (tt->ver_major < 3 || (tt->ver_major == 3 && tt->ver_minor < 3))
...
STATIC_OVL void writexlentry(rfile, tt)
    FILE *rfile;struct toptenentry *tt;
{
...
#define XLOG_SEP '\t' /* xlogfile field separator. */
    ...
    XLOG_SEP, tt->deaths, XLOG_SEP, tt->deathdate, XLOG_SEP, tt->birthdate,
    XLOG_SEP, tt->uid);
    Fprintf (rfile, "%s", buf);
    Sprintf (buf, "%crole=%s%crace=%s%cgender=%s%calign=%s", XLOG_SEP,
            tt->plrole,
            XLOG_SEP, tt->plrace, XLOG_SEP, tt->plgend, XLOG_SEP, tt->plalign);
    Fprintf (rfile, "%s%cname=%s%cdeath=%s", buf, /* (already includes separator) */
...
#define HUP
    ...
    if (wizard || discover)
    {
        ...
        HUP
        {
            char pbuf[BUFSZ];
            topten_print("");
            Sprintf (pbuf,"Since you were in %s mode, the score list will not be checked." ,
    ...
    for (rank = 1;;)
    {
        ...
        if ((sysopt.pers_is_uid ?
                t1->uid == t0->uid : strncmp(t1->name, t0->name, NAMSZ) == 0)
    ...
    if (rank0 >= rank) if (!done_stopprint) outentry(0, t0, TRUE);
#ifdef UPDATE_RECORD_IN_PLACE
    if (flg)
    {
#ifdef TRUNCATE_FILE
    ...
    showwin: if (iflags.toptenwin && !done_stopprint)
    display_nhwindow(toptenwin, 1);
    destroywin: if (!t0_used)
    dealloc_ttentry(t0);
    if (iflags.toptenwin)
    {
...
STATIC_OVL void outentry(rank, t1, so)
    struct toptenentry *t1;int rank;boolean so;
{
...
STATIC_OVL int score_wanted(current_ver, rank, t1, playerct, players, uid)
    boolean current_ver;int rank;struct toptenentry *t1;int playerct;const char **players;int uid;
{
    int i;
    ...
    for (i = 0; i < playerct; i++)
    {
                ...
                && players[i][2] == 0 && i + 1 < playerct)
        {
            ...
            if ((players[i][1] == 'p' && str2role(arg) == str2role(t1->plrole))
                    || (players[i][1] == 'r'
                            && str2race(arg) == str2race(t1->plrace))) return 1;
            i++;
        }
                ...
                || (players[i][0] == '-' && players[i][1] == t1->plrole[0]
                        && players[i][2] == 0)
                || (digit(players[i][0]) && rank <= atoi(players[i]))) return 1;
    }
    return 0;
}
