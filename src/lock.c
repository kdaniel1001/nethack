...
STATIC_PTR int picklock(VOID_ARGS)
{
    if (xlock.box)
    {
...
void breakchestlock(box, destroyit)
    struct obj *box;boolean destroyit;
{
    if (!destroyit)
    ...
    { /* #force has destroyed this box (at <u.ux,u.uy>) */
        struct obj *otmp;
        struct monst *shkp =
                (*u.ushops && costly_spot(u.ux, u.uy)) ?
                        shop_keeper(*u.ushops) : 0;
        boolean costly = (boolean) (shkp != 0), peaceful_shk = costly
                && (boolean) shkp->mpeaceful;
        long loss = 0L;
        ...
        if (costly) loss += stolen_value(box, u.ux, u.uy, peaceful_shk, TRUE);
        if (loss)
...
STATIC_PTR int forcelock(VOID_ARGS)
{
    if ((xlock.box->ox != u.ux) || (xlock.box->oy != u.uy))
        return ((xlock.usedtime = 0)); /* you or it moved */
...
int doforce()
{
    ...
    for (otmp = level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere)
        if (Is_box(otmp))
        {
            ...
            (void) safe_qbuf(qbuf, "There is ", " here; force its lock?", otmp,
...
boolean stumble_on_door_mimic(x, y)
    int x, y;
{
    struct monst *mtmp;

    if ((mtmp = m_at(x, y))
            && is_door_mappear(mtmp) && !Protection_from_shape_changers)
    {
        stumble_onto_mimic(mtmp);
        return TRUE;
    }
...
STATIC_OVL boolean obstructed(x, y, quietly)
    register int x, y;boolean quietly;
{
...
int doclose()
{
    ...
    if (portcullis || !IS_DOOR(door->typ))
    {
        ...
        {
            nodoor: You("%s no door there.", Blind ? "feel" : "see");
        }
        return res;
    }
...
/* Door/secret door was hit with spell or wand effect otmp;
 returns true if something happened */
boolean doorlock(otmp, x, y)
    struct obj *otmp;int x, y;
{
...
STATIC_OVL void chest_shatter_msg(otmp)
    struct obj *otmp;
{
