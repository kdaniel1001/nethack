 ...
 */
void magic_map_background(x, y, show)
    xchar x, y;int show;
{
    ...
    if (!cansee(x, y) && !lev->waslit)
    {
        ...
        if (lev->typ == ROOM && glyph == cmap_to_glyph(S_room))
            glyph =
                    cmap_to_glyph(
                            (flags.dark_room && iflags.use_color) ? (DARKROOMSYM) : S_stone);
        else if (lev->typ == CORR && glyph == cmap_to_glyph(S_litcorr)) glyph =
 ...
 */
void map_background(x, y, show)
    register xchar x, y;register int show;
{
 ...
 */
void map_trap(trap, show)
    register struct trap *trap;register int show;
{
 ...
 */
void map_object(obj, show)
    register struct obj *obj;register int show;
{
    ...
    register int x, y;
{
    ...
    else if (levl[x][y].seenv)
    {
        ...
        if (!lev->waslit && lev->glyph == cmap_to_glyph(S_room)
                && lev->typ == ROOM) lev->glyph = cmap_to_glyph(S_stone);
    }
    else
 ...
 *
 */
STATIC_OVL void display_monster(x, y, mon, sightflags, worm_tail)
 ...
 */
STATIC_OVL void display_warning(mon)
    ...
    xchar x, y;
{
    ...
    if (Underwater && !Is_waterlevel(&u.uz) && !is_pool(x, y)) return;
    ...
    int x, y;
{
    ...
    case DISP_ALWAYS:
        if (!tglyph)
            ...
            /* nested effect; we need dynamic memory */
    ...
    int x, y, glyph;
{
    ...
    if (!isok(x, y))
    {
        ...
        if (glyph >= GLYPH_WARNING_OFF && glyph < GLYPH_STATUE_OFF)
        { /* a warning */
            text = "warning";
 ...
 */
STATIC_OVL int swallow_to_glyph(mnum, loc)
    int mnum;int loc;
{
 ...
 *      /  S_rslant     (-1, 1) or ( 1,-1)
 */
int zapdir_to_glyph(dx, dy, beam_type)
    register int dx, dy;int beam_type;
{
...
STATIC_OVL int get_bk_glyph(x, y)
    xchar x, y;
{
    ...
    if (iflags.use_background_glyph)
    {
        ...
        if (idx != S_room) bkglyph = cmap_to_glyph(idx);
    }
    return bkglyph;
}
 ...
 */
STATIC_OVL int check_pos(x, y, which)
    int x, y, which;
{
    int type;
...
STATIC_OVL int set_twall(x0, y0, x1, y1, x2, y2, x3, y3)
...
STATIC_OVL int set_wall(x, y, horiz)
    int x, y, horiz;
{
    int wmode, is_1, is_2;
...
STATIC_OVL int set_corn(x1, y1, x2, y2, x3, y3, x4, y4)
...
STATIC_OVL int set_crosswall(x, y)
    int x, y;
{
    int wmode, is_1, is_2, is_3, is_4;
    ...
    for (x = 0; x < COLNO; x++)
        for (lev = &levl[x][0], y = 0; y < ROWNO; y++, lev++)
        {
            ...
            case TRWALL:
                ...
                break;
            case TLCORNER:
                wmode = set_corn(x - 1, y - 1, x, y - 1, x - 1, y, x + 1,
                        y + 1);
                break;
            case TRCORNER:
                wmode = set_corn(x, y - 1, x + 1, y - 1, x + 1, y, x - 1,
                        y + 1);
                break;
            case BLCORNER:
                wmode = set_corn(x, y + 1, x - 1, y + 1, x - 1, y, x + 1,
                ...
                break;
            case BRCORNER:
                wmode = set_corn(x + 1, y, x + 1, y + 1, x, y + 1, x - 1,
                        y - 1);
                break;
            case CROSSWALL:
...
STATIC_OVL void set_seenv(lev, x0, y0, x, y)
    struct rm *lev;int x0, y0, x, y; /* from, to */
{
    int dx = x - x0, dy = y0 - y;
...
void unset_seenv(lev, x0, y0, x1, y1)
...
#define only(sv, bits) (((sv) & (bits)) && !((sv) & ~(bits)))
    ...
    case TDWALL:
        row = wall_matrix[T_d];
        do_twall: switch (lev->wall_info & WM_MASK)
        {
        case 0:
            if (seenv == SV4)
            {
    ...
    case HWALL:
        horiz: switch (lev->wall_info & WM_MASK)
        {
        case 0:
