...
STATIC_DCL char *
FDECL(oclass_to_name, (CHAR_P, char *));
...
STATIC_OVL void setgemprobs(dlev)
    d_level *dlev;
{
    int j, first, lev;
...
STATIC_OVL void shuffle(o_low, o_high, domaterial)
    int o_low, o_high;boolean domaterial;
{
...
void obj_shuffle_range(otyp, lo_p, hi_p)
    ...
    int *lo_p, *hi_p; /* output: range that item belongs among */
{
    ...
    *lo_p = *hi_p = otyp;
    ...
    case ARMOR_CLASS:
        ...
        else if (otyp >= SPEED_BOOTS && otyp <= LEVITATION_BOOTS) *lo_p =
        SPEED_BOOTS, *hi_p = LEVITATION_BOOTS;
        break;
    case POTION_CLASS:
...
void discover_object(oindx, mark_as_known, credit_hero)
    ...
    register int oindx;
{
    if (!objects[oindx].oc_name_known)
    {
        ...
        for (dindx = bases[acls];
            ...
            else if (disco[dindx] == oindx) found = TRUE;
...
STATIC_OVL boolean interesting_to_discover(i)
    register int i;
{
...
STATIC_OVL char * oclass_to_name(oclass, buf)
    char oclass;char *buf;
{
    char *s;
...
int doclassdisco()
{
    ...
    char *s, c, oclass, menulet, allclasses[MAXOCLASSES], discosyms[2
            + MAXOCLASSES + 1], buf[BUFSZ];
    int i, ct, dis, xtras;
    boolean traditional;
    winid tmpwin;
    anything any;
...
void rename_disco()
{
    ...
    for (s = flags.inv_order; *s; s++)
    {
                ...
                i < NUM_OBJECTS && objects[i].oc_class == oclass; i++)
        {
            ...
            any.a_int = dis;
            add_menu(tmpwin, NO_GLYPH, &any, 0, 0, ATR_NONE, obj_typename(dis),
            MENU_UNSELECTED);
        }
    }
    if (ct == 0)
