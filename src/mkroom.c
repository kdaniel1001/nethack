...
STATIC_OVL boolean isbig(sroom)
...
STATIC_OVL void mkshop()
{
    ...
    if (wizard)
    {
        ...
        if (ep)
        {
            ...
            for (i = 0; shtypes[i].name; i++)
                if (*ep == def_oc_syms[(int) shtypes[i].symb].sym) goto gottype;
            if (*ep == 'g' || *ep == 'G')
                i = 0;
            else if (*ep == 'v' || *ep == 'V')
    ...
    for (sroom = &rooms[0];; sroom++)
    {
        ...
        if ((wizard && ep && sroom->doorct != 0) || sroom->doorct == 1) break;
    }
    if (!sroom->rlit)
    {
        int x, y;
...
STATIC_OVL struct mkroom * pick_room(strict)
    register boolean strict;
{
    ...
    for (sroom = &rooms[rn2(nroom)]; i--; sroom++)
    {
        ...
        if (sroom->doorct == 1 || !rn2(5) || wizard) return sroom;
    }
    return (struct mkroom *) 0;
}

STATIC_OVL void mkzoo(type)
    int type;
{
    register struct mkroom *sroom;
        ...
        for (sy = sroom->ly; sy <= sroom->hy; sy++)
        {
            if (sroom->irregular)
            {
                if ((int) levl[sx][sy].roomno != rmno || levl[sx][sy].edge
                        || (sroom->doorct
                                && distmin(sx, sy, doors[sh].x, doors[sh].y)
                                        <= 1)) continue;
            }
            ...
            mon =
                    makemon(
                            ...
                            (type == ANTHOLE) ?
                                    antholemon() : (struct permonst *) 0, sx,
                            sy, NO_MM_FLAGS);
            if (mon)
            {
                mon->msleeping = 1;
            ...
            case LEPREHALL:
                ...
                break;
            case MORGUE:
                if (!rn2(5)) (void) mk_tt_object(CORPSE, sx, sy);
                ...
                break;
            case BEEHIVE:
                if (!rn2(3)) (void) mksobj_at(LUMP_OF_ROYAL_JELLY, sx, sy, TRUE,
                FALSE);
                break;
            case BARRACKS:
            ...
            case COCKNEST:
                ...
                break;
            case ANTHOLE:
                if (!rn2(3)) (void) mkobj_at(FOOD_CLASS, sx, sy, FALSE);
                break;
            }
        }
    switch (type)
...
void mkundead(mm, revive_corpses, mm_flags)
    coord *mm;boolean revive_corpses;int mm_flags;
{
...
morguemon()
{
    ...
    if (hd > 10 && i < 10)
    {
        ...
        else
        {
            ...
            if (ndemon_res != NON_PM) return &mons[ndemon_res];
...
STATIC_OVL coord * shrine_pos(roomno)
    int roomno;
{
    static coord buf;
    int delta;
...
boolean has_dnstairs(sroom)
    register struct mkroom *sroom;
{
    if (sroom == dnstairs_room) return TRUE;
    if (sstairs.sx && !sstairs.up) return (boolean) (sroom == sstairs_room);
    return FALSE;
}

boolean has_upstairs(sroom)
    register struct mkroom *sroom;
{
...
boolean inside_room(croom, x, y)
    struct mkroom *croom;xchar x, y;
{
    return (boolean) (x >= croom->lx - 1 && x <= croom->hx + 1
            && y >= croom->ly - 1 && y <= croom->hy + 1);
}

boolean somexy(croom, c)
    struct mkroom *croom;coord *c;
{
    int try_cnt = 0;
    int i;
        ...
        for (c->x = croom->lx; c->x <= croom->hx; c->x++)
            for (c->y = croom->ly; c->y <= croom->hy; c->y++)
                if (!levl[c->x][c->y].edge
                        && (int) levl[c->x][c->y].roomno == i) return TRUE;
        return FALSE;
    }
    ...
    while (try_cnt++ < 100)
    {
        ...
        for (i = 0; i < croom->nsubrooms; i++)
            if (inside_room(croom->sbrooms[i], c->x, c->y)) goto you_lose;
        break;
        you_lose: ;
    }
 ...
 *		- ANY_TYPE
 */
struct mkroom * search_special(type)
    schar type;
{
    register struct mkroom *croom;
...
squadmon()
{
    ...
    gotone: if (!(mvitals[mndx].mvflags & G_GONE))
        return &mons[mndx];
    else
 ...
 * (if any).
 */
STATIC_OVL void save_room(fd, r)
    int fd;struct mkroom *r;
{
    short i;
...
STATIC_OVL void rest_room(fd, r)
    int fd;struct mkroom *r;
{
    short i;
