 ...
 */
STATIC_OVL void insert_rtoptions(win, buf, finalphrase)
    winid win;char *buf;const char *finalphrase;
{
    char rtbuf[BUFSZ];
    int l, i;
    const char *s1 = 0, *s2 = 0, *s3 = 0, *s4 = 0;

    if ((int) strlen(buf) >= (BUFSZ - 1)) return;
...
boolean check_version(version_data, filename, complain)
    struct version_info *version_data;const char *filename;boolean complain;
{
    if (
...
/*ARGUSED*/
const char * copyright_banner_line(indx)
    int indx;
{
