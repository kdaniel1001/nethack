...
STATIC_DCL char *
FDECL(trapnote, (struct trap *, BOOLEAN_P));
#if 0
...
STATIC_VAR const char * const a_your[2] =
{ "a", "your" };
STATIC_VAR const char * const A_Your[2] =
{ "A", "Your" };
    ...
    struct monst *victim;
{
    ...
    while (item)
    {
        if (is_wet_towel(item))
        {
            ...
            if (item->spe != oldspe) break; /* stop once one towel has been affected */
        }
        item = item->nobj;
    }
 ...
 */
int erode_obj(otmp, ostr, type, ef_flags)
    register struct obj *otmp;const char *ostr;int type;int ef_flags;
{
    ...
    else if (erosion < MAX_ERODE)
    {
        ...
        if (ef_flags & EF_PAY) costly_alteration(otmp, cost_type);
 ...
 * wears off.
 */
boolean grease_protect(otmp, ostr, victim)
    register struct obj *otmp;const char *ostr;struct monst *victim;
{
    ...
    register int x, y, typ;
{
    ...
    case SQKY_BOARD:
    {
        ...
        for (t = ftrap; t; t = t->ntrap)
            if (t->ttyp == SQKY_BOARD && t != ttmp) tavail[t->tnote] = 1;
    ...
    boolean td; /* td == TRUE : trap door or hole */
{
    ...
    if (In_quest(&u.uz))
    {
        ...
        if (dunlev_reached(&u.uz) < qlocate_depth) bottom = qlocate_depth; /* early cut-off */
    }
...
struct monst * animate_statue(statue, x, y, cause, fail_reason)
    struct obj *statue;xchar x, y;int cause;int *fail_reason;
{
    ...
    boolean historic = (Role_if(PM_ARCHEOLOGIST)
            && (statue->spe & STATUE_HISTORIC) != 0), golem_xform = FALSE,
            use_saved_traits;
    ...
    if (!mon)
    {
        if (fail_reason) *fail_reason =
        unique_corpstat(&mons[statue->corpsenm]) ?
        AS_MON_IS_UNIQUE :
                                                   AS_NO_MON;
    ...
    if (statue->spe & STATUE_MALE)
        mon->female = FALSE;
    else if (statue->spe & STATUE_FEMALE) mon->female = TRUE;
    /* if statue has been named, give same name to the monster */
    if (has_oname(statue)) mon = christen_monst(mon, ONAME(statue));
 ...
 * or pick-axe.
 */
struct monst * activate_statue_trap(trap, x, y, shatter)
    struct trap *trap;xchar x, y;boolean shatter;
{
...
STATIC_OVL boolean keep_saddle_with_steedcorpse(steed_mid, objchn, saddle)
    unsigned steed_mid;struct obj *objchn, *saddle;
{
...
void dotrap(trap, trflags)
    register struct trap *trap;unsigned trflags;
{
    register int ttype = trap->ttyp;
    register struct obj *otmp;
    boolean already_seen = trap->tseen, forcetrap = (trflags & FORCETRAP) != 0,
            webmsgok = (trflags & NOWEBMSG) == 0, forcebungle = (trflags
                    & FORCEBUNGLE) != 0, plunged = (trflags & TOOKPLUNGE) != 0,
    ...
    if (u.usteed)
    {
        ...
        if (has_mname(u.usteed) && !Hallucination) steed_article = ARTICLE_NONE;
    }
    ...
    case DART_TRAP:
        ...
        {
            if (otmp->opoisoned) poisoned("dart", A_CON, "little dart",
    ...
    case BEAR_TRAP:
    {
        ...
        if (u.usteed)
        {
            ...
            if (thitm(0, u.usteed, (struct obj *) 0, dmg, FALSE)) u.utrap = 0; /* steed died, hero not trapped */
        }
        else
        {
    ...
    case RUST_TRAP:
        ...
        default:
            ...
            else if (uarmu) (void) water_damage(uarmu, "shirt", TRUE);
        }
        update_inventory();
        ...
        if (ttype == SPIKED_PIT)
        {
            ...
            if (u.usteed)
            {
                pline("%s %s %s!",
                        ...
                        SUPPRESS_SADDLE,
                        FALSE)), adj_pit ? "steps" : "lands", predicament);
            }
            else
    ...
    case ANTI_MAGIC:
        ...
        else
        {
            ...
            if (Half_physical_damage || Half_spell_damage) dmgval2 += rnd(4);
            /* give Magicbane wielder dose of own medicine */
            if (uwep && uwep->oartifact == ART_MAGICBANE) dmgval2 += rnd(4);
            ...
            You_feel(
                    (dmgval2 >= hp) ? "unbearably torpid!" :
                    (dmgval2 >= hp / 4) ? "very lethargic." : "sluggish.");
    ...
    case LANDMINE:
    {
        ...
        if ((Levitation || Flying) && !forcetrap)
        {
            ...
            pline("KAABLAMM!!!  %s %s%s off!",
                    forcebungle ?
                            "Your inept attempt sets" : "The air currents set",
...
STATIC_OVL char * trapnote(trap, noprefix)
    struct trap *trap;boolean noprefix;
{
    static char tnbuf[12];
    const char *tn, *tnnames[12] =
    { "C note", "D flat", "D note", "E flat", "E note", "F note", "F sharp",
            "G note", "G sharp", "A note", "B flat", "B note" };
...
STATIC_OVL int steedintrap(trap, otmp)
    struct trap *trap;struct obj *otmp;
{
    ...
    case SPIKED_PIT:
        trapkilled = (steed->mhp <= 0
                || thitm(0, steed, (struct obj *) 0, rnd((tt == PIT) ? 6 : 10),
                FALSE));
        steedhit = TRUE;
        break;
    case POLY_TRAP:
...
static void launch_drop_spot(obj, x, y)
    struct obj *obj;xchar x, y;
{
    if (!obj)
    {
 ...
 */
int launch_obj(otyp, x1, y1, x2, y2, style)
    short otyp;register int x1, y1, x2, y2;int style;
{
    ...
    case ROLL | LAUNCH_UNSEEN:
        ...
        {
            You_hear(
            Hallucination ? "someone bowling." : "rumbling in the distance.");
        }
        style &= ~LAUNCH_UNSEEN;
        goto roll;
    ...
    while (dist-- > 0 && !used_up)
    {
        ...
        if (cansee(bhitpos.x, bhitpos.y)) while (tmp-- > 0)
            delay_output();
        ...
        else if (bhitpos.x == u.ux && bhitpos.y == u.uy)
        {
            ...
            if (thitu(9 + singleobj->spe, dmgval(singleobj, &youmonst),
                    singleobj, (char *) 0)) stop_occupation();
        }
        if (style == ROLL)
        {
                    ...
                    && (otmp2 = sobj_at(BOULDER, bhitpos.x, bhitpos.y)) != 0)
            {
                ...
                || IS_ROCK(levl[bhitpos.x + dx][bhitpos.y + dy].typ)) bmsg =
...
STATIC_OVL int mkroll_launch(ttmp, x, y, otyp, ocount)
    struct trap *ttmp;xchar x, y;short otyp;long ocount;
{
    struct obj *otmp;
    register int tmp;
    schar dx, dy;
...
STATIC_OVL boolean isclearpath(cc, distance, dx, dy)
    coord *cc;int distance;schar dx, dy;
{
    uchar typ;
    xchar x, y;
    ...
    register struct monst *mtmp;
{
    ...
    else
    {
        ...
        boolean in_sight, tear_web, see_it, inescapable = force_mintrap
        ...
        case FIRE_TRAP:
            ...
            if (burn_floor_objects(mtmp->mx, mtmp->my, see_it, FALSE) && !see_it
                    && distu(mtmp->mx, mtmp->my) <= 3 * 3) You("smell smoke.");
        ...
        case ANTI_MAGIC:
            ...
            { /* take some damage */
                ...
                if ((otmp = MON_WEP(mtmp))
                        != 0&& otmp->oartifact == ART_MAGICBANE)
                    dmgval2 += rnd(4);
                for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj)
                    if (otmp->oartifact && defends_when_carried(AD_MAGM, otmp))
                        break;
                if (otmp) dmgval2 += rnd(4);
...
void minstapetrify(mon, byplayer)
    struct monst *mon;boolean byplayer;
{
...
void mselftouch(mon, arg, byplayer)
    struct monst *mon;const char *arg;boolean byplayer;
{
    ...
    else if (u.uswallow)
    {
        You(
                is_animal(u.ustuck->data) ?
                        "float away from the %s." : "spiral up into %s.",
                is_animal(u.ustuck->data) ?
                        surface(u.ux, u.uy) : mon_nam(u.ustuck));
    }
    else if (Hallucination)
    {
    ...
    long hmask, emask; /* might cancel timeout */
{
    ...
    if (trap)
    {
        ...
        case HOLE:
        case TRAPDOOR:
            if (!Can_fall_thru(&u.uz) || u.ustuck) break;
            /*FALLTHRU*/
        default:
    ...
    && on_level(&u.uz, &current_dungeon_level)) (void) pickup(1);
    return 1;
}
...
STATIC_OVL void dofiretrap(box)
    struct obj *box; /* null for floor trap */
{
    ...
    if (is_ice(u.ux, u.uy)) melt_ice(u.ux, u.uy, (char *) 0);
}
 ...
 */
boolean fire_damage(obj, force, x, y)
    struct obj *obj;boolean force;xchar x, y;
{
    int chance;
    ...
    if (Is_container(obj))
    {
        ...
        if ((!force && (Luck + 5) > rn2(chance))
                || (is_flammable(obj) && obj->oerodeproof)) return FALSE;
        ...
        if (Has_contents(obj))
        {
            ...
            for (otmp = obj->cobj; otmp; otmp = ncobj)
            {
                ncobj = otmp->nobj;
                obj_extract_self(otmp);
                if (!flooreffects(otmp, x, y, "")) place_object(otmp, x, y);
            }
        }
        setnotworn(obj);
        delobj(obj);
 ...
 */
int fire_damage_chain(chain, force, here, x, y)
    struct obj *chain;boolean force, here;xchar x, y;
{
    struct obj *obj, *nobj;
    int num = 0;
    for (obj = chain; obj; obj = nobj)
    {
        ...
        if (fire_damage(obj, force, x, y)) ++num;
    }

    if (num && (Blind && !couldsee(x, y))) You("smell smoke.");
    return num;
}

void acid_damage(obj)
    struct obj *obj;
{
 ...
 * Returns an erosion return value (ER_*)
 */
int water_damage(obj, ostr, force)
    struct obj *obj;const char *ostr;boolean force;
{
    if (!obj) return ER_NOTHING;

    if (snuff_lit(obj)) return ER_DAMAGED;
    ...
    {
        if (carried(obj)) pline("Water gets into your %s!", ostr);
...
#endif
        if (carried(obj)) pline("Your %s %s.", ostr, vtense(ostr, "fade"));
    ...
    else if (obj->oclass == POTION_CLASS)
    {
        ...
        {
            char *bufp;
            boolean one = (obj->quan == 1L), update = carried(obj), exploded =
            FALSE;
            ...
            !exploded ? (one ? "A" : "Some") : (one ? "Another" : "More"), bufp,
                    vtense(bufp, "explode"));
            if (acid_ctx.ctx_valid)
            {
                if (obj->dknown)
 ...
 */
STATIC_OVL boolean emergency_disrobe(lostsome)
    boolean *lostsome;
{
...
STATIC_OVL int untrap_prob(ttmp)
    struct trap *ttmp;
{
    int chance = 3;
...
void cnv_trap_obj(otyp, cnt, ttmp, bury_it)
    int otyp;int cnt;struct trap *ttmp;boolean bury_it;
{
...
STATIC_OVL void move_into_trap(ttmp)
    struct trap *ttmp;
{
    int bc = 0;
 ...
 * 2: succeeds
 */
STATIC_OVL int try_disarm(ttmp, force_failure)
    struct trap *ttmp;boolean force_failure;
{
...
STATIC_OVL void reward_untrap(ttmp, mtmp)
    struct trap *ttmp;struct monst *mtmp;
{
    if (!ttmp->madeby_u)
    {
...
STATIC_OVL int disarm_holdingtrap(ttmp)
    /* Helge Hafting */
    struct trap *ttmp;
{
    struct monst *mtmp;
...
STATIC_OVL int disarm_landmine(ttmp)
    /* Helge Hafting */
    struct trap *ttmp;
{
    int fails = try_disarm(ttmp, FALSE);
...
static NEARDATA const char oil[] =
{ ALL_CLASSES, TOOL_CLASS, POTION_CLASS, 0 };

/* it may not make much sense to use grease on floor boards, but so what? */
STATIC_OVL int disarm_squeaky_board(ttmp)
    struct trap *ttmp;
{
    struct obj *obj;
    boolean bad_tool;
...
STATIC_OVL int disarm_shooting_trap(ttmp, otyp)
    struct trap *ttmp;int otyp;
{
    int fails = try_disarm(ttmp, FALSE);
...
STATIC_OVL int try_lift(mtmp, ttmp, wt, stuff)
    struct monst *mtmp;struct trap *ttmp;int wt;boolean stuff;
{
    int wc = weight_cap();
...
STATIC_OVL int help_monster_out(mtmp, ttmp)
    struct monst *mtmp;struct trap *ttmp;
{
    int wt;
    struct obj *otmp;
    boolean uprob;
    ...
    boolean force;
{
            ...
            for (otmp = level.objects[x][y]; otmp; otmp = otmp->nexthere)
                if (Is_box(otmp))
                {
                    (void) safe_qbuf(qbuf, "There is ",
                            " here.  Check it for traps?", otmp, doname,
                            ansimpleoname, "a box");
                    switch (ynq(qbuf))
                    {
                    case 'q':
    ...
    {
        if (!trap_skipped) You("know of no traps there.");
        return 0;
    }
...
boolean openholdingtrap(mon, noticed)
    struct monst *mon;boolean *noticed; /* set to true iff hero notices the effect; */
...
boolean closeholdingtrap(mon, noticed)
    struct monst *mon;boolean *noticed; /* set to true iff hero notices the effect; */
{ /* otherwise left with its previous value intact */
    struct trap *t;
...
boolean openfallingtrap(mon, trapdoor_only, noticed)
    struct monst *mon;boolean trapdoor_only;boolean *noticed; /* set to true iff hero notices the effect; */
{ /* otherwise left with its previous value intact */
    struct trap *t;
    boolean ishero = (mon == &youmonst), result;
    ...
    if (!t
            || ((t->ttyp != TRAPDOOR && t->ttyp != ROCKTRAP)
                    && (trapdoor_only
                            || (t->ttyp != HOLE && t->ttyp != PIT
                                    && t->ttyp != SPIKED_PIT)))) return FALSE;
...
boolean chest_trap(obj, bodypart, disarm)
    register struct obj *obj;register int bodypart;boolean disarm;
{
    ...
    register struct trap *trap;
{
    ...
    else
    {
        ...
        if (!ttmp) panic("deltrap: no preceding trap!");
...
boolean conjoined_pits(trap2, trap1, u_entering_trap2)
    struct trap *trap2, *trap1;boolean u_entering_trap2;
{
    ...
    for (diridx = 0; diridx < 8; diridx++)
        if (xdir[diridx] == dx && ydir[diridx] == dy) break;
    /* diridx is valid if < 8 */
    if (diridx < 8)
    {
        adjidx = (diridx + 4) % 8;
        if ((trap1->conjoined & (1 << diridx))
                && (trap2->conjoined & (1 << adjidx))) return TRUE;
    }
    return FALSE;
}

void clear_conjoined_pits(trap)
    struct trap *trap;
{
 ...
 */
boolean uteetering_at_seen_pit(trap)
    struct trap *trap;
{
...
void b_trapped(item, bodypart)
    const char *item;int bodypart;
{
    int lvl = level_difficulty(), dmg = rnd(5 + (lvl < 5 ? lvl : 2 + lvl / 2));
...
STATIC_OVL boolean thitm(tlev, mon, obj, d_override, nocorpse)
    int tlev;struct monst *mon;struct obj *obj;int d_override;boolean nocorpse;
{
    int strike;
    boolean trapkilled = FALSE;
...
boolean lava_effects()
{
    ...
    if (!Fire_resistance)
    {
        ...
        boil_away = (u.umonnum == PM_WATER_ELEMENTAL
                || u.umonnum == PM_STEAM_VORTEX || u.umonnum == PM_FOG_CLOUD);
        for (;;)
        {
            u.uhp = -1;
            /* killer format and name are reconstructed every iteration
    ...
    else if (!Wwalking && (!u.utrap || u.utraptype != TT_LAVA))
    {
        ...
        if (u.uhp > 1) losehp(!boil_away ? 1 : (u.uhp / 2), lava_killer,
        KILLED_BY); /* lava damage */
    }

    burn_stuff: destroy_item(SCROLL_CLASS, AD_FIRE);
    destroy_item(SPBOOK_CLASS, AD_FIRE);
    destroy_item(POTION_CLASS, AD_FIRE);
    return FALSE;
}
...
STATIC_OVL void maybe_finish_sokoban()
{
    struct trap *t;
