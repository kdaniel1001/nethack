...
STATIC_OVL boolean cant_wield_corpse(obj)
    struct obj *obj;
{
    char kbuf[BUFSZ];
...
STATIC_OVL int ready_weapon(wep)
    struct obj *wep;
{
...
int dowield()
{
    ...
    else if (wep == uwep)
    {
        ...
        if (is_weptool(wep) || is_wet_towel(wep)) unweapon = FALSE; /* [see setuwep()] */
        return 0;
    }
    else if (welded(uwep))
    {
...
boolean wield_tool(obj, verb)
    struct obj *obj;const char *verb; /* "rub",&c */
{
    const char *what;
    boolean more_than_1;
...
int chwepon(otmp, amount)
    register struct obj *otmp;register int amount;
{
    ...
    {
        if (!Blind) pline("%s %s.", Yobjnam2(uwep, "faintly glow"), color);
        return 1;
    }
