...
/*ARGSUSED*/
int mapglyph(glyph, ochar, ocolor, ospecial, x, y)
    int glyph, *ocolor, x, y;int *ochar;unsigned *ospecial;
{
     ...
     */
    if ((offset = (glyph - GLYPH_STATUE_OFF)) >= 0)
    { /* a statue */
        ...
        if (has_rogue_color)
            ...
            obj_color(STATUE);
        special |= MG_STATUE;
        if (level.objects[x][y] && level.objects[x][y]->nexthere) special |=
        MG_OBJPILE;
    }
    ...
    { /* object */
        ...
        if (offset == BOULDER) idx = SYM_BOULDER + SYM_OFF_X;
        if (has_rogue_color && iflags.use_color)
        {
        ...
        if (offset != BOULDER && level.objects[x][y]
                && level.objects[x][y]->nexthere) special |= MG_OBJPILE;
    }
    ...
    { /* a monster */
        ...
        else
        {
            ...
            && flags.showrace && !Upolyd) color = HI_DOMESTIC;
#endif
        }
    }
...
void genl_putmixed(window, attr, str)
