...
FDECL(muse_newcham_mon, (struct monst *));
 ...
 */
STATIC_OVL int precheck(mon, obj)
...
STATIC_OVL void mzapmsg(mtmp, otmp, self)
    struct monst *mtmp;struct obj *otmp;boolean self;
{
    if (!canseemon(mtmp))
    {
        if (!Deaf)
...
STATIC_OVL void mreadmsg(mtmp, otmp)
...
STATIC_OVL void mquaffmsg(mtmp, otmp)
    struct monst *mtmp;struct obj *otmp;
{
    if (canseemon(mtmp))
    {
    ...
    else if (!Deaf) You_hear("a chugging sound.");
}
    ...
    struct monst *mtmp;
{
    ...
    else if (levl[x][y].typ == STAIRS)
    {
        ...
        else if (x == xupstair && y == yupstair)
        {
            ...
            if (ledger_no(&u.uz) != 1) m.has_defense = MUSE_UPSTAIRS;
        }
        else if (sstairs.sx && x == sstairs.sx && y == sstairs.sy)
        {
            if (sstairs.up || !is_floater(mtmp->data)) m.has_defense =
            MUSE_SSTAIRS;
        }
    }
    ...
    else
    {
        ...
        boolean ignore_boulders = (verysmall(mtmp->data)
                || throws_rocks(mtmp->data) || passes_walls(mtmp->data)),
    ...
    if (t
            && (t->ttyp == PIT || t->ttyp == SPIKED_PIT || t->ttyp == WEB
                    || t->ttyp == BEAR_TRAP)) t = 0; /* ok for monster to dig here */
...
#define nomore(x)       if (m.has_defense == x) continue;
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj)
    {
        ...
        if (obj->otyp == SCR_TELEPORTATION && mtmp->mcansee
                && haseyes(mtmp->data)
                && (!obj->cursed
                        || (!(mtmp->isshk && inhishop(mtmp)) && !mtmp->isgd
                                && !mtmp->ispriest)))
        {
...
#define m_flee(m)                          \
    ...
    case MUSE_BUGLE:
        ...
        else if (!Deaf) You_hear("a bugle playing reveille!");
    ...
    case MUSE_WAN_TELEPORTATION_SELF:
        ...
        mon_tele: if (tele_restrict(mtmp))
        { /* mysterious force... */
    ...
    case MUSE_WAN_CREATE_MONSTER:
    {
        ...
        struct permonst *pm =
                !is_pool(mtmp->mx, mtmp->my) ?
                        0 : &mons[u.uinwater ? PM_GIANT_EEL : PM_CROCODILE];
        struct monst *mon;
    ...
    case MUSE_SCR_CREATE_MONSTER:
    {
        ...
        if (known)
            makeknown(SCR_CREATE_MONSTER);
                ...
                && !objects[SCR_CREATE_MONSTER].oc_uname) docall(otmp);
        m_useup(mtmp, otmp);
        return 2;
    }
    case MUSE_TRAPDOOR:
    ...
    struct monst *mtmp;
{
    ...
    try_again: switch (rn2(
            8 + (difficulty > 3) + (difficulty > 6) + (difficulty > 8)))
    {
    case 6:
    case 9:
...
int mbhitm(mtmp, otmp)
    register struct monst *mtmp;register struct obj *otmp;
{
    int tmp;
 ...
 */
STATIC_OVL void mbhit(mon, range, fhitm, fhito, obj)
    ...
    struct monst *mtmp;
{
    ...
    case 0:
    {
        ...
        if ((helmet && is_metallic(helmet))
                || amorphous(
                        pm) || passes_walls(pm) || noncorporeal(pm) || unsolid(pm))
            return SCR_EARTH;
    } /* fall through */
    case 1:
        return WAN_STRIKING;
    ...
    struct monst *mtmp;
{
    ...
    if (!stuck && !immobile && (mtmp->cham == NON_PM) && monstr[(pmidx =
            monsndx(mdat))] < 6)
    {
        boolean ignore_boulders = (verysmall(mdat) || throws_rocks(mdat)
                || passes_walls(mdat)), diag_ok = !NODIAG(pmidx);
...
#define nomore(x)       if (m.has_misc == x) continue
    ...
    for (obj = mtmp->minvent; obj; obj = obj->nobj)
    {
        ...
        if (obj->otyp == WAN_POLYMORPH && obj->spe > 0 && (mtmp->cham == NON_PM)
...
static struct permonst * muse_newcham_mon(mon)
    struct monst *mon;
{
    struct obj *m_armr;
    ...
    if ((m_armr = which_armor(mon, W_ARM)) != 0)
    {
        ...
        else if (Is_dragon_mail(m_armr)) return Dragon_mail_to_pm(m_armr);
    }
    return rndmonst();
}
    ...
    struct monst *mtmp;
{
            ...
            if (Can_rise_up(mtmp->mx, mtmp->my, &u.uz))
            {
                ...
                if (vismon)
                {
                    ...
                    if (!objects[POT_GAIN_LEVEL].oc_name_known
                            && !objects[POT_GAIN_LEVEL].oc_uname) docall(otmp);
                }
                m_useup(mtmp, otmp);
        ...
        /* attempt to disarm hero */
    {
        ...
        if (!obj || !canletgo(obj, "")
                || (u.twoweap && canletgo(uswapwep, "") && rn2(2))) obj =
                uswapwep;
...
STATIC_OVL void you_aggravate(mtmp)
    struct monst *mtmp;
{
...
boolean searches_for_item(mon, obj)
    struct monst *mon;struct obj *obj;
{
    int typ = obj->otyp;
    ...
    case WAND_CLASS:
        ...
        if (typ == WAN_DIGGING) return (boolean) !is_floater(mon->data);
    ...
    case POTION_CLASS:
        if (typ == POT_HEALING || typ == POT_EXTRA_HEALING
                ...
                || typ == POT_SLEEPING || typ == POT_ACID
                || typ == POT_CONFUSION) return TRUE;
        if (typ == POT_BLINDNESS && !attacktype(mon->data, AT_GAZE))
            return TRUE;
        break;
    case SCROLL_CLASS:
        if (typ == SCR_TELEPORTATION || typ == SCR_CREATE_MONSTER
                || typ == SCR_EARTH || typ == SCR_FIRE) return TRUE;
        break;
    case AMULET_CLASS:
        ...
        break;
    case TOOL_CLASS:
        if (typ == PICK_AXE) return (boolean) needspick(mon->data);
        if (typ == UNICORN_HORN)
    ...
    case FOOD_CLASS:
        ...
        if (typ == EGG) return (boolean) touch_petrifies(&mons[obj->corpsenm]);
        break;
    default:
        break;
    }
...
boolean mon_reflects(mon, str)
    struct monst *mon;const char *str;
{
...
boolean munstone(mon, by_you)
    struct monst *mon;boolean by_you;
{
    struct obj *obj;
    boolean tinok;
...
STATIC_OVL void mon_consume_unstone(mon, obj, by_you, stoning)
    struct monst *mon;struct obj *obj;boolean by_you;boolean stoning;
{
    boolean vis = canseemon(mon), tinned = obj->otyp == TIN, food = obj->otyp
            == CORPSE || tinned, acid = obj->otyp == POT_ACID
            || (food && acidic(&mons[obj->corpsenm])), lizard = food
    ...
    if (vis)
    {
        ...
        pline("%s %s %s.", Monnam(mon),
                ...
                (obj->otyp == TIN) ? "opens and eats the contents of" : "eats",
    ...
    {
        mon->mhp -= rnd(15);
        if (vis) pline("%s has a very bad case of stomach acid.", Monnam(mon));
        if (mon->mhp <= 0)
        {
...
STATIC_OVL boolean cures_stoning(mon, obj, tinok)
    struct monst *mon;struct obj *obj;boolean tinok;
{
...
STATIC_OVL boolean mcould_eat_tin(mon)
    struct monst *mon;
{
    struct obj *obj, *mwep;
    boolean welded_wep;
    ...
    struct monst *mon;boolean by_you;
{
    ...
    for (obj = mon->minvent; obj; obj = obj->nobj)
        if (cures_sliming(mon, obj)) return muse_unslime(mon, obj, by_you);
...
STATIC_OVL boolean muse_unslime(mon, obj, by_you)
    struct monst *mon;struct obj *obj;boolean by_you; /* true: if mon kills itself, hero gets credit/blame */
{
    struct obj *odummyp;
    ...
    if (otyp == STRANGE_OBJECT)
    {
        ...
        if (vis) pline("%s breathes fire on %sself.", Monnam(mon), mhim(mon));
    ...
    else if (otyp == SCR_FIRE)
    {
        ...
        if (mon->mconf)
        {
            if (cansee(mon->mx, mon->my)) pline("Oh, what a pretty fire!");
...
STATIC_OVL int cures_sliming(mon, obj)
    struct monst *mon;struct obj *obj;
{
    /* scroll of fire, non-empty wand or horn of fire */
    if (obj->otyp == SCR_FIRE) return (haseyes(mon->data) && mon->mcansee);
...
STATIC_OVL boolean green_mon(mon)
    struct monst *mon;
{
