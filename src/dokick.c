...
STATIC_OVL void kickdmg(mon, clumsy)
    register struct monst *mon;register boolean clumsy;
{
    ...
    if ((is_undead(mon->data) || is_demon(mon->data) || is_vampshifter(mon))
            && uarmf && uarmf->blessed) blessed_foot_damage = 1;
...
STATIC_OVL boolean maybe_kick_monster(mon, x, y)
    struct monst *mon;xchar x, y;
{
    if (mon)
    {
        ...
        bhitpos.x = x;
        bhitpos.y = y;
        if (!mon->mpeaceful || !canspotmon(mon)) context.forcefight = TRUE; /* attack even if invisible */
        ...
        if (attack_checks(mon, (struct obj *) 0) || overexertion()) mon = 0; /* don't kick after all */
        context.forcefight = save_forcefight;
    }
    return (boolean) (mon != 0);
}

STATIC_OVL void kick_monster(mon, x, y)
    struct monst *mon;xchar x, y;
{
    ...
    if (mon->mundetected || (mon->m_ap_type && mon->m_ap_type != M_AP_MONSTER))
    {
        if (mon->m_ap_type) seemimic(mon);
        mon->mundetected = 0;
        if (!canspotmon(mon))
    ...
    {
        struct attack *uattk;
        int sum, kickdieroll, armorpenalty, attknum = 0, tmp = find_roll_to_hit(
                mon, AT_KICK, (struct obj *) 0, &attknum, &armorpenalty);
 ...
 */
boolean ghitm(mtmp, gold)
    register struct monst *mtmp;register struct obj *gold;
{
        ...
        else if (is_mercenary(mtmp->data))
        {
            ...
            if (rn2(3))
            {
                if (mtmp->data == &mons[PM_SOLDIER])
                    ...
                    goldreqd = 500L;
                else if (mtmp->data == &mons[PM_CAPTAIN]) goldreqd = 750L;
                ...
                if (goldreqd)
                {
                    ...
                    if (value
                            > goldreqd
                                    + (umoney + u.ulevel * rn2(5))
                                            / ACURR(A_CHA)) mtmp->mpeaceful =
                    TRUE;
                }
            }
            if (mtmp->mpeaceful)
...
void container_impact_dmg(obj, x, y)
    struct obj *obj;xchar x, y; /* coordinates where object was before the impact, not after */
{
    struct monst *shkp;
    ...
    for (otmp = obj->cobj; otmp; otmp = otmp2)
    {
        ...
        if (result)
        {
            ...
            if (costly)
            {
                ...
                loss += stolen_value(otmp, x, y, (boolean) shkp->mpeaceful,
                TRUE);
            }
            if (otmp->quan > 1L)
            {
...
STATIC_OVL int kick_object(x, y)
    xchar x, y;
{
    int res = 0;
...
/* guts of kick_object */
STATIC_OVL int really_kick_object(x, y)
    xchar x, y;
{
    int range;
    ...
    if (!kickedobj || kickedobj->otyp == BOULDER || kickedobj == uball
            || kickedobj == uchain) return 0;

    if ((trap = t_at(x, y)) != 0
            && (((trap->ttyp == PIT || trap->ttyp == SPIKED_PIT)
                    && !Passes_walls) || trap->ttyp == WEB))
    {
    ...
    if (!ZAP_POS(levl[x + u.dx][y + u.dy].typ)
            || closed_door(x + u.dx, y + u.dy)) range = 1;

    costly = (!(kickedobj->no_charge && !Has_contents(kickedobj)) && (shkp =
            shop_keeper(*in_rooms(x, y, SHOPBASE))) != 0 && costly_spot(x, y));
    ...
    if (hero_breaks(kickedobj, kickedobj->ox, kickedobj->oy, FALSE)) return 1;
    ...
    if (mon)
    {
        if (mon->isshk && kickedobj->where == OBJ_MINVENT
                && kickedobj->ocarry == mon) return 1; /* alert shk caught it */
...
int dokick()
{
    ...
    if (mtmp)
    {
        ...
        if (!maybe_kick_monster(mtmp, x, y)) return context.move;
    }
    ...
    if (mtmp)
    {
        ...
        && mtmp->mx == x && mtmp->my == y && !glyph_is_invisible(glyph)
                    ...
                    || sobj_at(BOULDER, x, y)))
    {
        if (kick_object(x, y))
        {
            if (Is_airlevel(&u.uz)) hurtle(-u.dx, -u.dy, 1, TRUE); /* assume it's light */
            return 1;
        }
        goto ouch;
    }
    ...
    if (!IS_DOOR(maploc->typ))
    {
            ...
            if (!Levitation && rn2(30) < avrg_attrib)
            {
                ...
                if (maploc->doormask & D_TRAPPED)
                {
                    ...
                    b_trapped("door", FOOT);
                        ...
                        && !(maploc->doormask & D_LOCKED)) maploc->doormask =
                D_ISOPEN;
        ...
        if (maploc->typ == IRONBARS) goto ouch;
        if (IS_TREE(maploc->typ))
        {
            ...
            if (rn2(15) && !(maploc->looted & TREE_LOOTED) && (treefruit =
        ...
        if (IS_SINK(maploc->typ))
        {
            ...
            else if (!rn2(3))
            {
                ...
                { /* once per sink */
                    if (!Blind) You_see("a ring shining in its midst.");
...
STATIC_OVL void drop_to(cc, loc)
    coord *cc;schar loc;
{
...
void impact_drop(missile, x, y, dlev)
 ...
 */
boolean ship_object(otmp, x, y, shop_floor_obj)
    xchar x, y;struct obj *otmp;boolean shop_floor_obj;
{
    schar toloc;
    xchar ox, oy;
    coord cc;
...
STATIC_OVL void otransit_msg(otmp, nodrop, num)
    register struct obj *otmp;register boolean nodrop;long num;
{
    char obuf[BUFSZ];
    ...
    else if (!nodrop) pline("%s %s %s.", obuf, otense(otmp, "fall"), gate_str);
}
    ...
    xchar x, y;
{
    ...
    if (on_level(&u.uz, &qstart_level) && !ok_to_quest()) return MIGR_NOWHERE;
