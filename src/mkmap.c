...
STATIC_OVL void init_map(bg_typ)
    schar bg_typ;
{
    register int i, j;
...
STATIC_OVL void init_fill(bg_typ, fg_typ)
    schar bg_typ, fg_typ;
{
    register int i, j;
    long limit, count;
...
STATIC_OVL schar get_map(col, row, bg_typ)
    int col, row;schar bg_typ;
{
...
STATIC_OVL void pass_one(bg_typ, fg_typ)
    schar bg_typ, fg_typ;
{
    register int i, j;
    short count, dr;
...
STATIC_OVL void pass_two(bg_typ, fg_typ)
    schar bg_typ, fg_typ;
{
    register int i, j;
    short count, dr;
...
STATIC_OVL void pass_three(bg_typ, fg_typ)
    schar bg_typ, fg_typ;
{
    register int i, j;
    short count, dr;
 ...
 */
void flood_fill_rm(sx, sy, rmno, lit, anyroom)
    int sx;register int sy;register int rmno;boolean lit;boolean anyroom;
{
    register int i;
    int nx;
...
STATIC_OVL void join_map(bg_typ, fg_typ)
    schar bg_typ, fg_typ;
{
...
STATIC_OVL void finish_map(fg_typ, bg_typ, lit, walled, icedpools)
    schar fg_typ, bg_typ;boolean lit, walled, icedpools;
{
    int i, j;
    ...
    int lx, ly, hx, hy;
{
    ...
    for (i = nroom - 1; i >= 0; --i)
    {
        ...
        if (croom->hx < lx || croom->lx >= hx || croom->hy < ly
                || croom->ly >= hy) continue; /* no overlap */
 ...
 */
STATIC_OVL void remove_room(roomno)
    unsigned roomno;
{
    ...
    lev_init *init_lev;
{
    ...
    if (lit < 0) lit = (rnd(1 + abs(depth(&u.uz))) < 11 && rn2(77)) ? 1 : 0;
    ...
    if (smooth) for (i = 0; i < N_P3_ITER; i++)
