...
STATIC_DCL char *
FDECL(mon_nam_too, (char *, struct monst *, struct monst *));
...
STATIC_OVL char * mon_nam_too(outbuf, mon, other_mon)
    char *outbuf;struct monst *mon, *other_mon;
{
...
STATIC_OVL void noises(magr, mattk)
    register struct monst *magr;register struct attack *mattk;
{
...
STATIC_OVL
void missmm(magr, mdef, mattk)
    register struct monst *magr, *mdef;struct attack *mattk;
{
    const char *fmt;
 ...
 */
int mdisplacem(magr, mdef, quietly)
    register struct monst *magr, *mdef;boolean quietly;
{
    ...
    if (touch_petrifies(pd) && !resists_ston(magr))
    {
        if (which_armor(magr, W_ARMG) != 0)
        {
            ...
            else if (magr->mtame && !vis) You(brief_feeling, "peculiarly sad");
            return MM_AGR_DIED;
        }
    }
...
STATIC_OVL int hitmm(magr, mdef, mattk)
    register struct monst *magr, *mdef;struct attack *mattk;
{
    if (vis)
    {
...
STATIC_OVL int gazemm(magr, mdef, mattk)
    register struct monst *magr, *mdef;struct attack *mattk;
{
    char buf[BUFSZ];
...
boolean engulf_target(magr, mdef)
    struct monst *magr, *mdef;
{
    ...
    if (mdef->data->msize >= MZ_HUGE) return FALSE;
    ...
    || (lev->typ == IRONBARS && !is_whirly(magr->data))) return FALSE;

    return TRUE;
}

/* Returns the same values as mattackm(). */
STATIC_OVL int gulpmm(magr, mdef, mattk)
    register struct monst *magr, *mdef;register struct attack *mattk;
{
    xchar ax, ay, dx, dy;
    int status;
    char buf[BUFSZ];
    ...
    { /* defender died */
        ...
        if (minliquid(magr) || (t_at(dx, dy) && mintrap(magr) == 2)) status |=
        MM_AGR_DIED;
    }
...
STATIC_OVL int explmm(magr, mdef, mattk)
 ...
 */
STATIC_OVL int mdamagem(magr, mdef, mattk)
    register struct monst *magr, *mdef;register struct attack *mattk;
{
    struct obj *obj;
    char buf[BUFSZ];
    struct permonst *pa = magr->data, *pd = mdef->data;
    int armpro, num, tmp = d((int) mattk->damn, (int) mattk->damd), res =
    MM_MISS;
    boolean cancelled;
    ...
    case AD_DGST:
        ...
        if (is_rider(pd))
        {
            ...
            if (magr->mhp > 0)
                return 0; /* lifesaved */
            else if (magr->mtame && !vis) You(brief_feeling, "queasy");
            return MM_AGR_DIED;
        }
    ...
    case AD_HEAL:
    case AD_PHYS:
        physical: if (mattk->aatyp == AT_KICK && thick_skinned(pd))
        {
            tmp = 0;
        }
    ...
    case AD_FIRE:
        ...
        if (vis) pline("%s is %s!", Monnam(mdef), on_fire(pd, mattk));
    ...
    case AD_STON:
        ...
        if (!resists_ston(mdef))
        {
            ...
            post_stone: if (mdef->mhp > 0)
                return 0;
            else if (mdef->mtame && !vis) You(brief_feeling, "peculiarly sad");
            return (MM_DEF_DIED | (grow_up(magr, mdef) ? 0 : MM_AGR_DIED));
        }
...
void paralyze_monst(mon, amt)
    struct monst *mon;int amt;
{
    if (amt > 127) amt = 127;
...
int sleep_monst(mon, amt, how)
    struct monst *mon;int amt, how;
{
...
STATIC_OVL void mswingsm(magr, mdef, otemp)
    struct monst *magr, *mdef;struct obj *otemp;
{
 ...
 */
STATIC_OVL int passivemm(magr, mdef, mhit, mdead)
    register struct monst *magr, *mdef;boolean mhit;int mdead;
{
        ...
        case AD_PLYS: /* Floating eye */
            ...
            if (mddat == &mons[PM_FLOATING_EYE])
            {
                        ...
                        && (perceives(madat) || !mdef->minvis))
                {
                    ...
                    canseemon(magr) ? buf : (char *) 0)) return (mdead | mhit);
                    Strcpy (buf, Monnam(magr));
                    if (canseemon(magr))
...
void xdrainenergym(mon, givemsg)
    struct monst *mon;boolean givemsg;
{
