    ...
    for (upp = u.uprops; upp < u.uprops + SIZE(u.uprops); upp++)
        if ((upp->intrinsic & TIMEOUT) && !(--upp->intrinsic & TIMEOUT))
        {
            ...
            case SICK:
                ...
                break;
            case FAST:
                if (!Very_fast) You_feel("yourself slowing down%s.",
                Fast ? " a bit" : "");
                break;
            case CONFUSION:
...
void fall_asleep(how_long, wakeup_msg)
    int how_long;boolean wakeup_msg;
{
    stop_occupation();
    nomul(how_long);
 ...
 */
void attach_egg_hatch_timeout(egg, when)
    struct obj *egg;long when;
{
    int i;
...
void attach_fig_transform_timeout(figurine)
    struct obj *figurine;
{
    int i;
...
STATIC_OVL void see_lamp_flicker(obj, tailer)
    struct obj *obj;const char *tailer;
{
    switch (obj->where)
    {
    case OBJ_INVENT:
...
STATIC_OVL void lantern_message(obj)
    struct obj *obj;
{
    /* from adventure */
    switch (obj->where)
 ...
 */
void begin_burn(obj, already_lit)
    struct obj *obj;boolean already_lit;
{
    int radius = 3;
    long turns = 0;
 ...
 * light source.
 */
void end_burn(obj, timer_attached)
    struct obj *obj;boolean timer_attached;
{
    if (!obj->lamplit)
    {
 ...
 */
static void cleanup_burn(arg, expire_time)
    anything *arg;long expire_time;
{
    struct obj *obj = arg->a_obj;
    if (!obj->lamplit)
    {
...
void do_storms()
{
    ...
    if (!Is_airlevel(&u.uz) || rn2(8)) return;
    ...
    for (nstrike = rnd(64); nstrike <= 64; nstrike *= 2)
    {
        ...
        if (count < 100)
        {
            dirx = rn2(3) - 1;
            diry = rn2(3) - 1;
            if (dirx != 0 || diry != 0) buzz(-15, /* "monster" LIGHTNING spell */
            8, x, y, dirx, diry);
        }
    }
...
STATIC_OVL const char * kind_name(kind)
    short kind;
{
    switch (kind)
    {
...
STATIC_OVL void print_queue(win, base)
    winid win;timer_element *base;
{
    timer_element *curr;
    char buf[BUFSZ];
 ...
 */
boolean start_timer(when, kind, func_index, arg)
    long when;short kind;short func_index;anything *arg;
{
    timer_element *gnu;
 ...
 */
long stop_timer(func_index, arg)
    short func_index;anything *arg;
{
    ...
    if (doomed)
    {
        ...
        if (doomed->kind == TIMER_OBJECT) (arg->a_obj)->timed--;
 ...
 */
long peek_timer(type, arg)
    short type;anything *arg;
{
    timer_element *curr;
 ...
 */
void obj_split_timers(src, dest)
    struct obj *src, *dest;
{
 ...
 * Check whether object has a timer of type timer_type.
 */
boolean obj_has_timer(object, timer_type)
    struct obj *object;short timer_type;
{
 ...
 *
 */
void spot_stop_timers(x, y, func_index)
    xchar x, y;short func_index;
{
    timer_element *curr, *prev, *next_timer = 0;
 ...
 */
long spot_time_expires(x, y, func_index)
    xchar x, y;short func_index;
{
    timer_element *curr;
    long where = (((long) x << 16) | ((long) y));
    ...
    {
        if (curr->kind == TIMER_LEVEL && curr->func_index == func_index
                && curr->arg.a_long == where) return curr->timeout;
    }
    return 0L;
}

long spot_time_left(x, y, func_index)
    xchar x, y;short func_index;
{
...
STATIC_OVL void insert_timer(gnu)
    timer_element *gnu;
{
...
STATIC_OVL timer_element * remove_timer(base, func_index, arg)
    timer_element **base;short func_index;anything *arg;
{
...
STATIC_OVL void write_timer(fd, timer)
    int fd;timer_element *timer;
{
    anything arg_save;
 ...
 * level is saved.
 */
STATIC_OVL boolean mon_is_local(mon)
    struct monst *mon;
{
    struct monst *curr;
 ...
 */
STATIC_OVL boolean timer_is_local(timer)
    timer_element *timer;
{
    switch (timer->kind)
    {
 ...
 */
STATIC_OVL int maybe_write_timer(fd, range, write_it)
    int fd, range;boolean write_it;
{
    int count = 0;
    timer_element *curr;
 ...
 */
void save_timers(fd, mode, range)
    int fd, mode, range;
{
    timer_element *curr, *prev, *next_timer = 0;
    int count;
 ...
 * monster pointers.
 */
void restore_timers(fd, range, ghostly, adjust)
    int fd, range;boolean ghostly; /* restoring from a ghost level */
