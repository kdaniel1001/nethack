...
STATIC_OVL NEARDATA const char comestibles[] =
{ FOOD_CLASS, 0 };
STATIC_OVL NEARDATA const char offerfodder[] =
...
const char *hu_stat[] =
{ "Satiated", "        ", "Hungry  ", "Weak    ", "Fainting", "Fainted ",
        "Starved " };
...
STATIC_PTR int eatmdone(VOID_ARGS)
{
    /* release `eatmbuf' */
    if (eatmbuf)
    {
...
STATIC_OVL const char * food_xname(food, the_pfx)
    struct obj *food;boolean the_pfx;
{
    const char *result;
...
STATIC_OVL void recalc_wt()
{
    ...
    if (!piece)
    {
        ...
        return;
    }debugpline1("Old weight = %d", piece->owt);debugpline2("Used time = %d, Req'd time = %d", context.victual.usedtime,
...
STATIC_OVL struct obj * touchfood(otmp)
    struct obj *otmp;
{
    if (otmp->quan > 1L)
    {
...
void food_substitution(old_obj, new_obj)
...
STATIC_PTR int eatfood(VOID_ARGS)
{
    if (!context.victual.piece
...
STATIC_OVL void done_eating(message)
    boolean message;
{
...
void eating_conducts(pd)
    struct permonst *pd;
{
    u.uconduct.food++;
    if (!vegan(pd)) u.uconduct.unvegan++;
    if (!vegetarian(pd)) violated_vegetarian();
}

/* handle side-effects of mind flayer's tentacle attack */
int eat_brains(magr, mdef, visflag, dmg_p)
    struct monst *magr, *mdef;boolean visflag;int *dmg_p; /* for dishing out extra damage in lieu of Int loss */
{
...
STATIC_OVL boolean maybe_cannibal(pm, allowmsg)
    int pm;boolean allowmsg;
{
    ...
    register int pm;
{
    ...
    if (flesh_petrifies(&mons[pm]))
    {
                ...
                && !(poly_when_stoned(youmonst.data) && polymon(PM_STONE_GOLEM)))
        {
            ...
            if (context.victual.piece) context.victual.eating = FALSE;
            return; /* lifesaved */
        }
    }
...
STATIC_OVL int intrinsic_possible(type, ptr)
    int type;register struct permonst *ptr;
{
    int res = 0;
 ...
 */
STATIC_OVL void givit(type, ptr)
    int type;register struct permonst *ptr;
{
    register int chance;
...
STATIC_PTR void costly_tin(alter_type)
    int alter_type; /* COST_xxx */
{
    struct obj *tin = context.tin.tin;

    if (carried(tin) ?
            tin->unpaid : (costly_spot(tin->ox, tin->oy) && !tin->no_charge))
    {
        if (tin->quan > 1L)
        {
...
int tin_variety_txt(s, tinvariety)
    char *s;int *tinvariety;
{
    int k, l;
 ...
 */
void tin_details(obj, mnum, buf)
    struct obj *obj;int mnum;char *buf;
{
    char buf2[BUFSZ];
...
void set_tin_variety(obj, forcetype)
    struct obj *obj;int forcetype;
{
    register int r;

    if (forcetype == SPINACH_TIN
            || (forcetype == HEALTHY_TIN && (obj->corpsenm == NON_PM /* empty or already spinach */
    ...
    { /* RANDOM_TIN */
        ...
        if (r == ROTTEN_TIN && nonrotting_corpse(obj->corpsenm)) r =
        HOMEMADE_TIN; /* lizards don't rot */
    }
    obj->spe = -(r + 1); /* offset by 1 to allow index 0 */
}

STATIC_OVL int tin_variety(obj, disp)
    struct obj *obj;boolean disp; /* we're just displaying so leave things alone */
{
    register int r;
    ...
    if (!disp && r == HOMEMADE_TIN && !obj->blessed && !rn2(7)) r = ROTTEN_TIN; /* some homemade tins go bad */

    if (r == ROTTEN_TIN && nonrotting_corpse(obj->corpsenm)) r = HOMEMADE_TIN; /* lizards don't rot */
    return r;
}

STATIC_OVL void consume_tin(mesg)
    const char *mesg;
{
    const char *what;
    int which, mnum, r;
...
STATIC_PTR int opentin(VOID_ARGS)
{
    ...
    if (context.tin.usedtime < context.tin.reqtime) return 1; /* still busy */
...
STATIC_OVL void start_tin(otmp)
    struct obj *otmp;
{
    const char *mesg = 0;
    register int tmp;
    ...
    {
        no_opener: pline("It is not so easy to open this tin.");
        if (Glib)
        {
...
STATIC_OVL void start_eating(otmp)
    struct obj *otmp;
{
    const char *old_nomovemsg, *save_nomovemsg;

    debugpline2("start_eating: %lx (victual = %lx)", (unsigned long) otmp,
            (unsigned long) context.victual.piece);debugpline1("reqtime = %d", context.victual.reqtime);debugpline1("(original reqtime = %d)", objects[otmp->otyp].oc_delay);debugpline1("nmod = %d", context.victual.nmod);debugpline1("oeaten = %d", otmp->oeaten);
    ...
    default:
        ...
        else if (otmp->otyp == APPLE || otmp->otyp == PEAR)
        {
            ...
            else
            {
                ...
                pline("%s -- core dumped.",
                        (x <= 75) ? "Segmentation fault" :
                        (x <= 99) ? "Bus error" : "Yo' mama");
            }
#endif
        }
...
STATIC_OVL int bounded_increase(old, inc, typ)
    int old, inc, typ;
{
...
STATIC_OVL void accessory_has_effect(otmp)
    struct obj *otmp;
{
...
STATIC_OVL void eataccessory(otmp)
    struct obj *otmp;
{
    int typ = otmp->otyp;
    long oldprop;
...
{ "meal", "liquid", "wax", "food", "meat", "paper", "cloth", "leather", "wood",
        "bone", "scale", "metal", "metal", "metal", "silver", "gold",
        "platinum", "mithril", "plastic", "glass", "rich food", "stone" };

STATIC_OVL const char * foodword(otmp)
    struct obj *otmp;
{
...
STATIC_OVL void fpostfx(otmp)
    struct obj *otmp;
{
    switch (otmp->otyp)
    {
    case SPRIG_OF_WOLFSBANE:
        if (u.ulycn >= LOW_PM || is_were(youmonst.data)) you_unwere(TRUE);
        break;
    case CARROT:
 ...
 */
STATIC_OVL int edibility_prompts(otmp)
    struct obj *otmp;
{
...
int doeat()
{
    ...
    if (!(
    carried(otmp) ?
            retouch_object(&otmp, FALSE) : touch_artifact(otmp, &youmonst)))
    {
        return 1;
    }
    ...
    if (is_metallic(otmp) && u.umonnum == PM_RUST_MONSTER && otmp->oerodeproof)
    {
        ...
        You("spit %s out onto the %s.", the(xname(otmp)), surface(u.ux, u.uy));
        if (carried(otmp))
        {
            freeinv(otmp);
            dropy(otmp);
    ...
    if (otmp->otyp == RIN_SLOW_DIGESTION)
    {
        ...
        if (otmp->dknown && !objects[otmp->otyp].oc_name_known
                && !objects[otmp->otyp].oc_uname) docall(otmp);
        return 1;
    }
    if (otmp->oclass != FOOD_CLASS)
    {
        ...
        else if (material == WAX) u.uconduct.unvegan++;
        u.uconduct.food++;
        ...
        else if (!otmp->cursed && !nodelicious)
        {
            pline("%s%s is delicious!",
                    ...
                    (otmp->oclass == COIN_CLASS) ?
                            foodword(otmp) : singular(otmp, xname));
        }
        eatspecial();
        return 1;
    }
    ...
    else
    {
        ...
        default:
            ...
            || otmp->otyp == LUMP_OF_ROYAL_JELLY) u.uconduct.unvegan++;
            break;
        }
    ...
    debugpline1("before rounddiv: context.victual.reqtime == %d",
            context.victual.reqtime);debugpline2("oeaten == %d, basenutrit == %d", otmp->oeaten, basenutrit);
    context.victual.reqtime =
            (basenutrit == 0) ?
                    0 :
                    rounddiv(context.victual.reqtime * (long) otmp->oeaten,
                            basenutrit);
...
void gethungry()
{
    ...
    && (carnivorous(youmonst.data) || herbivorous(youmonst.data))
            && !Slow_digestion) u.uhunger--; /* ordinary food consumption */
    ...
    boolean incr;
{
    ...
    newhs = (h > 1000) ? SATIATED : (h > 150) ? NOT_HUNGRY : (h > 50) ? HUNGRY :
            (h > 0) ? WEAK : FAINTING;
    ...
    if (newhs != u.uhs)
    {
        ...
        else if (newhs < WEAK && u.uhs >= WEAK) losestr(-1);
        switch (newhs)
        {
        case HUNGRY:
            if (Hallucination)
        ...
        case WEAK:
            if (Hallucination)
                ...
                You(
                        (!incr) ? "feel weak now." :
                        (u.uhunger < 45) ?
                                "feel weak." : "are beginning to feel weak.");
            if (incr && occupation
                    && (occupation != eatfood && occupation != opentin))
                stop_occupation();
            context.travel = context.travel1 = context.mv = context.run = 0;
 ...
 */
struct obj * floorfood(verb, corpsecheck)
    const char *verb;int corpsecheck; /* 0, no check, 1, corpses, 2, tinnable corpses */
{
    ...
    if (!can_reach_floor(TRUE) || (feeding && u.usteed)
            || (is_pool_or_lava(u.ux, u.uy)
                    && (Wwalking || is_clinger(youmonst.data)
                            || (Flying && !Breathless)))) goto skipfloor;
    ...
    for (otmp = level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere)
    {
                    ...
                    otmp->oclass == FOOD_CLASS)
        {
            ...
            if ((c = yn_function(qbuf, ynqchars, 'n')) == 'y')
                return otmp;
            else if (c == 'q') return (struct obj *) 0;
        }
    }
...
int eaten_stat(base, obj)
    int base;struct obj *obj;
{
...
void consume_oeaten(obj, amt)
    struct obj *obj;int amt;
{
    /*
...
boolean maybe_finished_meal(stopping)
    boolean stopping;
{
