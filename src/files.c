...
#endif
STATIC_DCL char *
FDECL(set_bonesfile_name, (char *, d_level *));
STATIC_DCL char *
NDECL(set_bonestemp_name);
#ifdef COMPRESS
...
#endif
STATIC_DCL FILE *
FDECL(fopen_config_file, (const char *, int));
 ...
 */
char * fname_encode(legal, quotechar, s, callerbuf, bufsz)
    const char *legal;char quotechar;char *s, *callerbuf;int bufsz;
{
    char *sp, *op;
    int cnt = 0;
 ...
 *      bufsz       size of callerbuf
 */
char * fname_decode(quotechar, s, callerbuf, bufsz)
    char quotechar;char *s, *callerbuf;int bufsz;
{
    char *sp, *op;
    int k, calc, cnt = 0;
    static char hexdigits[] = "0123456789ABCDEF";
...
/*ARGSUSED*/
const char * fqname(basenam, whichprefix, buffnum)
    const char *basenam;int whichprefix UNUSED_if_not_PREFIXES_IN_USE;int buffnum UNUSED_if_not_PREFIXES_IN_USE;
{
    printf("fqname before PREFIXES_IN_USE\n");
#ifndef PREFIXES_IN_USE
    return basenam;
#else
    printf("FREFIXES_IN_USE defined\n");
    ...
    printf ("file path: %s",strcat(fqn_filename_buffer[buffnum], basenam));
    return strcat(fqn_filename_buffer[buffnum], basenam);
#endif
}

int validate_prefix_locations(reasonbuf)
...
FILE * fopen_datafile(filename, mode, prefix)
    const char *filename, *mode;int prefix;
{
    FILE *fp;
 ...
 */
void set_levelfile_name(file, lev)
    char *file;int lev;
{
    char *tf;
...
int create_levelfile(lev, errbuf)
    int lev;char errbuf[];
{
    int fd;
    const char *fq_lock;
 ...
 */
STATIC_OVL char * set_bonesfile_name(file, lev)
    char *file;d_level *lev;
{
    s_level *sptr;
    char *dptr;
...
int create_bonesfile(lev, bonesid, errbuf)
    d_level *lev;char **bonesid;char errbuf[];
{
    const char *file;
    int fd;
...
int open_bonesfile(lev, bonesid)
    d_level *lev;char **bonesid;
{
    const char *fq_bones;
    int fd;
...
void set_savefile_name(regularize_it)
...
STATIC_OVL void redirect(filename, mode, stream, uncomp)
    const char *filename, *mode;FILE *stream;boolean uncomp;
{
 ...
 */
STATIC_OVL void docompress_file(filename, uncomp)
    const char *filename;boolean uncomp;
{
    char cfn[80];
    FILE *cf;
    const char *args[10];
...
/* lock a file */
boolean lock_file(filename, whichprefix, retryct)
    const char *filename;int whichprefix;int retryct;
{
...
#pragma unused(retryct)
    ...
    if (lockfd == -1)
    {
        HUP raw_printf("Cannot open file %s. This is a program bug.", filename);
    }
...
STATIC_OVL FILE * fopen_config_file(filename, src)
    const char *filename;int src;
{
    FILE *fp;
#if defined(UNIX) || defined(VMS)
    ...
    if (filename)
    {
        ...
        if ((fp = fopenp(lastconfigfile, "r")) != (FILE *) 0) return fp;
 ...
 */
STATIC_OVL int get_uchars(fp, buf, bufp, list, modlist, size, name)
...
int parse_config_line(fp, origbuf, src)
    FILE *fp;char *origbuf;int src;
{
    ...
    else if (match_varname(buf, "ROLE", 4) || match_varname(buf, "CHARACTER", 4))
    {
    ...
    {
        if (sysopt.wizards) free((genericptr_t) sysopt.wizards);
    ...
    {
        if (sysopt.shellers) free((genericptr_t) sysopt.shellers);
        sysopt.shellers = dupstr(bufp);
    }
    else if (src == SET_IN_SYS && match_varname(buf, "EXPLORERS", 7))
    {
        if (sysopt.explorers) free((genericptr_t) sysopt.explorers);
    ...
    else if (src == SET_IN_SYS && match_varname(buf, "DEBUGFILES", 5))
    {
        ...
        {
            if (sysopt.debugfiles) free((genericptr_t) sysopt.debugfiles);
            sysopt.debugfiles = dupstr(bufp);
        }
    }
    else if (src == SET_IN_SYS && match_varname(buf, "SUPPORT", 7))
    {
        if (sysopt.support) free((genericptr_t) sysopt.support);
        sysopt.support = dupstr(bufp);
    }
    else if (src == SET_IN_SYS && match_varname(buf, "RECOVER", 7))
    {
        if (sysopt.recover) free((genericptr_t) sysopt.recover);
    ...
    else if (src == SET_IN_SYS && match_varname(buf, "PANICTRACE_LIBC", 15))
    {
        n = atoi(bufp);
...
#endif
        if (sysopt.gdbpath) free((genericptr_t) sysopt.gdbpath);
...
#endif
        if (sysopt.greppath) free((genericptr_t) sysopt.greppath);
...
boolean read_config_file(filename, src)
    const char *filename;int src;
{
    char buf[4 * BUFSZ], *p;
    FILE *fp;
...
STATIC_DCL void wizkit_addinv(obj)
    struct obj *obj;
{
    ...
    while (fgets(buf, (int) (sizeof buf), fp))
    {
        ...
        else
        {
            ...
            if (buf[0])
            {
                ...
                if (otmp)
                {
                    if (otmp != &zeroobj) wizkit_addinv(otmp);
                }
                else
                {
    ...
    int which_set;
{
    ...
    if (!chosen_symset_end)
    {
        raw_printf("Missing finish for symset \"%s\"",
                symset[which_set].name ? symset[which_set].name : "unknown");
        wait_synch();
    }
    return 1;
}
...
/* returns 0 on error */
int parse_sym_line(buf, which_set)
    char *buf;int which_set;
{
    int val, i;
    ...
    if (!bufp)
    {
        if (strncmpi(buf, "finish", 6) == 0)
        {
            ...
            if (chosen_symset_start) chosen_symset_end = TRUE;
    ...
    if (!symset[which_set].name)
    {
        ...
        if (symp->range == SYM_CONTROL)
        {
            ...
            {
            case 0:
                tmpsp = (struct symsetentry *) alloc(
                        sizeof(struct symsetentry));
            ...
            case 3: /* description:something */
                tmpsp = symset_list; /* most recent symset */
                if (tmpsp && !tmpsp->desc) tmpsp->desc = dupstr(bufp);
                break;
            case 5:
                /* restrictions: xxxx*/
                tmpsp = symset_list; /* most recent symset */
    ...
    if (symp->range)
    {
        if (symp->range == SYM_CONTROL)
        {
            ...
            case 2:
                /* handler type identified */
                if (chosen_symset_start) set_symhandling(bufp, which_set);
                break;
            ...
            case 4: /* color:off */
                if (chosen_symset_start)
                {
                    if (bufp)
                    {
                        ...
                        else if (!strcmpi(bufp, "false") || !strcmpi(bufp, "no")
                        || !strcmpi(bufp, "off")) symset[which_set].nocolor = 1;
                    }
                }
                break;
...
STATIC_OVL void set_symhandling(handling, which_set)
    char *handling;int which_set;
{
    int i = 0;
    ...
    if (!program_state.in_paniclog)
    {
        ...
        if (lfile)
        {
            ...
            (void) fprintf(lfile, "%s %08ld %06ld %d %c: %s %s\n",
                    version_string(buf), yyyymmdd(now), hhmmss(now), uid,
                    playmode, type, reason);
            (void) fclose(lfile);
        }
...
static int choose_passage(passagecnt, oid)
    ...
    unsigned oid; /* book.o_id, used to determine whether re-reading same book */
{
    ...
    if (passagecnt < 1) return 0;
    if (passagecnt > MAXPASSAGES) passagecnt = MAXPASSAGES;
    ...
    if (oid != context.novel.id || context.novel.count == 0)
    {
        ...
        for (idx = 0; idx < MAXPASSAGES ; idx++)
            context.novel.pasg[idx] =
                    (xchar) ((idx < passagecnt) ? idx + 1 : 0);
    }
...
boolean read_tribute(tribsection, tribtitle, tribpassage, nowin_buf, bufsz, oid)
    const char *tribsection, *tribtitle;int tribpassage, bufsz;char *nowin_buf;unsigned oid; /* book identifier */
{
    dlb *fp;
    char *endp;
    ...
    {
        if (!nowin_buf) pline("It's %s of \"%s\"!", badtranslation, tribtitle);
        return grasped;
    }
    ...
    if (!fp)
    {
        ...
        if (!nowin_buf) pline("You feel too overwhelmed to continue!");
        return grasped;
    }
    ...
    *line = *lastline = '\0';
            ...
            else if (!strncmpi(&line[1], "e ", sizeof("e ") - 1))
            {
                if (matchedtitle && scope == PASSAGESCOPE
                        && ((!nowin_buf && tribwin != WIN_ERR)
                                || (nowin_buf && foundpassage))) goto cleanup;
    ...
    else
    {
        ...
        else if (foundpassage) grasped = TRUE;
    }
    return grasped;
}

boolean Death_quote(buf, bufsz)
    char *buf;int bufsz;
{
