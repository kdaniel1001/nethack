...
FDECL(e_nam, (struct entity *));
STATIC_DCL const char *
FDECL(E_phrase, (struct entity *, const char *));
    ...
    int x, y;
{
    ...
    if (IS_DRAWBRIDGE(
            levl[x + 1][y].typ) && (levl[x + 1][y].drawbridgemask & DB_DIR) == DB_WEST)
        return DB_WEST;
    if (IS_DRAWBRIDGE(
            levl[x - 1][y].typ) && (levl[x - 1][y].drawbridgemask & DB_DIR) == DB_EAST)
        return DB_EAST;
    if (IS_DRAWBRIDGE(
            levl[x][y - 1].typ) && (levl[x][y - 1].drawbridgemask & DB_DIR) == DB_SOUTH)
        return DB_SOUTH;
    if (IS_DRAWBRIDGE(
            levl[x][y + 1].typ) && (levl[x][y + 1].drawbridgemask & DB_DIR) == DB_NORTH)
        return DB_NORTH;
 ...
 */
STATIC_OVL void get_wall_for_db(x, y)
    int *x, *y;
{
...
boolean create_drawbridge(x, y, dir, flag)
    int x, y, dir;boolean flag;
{
    int x2, y2;
    boolean horiz;
    ...
    int x, y;
{
    ...
    for (entitycnt = 0; entitycnt < ENTITIES; entitycnt++)
        if ((occupants[entitycnt].edata) && (occupants[entitycnt].ex == x)
                && (occupants[entitycnt].ey == y)) break;debugpline1("entitycnt = %d", entitycnt);
#ifdef D_DEBUG
    wait_synch();
#endif
...
STATIC_OVL void m_to_e(mtmp, x, y, etmp)
    struct monst *mtmp;int x, y;struct entity *etmp;
{
    etmp->emon = mtmp;
    if (mtmp)
    {
...
STATIC_OVL void set_entity(x, y, etmp)
    int x, y;struct entity *etmp;
{
    if ((x == u.ux) && (y == u.uy))
        u_to_e(etmp);
    else if (MON_AT(x, y))
...
STATIC_OVL const char * e_nam(etmp)
    struct entity *etmp;
{
...
STATIC_OVL const char * E_phrase(etmp, verb)
    struct entity *etmp;const char *verb;
{
    static char wholebuf[80];

    Strcpy (wholebuf, is_u(etmp) ? "You" : Monnam(etmp->emon));
    if (!verb || !*verb) return wholebuf;
    Strcat (wholebuf, " ");
    if (is_u(etmp))
...
STATIC_OVL boolean e_survives_at(etmp, x, y)
    struct entity *etmp;int x, y;
{
    if (noncorporeal(etmp->edata)) return TRUE;
    if (is_pool(x, y))
        return (boolean) ((is_u(etmp)
                && (Wwalking || Amphibious || Swimming || Flying || Levitation))
...
STATIC_OVL void e_died(etmp, dest, how)
    struct entity *etmp;int dest, how;
{
    if (is_u(etmp))
    {
        if (how == DROWNING)
...
STATIC_OVL boolean automiss(etmp)
    struct entity *etmp;
{
    return (boolean) ((is_u(etmp) ? Passes_walls : passes_walls(etmp->edata))
...
STATIC_OVL boolean e_missed(etmp, chunks)
    struct entity *etmp;boolean chunks;
{
    int misses;
    ...
    if (is_flyer(etmp->edata)
            && (is_u(etmp) ?
                    !Unaware : (etmp->emon->mcanmove && !etmp->emon->msleeping)))
        /* flying requires mobility */
        misses = 5; /* out of 8 */
    else if (is_floater(etmp->edata) || (is_u(etmp) && Levitation)) /* doesn't require mobility */
        misses = 3;
...
STATIC_OVL boolean e_jumps(etmp)
    struct entity *etmp;
{
    int tmp = 4; /* out of 10 */
    ...
    if (is_u(etmp) ?
            ...
            (!etmp->emon->mcanmove || etmp->emon->msleeping
                    || !etmp->edata->mmove || etmp->emon->wormno)) return FALSE;
...
STATIC_OVL void do_entity(etmp)
    struct entity *etmp;
{
            ...
            else
            {
                if (e_inview)
                    pline("%s crushed by the falling portcullis!",
                            E_phrase(etmp, "are"));
                else if (!Deaf) You_hear("a crushing sound.");
    ...
    int x, y;
{
    ...
    if (OBJ_AT(x, y) && !Deaf) You_hear("smashing and crushing.");
        ...
        if (lev1->typ == DRAWBRIDGE_UP)
        {
            if (cansee(x2, y2))
                pline_The("portcullis of the drawbridge falls into the %s!",
                        lava ? "lava" : "moat");
            else if (!Deaf) You_hear("a loud *SPLASH*!");
        }
        else
        {
            if (cansee(x, y))
