...
const char * align_str(alignment)
    aligntyp alignment;
{
        ...
        Sprintf (eos(info), ", mimicking %s",
                ...
                (mtmp->m_ap_type == M_AP_OBJECT) ?
                        ((mtmp->mappearance == GOLD_PIECE) ?
                                "gold" : an(simple_typename(mtmp->mappearance))) :
                (mtmp->m_ap_type == M_AP_MONSTER) ?
                        an(mons[mtmp->mappearance].mname) : something); /* impossible... */
    if (mtmp->mcan)
...
void pudding_merge_message(otmp, otmp2)
    struct obj *otmp;struct obj *otmp2;
{
    boolean visible =
