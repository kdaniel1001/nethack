 ...
 * sight.
 */
int does_block(x, y, lev)
    int x, y;register struct rm *lev;
{
    struct obj *obj;
    struct monst *mon;
    ...
    if (lev->typ == CLOUD || lev->typ == WATER
            || (lev->typ == MOAT && Underwater)) return 1;
    ...
    if ((mon = m_at(x, y)) && (!mon->minvis || See_invisible)
            && (is_door_mappear(mon) || is_obj_mappear(mon, BOULDER))) return 1;
 ...
 */
STATIC_OVL void get_unused_cs(rows, rmin, rmax)
    char ***rows;char **rmin, **rmax;
{
    register int row;
 ...
 */
STATIC_OVL void rogue_vision(next, rmin, rmax)
    ...
    int control;
{
    ...
    for (row = 0; row < ROWNO; row++)
    {
        ...
        for (col = start; col <= stop; lev += ROWNO, sv += (int) colbump[++col])
        {
                    ...
                    && (lev->lit || (next_row[col] & TEMP_LIT)))
            {
                        ...
                        && !viz_clear[row][col])
                {
                    ...
                    if (flev->lit || next_array[row + dy][col + dx] & TEMP_LIT)
                    {
                         ...
                         * angle.*/
                        if (!(old_row[col] & IN_SIGHT)
                                || oldseenv != lev->seenv) newsym(col, row);
                    }
                    else
                        goto not_in_sight;
                    /* we don't see it */
            ...
            {
                not_in_sight: if ((old_row[col] & IN_SIGHT)
                        || ((next_row[col] & COULD_SEE)
                                ^ (old_row[col] & COULD_SEE))) newsym(col, row);
            }
     ...
     */
    if (!program_state.panicking) newsym(u.ux, u.uy); /* Make sure the hero shows up! */
 ...
 */
STATIC_OVL void dig_point(row, col)
    int row, col;
{
    int i;
...
STATIC_OVL void fill_point(row, col)
    int row, col;
{
    int i;
 ...
 *      do_light_sources()
 */
boolean clear_path(col1, row1, col2, row2)
 ...
 */
STATIC_OVL void right_side(row, left, right_mark, limits)
    int row; /* current row */
    ...
    char *limits; /* points at range limit for current row, or NULL */
{
    ...
    while (left <= right_mark)
    {
        ...
        if (left <= right)
        {
            ...
            && !is_clear(row, start_col + 1)) right = start_col + 1;
 ...
 * extensive comments.
 */
STATIC_OVL void left_side(row, left_mark, right, limits)
    int row, left_mark, right;char *limits;
{
    ...
    while (right >= left_mark)
    {
        ...
        if (left <= right)
        {
            ...
            && !is_clear(row, start_col - 1)) left = start_col - 1;
 ...
 */
void do_clear_area(scol, srow, range, func, arg)
    int scol, srow, range;void FDECL((*func), (int, int, genericptr_t));
genericptr_t arg;
{
    ...
    struct monst *mon;
{
    ...
    if ((mon->wormno ?
            ...
            && mon_visible(mon) && !mon->minvis) how_seen |= MONSEEN_NORMAL;
    /* see invisible */
    if (useemon && mon->minvis) how_seen |= MONSEEN_SEEINVIS;
    /* infravision */
    if ((!mon->minvis || See_invisible) && see_with_infrared(mon)) how_seen |=
    MONSEEN_INFRAVIS;
    /* telepathy */
    ...
    /* extended detection */
    if (Detect_monsters) how_seen |= MONSEEN_DETECT;
