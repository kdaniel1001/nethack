...
STATIC_DCL struct permonst *
FDECL(accept_newcham_form, (int));
STATIC_DCL struct obj *
FDECL(make_corpse, (struct monst *, unsigned));
...
void sanity_check_single_mon(mtmp, msg)
    struct monst *mtmp;const char *msg;
{
...
void mon_sanity_check()
{
    ...
    for (x = 0; x < COLNO; x++)
        for (y = 0; y < ROWNO; y++)
            if ((mtmp = m_at(x, y)) != 0) sanity_check_single_mon(mtmp, "m_at");
 ...
 * etc....
 */
STATIC_OVL struct obj * make_corpse(mtmp, corpseflags)
    register struct monst *mtmp;unsigned corpseflags;
{
    ...
    case PM_BLACK_PUDDING:
        ...
        obj = mksobj_at(GLOB_OF_BLACK_PUDDING - (PM_BLACK_PUDDING - mndx), x, y,
        TRUE, FALSE);
    ...
    if (has_mname(mtmp)) obj = oname(obj, MNAME(mtmp));
    ...
    register struct monst *mtmp;
{
    ...
    if (mtmp == u.usteed && (Flying || Levitation)) return 0;
    ...
    struct monst *mon;
{
    ...
    if (mon->mspeed == MSLOW)
        mmove = (2 * mmove + 1) / 3;
    else if (mon->mspeed == MFAST) mmove = (4 * mmove + 2) / 3;
    ...
    for (mtmp = fmon; mtmp; mtmp = nmtmp)
    {
        ...
        if (is_hider(mtmp->data))
        {
            ...
            if (mtmp->m_ap_type == M_AP_FURNITURE
                    || mtmp->m_ap_type == M_AP_OBJECT) continue;
        ...
        if (Conflict && !mtmp->iswiz && mtmp->mcansee)
        {
            ...
            if (couldsee(mtmp->mx, mtmp->my)
                    ...
                    && fightm(mtmp)) continue; /* mon might have died */
        }
        if (dochugw(mtmp)) /* otherwise just move the monster */
        continue;
    }

    if (any_light_source()) vision_full_recalc = 1; /* in case a mon moved with a light source */
    ...
    register struct monst *mtmp;
{
    ...
    for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp->nexthere)
    {
        ...
        if ((mtmp->data == &mons[PM_RUST_MONSTER] && !is_rustprone(otmp))
                ...
                || (otmp->otyp == RIN_SLOW_DIGESTION)) continue;
                ...
                && touch_artifact(otmp, mtmp))
        {
            ...
            else
            {
                if (cansee(mtmp->mx, mtmp->my) && flags.verbose)
                    pline("%s eats %s!", Monnam(mtmp),
                            distant_name(otmp, doname));
                else if (flags.verbose) You_hear("a crunching sound.");
    ...
    struct monst *mtmp;
{
    ...
    for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp2)
    {
        ...
        else if ((otmp->otyp == CORPSE && touch_petrifies(&mons[otmp->corpsenm])
                && !resists_ston(mtmp))
        /* don't engulf boulders and statues or ball&chain */
        || otmp->oclass == ROCK_CLASS || otmp == uball || otmp == uchain)
        {
        ...
        else
        {
            ...
            else if (flags.verbose) You_hear("a slurping sound.");
            /* Heal up to the object's weight in hp */
            if (mtmp->mhp < mtmp->mhpmax)
            {
                mtmp->mhp += objects[otmp->otyp].oc_weight;
            ...
            if (poly)
            {
                if (newcham(mtmp, (struct permonst *) 0, FALSE, FALSE)) ptr =
                        mtmp->data;
            }
            else if (grow)
            {
...
boolean mpickstuff(mtmp, str)
    register struct monst *mtmp;register const char *str;
{
    ...
    for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp2)
    {
        ...
        if (!str ? searches_for_item(mtmp, otmp) : !!(index(str, otmp->oclass)))
        {
            if (otmp->otyp == CORPSE && mtmp->data->mlet != S_NYMPH
                    ...
                    && !touch_petrifies(
                            &mons[otmp->corpsenm]) && otmp->corpsenm != PM_LIZARD
    ...
    struct monst *mtmp;struct obj *otmp;
{
    ...
    if (notake(mdat)) return 0; /* can't carry anything */

    if (otyp == CORPSE
            && touch_petrifies(
                    &mons[otmp->corpsenm]) && !(mtmp->misc_worn_check & W_ARMG) && !resists_ston(mtmp))
        return 0;
    if (otyp == CORPSE && is_rider(&mons[otmp->corpsenm])) return 0;
    if (objects[otyp].oc_material == SILVER && mon_hates_silver(mtmp)
            && (otyp != BELL_OF_OPENING || !is_covetous(mdat))) return 0;
...
int mfndpos(mon, poss, info, flag)
    struct monst *mon;coord *poss; /* coord poss[9] */
    ...
    long flag;
{
    ...
    for (nx = max(1, x - 1); nx <= maxx; nx++)
        for (ny = max(0, y - 1); ny <= maxy; ny++)
        {
            ...
            ntyp = levl[nx][ny].typ;
            if (IS_ROCK(ntyp) && !((flag & ALLOW_WALL) && may_passwall(nx, ny))
...
/* Monster against monster special attacks; for the specified monster
 ...
 next to each other. */
STATIC_OVL long mm_aggression(magr, mdef)
...
STATIC_OVL long mm_displacement(magr, mdef)
    struct monst *magr, /* monster that is currently deciding where to move */
    *mdef; /* another monster which is next to it */
{
    ...
    && (is_rider(pa) || pa->msize >= pd->msize)) return ALLOW_MDISP;
    return 0L;
}

/* Is the square close enough for the monster to move or attack into? */
boolean monnear(mon, x, y)
    struct monst *mon;int x, y;
{
...
void relmon(mon, monst_list)
    struct monst *mon;struct monst **monst_list; /* &migrating_mons or &mydogs or null */
{
    struct monst *mtmp;
    ...
    if (unhide)
    {
        ...
        mon->mundetected = 0;
        if (mon->m_ap_type && mon->m_ap_type != M_AP_MONSTER) seemimic(mon);
    }
...
void dealloc_monst(mon)
    struct monst *mon;
{
    if (mon->nmon) panic("dealloc_monst with nmon");
    if (mon->mextra) dealloc_mextra(mon);
    free((genericptr_t) mon);
}

/* remove effects of mtmp from other data structures */
STATIC_OVL void m_detach(mtmp, mptr)
    struct monst *mtmp;struct permonst *mptr; /* reflects mtmp->data _prior_ to mtmp's death */
{
    if (mtmp == context.polearm.hitmon) context.polearm.hitmon = 0;
    ...
    if (emits_light(mptr)) del_light_source(LS_MONSTER, monst_to_any(mtmp));
    newsym(mtmp->mx, mtmp->my);
    unstuck(mtmp);
...
STATIC_OVL void lifesaved_monster(mtmp)
    struct monst *mtmp;
{
    boolean surviver;
    ...
    register struct monst *mtmp;
{
        ...
        if (mndx >= LOW_PM && mndx != monsndx(mtmp->data))
        {
            ...
            Sprintf (buf, "The %s%s suddenly %s and rises as %%s!",
                    ...
                    ARTICLE_NONE, (char *) 0,
                            ...
                            FALSE), spec_mon ? "reconstitutes" : "transforms");
            mtmp->mcanmove = 1;
            mtmp->mfrozen = 0;
            if (mtmp->mhpmax <= 0) mtmp->mhpmax = 10;
            mtmp->mhp = mtmp->mhpmax;
    ...
    if (mtmp == u.usteed) dismount_steed(DISMOUNT_GENERIC);
    ...
    if (mtmp->m_id == quest_status.leader_m_id) quest_status.leader_is_dead =
    TRUE;
#ifdef MAIL
    ...
    if (mtmp->data == &mons[PM_MEDUSA]) u.uachieve.killed_medusa = 1;
...
boolean corpse_chance(mon, magr, was_swallowed)
    struct monst *mon;struct monst *magr; /* killer, if swallowed */
    ...
    struct monst *mdef;
{
    ...
    if (mdef == u.usteed) dismount_steed(DISMOUNT_GENERIC);
...
void monkilled(mdef, fltxt, how)
    struct monst *mdef;const char *fltxt;int how;
{
...
void xkilled(mtmp, dest)
    struct monst *mtmp;int dest; /* dest==1, normal; dest==0, don't print message; dest==2, don't
     drop corpse either; dest==3, message but no corpse */
{
    ...
    { /* monster lifesaved */
         ...
         */
        stoned = FALSE;
        if (!cansee(x, y) && !vamp_rise_msg) pline("Maybe not...");
        return;
    }
    ...
    else if (mtmp->ispriest)
    {
        ...
        if (mdat->maligntyp == A_NONE) adjalign((int) (ALIGNLIM / 4)); /* BIG bonus */
    }
    else if (mtmp->mtame)
    {
        adjalign(-15); /* bad!! */
    ...
    else if (mtmp->mpeaceful) adjalign(-5);
 ...
 *      0 - in most cases.
 */
boolean mnearto(mtmp, x, y, move_other)
    register struct monst *mtmp;xchar x, y;boolean move_other; /* make sure mtmp gets to x, y! so move m_at(x, y) */
{
    ...
    && mtmp->data == &mons[quest_info(MS_LEADER)])
    {
        ...
        if (got_mad && !Hallucination)
            pline_The("%s appear%s to be angry too...",
                    got_mad == 1 ?
                            q_guardian->mname : makeplural(q_guardian->mname),
                    got_mad == 1 ? "s" : "");
    }
}
...
void wake_nearby()
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
    {
        ...
        {
            mtmp->msleeping = 0;
            if (!unique_corpstat(mtmp->data)) mtmp->mstrategy &=
                    ~STRAT_WAITMASK;
            if (mtmp->mtame && !mtmp->isminion)
...
STATIC_OVL boolean restrap(mtmp)
    register struct monst *mtmp;
{
    struct trap *t;
    ...
    if (mtmp->mcan || mtmp->m_ap_type || cansee(mtmp->mx, mtmp->my) || rn2(3)
            ...
            || (mtmp->mtrapped && (t = t_at(mtmp->mx, mtmp->my)) != 0
                    && !(t->ttyp == PIT || t->ttyp == SPIKED_PIT))
            || (sensemon(mtmp) && distu(mtmp->mx, mtmp->my) <= 2)) return FALSE;
    ...
    struct monst *mtmp;
{
    ...
    else if (is_u ? (u.utrap && u.utraptype != TT_PIT) : (mtmp->mtrapped && (t =
            t_at(x, y)) != 0 && !(t->ttyp == PIT || t->ttyp == SPIKED_PIT)))
    {
    ...
    else if (hides_under(mtmp->data) && OBJ_AT(x, y))
    {
        ...
        if (otmp->nexthere || otmp->otyp != CORPSE
                ...
                || !touch_petrifies(&mons[otmp->corpsenm])) undetected = TRUE;
    }
...
void mon_animal_list(construct)
    boolean construct;
{
    if (construct)
    {
...
STATIC_OVL int pick_animal()
{
    ...
    if (Is_rogue_level(&u.uz) && !isupper((uchar ) mons[res].mlet)) res =
            animal_list[rn2(animal_list_count)];
    return res;
}

void decide_to_shapeshift(mon, shiftflags)
    struct monst *mon;int shiftflags;
{
    struct permonst *ptr;
    ...
    ptr = mon->data;
    if (!is_male(ptr) && !is_female(ptr) && !is_neuter(ptr)) mon->female =
            was_female;
}

STATIC_OVL int pickvampshape(mon)
    struct monst *mon;
{
    int mndx = mon->cham;
...
STATIC_OVL boolean isspecmon(mon)
    struct monst *mon;
{
...
STATIC_OVL boolean validspecmon(mon, mndx)
    struct monst *mon;int mndx;
{
...
STATIC_OVL boolean validvamp(mon, mndx_p, monclass)
    struct monst *mon;int *mndx_p, monclass;
{
    ...
    if (*mndx_p == PM_WOLF) return (boolean) (mon->cham != PM_VAMPIRE);
    ...
    struct monst *mon;
{
    ...
    if (wizard && iflags.mon_polycontrol)
    {
        ...
        if (is_vampshifter(mon) && !validvamp(mon, &mndx, monclass)) mndx =
                pickvampshape(mon); /* don't resort to arbitrary */
    }
...
STATIC_OVL struct permonst * accept_newcham_form(mndx)
    int mndx;
{
    struct permonst *mdat;
...
void mgender_from_permonst(mtmp, mdat)
    struct monst *mtmp;struct permonst *mdat;
{
    if (is_male(mdat))
    {
...
int newcham(mtmp, mdat, polyspot, msg)
    struct monst *mtmp;struct permonst *mdat;boolean polyspot; /* change is the result of wand or spell of polymorph */
    boolean msg; /* "The oldmon turns into a newmon!" */
{
    ...
    if (mdat == 0)
    {
        ...
        do
        {
            ...
            && mdat && !isupper((uchar) mdat->mlet)) mdat = 0;
            if (mdat) break;
        }
        while (--tryct > 0);
        if (!tryct) return 0;
    }
    else if (mvitals[monsndx(mdat)].mvflags & G_GENOD) return 0; /* passed in mdat is genocided */
    ...
    if (mtmp->m_ap_type && mdat->mlet != S_MIMIC) seemimic(mtmp); /* revert to normal monster */
    ...
    if (!mtmp->perminvis || pm_invisible(olddata)) mtmp->perminvis =
            pm_invisible(mdat);
    ...
    if (u.ustuck == mtmp)
    {
        ...
        else if (!sticks(mdat) && !sticks(youmonst.data)) unstuck(mtmp);
    }
    ...
    if (msg)
    {
        ...
        Strcpy (newname,
                (mdat == &mons[PM_GREEN_SLIME]) ?
                        "slime" : x_monnam(mtmp, ARTICLE_A, (char *) 0,
                        SUPPRESS_SADDLE,
                        FALSE));
...
int egg_type_from_parent(mnum, force_ordinary)
...
boolean dead_species(m_idx, egg)
    int m_idx;boolean egg;
{
    int alt_idx;
...
STATIC_OVL void kill_eggs(obj_list)
    struct obj *obj_list;
{
    struct obj *otmp;
...
void golemeffects(mon, damtype, dam)
    register struct monst *mon;int damtype, dam;
{
    int heal = 0, slow = 0;
    ...
    if (slow)
    {
        if (mon->mspeed != MSLOW) mon_adjust_speed(mon, -1, (struct obj *) 0);
    }
    if (heal)
    {
...
void mimic_hit_msg(mtmp, otyp)
    struct monst *mtmp;short otyp;
{
