...
public:
    ...
    void
    fakeEvent(int key, int ascii, int state);
};
...
public:
    ...
    void
    SetDefault(const char *);
    bool
    Get(char *buffer, int maxchar = 80);
    virtual void
...
public:
    ...
    virtual void
    PutStr(int attr, const char *text);
    virtual void
    StartMenu();
    virtual void
    AddMenu(int glyph, const ANY_P *identifier, char ch, char gch, int attr,
            const char *str, bool presel);
    virtual void
    EndMenu(const char *prompt);
    virtual int
    SelectMenu(int how, MENU_ITEM_P **menu_list);
    virtual void
    ClipAround(int x, int y);
    virtual void
    PrintGlyph(int x, int y, int glyph);
    virtual void
    UseRIP(int how, time_t when);
...
public:
    ...
    void
    drawGlyph(QPainter &, int glyph, int pixelx, int pixely);
    void
    drawCell(QPainter &, int glyph, int cellx, int celly);
...
public:
    ...
    // For messages
    void
    displayMessages(bool block);
    void
    putMessage(int attr, const char *text);
    void
    clearMessages();
...
public:
    ...
    void
    setLabel(const char *, bool lower = TRUE);           // a string
    void
    setLabel(const char *, long, const char *tail = ""); // a number
    void
    setLabel(const char *, long show_value, long comparative_value,
            const char *tail = "");
    void
...
private:
    ...
    setAlignments();
    void
    highlight(const QPalette &highlight);
    void
    unhighlight();
...
private:
    void
    drawWorn(QPainter &painter, obj *, int x, int y, bool canbe = TRUE);
};
...
public:
    ...
    void
    AddMessageWindow(NetHackQtMessageWindow *window);
    void
    AddMapWindow(NetHackQtMapWindow *window);
    void
    AddStatusWindow(NetHackQtStatusWindow *window);
    void
    RemoveWindow(NetHackQtWindow *window);
    void
    updateInventory();
...
public:
    NetHackQtYnDialog(NetHackQtKeyBuffer &keysource, const char *, const char *,
            char);
...
public:
    static void
    qt_init_nhwindows(int *argc, char **argv);
    static void
    qt_player_selection();
    static void
    qt_askname();
    ...
    static void
    qt_clear_nhwindow(winid wid);
    static void
    qt_display_nhwindow(winid wid, BOOLEAN_P block);
    static void
    qt_destroy_nhwindow(winid wid);
    static void
    qt_curs(winid wid, int x, int y);
    static void
    qt_putstr(winid wid, int attr, const char *text);
    static void
    qt_display_file(const char *filename, BOOLEAN_P must_exist);
    static void
    qt_start_menu(winid wid);
    static void
    qt_add_menu(winid wid, int glyph, const ANY_P *identifier, CHAR_P ch,
            CHAR_P gch, int attr, const char *str, BOOLEAN_P presel);
    static void
    qt_end_menu(winid wid, const char *prompt);
    static int
    qt_select_menu(winid wid, int how, MENU_ITEM_P **menu_list);
    static void
    qt_update_inventory();
    static void
    qt_mark_synch();
    ...
    static void
    qt_cliparound(int x, int y);
    static void
    qt_cliparound_window(winid wid, int x, int y);
    static void
    qt_print_glyph(winid wid, XCHAR_P x, XCHAR_P y, int glyph, int bkglyph);
    static void
    qt_raw_print(const char *str);
    static void
    qt_raw_print_bold(const char *str);
    static int
    qt_nhgetch();
    static int
    qt_nh_poskey(int *x, int *y, int *mod);
    static void
    qt_nhbell();
    static int
    qt_doprev_message();
    static char
    qt_yn_function(const char *question, const char *choices, CHAR_P def);
    static void
    qt_getlin(const char *prompt, char *line);
    static int
    qt_get_ext_cmd();
    static void
    qt_number_pad(int);
    ...
    static void
    qt_outrip(winid wid, int how, time_t when);
    static int
    qt_kbhit();
