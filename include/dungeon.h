...
{ /* basic dungeon level element */
    xchar dnum; /* dungeon number */
    xchar dlevel; /* level number */
} d_level;

typedef struct s_level
{ /* special dungeon level element */
    struct s_level *next;
...
{ /* non-stairway level change identifier */
...
typedef struct mapseen
{
    ...
    struct mapseen_feat
    {
        ...
        Bitfield(nfount, 2);Bitfield(nsink, 2);Bitfield(naltar, 2);Bitfield(nthrone, 2);

        Bitfield(ngrave, 2);Bitfield(ntree, 2);Bitfield(water, 2);Bitfield(lava, 2);

        Bitfield(ice, 2);
        /* calculated from rooms array */
        Bitfield(nshop, 2);Bitfield(ntemple, 2);
    ...
    struct mapseen_flags
    {
        ...
        Bitfield(oracle, 1);Bitfield(sokosolved, 1);Bitfield(bigroom, 1);Bitfield(castle, 1);Bitfield(castletune, 1); /* add tune hint to castle annotation */

        Bitfield(valley, 1);Bitfield(msanctum, 1);Bitfield(ludios, 1);Bitfield(roguelevel, 1);
        ...
        Bitfield(questing, 1);
        /* quest leader has unlocked quest stairs */
    } flags;
    /* custom naming */
    char *custom;
    unsigned custom_lth;
    ...
    {
        Bitfield(seen, 1);Bitfield(untended, 1);
        /* flag for shop without shk */
