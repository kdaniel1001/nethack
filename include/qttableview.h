...
protected:
    ...
    setLeftCell(int col);
    virtual void
    setTopLeftCell(int row, int col);
    ...
    setYOffset(int);
    virtual void
    setOffset(int x, int y, bool updateScrBars = TRUE);
    ...
    tableFlags() const;
    bool
    testTableFlags(uint f) const;
    virtual void
    setTableFlags(uint f);
    void
    clearTableFlags(uint f = ~0);
    ...
    int
    lastRowVisible() const;
    int
    lastColVisible() const;

    bool
    rowIsVisible(int row) const;
    bool
    colIsVisible(int col) const;
...
protected:
    virtual void
    paintCell(QPainter *, int row, int col) = 0;
    virtual void
    ...
    void
    scroll(int xPixels, int yPixels);
    void
    updateScrollBars();
    void
    updateTableSize();
...
private:
    void
    coverCornerSquare(bool);
    void
    snapToGrid(bool horizontal, bool vertical);
    virtual void
    setHorScrollBar(bool on, bool update = TRUE);
    virtual void
    setVerScrollBar(bool on, bool update = TRUE);
    void
    updateView();
    int
    findRawRow(int yPos, int *cellMaxY, int *cellMinY = 0, bool goOutsideView =
            FALSE) const;
    int
    findRawCol(int xPos, int *cellMaxX, int *cellMinX = 0, bool goOutsideView =
            FALSE) const;
    int
    maxColsVisible() const;

    void
    updateScrollBars(uint);
    void
    updateFrameSize();
