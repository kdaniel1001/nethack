...
#define MAC_PATH_VALUE ".app/Contents/MacOS/"
     ...
     */
    attempt_restore: if ((fd = restore_saved_game()) >= 0)
    {
...
static void process_options(int argc, char *argv[])
{
    ...
    while (argc > 1 && argv[1][0] == '-')
    {
        ...
        case 'D':
        case 'd':
            if ((argv[0][1] == 'D' && !argv[0][2]) || !strcmpi(*argv, "-debug"))
            {
...
void sethanguphandler(void FDECL((*handler), (int)))
...
static void wd_message()
{
    ...
    else if (discover) You("are in non-scoring explore/discovery mode.");
}
